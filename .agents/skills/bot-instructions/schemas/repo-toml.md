...
| `code_review_path` | string | no | Where the complete review doctrine is written, and the path the `AGENTS.md` directive, the Copilot pointer and CodeRabbit's `code_guidelines.filePatterns` all name. Defaults to `.github/instructions/code-review.md`. A repo-relative markdown path holding only `[A-Za-z0-9._/-]`, with no leading slash and no empty, `.` or `..` component; an `AGENTS.md` basename is refused, since this package owns a region inside the root one and would own this file whole. A path this render already writes is refused where the whole output set exists, naming it |
