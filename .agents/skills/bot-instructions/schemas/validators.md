...
**Silent failure.** Codex reads `AGENTS.md` § Code Review Rules and nothing else, and that section is what sends it to the pointed file. Rename the heading, move the section, or delete the file, and Codex reviews every pull request with no repo rules at all, posting normally throughout.
...
**The scan is recursive, over what each bot reads rather than what the generator writes.** `.github/instructions/**`, `.macroscope/correctness/**`, `.macroscope/check-run-agents/**` and `.macroscope/approvability.md`, since Copilot and Macroscope read all four and the policy set already counts them. The configured `code_review_path` is added to that set by name: the default sits under the first of those trees, and a repo that moved the file elsewhere would otherwise leave a marked file nothing here judges. A marked file one directory down, left by a hand move or a directory rename, is at no path the generator would write, so a flat scan would miss it while Copilot kept loading it — and the two `.macroscope/` paths this package never writes are exactly where a marked generated file could be moved and stay active, particularly after `macroscope = false`, with neither this nor `drift` reporting it.
