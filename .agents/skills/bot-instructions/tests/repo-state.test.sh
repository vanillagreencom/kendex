...
# A pointed file left behind when `codex` goes false is an orphan wherever it
# sits. The default path is under a scanned tree; a configured one is not, and
# the bot keeps loading it until someone deletes it.
stray="$(bi_minimal_repo stray-pointed-file)"
{
  printf '%s' "$BI_MIN_HEAD"
  printf 'code_review_path = "docs/code-review.md"\n'
} > "$stray/kendex.toml"
mkdir -p "$stray/docs"
printf '<!-- generated by bot-instructions 1.0.0 from x. y -->\n\n# Code review rules\n' \
  > "$stray/docs/code-review.md"
git -C "$stray" add -A >/dev/null 2>&1
expect_red orphan 'a marked pointed file at a configured path with codex false is an orphan' \
  check --repo "$stray"
rm -f -- "${stray:?}/docs/code-review.md"
git -C "$stray" add -A >/dev/null 2>&1
