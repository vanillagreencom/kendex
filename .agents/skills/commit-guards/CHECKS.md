...
The batch names the scope: `commit-guards all` hands the lane `--all`, `all --base REF` hands `--base REF`, `all --against REF` hands `--against REF`, `all --staged` hands `--staged`. The two range scopes reach md-format and md-refs as well, on the same dot conventions. A copy is an addition; symlinks and gitlinks are not sized.
