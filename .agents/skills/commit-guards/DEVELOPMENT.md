...
- The base is the remote oid where this repository has that commit. That oid is the one thing here bound to the destination: git read it from the destination's own advertisement, so the range is exactly what the destination lacks and byte-ceiling's baseline is its own blob.
- Otherwise the base is the first boundary commit of `git rev-list --boundary <local-oid> --not --remotes=<remote>`, and it is a best-effort **local** scope rather than a claim about the destination: `refs/remotes/<name>/*` record a past fetch and nothing local dates them. After a `git remote set-url` they still describe the previous repository, so on a new destination branch byte-ceiling can treat an oversized file as pre-existing. `git ls-remote` would close that and is refused — a network round trip and an offline failure mode inside a hook, for the new-branch case alone.
