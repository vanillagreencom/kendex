...
- kendex runs the installer through the `repo-effects` declaration in `SKILL.md`; every verb that drops the package runs `--uninstall` while the scripts are still on disk; `kendex guard install`, `guard uninstall` and `guard check` call it directly; the same declaration's `checker` names `--check` and `.git/hooks/kendex-guards` as its evidence, which is what `kendex check` and the app's per-project setup status both relay `--check` behind.
