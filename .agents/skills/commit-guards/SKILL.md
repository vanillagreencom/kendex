...
The inventory uses the exclusion list’s index-first read contract and is required at the Git repository root. An absent, unreadable or malformed inventory fails with exit `2`. Install or refresh kendex at that root in the main checkout and stage the inventory with the renders. An installation confined below the Git root produces no repository inventory. Explicit `!` rows still restore matching paths to the scan. Keep non-render exceptions in the reasoned exclusion list.
