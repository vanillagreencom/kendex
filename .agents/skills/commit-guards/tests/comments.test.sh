...
OUT="$(cd "$R" && COMMIT_GUARDS_COMMENT_REFERENCE_TYPES=issue-id "$CM" 2>&1)" && RC=0 || RC=$?
...
OUT="$(cd "$R" && GH_ISSUE_PATTERN= COMMIT_GUARDS_COMMENT_REFERENCE_TYPES=issue-id "$CM" 2>&1)" && RC=0 || RC=$?
...
for mode in index staged; do
  args=()
  [ "$mode" != staged ] || args=(--staged)
  OUT="$(cd "$R" && COMMIT_GUARDS_COMMENT_REFERENCE_TYPES=' ' "$CM" ${args[@]+"${args[@]}"} 2>&1)" && RC=0 || RC=$?
...
OUT="$(cd "$R" && COMMIT_GUARDS_COMMENT_PATHS='run' "$CM" 2>&1)" && RC=0 || RC=$?
...
put notes.txt "# $W"
OUT="$(cd "$R" && COMMIT_GUARDS_COMMENT_PATHS='*.txt' "$CM" 2>&1)" && RC=0 || RC=$?
...
chmod +x "$R/shim/head"
put run "#!/usr/bin/env bash" "# $W"
OUT="$(cd "$R" && PATH="$R/shim:$PATH" COMMIT_GUARDS_COMMENT_PATHS='run' "$CM" 2>&1)" && RC=0 || RC=$?
...
echo "=== COMMIT_GUARDS_COMMENT_PATHS REPLACES the list, and is validated ==="
new_repo override
put a.rs "// $W"
put notes.txt "# $W"
OUT="$(cd "$R" && COMMIT_GUARDS_COMMENT_PATHS='*.txt' "$CM" 2>&1)" && RC=0 || RC=$?
...
OUT="$(cd "$R" && COMMIT_GUARDS_COMMENT_PATHS='no/such/*.rs' "$CM" 2>&1)" && RC=0 || RC=$?
[ "$RC" -eq 0 ] && case "$OUT" in *"no tracked file matches COMMIT_GUARDS_COMMENT_PATHS"*) true ;; *) false ;; esac \
  && ok "a list matching no tracked file passes, naming the list" || bad "unmatched list" "rc=$RC out=$OUT"
OUT="$(cd "$R" && COMMIT_GUARDS_COMMENT_PATHS=' ' "$CM" 2>&1)" && RC=0 || RC=$?
...
git -C "$R" add -A
OUT="$(cd "$R" && COMMIT_GUARDS_COMMENT_EXCLUDES=alt "$CM" 2>&1)" && RC=0 || RC=$?
...
[ "$RC" -eq 1 ] && ok "'commit-guards comments' reaches the lane" || bad "dispatcher routes by name" "rc=$RC out=$OUT"
OUT="$(cd "$R" && COMMIT_GUARDS_CHECKS=comments "$GG" all --staged 2>&1)" && RC=0 || RC=$?
[ "$RC" -eq 1 ] && case "$OUT" in *"commit-guards: comments --staged"*) true ;; *) false ;; esac \
  && ok "the batch hands comments --staged at commit scope" || bad "batch forwards --staged" "rc=$RC out=$OUT"
OUT="$(cd "$R" && "$GG" 2>&1)" && RC=0 || RC=$?
[ "$RC" -eq 0 ] && case "$OUT" in *"commit-guards: comments"*) false ;; *) true ;; esac \
  && ok "the default batch does not run the lane (it is opt-in through COMMIT_GUARDS_CHECKS)" \
...
git -C "$R" add -A
OUT="$(cd "$R" && COMMIT_GUARDS_COMMENT_PATHS='*.sh skills/*/scripts/*' "$CM" 2>&1)" && RC=0 || RC=$?
