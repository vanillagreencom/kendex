...
# The two installs, each in another checkout, the tests subtree cut since
# a consumer install never carries it. The bare one sits under a skill root
...
# three stubs are reached only through the install's own side.
...
# The bare install's skip line: the tree, the install's checkout, the roots, the install's own side.
skip() { printf '=== pre-commit: %s not installed — skipped (no %s skill under <repo> and <root>/other-checkout (%s), nor at <root>/%s/commit-guards/scripts/../../%s)' "$1" "$1" "$ROOTS" "$BARE" "$1"; } # LANE
...
fx_tree_bot_broken() { repo tree-bot-broken; mkdir -p "$R/.agents/skills/bot-instructions/scripts"; }
...
run_rows \
  ...
  "a tree-carried bot-instructions skill without a runnable script is a broken install, never a skip|fx_tree_bot_broken||$BARE||rc=2 $(skip doc-limits);$(skip preflight);$(broken bot-instructions)" \
...
fx_local_missing() { repo local-missing; }
...
run_rows \
  ...
  "a missing entry is the same config error|fx_local_missing|$LOCAL_ENV|$BARE||rc=2 $SKIPS;$BATCH_OK;${ERR}COMMIT_GUARDS_PRE_COMMIT_LOCAL names 'tools/local-check', which is missing or not executable"
