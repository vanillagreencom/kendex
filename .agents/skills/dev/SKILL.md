...
- When a change deletes a call, search the repository for its callee. Delete a public item in the same commit when no caller remains.
- A field, setting, or view member added by the change has both a producer and a consumer in the change; otherwise, do not add it.
