...
- When a change deletes a call, apply [code-quality § Cleanup](../code-quality/SKILL.md#cleanup) to its callee; its deletion maps to the call removal's Done-when item, but no internal caller is not proof that a supported external API is unused. A field, setting, or view member added by the change has a real producer and consumer; a named and documented external endpoint is valid when the change adds its in-repository counterpart.
