...
#     required:<context> a ruleset requiring that one context;
...
word() {
  ...
  case "$1" in
    ...
    admin-contexts:*) W_ENV+=("STUB_GATE_RULES=$(printf '%s' "$v" | tr ',' '\n' | jq -R -s -c '
        [ split("\n")[] | select(. != "") | {context: gsub("[+]"; " ")} ] as $c
        | [{type:"required_status_checks", parameters:{required_status_checks:$c}}]')") ;;
...
ADMIN_MERGE_CALLS="compare,queue-state,view:head,merge:admin,graphql:queue"
...
the review gate is met and every required context green merges|admin-dir checks:ci-required head:$AHEAD admin-contexts:CI+Required post:MERGED merge-commit:admin-merge-oid|admin-credential:$AHEAD|0|$REC merged pr=123 head=$AHEAD route=on class=any head-match=ok review=ok checks=ok base=fresh dequeue=none|MERGED PR #123|calls=$ADMIN_PRE,$ADMIN_CHECK,$ADMIN_MERGE_CALLS auth=<unset>
a PR under review-required with no approval refuses, the review gate its own field|admin-dir checks:ci-required head:$AHEAD review:REVIEW_REQUIRED|admin-credential:$AHEAD|1|$REC refused pr=123 head=$AHEAD route=on class=any head-match=ok review=required checks=ok base=- dequeue=- reason=review-required|REFUSED PR #123 — the review gate is not met: not_approved: Review status is 'REVIEW_REQUIRED';Nothing dequeued, nothing merged.|calls=$ADMIN_PRE,$ADMIN_CHECK auth=<unset>
...
a required context absent from the head rollup refuses|admin-dir checks:ci-required head:$AHEAD admin-contexts:Absent+Check|admin-credential:$AHEAD|1|$REC refused pr=123 head=$AHEAD route=on class=any head-match=ok review=ok checks=missing-context base=- dequeue=- reason=checks-unmet|REFUSED PR #123 — required context(s) not green on this head: Absent Check;Nothing dequeued, nothing merged.|calls=$ADMIN_PRE,$ADMIN_CHECK auth=<unset>
...
a queued PR with no node id cannot be dequeued, so it is not merged|admin-dir checks:ci-required head:$AHEAD admin-queue admin-node:-|admin-credential:$AHEAD|1|$REC refused pr=123 head=$AHEAD route=on class=any head-match=ok review=ok checks=ok base=fresh dequeue=failed reason=dequeue-failed|REFUSED PR #123 — the PR is queued or armed and GitHub returned no node id to dequeue it by;Nothing dequeued, nothing merged.|calls=$ADMIN_PRE,$ADMIN_CHECK,compare,queue-state auth=<unset>
