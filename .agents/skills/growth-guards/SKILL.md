...
| Check | Verdict |
|---|---|
| **todo-ban** | Any work marker (TODO, FIXME, HACK, XXX in comment-marker shapes) in a tracked, non-excluded file fails. No baseline. Prose naming a marker word does not fire. Two scopes: the default reads the whole index, over the blobs git calls text (the CI subject); `--staged` judges only the lines the staged diff ADDS, on content rather than attributes, so a marker the commit does not add is CI's finding rather than this commit's. |
