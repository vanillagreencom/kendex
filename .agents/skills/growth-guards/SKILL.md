...
| **changelog-entries** | The changelog, over two scopes. Fragments (`GROWTH_GUARDS_CHANGELOG_PATHS`): each matched tracked path must be a real text file — a symlink, a gitlink or a binary blob is refused — sit exactly two segments past its pattern's root — one directory of section, one of name — under a Keep a Changelog section directory (`added`, `changed`, `deprecated`, `removed`, `fixed`, `security`), depth counted from the root and never from the immediate parent, hold exactly one Markdown list item (first non-blank line opens with a hyphen and a space and says something; every later non-blank line indents under it, so an indented second paragraph is part of the entry), and measure within the character cap (default 200). Measuring joins the fragment's lines with CR stripped and whitespace runs collapsed to one space, counted in characters — so wrapping spends no cap. Every OTHER tracked path in the fragment tree is refused too, a `README.md` directly under a root excepted; `--list` writes NUL-terminated `fragment<TAB>SECTION<TAB>PATH` and `record<TAB>PATH` records on stdout, which is what a collator folds in and what it guards. The record (`GROWTH_GUARDS_CHANGELOG_RECORD`): a line the index carries under its `## [Unreleased]` heading that HEAD does not is refused, so two branches never insert at the same place; the heading is found by ATX structure outside fenced code, never by substring (a fence closes only on a run of at least its opening length in the same character, and an unterminated one is exit 2), and `GROWTH_GUARDS_CHANGELOG_COLLATE=1` declares the collator's own write, bypassing that comparison alone and leaving the type and text rules judging. Text that is not valid UTF-8 is a collection error; paths matching no tracked file are a clean pass. |
