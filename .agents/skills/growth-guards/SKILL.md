...
| **changelog-entries** | A changelog entry over the character cap (default 200) fails. An entry opens on a list marker (`-`, `*`, `+`) at column 0 followed by a space or tab, and runs to the next such marker, an ATX heading (one to six hashes then whitespace or end of line), or a blank line followed by a line that is neither indented nor a marker — an indented second paragraph is part of the entry. Its text is those lines with CR stripped and whitespace runs collapsed to one space, counted in characters. A symlink, a gitlink or a binary blob at a configured path is named as unmeasured, and text that is not valid UTF-8 is a collection error; paths matching no tracked file are a clean pass. |
