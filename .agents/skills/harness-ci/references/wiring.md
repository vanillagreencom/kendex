...
- **`render` reaches only the paths `kendex verify` itself names.** Each changed path has to be the Claude instruction shim that run listed, whose shim IS the whole file, or one of kendex's own two bookkeeping files, `.kendex-lock.json` and `.kendex-generated.json`: those hold no rendered content and are covered by name, nothing reading the record and the inventory being what `harness-only` has already judged the diff by. Every other changed path answers `standard` with `cause=render-path-unowned`. So today a refresh that changes a rendered skill, agent, hook or command answers `standard`, and `render` is reached by a shim-only or a bookkeeping-only refresh. The wider reach is KEN-1673, where `kendex verify` prints the rendered positions of each row it prints and the classifier owns paths from those rows; until then nothing about the install record is weighed, because it is a file the pull request's own branch writes. `.gemini/settings.json` is refused for a second reason besides: the shim row kendex prints for it weighs one key of a document whose other keys decide what that harness runs, so it is a configuration source and answers `standard` ahead of every render.
