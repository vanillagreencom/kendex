...
- **`render` needs a `kendex` on the runner AND a primed source mirror**, which is what the two steps above give it. `kendex verify` re-renders out of the local mirror and never fetches it, so on a runner that has never fetched the source every package reports that where it comes from is unavailable, the proof fails and the answer is `standard`. The priming step fetches the marketplaces the judged tree's own manifest declares into the runner's cache and leaves `subject` exactly as it was committed: it installs nothing and writes no file there. That matters because the classifier refuses a `subject` whose working tree differs from its own HEAD — a step that wrote renders back would have the proof attest to its own repair instead of to the commit it was checked out at. That HEAD is the pull request's merge ref here, since the `subject` checkout names no `ref:`, while the install record is read at `--head`; every way those two commits differ costs a package its row and the paths under it their owner, so the difference is conservative rather than exact. The proof also reads the wording of `kendex verify`'s rows and its closing counts, which is why the install step pins a version: a kendex that prints either differently answers `standard` rather than failing, and the pin is the consumer's own to move once a newer release has been tried against its lanes. A consumer that will not pay for these steps has no `render` class and keeps publishing `harness_only` beside `change_class` to gate its lanes.
- **`render` reaches the changed paths the install record can place.** Each changed path has to sit under an `emitted.paths` position of a package `kendex verify` listed as checked and OK, be the Claude instruction shim that run listed, or be one of kendex's own two bookkeeping files, `.kendex-lock.json` and `.kendex-generated.json`: those hold no rendered content and are covered by name, the record being the table the rows are looked up in and the inventory being what `harness-only` has already judged the diff by. A position counts only where its last segment is the name in the key that records it, and where no second entry records that same position under another `kind:name`; the one exception is the rename kendex performs itself, a command whose name a skill already holds installing under a longer spelling of that name, so a command's position may extend its own name. Nothing else about the record is weighed and the record at the other end of the range is never read, so a branch cannot widen a position, move it onto a sibling's tree, or rename the entry around the tree it wants owned, in this pull request or an earlier one. A declared name carrying a slash installs under a folded spelling and answers `standard`, which is what a rule that tight costs. A skill, a Pi extension and a command on a harness that stores it as a skill tree record those positions; an agent, a hook, an MCP server, a plugin, a command on every other harness, and the harness registry files a refresh rewrites, such as `.claude/settings.json`, `.codex/hooks.json`, `.codex/config.toml` and `.pi/prompts/*`, record none, so a pull request that re-renders one of those answers `standard`. `.gemini/settings.json` answers `standard` too, for the same reason as every other configuration source: the shim row kendex prints for it weighs one key of a document whose other keys decide what that harness runs, so a refresh that first writes or restores the Gemini shim buys no waiver.
