...
review-gate's `validate-standard.sh` checks the merge at the default branch head. Its `standard-ci-context` row fails with a value starting `ci-context-missing` when the pull request or its merge group ran no job named `CI`, naming the job names that did run, and with `merge-group-unobserved` when no merge_group run exists.
