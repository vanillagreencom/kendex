...
- **`render` reaches the changed paths the install record can place.** Each changed path has to sit under an `emitted.paths` position of a package `kendex verify` listed as checked and OK, be the Claude instruction shim that run listed, or be one of kendex's own two bookkeeping files, `.kendex-lock.json` and `.kendex-generated.json`: those hold no rendered content and are covered by name, the record being the table the rows are looked up in and the inventory being what `harness-only` has already judged the diff by. A skill, a Pi extension and a command on a harness that stores it as a skill tree record those positions; an agent, a hook, an MCP server, a plugin, a command on every other harness, and the harness registry files a refresh rewrites (`.claude/settings.json`, `.codex/hooks.json`, `.codex/config.toml`, `.pi/prompts/*`) record none, so a pull request that re-renders one of those answers `standard`. `.gemini/settings.json` answers `standard` too, for the same reason as every other configuration source: the shim row kendex prints for it weighs one key of a document whose other keys decide what that harness runs, so a refresh that first writes or restores the Gemini shim buys no waiver.
