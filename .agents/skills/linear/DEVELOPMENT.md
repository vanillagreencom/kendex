...
- Build any timestamp compared against a cached `startsAt` or `updatedAt` with `cache_now_utc` or `cache_utc_days_ago` from `lib/cache-dates.sh`, never `date -Iseconds`. The comparison is lexical against records sync stores in UTC, so a local-time value with an offset suffix only agrees on a UTC host; the same mistake stood in five places at once (KEN-1175). `date -Iseconds` is right for a timestamp this skill *writes*, such as `sync`'s `synced_at`.
- Select a cycle by date, not by position in the sorted set. `cache_working_cycle`, `cache_cycles_before` and `cache_cycles_after` are the three definitions; with no cycle running they cut at today, where reading the ends of the list answered "previous" with a cycle that had not started.
