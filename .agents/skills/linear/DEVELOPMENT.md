...
An assertion made in a subshell — a command substitution, a pipeline element, a backgrounded or parenthesised block — increments a counter the suite never sees and records a failure nobody reads, so the library refuses the shape rather than leaving authors to avoid it: every assertion also appends to a ledger file, which a subshell shares with its parent, and a count that disagrees with the in-memory one fails the suite naming how many were lost. Capture the status in the suite and assert on it there.
