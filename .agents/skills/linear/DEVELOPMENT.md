...
- Select a cycle by date, not by position in the sorted set. `cache_working_cycle`, `cache_cycles_before` and `cache_cycles_after` are the three definitions, and every caller hands the working cycle over unguarded: with none running they cut at today, where reading the ends of the list answered "previous" with a cycle that had not started. A caller that guards on `working == null` instead reintroduces the divergence — one cache answering `--type past` with a cycle and `--cycle previous` with a refusal.
