...
Output:
  ...
  - issues: categorized issue arrays (sub-issues excluded, shown via parent):
    ...
    - Each issue includes blocked_by and blocked_by_open
...
get_session_status() {
    ...
    if [[ -n "$project_ids" ]]; then
        issues_json=$(jq --arg pids "$project_ids" --argjson projects "$projects_json" "$ISSUE_RELATION_JQ"'
            ...
            def format_research: . as $issue | format_issue + {
                blocks: [issue_blocks_ids($issue.relations.nodes)[] as $blocked_id |
                    ([$all[] | select(.identifier == $blocked_id)] | first) as $blocked_issue |
                    select($blocked_issue | issue_is_open) | $blocked_id]
            };
            {
