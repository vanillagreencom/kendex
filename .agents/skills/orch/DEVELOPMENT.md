...
- The inventory is discovered; config is only an overlay. `ORCH_LANE_ALIASES` renames discovered lanes, `ORCH_LANE_DIRS` covers a layout discovery cannot reach, `ORCH_LANE_EXCLUDE` and `ORCH_LANE_RETIRE` remove lanes. An excluded or retired lane's directory is never looked inside, because an excluded account can be one agents must not touch.
