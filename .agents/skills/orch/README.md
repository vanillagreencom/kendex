...
| `ORCH_OVERSEER_PREFERENCE` | Comma-separated `harness:rank:effort` entries that `oversee-succeed` tries in order, `rank` a kendex tier ladder position with 1 the top tier; what a named entry and the caller fallback carry, question-tool words included, is `oversee-succeed --help` | empty |
| `ORCH_QUESTION_TOOL` | `off`: an overseer drops its question tool where a lane does (skill-rules.md § Coordination); `overseer`: keeps it | `off` |
| `ORCH_ASK_WAIT_MINUTES` | Minutes a recommended owner ask waits before the watch takes the recommendation | `120` |
