...
| `ORCH_HANDOFF_HEADROOM_PCT` | Account headroom at or below which `lanes context` marks a live lane for handoff and the `lane-mail-check` turn-end hook refuses that lane's turn end. The hook resolves it through `orch-env`, so a settings file sets it; the poll reads the process environment alone | `3` |
...
| `ORCH_LANE_MAX_PCT` | Usage share of its binding bucket at or above which `lanes pick` refuses an account, so a lane launches only on one with more than five percent headroom. `lanes` reads it; `open-terminal --lane-max-pct` and `lanes --max-pct` override it | `95` |
| `ORCH_LANE_BINDING_FLOOR` | `on` holds the account's own binding bucket to that same threshold on every `open-terminal` lane judgement, beside the window of the model being launched; `off` judges the model window alone | `on` |
