...
| `ORCH_HANDOFF_HEADROOM_PCT` | Account headroom at or below which `lanes context` marks a live lane for handoff and the `lane-mail-check` turn-end hook refuses that lane's turn end, read against the binding bucket. Both resolve it through the kendex settings ladder, so a settings file sets one mark for both. Never compared with `ORCH_LANE_MAX_PCT` | `3` |
| `ORCH_OVERSEER_PREFERENCE` | Comma-separated `harness:rank:effort` entries that `oversee-succeed` tries in order. `rank` is a position on the kendex tier ladder, 1 for the top tier. A named entry writes its harness's model and effort, keeps same-harness permission words exact, and carries only full bypass across harnesses. The caller fallback keeps the flags after `--`. See `oversee-succeed --help` for lane selection. | empty |
...
| Lane settings | `ORCH_LANE_DIRS`, `ORCH_LANE_ALIASES`, `ORCH_LANE_EXCLUDE`, `ORCH_LANE_RETIRE`, `ORCH_LANES_USAGE_TTL`, `ORCH_LANES_USAGE_MAX_AGE`, `ORCH_TMUX_VERIFY_SECS`, `ORCH_LANE_SSH_PROMPT_SECS`: `lanes --help`, `open-terminal --help` | |
