...
3. Put non-secret settings in `kendex.settings.toml` under `[env]` and secrets in `.env.local`. This skill's `kendex.settings.toml.example` seeds those defaults with a comment on each; a key orch shares with another skill is seeded by that skill's template.
...
| `REVIEW_MAX_CYCLES` | Max internal re-review cycles per issue in review-pr § 4; the `rereview_panel` write raises `rereview_cycles` and refuses at it, so the number configured is the number of re-entries allowed. Nothing else spends it — § 7 QA re-checks write `qa_recheck_panel`, and fix rounds tally in `cycles` | `4` |
| `REVIEW_MAX_EXTERNAL_ROUNDS` | Max external review rounds on an open PR, counted in `pr_comment_review.iterations` by review-pr-comments § 6.3. Past it findings get dispositions and no fix push, except a defect the diff itself introduces or arms | `4` |
