...
| `ORCH_HANDOFF_HEADROOM_PCT` | Account headroom at or below which `lanes context` marks a live lane for handoff and the `lane-mail-check` turn-end hook refuses that lane's turn end, read against the binding bucket. Never compared with `ORCH_LANE_MAX_PCT` | `3` |
...
| Watch | `ORCH_WATCH_TAIL_LINES`, `ORCH_WATCH_PREPARE_SECS`, `ORCH_WATCH_MAIL_INTERVAL`, `ORCH_DIRECTIVE_UNREAD_SECS`: `oversee-watch --help` § Environment | |
