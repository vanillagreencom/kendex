...
| `ORCH_HANDOFF_CONTEXT_TOKENS` | Context tokens at or past which the `lane-mail-check` turn-end hook refuses a turn end, a lane's and the overseer's alike, until that session's handoff record stands. `oversee-succeed` judges the overseer's own context mark against the same number. The hook resolves it through `orch-env`, so a settings file sets it. `lanes context` neither reads this setting nor marks a lane on it: its `HANDOFF` column answers for the headroom mark alone, so the overseer's backstop for a lane is reading the poll's `CONTEXT_TOKENS` column against this number by hand | `500000` |
...
| `ORCH_OVERSEER_HEADROOM_PCT` | Account headroom at or below which `oversee-succeed` succeeds the overseer, opens no successor, and the `lane-mail-check` turn-end hook refuses its turn end | `20` |
