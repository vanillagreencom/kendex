...
| `ORCH_HANDOFF_HEADROOM_PCT` | Account headroom at or below which `lanes context` marks a live lane for handoff and the `lane-mail-check` turn-end hook refuses that lane's turn end, read against the binding bucket. The hook resolves it through `orch-env`, so a settings file sets it; the poll reads the process environment alone. Never compared with `ORCH_LANE_MAX_PCT` | `3` |
