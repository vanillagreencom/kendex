...
| `ORCH_USER_MODE` | `ceo` asks only what [communication-modes.md](references/communication-modes.md) lists, frames it as outcomes, and composes the autonomy keys; `engineer` keeps their defaults | `ceo` |
| `ORCH_DECISION_MODE` | `ask` presents decision points; `auto-recommended` executes the recommended option | `auto-recommended` |
| `ORCH_MERGE_AUTONOMY` | `auto` uses existing user authorization to merge once every gate is green; `ask` requires user authorization for each merge | `auto` |
