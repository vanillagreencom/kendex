...
| `REVIEW_MAX_CYCLES` | Max internal re-review cycles per issue in review-pr § 4; the `rereview_panel` write raises `rereview_cycles` and refuses once that count is past it. Fix rounds elsewhere (QA, pre-loop review/submit) tally in `cycles` and spend none of it | `4` |
