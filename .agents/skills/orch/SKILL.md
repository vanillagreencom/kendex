...
| `round-prune` | At a dev round's start, prune the item worktree's build output under its own lease when the disk is past `ORCH_ROUND_PRUNE_DISK_PCT`, recording the bytes in `round_prunes` |
