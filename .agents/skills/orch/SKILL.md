...
- **A lane is quiet.** Under `ORCH_LANE_OUTPUT` a lane prints one line per completed step and writes every `<output_format>` block it fills itself to a file, the artifact the step owns or the lane status file: [references/skill-rules.md](references/skill-rules.md) § Lane Output.
