...
| `worktree-push` | Push an issue worktree via `worktree push` and reconcile rebased SHAs in workflow state (`.rebase_map`, `fixed_items`, `pr_comment_review.fixes`) in the same call. `--check-live-round` answers whether a fix round is in flight and pushes nothing, for the restack path that never reaches the push. `--help` |
...
The three waiters exit `3` on hard auth failure. See [references/gates.md](references/gates.md).
