...
| `oversee` | `launch` opens a fleet's first overseer on the account `ORCH_OVERSEER_PREFERENCE` names, from outside tmux with `ORCH_TMUX_SESSION` set, and refuses `overseer-live` while the recorded session runs; `register` writes the session record for a session opened by hand. Both write the `overseer` record [schemas/workflow-state.md § Oversee state](schemas/workflow-state.md#oversee-state) states |
...
| `oversee-cycle` | `record` writes a merged lane's `cycle` onto its fleet record: the seven phase stamps from `launched_at` and the github skill's `pr-timeline`, the class harness-ci's classifier gives the merge, its rounds, and its verdict against the class's merge-time target with the phase that dominated; it logs that line, and a `repeat-miss` line at the third miss on one phase. `rollup` logs and prints the per-class count, median, p90, rounds and quality counts. The target table is its `--help` |
