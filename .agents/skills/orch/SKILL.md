...
| `lane-close` | Stop one finished recorded lane's harness by signal, unless a hosted stop answers the item's worktree is gone, close its hosted sandbox and tmux window, remove the item's files from the state directory on a full close, and update its fleet record |
