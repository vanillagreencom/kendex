...
| `lane-close` | Stop one finished recorded lane's harness by signal, close its hosted sandbox and tmux window, and update its fleet record |
...
| `lane-mail` | The lane-to-overseer mailbox. A lane runs `ask`, `notice`, `wait` and `inbox`; the overseer runs `send`, `drain`, `pending` and `receipts`, adding `--root` and `--host` for a lane on another host |
