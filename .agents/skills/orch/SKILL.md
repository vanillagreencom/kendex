...
- **Post-PR autonomy.** After a PR exists, `ORCH_DECISION_MODE=auto-recommended` takes and logs the continuing option while a bounded wait, retry, or triage round remains. `ask` presents the listed choice. When no budget remains, never ask or claim completion: write `tmp/post-pr-stop-[ISSUE_ID].json` as `{name, gate, remaining}` and a matching Markdown comment, store the JSON with `workflow-state update [ISSUE_ID] --slurpfile stop [JSON_PATH] '.post_pr_stop = $stop[0]'`, post the Markdown to the PR, and return the same named stop. A later continuing action clears `.post_pr_stop`. `ORCH_MERGE_AUTONOMY` controls merge consent only.
