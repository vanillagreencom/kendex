...
- **A lane is quiet.** Under `ORCH_LANE_OUTPUT` a lane prints one line per completed step and writes every `<output_format>` block it fills itself to the file the step owns: [references/skill-rules.md](references/skill-rules.md) § Lane Output.
