...
A Codex lane arms no mailbox monitor ([watch-delivery.md § Lane mailbox monitor](watch-delivery.md#lane-mailbox-monitor)): a lane idle at its prompt holds no turn, and Codex starts none for a monitor's output. The overseer follows each `lane-mail send` to a Codex lane with `open-terminal --wake` ([oversee.md § Talking to a lane](../workflows/oversee.md#talking-to-a-lane)), which resumes the lane's newest session in print mode with one line that runs `lane-mail inbox`. Codex publishes no idle signal, so the wake refuses a lane whose Codex process still runs, as `working` or `unjudged` ([lane-reach.md § Wake refusals](lane-reach.md#wake-refusals)). A hosted Codex lane takes the wake through its provider once the wake has that route; until then the wake refuses it as `wake-invalid`. A live Codex lane the wake refuses keeps its mail waiting by default, and reads it at its next tool call, a waiter poll's return included. Mail that cannot wait reaches a hosted Codex lane by ending its session: `lane-host stop --item [ITEM] --harness codex`, then the `lane-exited` wait, close and relaunch that the `usage-limit` handling in [oversee-events.md § Event kinds](oversee-events.md#event-kinds) sets out, under [oversee.md § Recovery relaunch](../workflows/oversee.md#recovery-relaunch) and never as a second session beside a running one. That relaunch resumes with no line, reported as `resume-lineless`, and takes its continuation line as that section says. The stop interrupts the turn in flight, so it is never the default. `lane-host stop` refuses a local lane, so a local Codex lane's mail waits. The guarded pane writer of KEN-1794 becomes the route that reaches a live lane without ending it; the Codex idle signal that would let the wake itself reach the lane stays with KEN-1659.
