...
|--------|---------------------|-------------------------|
| `working` | A turn in flight, or a pane that cannot be read as anything else: a frame scrolled up its own history reads `working` while it stays there, and so does a lane streaming its last message. | Send, and a lane still taking tool calls reads it at the next one, and a turn in flight reads it at its end. For a lane already idle, scroll the pane back to the bottom and read the state again; a live Codex lane is handled as [codex-runtime.md § Lane mailbox](codex-runtime.md#lane-mailbox) states. |
