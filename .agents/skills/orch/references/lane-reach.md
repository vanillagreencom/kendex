...
|--------|---------------------|-------------------------|
| `working` | A turn in flight, or a pane that cannot be read as anything else: a frame scrolled up its own history reads `working` while it stays there, and so does a lane streaming its last message. | Send, and a lane still taking tool calls reads it at the next one. Once the turn ends, a Claude Code or Pi lane's mailbox monitor wakes it. A Codex lane reads it at its turn end; for one already idle, scroll the pane back to the bottom and read the state again. |
