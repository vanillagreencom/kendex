...
**Compaction.** `open-terminal` and `oversee-succeed` use the launch settings in [`scripts/lib/lane-launch.sh`](../scripts/lib/lane-launch.sh), `LAUNCH_CHOICE_FLAGS`. Claude Code disables automatic compaction on models whose window the adapter names. A fleet launch onto an unnamed Claude window refuses as `launch-window-unknown`; succession refuses as `model-window-unknown`. A fleet `--cmd` launch on Claude or Codex must carry its required settings or it refuses as `launch-compaction-missing`. Pi disables automatic compaction through its settings file. [Context handoff](oversee-events.md#judgement-rules) defines the shared handoff rule.

Codex 0.157.1 defers normal compaction to its usable-window cap. The `body_after_prefix` scope uses the configured token limit directly, and the zero post-turn percentage disables the separate final-response trigger. The full-window cap still applies. The reported `token_count.info.model_context_window` equals that cap, including configured window overrides; the adapter uses that number without a second reserve. These semantics are verified in the versioned upstream [compaction checks](https://github.com/openai/codex/blob/rust-v0.157.1/codex-rs/core/src/session/context_window.rs) and [turn processing](https://github.com/openai/codex/blob/rust-v0.157.1/codex-rs/core/src/session/turn.rs). Automatic compaction remains possible if a response or tool result crosses the cap between handoff checks, if model compatibility changes, or during review-budget recovery. The frequency is unknown. Handoff checks cannot guarantee that Codex never compacts.
