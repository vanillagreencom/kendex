...
**Tracked issue creation.** A delegated session (lane) never creates a tracked issue. It records the proposal in its status file with the reached behavior and the reason. The overseer files through TPM (project-management). Only the overseer runs the direct-creation exceptions with their workflow-specified label sets: `plan-issues`, `start-new`, and the `merge-pr` rebundle.
