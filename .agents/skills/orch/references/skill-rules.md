...
1. **Round token.** Immediately before delegating, run `workflow-state new-round-id [ISSUE_ID] dev_round_id` for the `Round ID:` line and re-stamp `dev_delegated_at`. `dev_delegated_at` is the stall escalation's clock, and `round-recover` accepts a reported pass only from a validation run started at or after it, so a delegation that skips the re-stamp re-admits an earlier round's passing run. A fix round also runs `dev-round-write`, which records HEAD, items, the size report, and optional `Adds:` paths in an immutable round record under the worktree's `tmp/`. A missing or mismatched record requires a fresh round; never recreate one after delegation. A chosen cut uses `dev-round-write --cut`; [dev-round.md § Declared cuts](../schemas/dev-round.md#declared-cuts) owns its acceptance rule.
