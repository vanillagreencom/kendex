...
**Compaction.** Every command `open-terminal` and `oversee-succeed` build carries the words that turn the harness's own auto-compaction off, so a lane hands off at `ORCH_HANDOFF_CONTEXT_PCT` of its own window before its harness compacts it, and a fleet `--cmd` launch on claude or codex carries them inside its command or is refused as `launch-compaction-missing`. Claude Code takes `--settings={"env":{"DISABLE_AUTO_COMPACT":"1"}}` and Codex `-c model_auto_compact_token_limit=9223372036854775807`; Pi has no launch word, and its settings file turns compaction off. The words live in `scripts/lib/lane-launch.sh` beside the question-tool words.
