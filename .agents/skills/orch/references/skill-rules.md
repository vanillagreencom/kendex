...
**Tracked issue creation.** A delegated orchestration session (lane) never creates a tracked issue. TPM analysis from [audit-issues](../../project-management/workflows/audit-issues.md) § 2.1 / § 4.1 runs in a lane or delegated subagent, never in the overseer session. The lane materializes the audit JSON through § 4.2 and records its path, proposals, reached behavior and reasons in its launch brief's status file. The overseer checks duplicates across the fleet and applies its wider context to that JSON before presenting § 5 and running § 6 and § 7; project-order resumes at § 2.2. It returns issue IDs or decline reasons; the lane hands the step over with `lane-mail ask` and waits for that outcome, as above, before dependent replies or worktree removal, then resumes its caller. A sweep lane produces the same JSON and files nothing. [oversee.md](../workflows/oversee.md) owns the launch and status-file handoff. A non-delegated primary session routes creation through TPM, except for `plan-issues`, `start-new`, and the `merge-pr` rebundle, which it may run directly with their workflow-specified label sets.
