...
To stop the watch, write `stopped` into `[RUN_DIR]/watch.exit` with the harness file-write tool, then run the read. Where line 1 of `[RUN_DIR]/watch.log` reads `runner=systemd unit=[UNIT]`, stop that unit by its exact name with `.agents/skills/orch/scripts/lib/job-unit.sh stop [UNIT]`: exit 0 stopped it, exit 1 is a unit already ended, and any other exit is a failed stop to report with its `job-unit:` line. Where it reads `runner=setsid`, send `kill -TERM -- -[PID]` to the group the read proved. Then run the read again until it exits 1. Either stop ends the launch shell before it writes a status, so the mark written first is what every later check reads. When the first read exits 1, there is no watch to signal.
