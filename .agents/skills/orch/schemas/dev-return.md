...
| `validate` | Yes | `--validate` | `pass`, `no-verdict` or `FAILING: check1,check2` — a closed enumeration. With `--validate-run-dir`, `pass` needs a run that passed and `no-verdict` a run the timeout ended (`--record` reads `verdict=timeout`); `FAILING` is accepted beside any run, because it judges every gate of the round. `dev-artifact-check` accepts `pass` and `no-verdict` and retries `FAILING` |
...
`no-verdict` is a battery the timeout cut off: neither a pass nor a failure. The round's result is then the suites of the trees the diff touches, run once as `dev-implement.md` § 5 sets out, and CI is the full record; submit neither reuses nor re-runs it.
