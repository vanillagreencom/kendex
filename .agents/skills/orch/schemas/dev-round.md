...
The `Adds:` delegation line and `--adds` carry the same blank-separated path list. A blank or tab separates, so a path containing whitespace is read as two paths and cannot be authorized as one. The writer refuses the whole value when it is empty or blanks-only, when it contains a newline or carriage return, or when any path begins with `-`; it also rejects absolute paths, leading or trailing empty components, double slashes, `.` and `..` components, and duplicates. The reader refuses a recorded path beginning with `-` or carrying a space, tab, newline, carriage return, form feed or vertical tab — a path dev-round-write could not have produced. Omit the line and flag when no additions are allowed.
