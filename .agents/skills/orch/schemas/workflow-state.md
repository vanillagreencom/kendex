...
| `near_ceiling` | string[] | `byte-ceiling` `near-ceiling` lines from the last accepted dev return, each naming a file within reach of the byte ceiling. `dev-start.md` § Store Near-Ceiling Lines is the one writer, invoked by the implement and fix accept paths alike, and it REPLACES the list: state describes the branch as the last accepted round left it, never an accumulation across rounds. The next round's delegation renders one `Near-ceiling:` line per entry |
