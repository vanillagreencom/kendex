...
| `pr` | object | Pull-request size state. `size_check` initializes as null and holds the report `branch-size-check` records at launch, fix-round mint, cut acceptance and submission: `base_sha` and `head_sha`, the commits it compared; `production_lines`, `test_lines` and `mirror_lines`, the added lines in each part; `production_allowance` and `test_allowance` from the issue's optional `**Expected delta**` line, null where absent; the `verdict` (`pass`, `over`, `allowance_missing`) and its `reason`. Cut acceptance uses the comparison in [dev-round.md § Declared cuts](dev-round.md#declared-cuts). Every check overwrites the record. A reader binds it to a head by `head_sha`; review-gate's `pr-watch.sh` annotates its `disarmed` line with the record for the PR head and reports any other as stale |
