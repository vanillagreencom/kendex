...
| `merge_queue_watch` | object | Pointer to the lifecycle file owned by `merge-queue-watch`. `state_path`, `watch_id`, `repository`, `pr_number`, and `head_sha` bind workflow state to the prepared repository, PR, head, and generation. The lifecycle file lives under the shared git directory and records gate mode, recovery count, artifact/log/runtime paths, supervisor PID and deadline, producer diagnostics, claimed and resume actions, merge-pr completion, cleanup requirement and phase, and lane acknowledgment. Its terminal statuses are `failed`, `abandoned`, and `complete`; `awaiting_lane_postmerge` is not terminal |
