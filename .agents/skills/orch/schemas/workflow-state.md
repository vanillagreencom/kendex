...
| `round_prunes` | object | Round id to `{action, used_pct, mark_pct, bytes}`, one entry per round `round-prune` ran for: `action` is `pruned`, `below-mark` or `failed`, `bytes` what the prune freed, 0 unless pruned. `round-prune` is the one writer |
