...
| Context reading, `context.json` in a lane's mailbox and in the overseer mailbox `tmp/lane-mail/overseer/` | `lanes context` and `oversee-succeed`. The `lane-mail-check` turn-end hook writes it at every turn end it reads a transcript at | `lane_context_record` in `scripts/lib/lane-context.sh` | A lane's follows its lane mailbox, and each launch of the lane empties it. The overseer's is rewritten at each overseer turn end and removed where `oversee-watch` records a new overseer; `prune` never reaches it |
