...
- Every path of that directory whose name carries, as a whole token, the item of a lane whose record is not `done`: a `running` lane, and a `preparing` or `stopped` lane a relaunch resumes. With no fleet state, the item of each workflow state written within the retention.
...
Every other entry is a candidate by its age alone, whatever wrote it: a waiter or validation run directory, a review artifact named for no item, or a scratch directory such as `mutstab-diag/` that no script names.

### Item close-out

`workflow-state remove ITEM` is the one owner of what an item leaves in that directory once it ends. It removes every entry the prune would judge whose name carries the item as a whole token, whatever its age: the item's workflow state and lock, and the per-item files written beside them, such as `completion-summary-ITEM.md` or `dev-return-ITEM-ROUND.json`. The lane status file and the lane mailbox stay for their retention above, as do the keep list's named files. It prints `removed path=PATH` per path, and a path it cannot remove refuses as `remove-failed` naming it. A lane's own run calls it at [merge-pr.md § 5](../workflows/merge-pr.md#5-execute-the-merge) step 6, and `lane-close` calls it on the fleet's state directory at every close that records `done`; a `--keep-sandbox` close keeps the files a relaunch resumes into. A file with no item in its name is left to the prune's age. What a lane writes under its own worktree goes with the worktree.
