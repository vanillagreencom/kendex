...
- Every path of that directory whose name carries, as a whole token, the item of a lane whose record is not `done`: a `running` lane, and a `preparing` or `stopped` lane a relaunch resumes. With no fleet state, the item of each workflow state that stands.
