...
| `overseer` | object | The overseer session itself: `{server, pane, window, launch_line}`. `server` is the tmux server process id. `pane` and `window` identify the session in that server. `launch_line` is the whole successor command: lane prefix, harness, current model, effort, permission flags and brief. `oversee-watch` reuses the line only when both server and pane match. It rebuilds the line from `oversee-succeed --print-launch-line` for a manual replacement or an older record with no server. Watch recording always receives the current model and effort, even when `ORCH_OVERSEER_PREFERENCE` is set. Live self-succession uses that preference instead. `oversee-succeed` publishes the launched successor's server, pane, window and command before it closes the caller. `oversee-watch` hands the bound line back through a file on `overseer-dead` ([oversee-events.md § Event kinds](../references/oversee-events.md#event-kinds)) |
