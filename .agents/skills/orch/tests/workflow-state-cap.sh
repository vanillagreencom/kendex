...
[[ "$got" == "4" ]] && pass "REVIEW_MAX_EXTERNAL_ROUNDS defaults to 4 through the table" \
...
[[ "$got" == "below 3/4" ]] && pass "the external cap counts pr_comment_review.iterations" \
  || fail "the external cap counts pr_comment_review.iterations" "got=$got"
"$WS" --state-dir "$cd_sd" update KEN-CAP '.pr_comment_review.iterations = 4' >/dev/null
got="$("$WS" --state-dir "$cd_sd" cap REVIEW_MAX_EXTERNAL_ROUNDS --issue KEN-CAP)"
[[ "$got" == "at-cap 4/4" ]] && pass "the external cap reads at-cap once the budget is spent" \
  || fail "the external cap reads at-cap once the budget is spent" "got=$got"
...
[[ "$rc" -eq 2 ]] && pass "an unknown cap name is refused" || fail "an unknown cap name is refused" "rc=$rc"
rc=0; "$WS" --state-dir "$cd_sd" cap CI_FIX_MAX_CYCLES --issue KEN-CAP >/dev/null 2>&1 || rc=$?
[[ "$rc" -eq 2 ]] && pass "a cap with no state field refuses --issue" \
  || fail "a cap with no state field refuses --issue" "rc=$rc"
got="$(env -u CI_FIX_MAX_CYCLES "$WS" --state-dir "$cd_sd" cap CI_FIX_MAX_CYCLES --count 5)"
[[ "$got" == "below 5/6" ]] && pass "CI_FIX_MAX_CYCLES defaults to 6 through the table" \
  || fail "CI_FIX_MAX_CYCLES defaults to 6 through the table" "got=$got"
...
[[ "$got" == "below 5/6" ]] && pass "the table's default is what resolves where no setting overrides it" \
  || fail "the table's default is what resolves where no setting overrides it" "got=$got"
...
[[ -n "$refusal_roster" && "$refusal_roster" == "$help_roster" ]] \
  && pass "the refusal and the help name the same caps" \
  || fail "the refusal and the help name the same caps" "refusal=[$refusal_roster] help=[$help_roster]"
missing=""
while IFS= read -r setting; do
  [[ -n "$setting" ]] || continue
  "$WS" --state-dir "$cd_sd" cap "$setting" >/dev/null 2>&1 || missing="$missing $setting"
done <<<"$refusal_roster"
[[ -z "$missing" ]] && pass "every cap the roster names resolves through the table" \
  || fail "every cap the roster names resolves through the table" "unresolved:$missing"
...
[[ -z "$stray" ]] && pass "no orch script or workflow resolves a round cap outside the table" \
  || fail "no orch script or workflow resolves a round cap outside the table" "$stray"
...
[[ -n "$(cap_strays "$CTRL_WF")" ]] && pass "the stray check flags a workflow carrying its own cap default" \
  || fail "the stray check flags a workflow carrying its own cap default"
