...
- **Hand off a lane before it runs out.** On the § 1 tmux surface, at every § 4 event, read each live lane's context use:
  ...
  `lanes context` reports context use and account headroom. It sets `handoff_required` when the binding account bucket has headroom at or below `ORCH_HANDOFF_HEADROOM_PCT`. The default is `5`. At each event, tell every required lane to write its handoff at the next safe point, as for the context mark. Use § Talking to a lane until lane mail is installed; then send the directive through lane mail. The context mark is `CONTEXT_TOKENS` past about 500000. At either mark, wait for a safe point such as a landed merge or held PR. Never interrupt a round or an unpushed tree. The lane records the fixed shape below and exits. The watch reports `handoff [ISSUE_ID]` once. Relaunch the item with `open-terminal --relaunch` (§ 3), which resumes the newest matching harness session and uses the handoff record as durable recovery state.
...
- `window-gone`, or any lane whose session ended with no merged PR → inspect its worktree and PR state, then re-launch once with the same brief and `open-terminal --relaunch` (without the flag the existing worktree reads as another session's claim and the item is skipped). The launcher resumes the newest matching Claude Code, Codex, or Pi session. It starts from the normal brief only when no matching transcript exists. A second death is surfaced to the user, not retried.
...
**Resuming a dead or walled lane.** `open-terminal --relaunch` finds the newest transcript whose first user message names the item. It runs Claude Code with `--resume <id>`, Codex with `codex resume <id>`, or Pi with `--session <path>`. Send one message after launch to re-arm the lane's waiters. Claude Code transcripts are under `~/.claude-shared/projects/`. Codex transcripts are under `~/.codex/sessions/`. Pi transcripts are under its session directory.
