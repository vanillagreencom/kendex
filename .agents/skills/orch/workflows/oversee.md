...
| `unjudged` | Nothing gave a positive idle: a live Codex process, a harness process another user owns, or a host with no `/proc`, where the process read answers for no lane at all. | The turn may already have ended, so mail may never arrive. Reach it at the pane. |
