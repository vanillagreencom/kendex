...
2. Choose: `lanes pick --harness [HARNESS] --json`. The fewest live claims wins, then the most headroom on the binding bucket. Exit 3 means no lane qualifies, and nothing launches: read `lanes list`, wait only when its lanes are over the threshold, and report every `expired`, `unreachable`, `no_credentials`, `no_usage_data` or `error` lane to the operator. An `expired` lane is one whose token renewal failed; its `detail` names why.
