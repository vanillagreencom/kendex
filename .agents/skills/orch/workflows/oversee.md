...
4. Launch: the `handoff.md` § 2 `open-terminal` invocation plus `--lane [CONFIG_DIR]` from the picked record, the sized `--launch-flags` and `--state-dir [OVERSEE_STATE_DIR]` (§ Lane record), one item per launch. `open-terminal` reads the model and the effort out of those flags, or out of `--cmd` when they name either none, and refuses the launch that names either nowhere. It then judges the named lane on that model a second time, so a lane picked without `--model` can be refused here. A hosted `--relaunch` is judged on that window too; only an account nothing measured turns on the provider's answer, per `open-terminal --help` § `--host`. It still names a model and an effort. The gate emits six refusals, each with its own answer:
   ...
   - `lane-model-walled` (naming `lane`, `model`, `pct` and `max-pct`): that account's window for the model is at or above the threshold. Re-run step 3 with `--model`, never retry the same lane. A hosted relaunch meets it too: the window belongs to the account, so resuming would spend the sandbox start to open on a usage banner.
