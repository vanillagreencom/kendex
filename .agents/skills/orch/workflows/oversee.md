...
A dead or walled terminal lane uses native resume. Start with the `handoff.md` § 2 terminal command. Add `--relaunch`, the selected `--lane`, the chosen `--launch-flags` and `--state-dir [OVERSEE_STATE_DIR]`. Keep the tracker, repository, harness, and item arguments. Do not pass `--cmd`, because a custom command bypasses session lookup. A record carrying `host` adds `--host [HOST]`: the provider keeps its tree and the harness continues natively. A hosted relaunch runs on the copy the provider put on the host. Where the provider reports holding that account, the launcher does not judge the relaunch on the usage windows this machine reads and an account reading `expired` here does not stop it; the launcher reports `host-relaunch-credential` and the resumed session reports its own usage banner, which § 4 reads as `usage-limit`. Where the provider reports holding no such account, or implements no `accounts` verb, the relaunch keeps that usage gate, because a provider re-seeding the host from the account's config directory at every `create` sends this machine's copy again. A fresh hosted launch is judged on this machine's reading either way. Where the copy here has expired and cannot be renewed while the provider reports holding the same account, the refusal is `host-credential-dead` and the remedy is to log in again on this machine for that config directory. The launcher delivers the continuation line in the resumed command itself and keeps a merged item's tree as it stands, so nothing is pasted into the pane after the resume. A hosted codex lane is the exception: `codex resume` refuses a prompt beside `--last`, so it resumes with no line and the launcher reports `resume-lineless`. Paste that lane's continuation line into its pane through § Talking to a lane, Pane paste, the way a walled lane gets its nudge.
