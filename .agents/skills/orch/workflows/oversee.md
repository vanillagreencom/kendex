...
The watch reads bare window names in the tmux session of the pane it started from, resolved once at start, and refuses one as `session-unresolved` where tmux names no session. A hosted lane while `lane-host resolve` answers `local` refuses as `hosted-without-host`: set `ORCH_LANE_HOST`. The repeat loop records its own pid in `oversee-watch.pid` beside `[OVERSEE_STATE]`; to stop the watch, kill that pid, never the `$!` of the shell that started it. A second start on the same state refuses as `watch-running` naming the live pid. After a self-succession `oversee-succeed` has already restarted the watch from the successor pane, and the successor's own start from that pane takes it over and first prints what it reported in between, under a `watch-adopted` note.
