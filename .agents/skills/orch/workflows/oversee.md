...
4. Launch: the `handoff.md` § 2 `open-terminal` invocation plus `--lane [CONFIG_DIR]` from the picked record and the sized `--launch-flags`, one item per launch. `open-terminal` reads the model out of those flags and judges the named lane on it a second time, so a lane picked without `--model` can be refused here as `lane-model-walled` (it names `lane`, `model`, `pct` and `max-pct`) or as `lane-model-unreadable` when no window measures that model. The answer to either is to re-run step 3 with `--model`, never to retry the same lane.
