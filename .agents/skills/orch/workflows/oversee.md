...
3. Neither → no parallel surface. Say so once and work the queue sequentially in this session, running each item's § 3 Item Tier brief: `start [ISSUE_ID]`, or [micro.md](micro.md) in this session for a `micro` item, with § 2 selection between items. On a hosted fleet (§ 3 Placement) this session runs no item, `micro` included: report the queue once, naming the tmux surface as the route every item takes to a hosted lane.
...
A `micro` item launches as a lane like any other, sized under § Lane directive step 2 at the simplest complexity it names, and always with `--cmd`. On a hosted fleet (§ Placement) it launches only that way, waiting for a free lane, because the overseer's host is the control VM and runs nothing [references/control-host-toolchain.conf](../references/control-host-toolchain.conf) lists. Otherwise, with no lane free, or on the § 1 no-parallel surface, the overseer runs [micro.md](micro.md) itself in this session from the main checkout; that run holds the checkout on the item's branch until § 3 there returns it to the base, so start it between events and never beside another read of the base checkout.
...
Placement: before each launch, read `lane-host resolve`; any value but `local` makes a hosted fleet. There every launch this directive makes adds `--host [HOST]`, and any other surface or harness is reported, never launched locally. `start` never launches a hosted lane: only `oversee` and `handoff` launch through `open-terminal --host`. The overseer therefore never runs `/orch start [ISSUE_ID]` or `/orch micro [ISSUE_ID]` in its own session on a hosted fleet; every item launches as a hosted lane through this directive. The credential reaches the sandbox per [schemas/lane-host.md](../schemas/lane-host.md) § Provider protocol, with no local `CLAUDE_CONFIG_DIR` prefix.
