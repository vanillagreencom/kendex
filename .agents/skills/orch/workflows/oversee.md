...
- **Lane proposals.** Batch pending `Proposal:` comments from the fleet's Linear issues. When one fleet issue has no live lane and its worktree is released, use it only as the carrier for one TPM lane. Launch that lane with `open-terminal --cmd`, the project-management [proposal-sweep](../../project-management/workflows/proposal-sweep.md) brief, and launch flags for a low-cost model that can follow the TPM schema. The brief supplies the carrier worktree, fleet issue IDs, resolved comment IDs, and status file. The sweep changes no repository file and creates no issue. For a hosted carrier, first run `.agents/skills/orch/scripts/lane-host touch --item [CARRIER_ID]`; stop the sweep if the probe fails. Read the audit path with `.agents/skills/orch/scripts/lane-host cat --item [CARRIER_ID] [STATUS_FILE]`, then transfer that audit with `.agents/skills/orch/scripts/lane-host cat --item [CARRIER_ID] [REMOTE_AUDIT_PATH]`. Stop if either read fails. Write the exact audit output to `tmp/proposal-audit-[CARRIER_ID].json` in the overseer's checkout with the harness file-write tool. For a local carrier, read the local status file and copy its named audit to the same overseer-local path. Read no verification text. Check the local analyzed rows against live fleet work, then run [audit-issues](../../project-management/workflows/audit-issues.md) §§ 5-7 with only `--analyzed tmp/proposal-audit-[CARRIER_ID].json` under `PM_CREATE_AUTONOMY`. Reply on each source comment with `.agents/skills/linear/scripts/linear.sh comments create [ISSUE_ID] --parent [COMMENT_ID] --body-file [PATH]`. The reply carries the created issue ID or the one-line decline reason. Record the outcome and comment ID in the fleet log.
- **Heartbeat backstop for lane proposals.** Include every fleet issue in the next proposal sweep at each `heartbeat` and before fleet close. This catches a comment posted without another event. Never launch the sweep on an issue whose owning lane still holds its worktree.
