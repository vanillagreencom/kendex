...
Per item, mint the brief for the tier § Item Tier assigned: `/orch start [ISSUE_ID]` at `standard`, `/orch micro [ISSUE_ID]` at `micro` (or the `github [OWNER/REPO]#[N]` spelling of either). The brief also carries question routing: "Every question for the overseer goes through `.agents/skills/orch/scripts/lane-mail` in this worktree, per [skill-rules.md § Coordination](../references/skill-rules.md#coordination): `lane-mail ask --item [ISSUE_ID] --file [PATH]`, then `lane-mail wait` on the printed id. Never use your harness's question tool. As your first step, arm the mailbox monitor `lane-mail watch --item [ISSUE_ID]` per [watch-delivery.md § Lane mailbox monitor](../references/watch-delivery.md#lane-mailbox-monitor), and re-arm it at each expiry. Run the `lane-mail inbox` command each of its announcements prints, and read `lane-mail inbox --item [ISSUE_ID]` at every wait point." `/orch` slash syntax does nothing in Codex: a Codex CLI lane uses the form open-terminal renders — `Read .agents/skills/orch/SKILL.md and execute the orch start workflow for [ITEM]` — and a Codex Desktop thread uses `$orch start [ITEM]` (`handoff.md` § 2); a pi lane takes `/skill:orch start [ITEM]`, and an opencode lane the `/orch` form. At the `micro` tier each of those reads `micro` where it reads `start`. A `micro` item therefore always launches with `--cmd` carrying its brief: `open-terminal`'s own template renders `start` for every tracker and harness pair it handles, so a launch without `--cmd` runs the standard cycle whatever the tier said. Size launch flags to the item, then launch on the § 1 surface.
...
Text crosses `--file` ([SKILL.md](../SKILL.md) § Harness-Safe Shell). A directive answers no ask and `--halt` in place of `--directive` halts the lane; the Lane mail rule in [skill-rules.md](../references/skill-rules.md) says when each reaches it. The send's receipt says whether the lane's mailbox monitor stands ([watch-delivery.md § Lane mailbox monitor](../references/watch-delivery.md#lane-mailbox-monitor)): on `monitor=live` the monitor wakes the lane and the send is the whole step. On `monitor=none`, wake the lane after the send: a Codex lane arms no monitor ([codex-runtime.md § Lane mailbox](../references/codex-runtime.md#lane-mailbox)), and a Claude Code or Pi lane reads `none` when its monitor is not running. An OpenCode lane reads the mail at its `lane-mail inbox` wait points, and the wake does not take it. Keep the lane's tracker, repository, harness, item, `--lane` and `--launch-flags` arguments. The wake resumes the lane's own session with one line that runs `lane-mail inbox`, and reaches only lanes on this host, run from a checkout of the lane's own repository when its `mail_root` is another repository's worktree, because the wake resolves the lane's tree from the caller's own project; it wakes a lane only when the shared judge calls it `idle`, and refuses any other state as `wake-refused reason=[STATE]`. [lane-reach.md § Wake refusals](../references/lane-reach.md#wake-refusals) says what each reason licenses. Never send a lane text by keystroke.
...
**A refusal is a state, not a remedy.** Mail reaches a lane only through its mailbox monitor and the hooks the Lane mail rule in [skill-rules.md](../references/skill-rules.md) names, so a halt or an answer to a lane with no monitor that the wake refuses lands only while that lane still takes a tool call or ends a turn. Send where the reason allows it. A walled lane runs no turn, so neither its monitor nor a wake reaches it: it takes its continuation nudge by the paste below at the reset, or relaunches under § Recovery relaunch.
