...
A run that ends at [micro.md](micro.md) § Escape or [small.md](small.md) § Escape comes back as an item to launch at the next class up, on the branch it left: run `item-tier` again with `--floor` naming the class above the one the run held, plus `--base origin/[BASE_BRANCH] --head origin/[BRANCH]` once the pushed branch is fetched. So does a run that stops without reaching an escape, a failed push or create among the causes. Read a micro run's § 5 `Checkout` value: a main checkout still on the item's branch is returned to the base before the next launch.
