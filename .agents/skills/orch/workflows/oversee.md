...
Use the pane tail that `oversee-watch` prints as the lane state. It already contains at most the last 40 non-empty lines. Do not capture the pane again when that tail answers the event. When a fresh local pane read is necessary, retain only its last 40 non-empty lines before the overseer reads it. For a hosted lane, probe the host with `lane-host touch`, then read its bounded status file with `lane-host cat --item [ISSUE_ID] [STATUS_FILE]`. A failed probe or read stops the event. Never read a lane transcript.

### Bounded issue reads

For each `triage` or `heartbeat` pass, replace `[AGE]` with an `Nd` value that covers the fleet start and run these commands in order. The first command redirects the complete response to ignored scratch storage, so no issue body enters the overseer. The second command is the only issue-list result the overseer reads. It emits each identifier, title, and `## Done when` body. The last command removes the complete response.
...
- `triage` → team-wide: run § Bounded issue reads, then select the emitted identifiers. Give the issue identifiers to one low-cost verifier subagent. It reads the full issues and source, applies the filing bar regardless of lane authorship, posts its full evidence to Linear, and returns the bounded rows from [skill-rules.md § Coordination](../references/skill-rules.md#coordination). Route each kept item or cancel it from those verdicts, then record a kept or canceled verdict in the fleet state's `triaged` list before re-running the watch. An emitted item is never left without that record. triaged is the verdict log, not the watcher baseline. The watcher rebuilds acknowledged triage keys from it in the first repository's OVERSEE_WATCH_STATE_DIR baseline; the default directory is `<project-root>/tmp/oversee-watch`. Deleting that baseline resets watcher dedup: the next pass rebuilds kept and canceled keys, while pending items emit again.
