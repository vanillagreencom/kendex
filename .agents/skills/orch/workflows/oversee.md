...
The watch reads bare window names in the tmux session of the pane it started from, resolved once at start, and refuses one as `session-unresolved` where tmux names no session. A hosted lane while `lane-host resolve` answers `local` refuses as `hosted-without-host`: set `ORCH_LANE_HOST`. The repeat loop records its own pid in `oversee-watch.pid` beside `[OVERSEE_STATE]`; to stop the watch, kill that pid, never the `$!` of the shell that started it, and the loop stops the pass it is running. A second start on the same state refuses as `watch-running` naming the live pid while that watch serves a pane tmux still lists. After a self-succession `oversee-succeed` has had the watch restarted from the successor pane, and the successor's own start from that pane takes it over under a `watch-taken-over` note, as it does a watch whose pane is gone. Every start first prints, under a `watch-replayed` note, what a restarted watch reported where no session read it and how the restart went, so handle those lines as events too.
