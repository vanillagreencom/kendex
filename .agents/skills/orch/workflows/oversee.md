...
- **Lane proposals.** Record each lane's `Proposal comment:` notice in the fleet log as an authorized tracker, repository, issue, and returned comment ID or URL binding. Partition pending authorized comments by tracker and source PR repository. Use one fleet issue from that repository only after its owning lane returns and releases its worktree as the carrier for one batch. Launch the low-cost TPM [proposal-sweep](../../project-management/workflows/proposal-sweep.md) lane with `open-terminal --cmd`; its brief supplies the batch tracker, tracker repository, source repository, issue IDs, authorized and resolved comment bindings, status file, and mailbox. Add the carrier to `oversee-watch` and wait for its `lane-notice` whose text is exactly `Proposal sweep complete: [STATUS_FILE]`; read no output file before that durable event. For a hosted carrier, probe with `.agents/skills/orch/scripts/lane-host touch --item [CARRIER_ID]`, then use its `cat` verb to transfer the status and audit. Stop on any failure. Put the audit bytes at `tmp/proposal-audit-[CARRIER_ID].json` in the overseer's checkout; for a local carrier, copy the named audit there. Read no verification text. Check the local rows against live fleet work, then invoke [audit-issues](../../project-management/workflows/audit-issues.md) from § 1 as `audit-issues --analyzed tmp/proposal-audit-[CARRIER_ID].json` under `PM_CREATE_AUTONOMY`. Its normal route parses the tracker, runs its tracker preflight and label inventory, skips TPM analysis, and continues through §§ 5-7. For Linear, reply with `.agents/skills/linear/scripts/linear.sh comments create [ISSUE_ID] --parent [COMMENT_ID] --body-file [PATH]`. For GitHub, use `gh issue comment [NUMBER] --repo [OWNER/REPO] --body-file [PATH]`; its first line is `Proposal outcome for [COMMENT_URL]:`. Each outcome carries the created issue ID or one-line decline. Record the source and outcome in the fleet log. The completion notice releases lane ownership. After transfer, close a hosted carrier with `.agents/skills/orch/scripts/lane-host close --item [CARRIER_ID]` or remove a local carrier with `.agents/skills/worktree/scripts/worktree remove [CARRIER_ID]`. A failed release blocks reuse; record success before reuse.
