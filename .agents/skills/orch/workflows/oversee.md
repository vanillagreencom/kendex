...
The watch runs two passes on one clock. The mail pass starts every `ORCH_WATCH_MAIL_INTERVAL` seconds (default 20) and reads every mailbox and the lane records one after another, printing what it finds as it finds it. The long pass, holding pr-watch, the merged check, triage and the pane reads, runs every `--interval` seconds and makes every GitHub call but the two named below; one that overruns holds up no mail pass. `--repeat` waits at most the mail interval between runs, except after a run that exited 2: the first report of a lane read failure, a hosted lane close that fails (each long pass until it succeeds), or a global failure. Outside those, a lane's note is read within one mail interval, whatever `--interval` is, with these exceptions. A read that waits on a lock or a slow host delays the mailboxes after it. The run's one GitHub auth check before its first long pass, and the heartbeat's open-PR listing at a quiet run's end, run in the loop itself and delay the next mail pass while they wait. The overseer's own pane is read before every mail pass (off tmux there is none): while it reads exited, or walled on a wall its account confirms, no mailbox is read, and a successor finds the mail; a wall refuted or unjudged reads live, and a pane that cannot be read while a long pass is in flight, as in a succession, holds the mail until that pass ends. The mail pass never reads a lane's pane, so it runs on every surface and outside tmux.
