...
- `usage-limit-passed` → the wall named by `resets=` has already lifted and the lane is still sitting under the old banner. Send it a one-line continuation nudge; do not move the item and do not wait. The account is spendable again, so it is also eligible for a fresh launch.
