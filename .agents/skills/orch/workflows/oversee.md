...
First in every session, before the handoff file, read that mailbox: `.agents/skills/orch/scripts/lane-mail inbox --item overseer` prints every note no reader has taken yet and moves the mailbox's own cursor past them. Act on each or record it in the fleet log. The § 4 watch reads through the same cursor, so it does not report them again. `lane-mail pending --item overseer` takes nothing: it lists the owner and peer directives no reader has taken and the asks this overseer sent that no peer has answered. An unread peer ask or answer shows only through `inbox`.
...
The watch runs two passes on one clock. The mail pass reads every mailbox and the lane records every `ORCH_WATCH_MAIL_INTERVAL` seconds (default 20), printing what it finds as it finds it. The long pass, holding pr-watch, the merged check, triage and the pane reads, runs every `--interval` seconds and is the one that spends GitHub quota; one that overruns holds up no mail pass. `--repeat` waits at most the mail interval between runs, except after a run that exited 2: the first report of a lane read failure, a hosted lane close that fails (each long pass until it succeeds), or a global failure. Outside those, a lane's note is read within one mail interval, whatever `--interval` is. The exception is the overseer's own pane, read before every mail pass (off tmux there is none): while it reads exited, or walled on a wall its account confirms, no mailbox is read, and a successor finds the mail; a wall refuted or unjudged reads live. The mail pass never reads a lane's pane, so it runs on every surface and outside tmux.
