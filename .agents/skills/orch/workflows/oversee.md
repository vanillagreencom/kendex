...
The owner sends the overseer a note without typing into its pane: `.agents/skills/orch/scripts/lane-mail send --item overseer --directive --file [PATH]`, run from any checkout of the project, appends it to `tmp/lane-mail/overseer/to-lane.jsonl` in the project's main checkout, and the § 4 watch reports it once as `owner-note`.
...
- **Hand off a lane before it runs out.** On the § 1 tmux surface, at every § 4 event, read each live lane's context use:
  ...
  `lanes context` reports context use and account headroom. It sets `handoff_required` when the binding account bucket has headroom at or below `ORCH_HANDOFF_HEADROOM_PCT`. The default is `5`. At each event, tell every required lane to write its handoff at the next safe point, as for the context mark. Use § Talking to a lane until lane mail is installed; then send the directive through lane mail. The context mark is `CONTEXT_TOKENS` past about 500000. At either mark, wait for a safe point such as a landed merge or held PR. Never interrupt a round or an unpushed tree. The lane records the fixed shape below and exits. The watch reports `handoff [ISSUE_ID]` once. Relaunch the item through § 3 Recovery relaunch, which uses the handoff record as durable recovery state.
...
- `lane-closed` → the watch closed the merged hosted lane's sandbox: record the `kept=` line that follows in the fleet log.
- `lane-close-refused` → the merged hosted lane's remote worktree is dirty and its sandbox stays: surface the `dirty=` detail that follows to the user. Never run `lane-host close --force`.
- `usage-limit` → read the pane tail first and confirm the banner is the harness's own, not text the lane printed while working; a lane still working is left alone. Confirmed: keep the window and its watch registration while selecting a replacement. Run `lanes pick` for the same harness first. When a lane qualifies, kill the old window and use § 3 Recovery relaunch on the selected lane. When no lane of that harness qualifies, pick another terminal harness. When that lane qualifies, kill the old window and launch with the one-sentence continuation brief from `handoff.md` § 2, including the matching path from the prior harness's transcript store. When no replacement qualifies and the event has `resets=`, keep the old window and wait for that reset. The window then announces `usage-limit-passed`, and the overseer sends its one-line continuation nudge. The event's `resets=` is that reset in UTC. A banner naming a date states it outright. A banner naming only a clock or a weekday states no day, so `resets=` is the first occurrence after this watch's own first sighting of that banner. This is an upper bound. It can be a day later than the truth for a clock or a week later for a weekday when the watch did not see the wall start. Nothing on the event separates the two, so read the tail. Without `resets=` the banner either named no reset in a readable shape or named a time zone this host cannot resolve. Never launch onto the spent account again until its reset, and never park a lane against an estimate of your own when `resets=` states the time. A parked lane stays in the watch: its window is what announces the reset, as `usage-limit-passed`. A wall reported without `resets=` is announced once and nothing announces it again, so select another account on that event. If none qualifies, report the blocked recovery and keep the old window until a replacement qualifies.
...
- `idle-after-return` → read its pane tail. A lane that has returned is available for another fleet launch; a lane still inside `merge-pr.md` § 5 step 1 is blocked on its queue wait, which reads as work in flight rather than idle. A directive for an idle lane is sent and then woken per § Talking to a lane.
...
- `window-gone`, or any lane whose session ended with no merged PR → inspect its worktree and PR state, then re-launch once with the same brief through § 3 Recovery relaunch. Without `--relaunch`, the existing worktree reads as another session's claim and the item is skipped. The launcher starts from the normal brief only when no matching transcript exists. A second death is surfaced to the user, not retried.
...
- `owner-note` → the owner's note to the overseer; its text follows. Every text and `options:` line under `lane-question`, `lane-notice` and `owner-note` is indented two spaces, so only a watch record begins with `EVENT`. Act on it, or record it in the fleet log as a ruling; it needs no reply.
...
**Resuming a dead or walled lane.** Use § 3 Recovery relaunch. `open-terminal` finds the newest transcript whose harness kickoff names the item. It runs Claude Code with `--resume <id>`, Codex with `codex resume <id>`, or Pi with `--session <path>`. Send one message after launch to re-arm the lane's waiters. Claude Code transcripts are under `~/.claude-shared/projects/`. Codex transcripts are under `~/.codex/sessions/`. Pi transcripts are under its session directory.
