...
- `triage` → read the named item and apply the filing bar under **Triage what lanes file**: cancel it with that reason or route it to the right lane. Record the verdict in the fleet state's `triaged` list before re-running the watch.
