...
4. Launch on the § 1 surface, one item per launch so each launch matches its sizing. On the tmux surface: `open-terminal --lane [CONFIG_DIR]` with the picked record's `config_dir` and the sized `--launch-flags`. On surface 2: the harness's own session or thread launcher, carrying the picked lane's config dir only where that launcher accepts one.
