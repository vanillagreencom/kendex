...
When that table produces a terminal failed-validation report, rebuild the PR body per [`submit-pr.md` § 2](submit-pr.md) step 3 and post it with `pr-edit-body` before returning the report to any caller. Then return without entering the remaining triage sections. This step is the single owner for managed callers, including submission and late-findings merge paths.
