...
**Recurrence before the cap.** A finding sharing a root cause with one a prior round patched is dispositioned by [finding-disposition.md § Recurrence](../references/finding-disposition.md#recurrence), and no further patch round answers it. Check it here, ahead of the cap below. A cause this diff introduces or arms takes `structural-close`, whose item names the generating surface in place of the site and goes to the fix round below. A cause it neither introduces nor arms takes neither that nor the comment loop's `freeze`: record it with the At The Cap write below, its `reason` reading `recurrence outside this diff's authorship` in place of the cap's, so § 8 files it rather than re-deriving it as declined. A finding sharing a cause in `patched_causes` is the recurrence this rule ends, and one sharing a cause in `frozen_causes` is declined without re-triaging:
