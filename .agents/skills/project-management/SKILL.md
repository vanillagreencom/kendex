...
- **Name what reaches it.** Every issue carries a `Reached by:` line giving the user action, run, check, or shipped producer that arrives at the defect; an owner-directed item names the ask. The thread a finding came from, or a shape ("a name containing a quote"), is not a reach — `linear.sh issues create` refuses one under `LINEAR_REQUIRE_REACH`, and an item with nothing to name is a decline, not an issue. A review-born filing at priority 2 also carries a `Symptom:` line naming the run, the user, or the red check that already showed the defect (`--review-born`); priority 2 minted structurally by a planner, a roadmap layer, the merge-pr rebundle or a research spike carries no symptom and is not checked for one. Where a review-born finding files at all is [`../orch/references/finding-disposition.md`](../orch/references/finding-disposition.md) § Filing bar.
