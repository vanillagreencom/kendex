...
- **Name what reaches it.** Every issue carries a `Reached by:` line giving the user action, run, check, or shipped producer that arrives at the defect; an owner-directed item names the ask. A review thread, a reviewer, or a shape ("a name containing a quote") is not a reach — `linear.sh issues create` refuses one under `LINEAR_REQUIRE_REACH`, and an item with nothing to name is a decline, not an issue. Priority 2 is the reported tier and needs a `Symptom:` line naming the run, the user, or the red check that already showed the defect; where a review-born finding files at all is [`../orch/references/finding-disposition.md`](../orch/references/finding-disposition.md) § Filing bar.
