...
- The Linear cache holds the whole workspace: `sync` sends no team filter, and no `cache issues list` formatter returns a team (`--team X` filters, but a returned row cannot be checked against it). Team scope per path: § Scope by Path.
