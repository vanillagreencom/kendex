...
`REVIEW_GATE_CLASS_POLICY` is active by default: `review-policy` holds the default value, and an explicit empty assignment is the one opt-out. `validate.sh` refuses that opt-out unless `REVIEW_GATE_CLASS_POLICY_DECISION` names a tracked decision record, so an inactive policy is always a recorded choice and never an inherited default. The review-gate suites that pin the evidence engine or a legacy lane assign the key empty, because those lanes run only under an inactive policy.
