...
- `scripts/provision-environment.sh`: the organization owner's write half of the standard's environment. From the owner's own machine, never a lane or CI, it creates or corrects the environment, its default-branch-only policy and its secrets in every repository of an organization, and reports one record per repository. `--dry-run` writes nothing. `--help`
