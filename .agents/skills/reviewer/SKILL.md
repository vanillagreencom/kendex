...
- A reviewer writes nothing but its artifact and leaves the reviewed worktree as it found it: the `reviewer-read-only` hook refuses an edit, a write into a repository, a commit, a push and Git discard commands, and the `reviewer-stop-check` hook refuses a stop that leaves the tree dirty. Another revision is measured in a copy, `git worktree add` to a temporary path or `git show REV:PATH` redirected to a file outside the worktree, never by writing its bytes into the shared tree and restoring them by hand: the worktree is shared with the parallel review panel and the dev round, so the window belongs to their measurements as much as to this reviewer's.
