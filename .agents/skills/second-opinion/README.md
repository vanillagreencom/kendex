...
| `SECOND_OPINION_TIMEOUT` | `"1080"` | Seconds to wait per external CLI invocation; a review's one retry on a malformed response can double the total. Above the ~600s a harness allows a foreground shell call: background the run, or pass `--timeout` at or below the harness ceiling |
