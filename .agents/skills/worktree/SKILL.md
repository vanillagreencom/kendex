...
The layout table is data, one row per ecosystem, in `scripts/worktree-output-prune`. A row names the marker file that identifies the ecosystem, the lock files one of which must be in scope for it — beside the marker or in any enclosing directory, so a workspace writing its lock once at the root has every package under it reclaimed — and the output directories it owns with the lock file (or absence of one) that marks an independently prunable unit inside each. Covering a further ecosystem is a new row there and no other change. A marker is looked for below the worktree root as well as at it, so a manifest kept in a subdirectory (this repository installs its UI with `npm ci --prefix ui`) has its output reclaimed too. The walk that finds them descends into neither build output nor any dot-prefixed directory, `.git` among them, and follows no symlink, so a project hidden under a dotted directory keeps its output.

A Cargo profile is pruned under its own `.cargo-lock`, held from before the live-build check until after the delete, so no build can start in it meanwhile. An output with no lock file — `node_modules`, `.next` — has nothing to hold, so its live-build refusal is a point-in-time scan of running processes, taken once during inspection and again immediately before the delete. That narrows the window to the gap between the second scan and the first unlink; it cannot close it while no observable lock exists, and a package manager starting inside that gap or during a multi-second delete is not seen. Run `--apply` when no install is expected, or leave the worktree claimed, which refuses it outright.
