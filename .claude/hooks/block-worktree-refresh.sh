...
# segments and masks what the shell would not run. The read-only options are
# checked against its argument view, which drops redirection targets. The
# global scope is not this hook's, and `-g`, `--global` or
...
while IFS= read -r SEGMENT; do
  ...
  command_arguments "$TAIL"
  if [[ $COMMAND_ARGUMENTS =~ $READ_ONLY_RE ]]; then
    continue
  fi
