...
# plain `commit` word after a plain `git` word is a commit wherever it stands,
# a message, a heredoc body and a comment tail included, while a quoted
# `commit` is spared because the quote is still in the word. Git's armed hooks
# are the judge; this hook only decides whether to defer to them.
set -f
IFS=$' \t\n\r'
