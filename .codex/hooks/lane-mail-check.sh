...
# the install has its own line elsewhere.
ask_route() {
  local asker=lane-mail
  ! resolve_reader || asker="$READER"
  printf -v ASK_ROUTE '  %q ask --item %q --file [PATH]\n  %q wait --item %q --id [MSGID]' \
    "$asker" "$ITEM" "$asker" "$ITEM"
}
