...
# --no-verify or a cluster holding -n is the bypass. Nothing is rewritten and
# nothing is deleted, so a word is seen only where the command already spells
# it: a bypass the shell would join, unquote, expand or separate into a word is
# not seen here and reaches git, which skips its armed hooks. Rewrites that
# split on bash's metacharacters, or dropped a quote, a backslash, a line
# continuation or a brace, answered the shapes a scanner can always be shown
# one more of, at the cost of refusing read-only commands whose text held these
# words, and they are gone. That is the frozen lexical-scanner class: a finding
# of that shape against this file is declined, not patched.
#
# The reading runs the other way too: each of those three words counts wherever
# it stands, a message, a heredoc body and a comment tail included; which form
# falls where is pinned in the suite. Git's armed hooks are the judge; this hook
# only decides whether to defer to them.
set -f
IFS=$' \t\n\r'
