...
- `skill-tests.yml` is the suite runner [`../skills/AGENTS.md`](../skills/AGENTS.md) names; it carries the `tools/` and `hooks/` suites and the Bash 3.2 legs as well. The `Preflight (diff-scoped deterministic checks)` and `Markdown (format + references)` jobs are required checks on pull requests and skip merge groups.
