...
- `skill-tests.yml` is the suite runner [`../skills/AGENTS.md`](../skills/AGENTS.md) names; it carries the `tools/` and `hooks/` suites and the Bash 3.2 legs as well. Its docs-only classifier can skip shell and cargo lanes. The aggregators keep the required contexts present. The `Preflight (diff-scoped deterministic checks)` and `Markdown (format + references)` jobs are required checks on pull requests and skip merge groups.
