...
# the write only when its command tail contains no `<` or `>` character, so a
# redirection cannot make its operand look like an option. The global scope is
# not this hook's, and `-g`, `--global` or
...
while IFS= read -r SEGMENT; do
  ...
  if [[ $TAIL != *'<'* && $TAIL != *'>'* && $TAIL =~ $READ_ONLY_RE ]]; then
    continue
  fi
