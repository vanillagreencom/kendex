...
# --no-verify or a cluster holding -n is the bypass. Whole words: `'commit'` is
# not `commit`, so an ordinary `git log | grep 'commit'` is no commit at all.
#
# Nothing here models quoting, heredocs or comments. Every round that tried
# named one more construct and opened the next hole, and every scanner in this
# class has been deleted after it. The character rewrites that answered a quote
# inside the git word went the same way, once they started refusing read-only
# commands whose text happened to hold these words. A finding of that shape
# against this file is declined, not patched. The trade is stated in the
# frontmatter and runs both ways: a bypass spelled in a commit message is
# refused as the flag, and a bypass the shell assembles out of fragments is not
# seen. Git's armed hooks are the judge; this hook only decides whether to
# defer to them.
set -f
IFS=$' \t\n\r'
