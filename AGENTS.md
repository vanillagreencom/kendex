...
- **Git hooks come from the skill, not the CLI.** After a project-scope `add`/`refresh` has written its items, an installed growth-guards skill has its `scripts/install-git-hooks` run against the project so `.git/hooks/pre-commit` and `commit-msg` stay armed and repaired; `vstack remove` runs `--uninstall` before the skill's files go and REFUSES the removal when that cleanup fails, because the shims fail closed. `core.hooksPath` is never set; a non-git project is skipped with a note, and an installer failure is a warning, never an install failure. `vstack check` relays the installer's read-only `--check` verdict (`cli/src/git_hooks.rs::check_growth_guards_hooks`): a drifted, absent, or dormant-behind-`core.hooksPath` shim is drift with the checker's own line as the remedy, and a verdict that could not be measured is drift too, never a pass.
...
`harnesses:` accepts a YAML list or comma-separated string. Use it for hooks whose wire format or event has no parallel in another harness, or that a harness has no implementation for — a hook the `pi-hooks` package does not port must exclude `pi`, or Pi reports enforcement it does not have.
...
- **Three-file doc contract.** A nested `README.md` is the consumer's page — overview, feature list, setup and config need-to-know — bounded at 120 lines by the `*/README.md=120` size-ratchet class: tighten-only, so growth past 120 blocks and a frozen baseline row is reserved for vendored upstream content (the repo-root `README.md` is the product landing page and keeps the default class).
