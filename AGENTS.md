...
- **`vstack check` is the drift contract.** Exit `0` clean, `1` drift, `2` the check failed; `--quiet` prints nothing when clean, `--json` goes to stdout, `--offline` skips the CLI version lookup and the (six-hour TTL) source cache fetch. The `session-drift-check` hook and the Pi `pi-hooks` `sessionDriftCheck` setting are thin adapters over `check --quiet` — change the report in `cli/src/commands/check.rs`, not in the adapters.
