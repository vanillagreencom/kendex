...
Agreed shape for drovr, memsira, and hyprtrade (settled 2026-07-24; empirically tested, not inferred from docs). Recorded here so it is not re-litigated per repo.
...
- **GitHub does not name the ruleset in the error.** An operator sees only `Repository rule violations found` plus the conversation sentence — no indication which ruleset blocked, so point them at the thread ruleset by name when diagnosing.
...
- **Date-stamp measured state.** Branch-protection settings changed underneath this very conversation (hyprtrade `enforce_admins` read `false` and then `true` within an hour, as another session acted on the same brief). State as of 2026-07-24: memsira `conv_res=true`/`enforce_admins=false`; hyprtrade `true`/`true`; drovr `false`/`false`. Re-measure before relying on any of it; a later audit disagreeing with this list is expected, not a contradiction.
