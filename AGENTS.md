...
- **Upstream reports carry an `area:*` routing label.** vstack-targeted issues get one `area:` label (`cli`, `skills`, `harness`, `review-gate`, `docs`, `tech-debt`), derived from the asset selector or set explicitly with `--area`. The GitHub→Linear sync carries the label across, where a Linear triage rule files the synced issue into the matching project. Project-local reports never get one: the labels are defined on `vanillagreencom/vstack` only, and a `gh` call naming an undefined label fails.
