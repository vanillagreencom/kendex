...
- **`vstack check` is the drift contract.** Exit `0` clean, `1` drift, `2` the check failed; available-but-not-installed items are suggestions, never drift, while a source cache that has been failing to refresh for more than two TTL windows (or cannot be written at all) is. It reads only disk — no network on the check path: a cache older than the six-hour TTL is refreshed by a detached `vstack cache-refresh` nobody waits on, and its outcome is reported at the next session. `--quiet` prints nothing when clean, `--json` goes to stdout, `--offline` skips the CLI version lookup and the background refresh while still reporting recorded cache failures. The `session-drift-check` hook and the Pi `pi-hooks` `sessionDriftCheck` setting are thin adapters over `check --quiet` — change the report in `cli/src/commands/check.rs`, not in the adapters.
