...
- Every suite runs on the pull request as well as in the merge queue, so a green PR proves what the queue re-proves; `.github/workflows/skill-tests.yml`'s header comment names the three event-scoped exceptions. A red shard on the PR is the failure the queue would eject for — fix it there rather than requeuing.
