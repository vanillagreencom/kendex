...
- The safety check says what it found and nothing more. A package's page
  ...
  words with its score, and the dot now takes focus, so a row you install
  straight from the list gives up those words on hover, on Tab, and to a
  screen reader rather than leaving a bare colour; and Settings › Safety
  check says it about the check itself. A marketplace's About tab said
  ...
  and has not looked inside a single package. A clean check has never been
  an approval, and the wording no longer suggests otherwise.
