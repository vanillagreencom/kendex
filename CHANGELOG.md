...
- **Breaking:** the install record is version 11. Commit `.kendex-lock.json`; machine fields move to `.cache/kendex/lock-local.json`. Move a version 10 lock aside, then run `kendex apply`; clean tracked renders are updated, while hand edits remain conflicts.
