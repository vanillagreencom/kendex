...
- **Claude Code** — script copied under `<scope>/.claude/hooks/`, registered in `settings.json` plus the owning agent's frontmatter. Both artifacts are what `check`/`verify` require — a script whose `settings.json` entry was deleted, or one whose entry now points at a different script, is drift, because Claude Code would never run it. A registration you keep in `settings.local.json` instead counts: Claude Code merges it, so it runs.
