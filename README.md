...
`check` never touches the network and never touches the project's git state. Every answer comes from disk — the lock, the source trees, and each cache's recorded refresh outcome — so a session start is instant and works offline. Remote source caches under `~/.vstack/cache/` are vstack's own clones: when one is more than six hours old, `check` hands the fetch to a detached background process and does not wait for it (skipped entirely with `--offline`), so cache news lands at the next session rather than costing this one. A single failed refresh is a footnote — working offline stays quiet — but a cache that has been failing for more than two refresh windows, or one vstack cannot write to at all, counts as drift so a permanently broken remote cannot read as clean forever; the report names the cause and points at `vstack refresh`, which fetches synchronously and unbounded. Fetching an existing cache is serialized by a per-cache lock that every writing command takes (the first clone creates the cache instead of mutating it, so it is not covered). `vstack refresh` applies updates; `check` itself never installs or removes anything.
