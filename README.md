...
`check` never touches the project's git state, and computing drift never touches the network: the verdict comes from the lock, the source trees, and each cache's recorded refresh outcome. The human report additionally looks up the latest CLI version, which `--quiet` and `--offline` skip — so the session-start path (`--quiet`) is fully local and works offline. Remote source caches under `~/.vstack/cache/` are vstack's own clones: when one is more than six hours old, `check` hands the fetch to a detached background process and does not wait for it (skipped entirely with `--offline`), so cache news lands at the next session rather than costing this one. A single failed refresh is a footnote — working offline stays quiet — but a cache that has been failing for more than two refresh windows, or one vstack cannot write to at all, counts as drift so a permanently broken remote cannot read as clean forever; the report names the cause and points at `vstack refresh`, which fetches synchronously and unbounded. If the background refresh cannot even be started (a sandbox denying process spawn, for instance), the report says so on its own line — informational only, never drift. Bounded fetches (the background refresh, the wizard's short startup fetch) never prompt for credentials, so a private source needs a configured git credential helper or ssh key; interactive `vstack add` and `vstack refresh` keep git's normal prompting. Fetching an existing cache is serialized by a per-cache lock that every writing command takes (the first clone creates the cache instead of mutating it, so it is not covered). `vstack refresh` applies updates; `check` itself never installs or removes anything. `--quiet` is bounded by construction: each section lists at most ten items and closes with `… and M more (run `vstack check` for the full report)`, while the section headers keep the true counts — the full listing is always one `vstack check` away.
