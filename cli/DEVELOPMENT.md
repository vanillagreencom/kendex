...
- A `source` recorded anywhere in the cache SUBTREE (`vstack add ~/.vstack/cache/<entry>` is a supported invocation, and so is a subdirectory of one) is a remote source spelled as a path, and resolves as one — in `add` as well as in `refresh`, `check`, `verify` and `cache-refresh`. The remote comes from the entry's own `origin`: a cache key is derived from the repository identity and is not reversible, and one machine can hold `owner_repo` beside `owner_repo-<digest>` for a single repository. Resolution stays pinned to the entry the source named, and a path below an entry is read inside the tree that entry's fetch left behind; `add` at an entry root goes through the remote itself, because `add` is the one command that may mint the canonical clone. An entry whose remote cannot be established is refused rather than read as a local directory, everywhere — `cache-refresh` included, which reports it and exits nonzero. `refresh` rewrites an entry-root source to the remote spec only when that spec resolves to the same directory the path named, so the rewrite is correct by construction rather than by a freshness check; a legacy-key path naming a second clone of the same repository keeps its path and is moved by `vstack add`, which records the tree it read rather than the string it was given. Two invariants hold across the whole cache subtree: `add` and `check` reach the same verdict for any path under it — a directory there that is not one of vstack's clones is refused by both, and one that is simply absent is absent to both — and every command `check` prints succeeds when pasted, so a vanished entry is answered from the `source_repo` the lock still records rather than from the dead path — but only where the recorded path IS the entry, since an identity names the repository and not a directory inside it, and a refusal is given a remedy only where re-adding provably clears it (the redirect shapes, whose origin still reads and whose remote has an entry of its own). One probe answers "is this entry present?" for both spellings of a cache source, so a read it could not complete is never reported as an absence. `check`, `verify` and `refresh` compose the same cause and the same remedy, and a remedy is withheld wherever no command can be both correct and safe.
- A recorded source string must name the tree that was READ, resolved the way later readers resolve it — `refresh`, `check` and `verify` all re-resolve it, and any spelling whose resolution at install time differs from theirs writes a `source_hash` for a tree the install did not come from. That is why `add` resolves a remembered relative source against `config::project_root()` rather than the process CWD, and why a cache path is recorded as the remote it resolved through.
