...
- A project-scope install settles its destination through `commands::install_destination` — the walk up, else the folder the command was typed in, asked for first — and registers it through `commands::project::register_destination` after the write, so the app sees the project without a second command. A global install and every read-only verb register nothing.
