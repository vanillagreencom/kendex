...
- `add`, its collection form included, settles its destination through `commands::install_destination` — the walk up, else the folder the command was typed in, asked for first and never the home directory — and registers it through `commands::project::register_destination` on the strength of what landed, so the app sees the project without a second command. No other verb calls either: `apply` and `adopt` still resolve through `resolve_scopes` and register nothing, and a global install registers nothing.
