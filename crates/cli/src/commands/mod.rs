...
/// either: the walk lands in the main checkout's project, so the catalog's
/// `block-worktree-refresh` hook refuses a bare project-scope verb typed
/// there, and refuses a `cd` before it anywhere, because neither says
/// which checkout the write lands in. A named project is the other door:
/// the write lands in the checkout the command's own words carry, and
/// nowhere else. A worktree that owns its manifest is the project the walk
/// answers with, and the hook passes the bare verb there.
///
