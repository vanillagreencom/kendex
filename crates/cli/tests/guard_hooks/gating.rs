...
/// A lane the helper cannot run is not an armed repository.
///
/// The helper being ours settles what it WOULD run. It execs one program
/// per lane and exits 2 where the program is missing or has no execute bit,
/// so an install that lost either one refuses every commit — and reporting
/// that as armed describes a repository whose commits are blocked as one
/// whose commits are checked, which is the wrong direction to be wrong in.
#[test]
#[allow(clippy::unwrap_used)]
fn a_lane_the_helper_cannot_run_is_not_armed() {
    use std::os::unix::fs::PermissionsExt;
    for (lane, break_it) in [("pre-commit", true), ("commit-msg", false)] {
        let tmp = tempfile::tempdir().unwrap();
        let home = tmp.path();
        let root = repo(home);
        install_package(home, &root, &["growth-guards"]);
        let scripts = root.join(".agents/skills/growth-guards/scripts");
        let installer = scripts.join("install-git-hooks");
        let armed = run_with(
            home,
            &root,
            installer.to_str().unwrap(),
        ...
        assert!(armed.status.success(), "{}", said(&armed));
        // Armed is the silent verdict: there is nothing for a reader to
        // act on, so `check` says nothing about hooks at all.
        let clean = run(home, &root, "kendex", &["check"]);
        assert!(
            !said(&clean).contains("commit hooks"),
            "the control did not read armed: {}",
            said(&clean)
        );

        // One lane goes, by the two ways a lane can stop being runnable.
        let program = scripts.join(lane);
        match break_it {
            true => std::fs::remove_file(&program).unwrap(),
            false => {
        ...
        assert!(
            ...
            "{lane} gone and still read armed: {out}"
        );
        assert!(out.contains(lane), "{lane} is not named: {out}");
    }
}

/// A hook that only quotes a marker is somebody else's file.
///
/// Ownership and shape fail in opposite directions, and this is the
/// ownership one: a substring read claims a consumer's hook that mentions
/// the marker in a comment and reports it as our stale shim, which is a
/// commit-blocking accusation against a file that gates nothing of ours.
#[test]
#[allow(clippy::unwrap_used)]
fn a_hook_that_quotes_the_marker_is_not_reported_as_our_shim() {
    use std::os::unix::fs::PermissionsExt;
    let tmp = tempfile::tempdir().unwrap();
    let home = tmp.path();
    let root = repo(home);
    // No package here at all, which is the state orphan reporting is about.
    ...
    let out = said(&run(home, &root, "kendex", &["check"]));
    assert!(
        !out.contains("carries the package's shims"),
        "a quoting hook was claimed as ours: {out}"
    );

    // The must-fail control: a line the marker actually CLOSES is ours, so
    // the pin above is not passing on a predicate that matches nothing.
    std::fs::write(&hook, "#!/bin/sh\nexit 0 # kendex-guards-hook\n").unwrap();
    std::fs::set_permissions(&hook, std::fs::Permissions::from_mode(0o755)).unwrap();
    let out = said(&run(home, &root, "kendex", &["check"]));
    assert!(
        out.contains("carries the package's shims"),
        "a real shim was not reported: {out}"
    );
}
