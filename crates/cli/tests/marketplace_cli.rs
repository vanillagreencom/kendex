...
fn marketplace_browse_answers_over_an_unreadable_lock() {
    ...
    assert!(subscribed.status.success());
    fs::write(project.join(".kendex-lock.json"), "{not json").unwrap();
    ...
    let listed: serde_json::Value =
        serde_json::from_slice(&output.stdout).expect("browse --json emits JSON");
    assert_eq!(listed["schema"], 1, "the widening is additive: {listed:#}");
    let rows = listed["packages"].as_array().unwrap();
    assert!(!rows.is_empty(), "the offer is still listed: {listed:#}");
    for row in rows {
        assert_eq!(
            row["package"]["state"], "unknown",
            "every row's installed state is unknown: {listed:#}"
        );
    }
