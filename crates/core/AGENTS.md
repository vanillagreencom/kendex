...
- `src/fs/dacl.rs` holds the only `unsafe` in the workspace: the Win32 security-descriptor calls behind the private credential file's owner-only access-control list, which have no safe binding. Each function allows `unsafe_code` per item with a reason; the workspace lint denies it everywhere else.
