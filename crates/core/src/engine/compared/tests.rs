...
/// v1 nested its stamp under `metadata:`, which is where the real installs
/// this reads carry it. A column-0 match would miss every one.
#[test]
fn the_stamp_is_read_wherever_it_is_written() {
    let nested = b"---\nname: growth-guards\nmetadata:\n  author: vanillagreen\n  source: vstack\n---\nBody.\n";
    assert!(stamped_bytes(nested), "the stamp nests under metadata");
    assert!(stamped_bytes(b"---\nname: s\nsource: vstack\n---\nbody\n"));
}

/// Only that token, only in frontmatter, and only from the file that
/// speaks for the item: a reference file deep in a folder is content.
#[test]
#[allow(clippy::expect_used)]
fn nothing_but_the_marker_file_stamps_the_item() {
    assert!(!stamped_bytes(b"---\nname: s\nsource: kendex\n---\nbody\n"));
    assert!(!stamped_bytes(
        b"---\nname: s\nmetadata:\n  source: mine\n---\nbody\n"
    ));
    assert!(!stamped_bytes(b"source: vstack\n"));
    assert!(!stamped_bytes(b"---\nname: s\n---\nsource: vstack\n"));
    assert!(!stamped_bytes(&[0xff, 0xfe, 0x00]));
    ...
    .expect("write");
    assert!(
        !of_tree(&root, &[]).expect("comparable").vstack_stamped,
        "one stamped reference file spoke for the whole item"
    );
}

/// A catalog still named vstack stamps the same token today, so the flag
/// says what the frontmatter claims and nothing about when it was written.
#[test]
#[allow(clippy::expect_used)]
fn a_current_package_naming_vstack_carries_the_stamp() {
    let dir = tmp();
    let root = dir.path().join("skill");
    std::fs::create_dir_all(&root).expect("mkdir");
    std::fs::write(
    ...
    .expect("write");
    assert!(of_tree(&root, &[]).expect("comparable").vstack_stamped);
}
