...
use super::desired::{Artifact, Desired, DesiredState};
...
pub(crate) use migrated::record_finished;
use migrated::{Identity, every_pi_hook, legacy_registration, newly_installed};
pub(crate) use preflight::{Preflight, preflight};
use preflight::{claim, claims};
...
fn asked_for(
    ...
    manifest.hooks.contains_key(&entry.name)
        || state
            .processed
            .contains(&(ItemKind::Hook, entry.name.clone()))
        || (super::removal::derived_only(entry)
            && !super::removal::origin_readable(env, scope, manifest, state, &entry.source))
}
