...
use super::permission::{Access, PermissionIntent};
...
/// What one harness's own rules leave this agent able to use, as data
/// rather than as rendered text. Every answer comes from the same function
/// the renderer calls, so a rule the renderer grows — a deny keyed on the
/// agent's name, a permission a mode withholds — is answered here without
/// this dispatch being taught about it.
///
/// `Err` is the harness refusing the intent, the same refusal [`generate`]
/// returns: nothing installs, so there is no access at all.
pub fn access(agent: &EffectiveAgent) -> Result<Access, String> {
    Ok(match agent.harness {
        HarnessId::Claude => claude::access(agent),
        HarnessId::Opencode => opencode::access(agent),
        HarnessId::Pi => pi::access(agent)?,
        HarnessId::Gemini => gemini::access(agent),
        HarnessId::Copilot => copilot::access(agent),
        // Codex writes a sandbox mode and Cursor writes nothing: neither
        // states a per-tool policy, so neither has one for a name to widen.
        HarnessId::Codex | HarnessId::Cursor => Access::default(),
    })
}
