...
/// The cache is read, the feed is fetched, and the whole document is written
/// back — so two processes overlapping in that window cost more than a
/// second fetch. The one whose fetch fails writes the body it read *before*
/// the fetch back over the release the other just saved, under a fresh
/// `last_attempt_at`: the card says nothing, and nothing is due for six
/// hours, with a release sitting on the server the whole time.
///
/// The interleaving that does it is pinned here rather than raced for. The
/// second process is started only once the first is inside its fetch, which
/// is past the lock and past the read; the first then waits for the second
/// to have started, and the second's own fetch waits for the first's
/// document to land before failing. Unserialised that is two fetches and a
/// cache holding no release. Serialised, the second waits at the lock, reads
/// what the first left, finds the interval served, and answers from it
/// without going to the network at all.
#[test]
fn a_process_that_loses_the_race_does_not_write_over_the_release() {
    const ROOT: &str = "KENDEX_APP_UPDATE_CHILD_ROOT";
    const ROLE: &str = "KENDEX_APP_UPDATE_CHILD_ROLE";
    ...
    /// Poll until `path` reads and its content satisfies `holds`. Every wait
    /// here is bounded, so a child whose partner never arrives fails an
    /// assertion instead of hanging the suite.
    fn awaited(path: &Path, holds: impl Fn(&str) -> bool) -> bool {
        let deadline = Instant::now() + PATIENCE;
        while Instant::now() < deadline {
            if std::fs::read_to_string(path).is_ok_and(|text| holds(&text)) {
                return true;
            }
    ...
    impl Fetch for RaceFetch {
        fn get_auth(
            ...
            let tally = self.root.join("fetches");
            let mut file = std::fs::OpenOptions::new()
                .create(true)
                .append(true)
                .open(&tally)
                .map_err(|error| CoreError::io(&tally, error))?;
            writeln!(file, "fetch").map_err(|error| CoreError::io(&tally, error))?;
            if self.first {
                touch(&self.root.join("first-fetching"));
                awaited(&self.root.join("second-started"), |_| true);
                return response("5.1.0", "");
            }
            // Reached only when nothing serialised the two. Waiting for the
            // release to be on disk makes the write that follows this
            // failure the one that takes it away.
            awaited(
                &Env::fake(&self.root, FakeOs::Linux).app_update_cache_file(),
                |cache| cache.contains("5.1.0"),
            );
            Err(CoreError::RegistryUnavailable {
                why: "offline".to_owned(),
            })
    ...
    if let Some(root) = std::env::var_os(ROOT) {
        ...
        let first = std::env::var(ROLE).unwrap() == "first";
        if !first {
            touch(&root.join("second-started"));
        }
        ...
        .unwrap();
        let role = if first { "first" } else { "second" };
        std::fs::write(root.join(format!("{role}-status")), format!("{status:?}")).unwrap();
        return;
    }
    ...
    let spawn = |role: &str| {
        Command::new(&executable)
            .args([
                "--exact",
                "a_process_that_loses_the_race_does_not_write_over_the_release",
                "--nocapture",
            ])
            .env(ROOT, &root)
            .env(ROLE, role)
    ...
    assert!(
        awaited(&root.join("first-fetching"), |_| true),
        "the first process never reached its fetch"
    );
    let mut second = spawn("second");
    assert!(first.wait().unwrap().success(), "the first process failed");
    assert!(
        second.wait().unwrap().success(),
        "the second process failed"
    );

    let cache = std::fs::read_to_string(Env::fake(&root, FakeOs::Linux).app_update_cache_file())
        .expect("a check wrote the cache");
    assert!(
        cache.contains("5.1.0"),
        "the failed attempt wrote its pre-fetch body over the release: {cache}"
    );
    let second_status = std::fs::read_to_string(root.join("second-status")).unwrap();
    assert!(
        second_status.contains("5.1.0"),
        "the process that waited answered as if nothing had been checked: {second_status}"
    );
    let fetches = std::fs::read_to_string(root.join("fetches")).unwrap();
    assert_eq!(
        fetches.lines().count(),
        1,
        "both processes went out to the feed"
    );
}
