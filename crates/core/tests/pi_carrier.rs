...
fn a_registration_moved_to_another_event_is_never_doubled() {
    ...
    assert_eq!(after, moved, "the moved entry is left where it is");
    assert!(
        w.project.join(".pi/kendex/hooks/guard.sh").is_file(),
        "the conflict must hold the script the entry still runs"
    );
}
...
fn a_command_registered_twice_holds_rather_than_adding_a_third() {
    ...
    assert!(
        w.project.join(".pi/kendex/hooks/guard.sh").is_file(),
        "the conflict must hold the script the entries still run"
    );
}
