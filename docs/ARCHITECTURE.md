...
4. Locks record durable provenance; same-source reinstall is a no-op,
   ...
   restricting it further is refused. Its fork source keeps the edited rendered
   body as written, including that harness's vocabulary. The new name is proven
   free before the
...
6. Never touch the unowned: unmanaged files are reported, never deleted;
   ...
   from a position stating no remainder of the root the record names, which a travelled record rejoins onto the root reading it; outside either root is refused. A link the user put at a shared config file or a manifest
...
- **Intent in the manifest, closure in the plan, edges in the lock.** The
  ...
  catalog — and a set's members are its own catalog's items. A body key that
  is neither `description` nor one of the member lists makes the catalog
  unreadable naming the key: everything that reader skips is a member the
  set silently loses. A body carrying `source` is the manifest's record of
  an installed set, read by the manifest and not as a set on offer.
