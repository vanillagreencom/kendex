...
4. Locks record durable provenance; same-source reinstall is a no-op,
   ...
   restricting it further is refused. Its fork source removes renderer-owned
   wrapper sections and normalizes the remaining body's blank edges; the
   authored prose keeps the selected harness's vocabulary. The new name is
   proven free before the
