...
Review carry-forward is on for markdown here, and every policy path in `REVIEW_GATE_CARRY_FORWARD_EXCLUDE` needs fresh evidence. The current gate accepts configured bot or human evidence and does not require a human specifically for policy-file changes. Changes to review policy need a trusted human's approval under the package's adoption policy.
