...
`tools/harness-smoke` asks seven of the eight harnesses (every one but Antigravity, whose rows say so), on its own listing or startup surface, whether a package kendex installed into a scratch project loaded, and reads a marker its fixture hooks write when the harness runs them; one row per harness per question, with `unanswerable` and its reason where a harness or this machine has no surface. It spends model turns on the harnesses that expose nothing else, so it is run by hand after a render change, never in CI.
