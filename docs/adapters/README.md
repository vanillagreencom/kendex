...
A `[hooks.<name>]` declaration's `env` table sets variables for that hook's script. The command assigns each entry, its value quoted for the shell, directly before `bash`. A command that names its script outright binds the path to `h` first, so the script stays the first path the command names (`direct_command`, `crates/core/src/engine/targets.rs`). The Pi carrier spawns a project hook's script directly, so these assignments do not reach a Pi project hook.
