...
- Model dialect: tiers resolve to bare ids through `crates/core/src/harness/models.rs`; an omitted key is Codex's spelling of inherit. `model_reasoning_effort` is written as given (`minimal`, `low`, `medium`, `high`, `xhigh`); an absent key takes the model's own default.
