...
- Command file: a TOML table with `description` and `prompt`, the generated-file banner outside the prompt as a `#` comment. Only `.toml` loads from the commands directory, so the `.disabled` rename toggle is safe there. Gemini expands `{{args}}` and not Claude's `$ARGUMENTS`, so the prompt carries the catalog body with that one token translated (`gemini_prompt`, `crates/core/src/render/command.rs`).
