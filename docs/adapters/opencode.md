...
- Model dialect: tiers resolve to provider-qualified ids through `crates/core/src/harness/models.rs`. An explicit id must be `provider/model`; render validation refuses a bare id (`crates/core/src/render/validate/agent.rs`). An omitted key means inherit.
