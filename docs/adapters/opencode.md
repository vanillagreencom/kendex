...
- Model dialect: every tier resolves to `openai/gpt-6-astra`; a bare vendor id gains the `openai/` prefix because the loader requires `provider/model`; an omitted key means inherit (`crates/core/src/harness/models.rs`).
