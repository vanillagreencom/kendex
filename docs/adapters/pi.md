...
- Model dialect: `crates/core/src/harness/models.rs` resolves tiers to inheritance or provider-qualified ids; a bare id with no `/` passes through with a warning. An effort setting is written as its own `effort` key (`minimal`, `low`, `medium`, `high`, `xhigh`, `max`), which the `pi-agents-tmux` extension passes to the child as `--thinking`; a pinned model id also carries it as a `:<effort>` suffix. `off` or `none` writes no key, and an absent key runs the child at Pi's `defaultThinkingLevel`.
...
- Every payload carries the fields Claude Code names the caller with: `session_id` from `getSessionId()`, `transcript_path` from `getSessionFile()` where the session has a file, and `agent_type` from `PI_SUBAGENT_CHILD_AGENT` where that is set. A Pi subagent is its own process with its own session file, so that file is already the calling agent's transcript and no `agent_id` is sent.
- A process started with `PI_SUBAGENT_CHILD_AGENT` set dispatches no `turn_end` registrations: Claude Code ends a subagent with `SubagentStop`, never `Stop`, and `pi_listener` maps no `SubagentStop`, so a `Stop` or `TaskCompleted` hook judges the lead session alone. The carrier's own clippy check still runs there.
