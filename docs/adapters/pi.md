...
- `tool_call` is the only key the carrier dispatches: a hook mapped onto `tool_result`, `turn_end` or `session_start` is registered, labelled enforced, and does not fire.
- On a `tool_call` it runs every registration whose `matcher` covers the tool, in the order the registry names them, so a catalog guard and a `[[custom-hooks]]` command of the person's own both fire; the carrier knows no hook's name in advance, and a custom hook has no file at all, so the registry is the only place it exists. Absent, empty and `*` matchers cover every tool; anything else is a whole-string regex over the tool said in Claude Code's words (`vocab.ts`, held by a test to `render::vocab::claude_tool_name`).
...
Beside the root, kendex does nothing at all: `engine::targets::pi_hook` names only `<root>/kendex/hooks.json`, `engine::owned::hook_owned` derives only `<root>/kendex/hooks/<name>.sh`, and `harness::pi::hook_surfaces` lists only the first. A `hooks.json` or `hooks/` an older kendex left beside the root is read, written, scanned, listed and removed by nothing, `kendex remove` included, and the carrier reads `<root>/kendex/hooks.json` alone, so an installation left only in the older layout is unenforced until the item is installed fresh. What becomes of the leftovers is the person's to settle by hand.
