...
6. Manifest and lock carry a format version and nothing converts them: an older or newer file is refused, left byte-for-byte, with the move-aside remedy in the message. Enforced by `crates/core/tests/migration.rs`, `crates/core/tests/v1_lock.rs` and `crates/app/tests/apply_migration.rs`.
