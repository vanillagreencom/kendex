...
- Mutation ownership is what kendex wrote, read from the positions lock entries recorded, never from a lock key alone or from an entry merely on the books; a record carried in from another checkout resolves against the root reading it (`crates/core/src/lock/roots.rs`). Report routing and read-only verification can resolve matching manifest and render provenance when the record cannot answer. Enforced by `crates/core/tests/unmanaged_ownership.rs`, `crates/core/tests/lock_from_another_project.rs`, `crates/cli/tests/compat.rs` and `crates/cli/tests/pi_extension_only_lock.rs`.
