...
7. A Pi extension never enters a scope through `add`: a request naming one is the typed refusal `PiExtensionDirect` and writes nothing. Enforced by `crates/core/tests/install_seam/main.rs::a_direct_pi_extension_add_refuses_naming_the_carrier`.
8. One model-alias table serves every harness (`crates/core/src/harness/models.rs`): bare tiers resolve per harness, `inherit` is expressed in each tool's dialect, explicit vendor ids pass through. Enforced by the tests in `crates/core/src/harness/models.rs`.
