...
6. Pi hooks are enforced through the carrier: the `pi-hooks` extension hosts native listeners and hook content rides in the registry kendex renders under a `kendex` directory beside it, keyed by Pi's listener names; every listener key `pi_listener` maps onto is dispatched and only `tool_call` gates, and Pi reserves `hooks/` beside every root it loads, so nothing writes a registry there. The rules in full are in [../adapters/pi.md](../adapters/pi.md). Enforced by `crates/core/tests/pi_carrier.rs`.
