...
- The consumer train is the propagation path until KEN-1779 ships the refresh workflow [D003](../decisions/D003-one-merge-path.md) decides. It enters each consumer's checkout, refreshes it, and commits the refresh output alone through that repository's branch, review and merge path. On a hosted fleet the train does not run: the orch `merged` event in [../../skills/orch/references/oversee-events.md](../../skills/orch/references/oversee-events.md) § Event kinds sends each consumer's overseer a note and reports that consumer as not refreshed.
...
- The install record is recorded on `main` after each merge by `.github/workflows/lock-record.yml` and landed through one rolling pull request, never on a branch, per [D007](../decisions/D007-lock-record-on-main.md); no pull request check judges it, so `tools/ci-job-set` selects no shard for it.
