...
- In kendex itself `REVIEW_GATE_CARRY_FORWARD` is `docs`: evidence carries across markdown-only deltas, and excluded policy paths need fresh evidence. `REVIEW_GATE_CLASS_POLICY` decides which change classes need evidence, so the legacy `REVIEW_GATE_DOCS_ONLY` and `REVIEW_GATE_RENDER_PATHS` are inert.
