...
- `skills/review-gate/scripts/validate-standard.sh` reports whether a repository's GitHub settings match the organization standard. `skills/review-gate/standard.json` holds the required contexts, the app, the environment and its secret names; the script fixes the remaining rows against [D003](../decisions/D003-one-merge-path.md). The environment rows come from owner directive 1790333777, which the D003 amendment KEN-1777 carries records. The script only reads; the adoption step KEN-1779 adds creates the environment and its secrets.
