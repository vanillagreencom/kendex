...
- In kendex itself the gate's two waiver lanes are off: `REVIEW_GATE_CARRY_FORWARD` is empty and `REVIEW_GATE_RENDER_PATHS` takes its empty default. `REVIEW_GATE_DOCS_ONLY` is `none`, so a documentation diff waives reviewer evidence alone.
