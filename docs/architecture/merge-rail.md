...
1. The class is read from the diff alone. The classifier takes an event name and two commit shas, and every case it cannot prove answers `false`, which runs every lane. Enforced by `skills/harness-ci/tests/fail-closed.test.sh`.
...
3. A skipped lane satisfies a required context only where a successful classifier authorized that exact job. Enforced by `skills/harness-ci/tests/aggregate-needs.test.sh`.
4. The gate answers one question, whether this exact head was reviewed, and posts that answer as a commit status. Two green checks never stand in for a review. Enforced by `skills/review-gate/scripts/review-predicate-selftest.sh`.
5. A waiver of reviewer evidence waives reviewer evidence alone. An objection, an unresolved thread and a suppressed finding still block, and the gate posts only its own status, so the ruleset's other required contexts are untouched by it. Enforced by `skills/review-gate/tests/docs-only-lane.test.sh`, which carries a must-fail control for the suppressed-finding branch.
