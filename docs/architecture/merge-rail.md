...
- In kendex itself the gate carries evidence across markdown-only deltas and its render-only lane is off: `REVIEW_GATE_CARRY_FORWARD` is `docs`, excluded policy paths need fresh evidence, and `REVIEW_GATE_RENDER_PATHS` takes its empty default. `REVIEW_GATE_DOCS_ONLY` is `none`, so a documentation diff waives reviewer evidence alone.
