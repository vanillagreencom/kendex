...
- In kendex itself the gate's carry-forward is off: `REVIEW_GATE_CARRY_FORWARD` is empty. The class policy is active, so the docs-only and render-only lanes, which run only under an inactive policy, are unassigned; a documentation or render diff needs the review evidence its change class sets.
