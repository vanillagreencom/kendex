...
- Whether a repository's GitHub settings match [D003](../decisions/D003-one-merge-path.md) is read by `skills/review-gate/scripts/validate-standard.sh` against `skills/review-gate/standard.json`, the one statement of the standard's values. It only reads; nothing in kendex writes a ruleset, an installation or a secret.
