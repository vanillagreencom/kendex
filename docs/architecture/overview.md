...
The desktop app and CLI project one Rust model: scan, declare, diff and apply. The per-scope manifest stores user intent. Drift compares that intent with observed harness state. Apply makes disk match the declaration; adopt records observed state as intent.
...
1. Generated artifacts are always overwritable by kendex, and bytes no apply wrote are the user's: an edited installation is a conflict naming its exits. Enforced by `crates/core/tests/invariants.rs::invariant_1_generated_artifacts_regenerate_but_never_over_an_edit`.
...
3. A content hash covers every input that shapes the artifact: the source bytes and every manifest section, or the sections alone where the tree is the source ([in-place.md](in-place.md)). Enforced by `crates/core/tests/invariants.rs::invariant_3_shared_key_edits_invalidate_dependents`.
4. A lock records durable provenance for every installed kind, including Pi extensions: same-source reinstall is a no-op, a cross-source name collision is a hard error naming the original, and a fork is the one rebind. A read-only report or verify may use matching manifest and render bytes when the lock is unavailable, keeping the lock failure visible. Enforced by `crates/core/tests/invariants.rs::invariant_4_provenance_is_durable`, `crates/core/tests/collision_refusal.rs`, `crates/cli/tests/compat.rs` and `crates/cli/tests/pi_extension_only_lock.rs`.
...
16. A debug build gets its own home under `<data>/kendex-dev`, drops inherited harness-root variables, and only `KENDEX_REAL_HOME=1` opts out. Enforced by the tests in `crates/core/src/env/sandbox.rs` and the fixture-home and binary-home lanes in `tools/guard`.
