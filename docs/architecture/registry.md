...
5. A skills.sh hit is a lead, never an identity, installs through the same subscribe path, and `KENDEX_SKILLSSH=off` is its kill switch. Enforced by `crates/core/tests/registry.rs::skillssh_refuses_a_shape_it_does_not_know`.
