...
6. A switch or command that a file only names is a mention, not a finding: a markdown code span, a full-line shell comment, or a string that `echo`, `printf` or a function whose body only prints, assigns, tests and steers hands to the terminal. The same text in a command position, inside `eval` or `bash -c`, piped on, redirected into a file or captured by a command substitution is a finding. `quality::text::shell` reads the shell shapes and every needle goes through `Line::standing`; mentions are `AuditResult::mentions`, cost the score nothing, and print only under `refresh --verbose`. Enforced by `crates/core/tests/quality/corpus.rs` over the shipped guard packages and hooks, and `crates/cli/tests/safety_print.rs::a_verbose_refresh_prints_what_the_rules_read_as_a_mention`.
...
- A run that scored something closes on `safety: clean` when nothing is flagged, so a clean scan and a scan nobody ran read differently.
