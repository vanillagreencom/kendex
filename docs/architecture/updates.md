...
6. A build with a recorded main identity follows the rolling main channel. A tagged release reads that channel only when a user runs `kendex update --git`. Before publication, the release workflow classifier alone holds the built version to the run number and commit. `tools/release-digests` validates the identity fields and signs them without parsing the version shape. A main update refuses an older build number. Enforced by the tests in `crates/core/src/update_channel.rs`, `crates/core/src/update_feed.rs`, `crates/cli/tests/release_workflow/channel.rs` and `tools/tests/release-digests.test.sh`.
