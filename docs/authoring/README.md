...
Where kendex recorded arming the effect, `kendex verify` fails and `kendex refresh` names the package on a `1`, on a check that could not be taken, or on a declaration that will not read, with the way to apply its repository changes again as the remedy (`kendex guard install` for commit-guards; the package's page in the app, or a remove and an add with `--allow-repo-effects`, for any other). Neither verb runs the installer again: a yes is given against the disclosure of its day, and a later version may declare more. A scope with no such record reports nothing about the package.
