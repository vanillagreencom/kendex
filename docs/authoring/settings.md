...
Nothing under `[secrets]` is ever seeded into `kendex.settings.toml`. No value reaches a plan description, a displayed diff, an error, a log or a commit offer; the bytes exist only in the write op that carries them and in the apply journal's pre-image. On macOS and Linux both are owner-only: the file is created at mode 600 and the journal copy carries that mode across. On Windows the file is created with an access-control list naming the account kendex runs as and nobody else, whatever its folder grants, and the create is refused rather than made wider when that list cannot be applied; the journal copy takes the list of its own folder under the profile's application data.
