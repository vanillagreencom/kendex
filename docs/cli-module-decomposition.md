...
Impl 1–1348, tests 1349–3697 (64% of the file). No module-level state; the
couplers are two path helpers (`normalize_absolute_path`, 9 internal
...
| Agent install | 45 | `InstallResult`, `install_agent` | none | commands/add |
| `install_skill` | 437 (one fn) | `install_skill` | none | add, refresh, harness/mod, tui/disk_mutations |
| `remove_item` | 366 | `RemoveOutcome`, `remove_item`, `ExpectedArtifact`, `remove_expected_path` | none | commands/remove, tui/disk_mutations |
...
`~/dev/vstack2` is a ground-up rebuild with its own architecture invariants.
Before steps 4–6 of either plan, decide whether this layout informs vstack2's
module map or whether v1 stays the working system long enough to justify the
moves. Steps 1–3 (test hoist, leaf extraction) are cheap enough to do
