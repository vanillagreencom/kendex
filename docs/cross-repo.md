...
- **Send ONE LINE. A multi-line `send-keys -l` becomes a paste, and a paste does not submit.** Claude Code collapses multi-line input to `[Pasted text #1]`, and neither `Enter` nor `C-m` submits it — the message sits in the composer indefinitely. Observed 2026-07-25: a full measurement handoff to memsira looked delivered and had not been. The same text as a single line submitted immediately and queued normally. Newlines are the trigger, not length — a very long single line is fine.
- **A collapsed paste also defeats fragment verification**, because `capture-pane` only ever shows `[Pasted text #1]` and never the body, so the delivery grep above returns zero on landed and unlanded messages alike. One more reason single-line is the only reliable form.
- **Clear a stuck composer with `C-u` rather than leaving it.** Text abandoned there can later be submitted glued to the other session's own message. `C-u` doubles as the power check for the whole diagnosis: if it clears the composer, keystrokes *are* registering, so "Enter did not submit" is a real finding rather than a dead pane.
...
- **Keyed as anything else, the connector appears twice.** Verified live: keying a declaration `claudeai-slack` while the CLI loads its own `claude.ai Slack` yields **28** servers and two namespaces for one connector; keying it `claude.ai Slack` gives **27** and one namespace. Reusing the connector's *id* is not sufficient — the id was identical in both runs.
- **What merges is the namespace, not the connection.** Both of us initially said "the declaration merges", and at the server-count level that is right — but under the shared name the declaration and the CLI's own loader each still connect. Across 40 cold runs the baseline logged 1 Slack connect (7 proxy connects total) against the declared arm's 2 (8 total), in 20 of 20 runs. Declaring N connectors costs **~2N connections**. They connect in parallel (slowest-connect median 1192ms → 1278ms, worst 1544ms, inside the 5s cap), but that was measured with one declaration. The general lesson: we agreed cheaply, and the agreement was about the wrong noun — nobody had asked *what* merges, and the answer was one grep over logs already on disk.
- **This is a hard requirement for consumers that pin fully-qualified tool names — treat it as a compatibility contract.** memsira's `claude_tools.rs` hard-codes them into `--allowedTools` *and* into its executor system prompt, and never globs a namespace. Change the key format and every memsira connector **write** silently stops matching a real tool, surfacing as "unavailable" rather than as an error naming the cause — a missing-connector shape for what is actually a broken contract. Any repo doing the same must assume the `claude.ai <Connector>` form.
- **Guarded by a test, not just by this row.** `pi-extensions/pi-claude-bridge/tests/unit-connector-declarations.mjs` asserts the exact format under a name that says it is a contract, so a change fails CI and points the author here rather than shipping quietly.
- **Do not health-check the deferred-tool count.** `Dynamic tool loading` reports `0/N`, and N moves for two separate reasons. Across configurations it varies with the CLI's own async fetch (0/20 … 0/66 observed). And it goes **down** as you declare *more*: measured 0/8 with none declared, 0/37 with one, 0/11 with seven, each stable across ten runs. That looks backwards until you see why — declared tools are `alwaysLoad` and therefore not deferrable at all, so declaring servers pulls their tools *out* of the deferred pool. N is not a health signal in either direction.
- **Declaring is a latency WIN, not a cost.** Measured over 30 cold runs, 10 per arm (memsira, 2026-07-26). Barrier cost is real but small and sub-linear: manifest_ms median 490ms none → 1996ms one → 2574ms seven, ranges fully disjoint, worst of all 30 runs 2927ms. Seven declarations cost **+578ms over one**, not seven times one. What the user actually feels goes the other way: first token median 9840ms none → 6887ms seven, and the spread collapses — baseline's worst turn was **35.7s** to first token against 7.8s worst for seven. Declaring removes the model's flailing (no speculative `ToolSearch`, no dead end), so the barrier buys back more than it spends.
- **The 5s cap in the SDK doc comment does not match the runtime.** The CLI logs `Starting connection with timeout of 30000ms` on every server, declared or auto-fetched, with no logged distinction between them. Which value governs the `alwaysLoad` barrier is still unresolved — but nothing in 70 total runs came within an order of magnitude of either, because real failures reject fast (277–303ms) rather than hanging. The cap is only reachable by a mid-handshake hang, which unreachable addresses cannot produce.
