...
`.kendex-lock.json` is committed with the renders it records and reads as its own in every clone. Nothing in it names the checkout that wrote it: every position is spelled as a remainder of the root, a path source's provenance is its declaration in a dot-marked spelling rather than the directory it resolved to, and the root itself is not written. What only one machine knows — the delivery an install used, when it was made, the root it was written under — is the machine half of the record at `.cache/kendex/lock-local.json`, which the managed ignore block keeps out of git and which holds one row per checkout that wrote through it. The lock format is version 11. A version 10 project record is refused by name with the complete recovery: move `.kendex-lock.json` to `.kendex-lock.v10.json`, then run `kendex apply`. The sidecar proves an earlier kendex install per entry. Apply replaces a declared destination only when its bytes match that entry's `renderedHash`. For a proven script-backed hook with no current entry, apply also uses the saved registration identity to retire that exact old entry through normal reconciliation. Every mismatch remains a conflict, including a committed hand edit.

The KEN-1721 overseer ruling permits this read-only sidecar proof for recovery and forbids loading the version 10 record as the working lock.
...
The commit offer and the render inventory list the lock beside `.kendex-generated.json`, so a refresh commits the record with the render diff. A clone that carries renders and no record, and the clone every earlier build left, is settled by `kendex check` once the source's local mirror holds the commit the declaration resolves to (until then the declaration reads as pending and stays on the blocked line, and the background refresh fetches it): the check plans the scope once, inside the session hook's budget or in the background past it, records each render that matches its source at the commit the source resolved, and reports each one that differs as stale with the file count, naming `kendex apply --replace-unmanaged` where that sweep settles the whole scope. When a person follows the version 10 refusal, moves `.kendex-lock.json` to `.kendex-lock.v10.json` and runs normal `kendex apply`, the sidecar's entry identity and `renderedHash` prove ownership of each matching destination. That proof remains available when a pending source makes the first apply partial and in a project outside Git. Apply replaces stale matching renders and records current matching ones. A proven script-backed hook also retires its saved event and matcher before it writes a changed registration, so the old hook cannot remain active beside the new one. Every unmatched destination remains unmanaged and blocks replacement, including a committed hand edit. The managed ignore block drops the lock, and a consumer's own rule that still ignores it is reported with what a clone loses. A clone-local `.git/info/exclude` rule is also reported by its file path, because kendex cannot edit that consumer-owned file. A stale `WORKTREE_COPIES` entry cannot replace the lock because worktree setup leaves every Git-owned copy path to Git. A linked worktree whose `.cache` is a link to the main checkout's shares the machine-half file; each checkout keeps its own row in it, and the file names every checkout that applied through it. A fork in a fresh clone waits for one apply to record the delivery it reads back through.
