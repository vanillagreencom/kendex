...
`.kendex-lock.json` is committed with the renders it records and reads as its own in every clone. Nothing in it names the checkout that wrote it: every position is spelled as a remainder of the root, a path source's provenance is its declaration in a dot-marked spelling rather than the directory it resolved to, and the root itself is not written. What only one machine knows — the delivery an install used, when it was made, the root it was written under — is the machine half of the record at `.cache/kendex/lock-local.json`, which the managed ignore block keeps out of git and which holds one row per checkout that wrote through it. The lock format is version 11. A version 10 record is refused by name with the complete recovery: move the lock aside and run `kendex apply`. The old managed ignore rule identifies an earlier kendex install, and Git confirms which declared render roots are clean and tracked. Apply can replace those stale renders while a hand-edited render remains a conflict.
