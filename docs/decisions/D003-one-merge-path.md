...
1. **One merge path.** Every repository merges through its merge queue, and the lanes app arms auto-merge; the one direct-merge exception is the gate-repair break-glass [review-gate SKILL.md](../../skills/review-gate/SKILL.md#4-operations) § 4. Operations states. No ruleset has a bypass actor, no admin route remains, and no owner credential sits on the control VM (KEN-1777). The queue serializes concurrent lanes and batches up to five pull requests per CI run, so a pull request behind `main` never restacks to merge. The queue run is cheap by class: the `merge_group` diff classifies like a pull request, a `render` or `trivial` group runs one job, and KEN-1750 removes the cargo lanes where no Rust path changed.
2. **Consumers pull.** kendex ships a refresh workflow template in its render (KEN-1779). The template reaches a consumer's `.github/workflows/` by adoption copy, as the review-gate writer template does per [adoption.md](../../skills/review-gate/references/adoption.md), because `kendex refresh` syncs no workflow YAML ([merge-rail.md](../architecture/merge-rail.md) § Boundaries). The lanes app is installed on all repositories at the organization, and a workflow in kendex `main` sends `repository_dispatch` with the lanes app token to every repository the app is installed on. Each consumer's copy runs on that dispatch, on a 30-minute schedule, and on `workflow_dispatch`. It installs a pinned kendex, runs `kendex refresh --scope project --yes --leave` and `kendex verify --scope project`, commits to the one rolling branch `kendex/refresh`, opens or updates one rolling pull request, and arms auto-merge on it. Its token comes from the `kendex` environment's secrets `FLEET_GH_APP_ID` and `FLEET_GH_APP_PRIVATE_KEY` through `actions/create-github-app-token`, scoped to the running repository and valid for one hour. The scoped app token commits, pushes, opens the rolling pull request and arms auto-merge; the built-in `GITHUB_TOKEN` is never used for any of these, because a pull request it opens starts no workflow and its required checks would never report. The pull request is `render` class: no review, one CI job, then the queue. No overseer and no control-VM toolchain take part. kendex never pushes to a consumer.
   - The `vanillagreen-fleet-lanes` app carries, on each repository, Contents read and write, Pull requests read and write, Workflows read and write, and read-only Administration, Metadata, Actions, Environments and Secrets; on the organization it carries read-only Secrets. The set is fixed once on the app; the minted installation token requests the same set; no per-repository setting. Workflows write lets a commit change `.github/workflows/review-gate-writer.yml` or the refresh workflow itself. The read-only permissions carry KEN-1781's checks, whose endpoint-to-permission mapping KEN-1781 owns. GitHub returns a ruleset's bypass actors only to a caller with write access to that ruleset, so no lanes app token reads them.
   ...
   - The owner-run provisioning command, `skills/review-gate/scripts/provision-environment.sh --org ORG` (KEN-1799), creates that environment with its policy and sets its two secrets in every repository the lanes app is installed on that is not archived. It is idempotent, so a new repository is provisioned by running the same command again and the owner does no per-repository work by hand. It runs from the owner's own machine under a credential the owner holds and picks, which is never stored on the control VM or a lane host. The adoption step in a consumer does not create the environment. Today `validate-standard.sh` reads it and names `provision-environment.sh` when it is absent or drifted; the adoption step KEN-1779 adds reads it the same way and fails naming that command.
   - The lanes app never holds Administration write or Environments write. Administration write would let a lane create the environment or rewrite its deployment branch policy; Environments write would let a lane overwrite the environment's two secrets.
...
- A review-gate validation reports, read-only, whether a repository's rulesets, required contexts and app installation match this decision. Per repository it also reads that the `kendex` environment exists, carries the default-branch-only deployment policy and holds the secrets `FLEET_GH_APP_ID` and `FLEET_GH_APP_PRIVATE_KEY` (names only), and that no repository or organization secret of those names exists outside it. The check is review-gate `validate-standard`, `skills/review-gate/scripts/validate-standard.sh` (KEN-1781); a `kendex check` relay of its verdict is KEN-1818.
- `.github/workflows/publish-homebrew.yml` `publish` also mints a token from the two secrets. That job runs under `environment: kendex` and reads them from it. A dry run is its own `dry-run` job, with no environment and no secret, so the default-branch-only deployment policy does not refuse a dry-run dispatch from another ref. [../../packaging/README.md](../../packaging/README.md) § Publishing tells the reader to place the two secrets in that environment and nowhere else (KEN-1836).
...
- Owner GitHub steps: install the `vanillagreen-fleet-lanes` app on all repositories with the permission set step 2 records, its read-only repository Actions, Environments and Secrets and organization Secrets included; once a repository's pull requests report `CI`, the trigger [adoption.md](../../skills/review-gate/references/adoption.md#repo-side-wiring) § Repo-side wiring names, set its own ruleset to require exactly `CI` and `Review gate` and remove its Repository-admin bypass actor (KEN-1882); then stand the organization rulesets step 3 records (KEN-1778); run `skills/review-gate/scripts/provision-environment.sh --org ORG` from the owner's own machine, once and again for each new repository.
...
**Verification**: KEN-1781's validation, once it lands, runs under a lanes app installation token once the owner sets the read-only permissions step 2 records on the app: repository Administration, Metadata, Actions, Environments and Secrets, and organization Secrets. No lanes app token reads a ruleset's bypass actors or the app's own permission set, so the owner reads both in GitHub: the organization ruleset settings, with zero bypass actors, the merge queue and the two required contexts, and the lanes app's permission set, with neither Administration write nor Environments write.

**References**: KEN-1776, KEN-1646, KEN-1672, KEN-1750, KEN-1765, KEN-1773, KEN-1777, KEN-1778, KEN-1779, KEN-1780, KEN-1781, KEN-1799, KEN-1818, KEN-1882; superseded: KEN-1601, KEN-1602
