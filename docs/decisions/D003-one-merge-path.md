...
- `.github/workflows/publish-homebrew.yml` `publish` also mints a token from the two secrets. A real run of that job runs under `environment: kendex` and reads them from it; a dry run takes no environment and reads no secret, so the default-branch-only deployment policy does not refuse a dry-run dispatch from another ref. [../../packaging/README.md](../../packaging/README.md) § Publishing tells the reader to place the two secrets in that environment and nowhere else (KEN-1836).
