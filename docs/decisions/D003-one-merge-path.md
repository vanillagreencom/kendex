...
- A review-gate validation reports, read-only, whether a repository's rulesets, required contexts and app installation match this decision. Per repository it also reads that the `kendex` environment exists, carries the default-branch-only deployment policy and holds the secrets `FLEET_GH_APP_ID` and `FLEET_GH_APP_PRIVATE_KEY` (names only), and that no repository or organization secret of those names exists outside it. The check is review-gate `validate-standard`, `skills/review-gate/scripts/validate-standard.sh` (KEN-1781); a `kendex check` relay of its verdict is an overseer-filed follow-up.
