...
- Owner GitHub steps: install the `vanillagreen-fleet-lanes` app on all repositories with the permission set step 2 records, its read-only repository Actions, Environments and Secrets and organization Secrets included; set each repository's own ruleset to require exactly `CI` and `Review gate` and remove its Repository-admin bypass actor, in the order [adoption.md](../../skills/review-gate/references/adoption.md#repo-side-wiring) § Repo-side wiring states (KEN-1882); then stand the organization rulesets step 3 records (KEN-1778); run `skills/review-gate/scripts/provision-environment.sh --org ORG` from the owner's own machine, once and again for each new repository.
