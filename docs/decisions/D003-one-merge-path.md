...
**Approval**: the owner, directive 1790300329, 2026-09-25; amended by owner directive 1790322161 and owner note 1790324783, 2026-09-25
...
- A review-gate validation reports, read-only, whether a repository's rulesets, required contexts, app installation and the organization secrets `FLEET_GH_APP_ID` and `FLEET_GH_APP_PRIVATE_KEY` match this decision, and `kendex check` relays its verdict (KEN-1781).
...
- Owner GitHub steps, each set once at the organization: install the `vanillagreen-fleet-lanes` app on all repositories with the permission set step 2 records; add the organization secrets `FLEET_GH_APP_ID` and `FLEET_GH_APP_PRIVATE_KEY`; stand the organization rulesets step 3 records (KEN-1778).
