...
- Owner GitHub steps: install the `vanillagreen-fleet-lanes` app on all repositories with the permission set step 2 records, its read-only repository Actions, Environments and Secrets and organization Secrets included; stand the organization rulesets step 3 records (KEN-1778); run KEN-1799's provisioning command from the owner's own machine, once and again for each new repository.
