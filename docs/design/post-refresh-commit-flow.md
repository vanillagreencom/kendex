...
`GeneratedPaths` separates whole-file ownership, shared edit targets, and held positions. The inventory writer and its check use one set: whole-file paths, shared targets, and held positions already listed in the inventory at `HEAD`. A newly declared conflict contributes no inventory entry. The offer uses whole-file ownership for existing files; retaining a held entry grants no permission to replace that file's bytes.

- Take the collection's whole-file paths: each item's `Artifact::File` path, an `Artifact::Tree`'s files and link, an `Artifact::Registration`'s `script` path where it has one, and the instruction shims. In-place sources stay out. Items whose drift row is `Conflict` or `Unmanaged` stay outside whole-file ownership, including those whose inventory entries are retained.
...
- Add paths in `.kendex-generated.json` at `HEAD` that git reports as deleted and that current whole-file ownership does not cover. This includes a sweep's removals. Existing held files stay outside the offer even when their historical inventory entries remain. A never-recorded conflict cannot become deletion ownership through a later inventory commit.
...
The engine reads the inventory at `HEAD` to retain previously recorded held positions. It takes desired state from the manifest (`crates/core/src/engine/settings_scan.rs`, `crates/core/src/engine/ops.rs`) and judges write ownership by the lock (`crates/core/src/engine/removal.rs::orphans`, `crates/core/src/engine/stale.rs`). The inventory travels with render additions and removals because the offer reads the same committed copy through `commit_offer::git::committed_inventory` to identify deletions it can include.
