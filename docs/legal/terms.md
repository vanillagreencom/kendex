...
kendex writes into the directories your coding tools own, and it moves and deletes files it manages there. Removals go to a trash directory and an interrupted change is rolled back, but neither is a backup and neither is promised to work. The trash is emptied automatically at the end of a later apply, refresh, remove or update-pi command: entries older than 30 days are deleted, and while the trash holds more than 512 MB the oldest entries are deleted until it is back under that bound, apart from what that command itself just moved there. Both bounds are settings (`KENDEX_TRASH_KEEP_DAYS`, `KENDEX_TRASH_KEEP_MB`), and `kendex trash` lists or empties it sooner. Keep your own backups.
