...
- Flightdeck has four explicit lanes after PR #128 / #134 / #137 / #138 / #139:
  ...
  - `flightdeck plan start <path>` (plan-file orchestration; `workflows/plan/`). This plan document is itself a valid phase-style plan file under `skills/flightdeck/PLAN-FILE.md`: top-level context sections are shared context, and `### Phase ...` sections under `## Implementation phases` plus `## Additional workstream ...` are the work items. `flightdeck plan start docs/plans/flightdeck-app-run-history-and-pi-status-plan.md` is a viable execution path.
