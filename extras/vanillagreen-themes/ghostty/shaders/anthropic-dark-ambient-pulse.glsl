...
#define FOOT_ON      0.0        // 0.0 or 1.0 (some characters float)
...
#define BODY_COL     vec3(0.640, 0.320, 0.180)
#define OUTLINE_COL  vec3(0.140, 0.060, 0.030)
#define FOOT_COL     vec3(0.640, 0.320, 0.180)
...
#define BELL_COL     vec3(1.000, 0.612, 0.000)
#define BELL_OUTLINE vec3(0.140, 0.060, 0.030)

#define SHOW_BELL    1.0
#define SHOW_BELL_ID 1
#define BG_COL       vec3(0.078, 0.078, 0.074)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Rising ember sparks: tiny dots ascending from below, fading near the top.
    for (int i = 0; i < 14; i++) {
        float fi = float(i);
        float x = hash11(fi * 3.3) * iResolution.x;
        float life = 14.0 + 6.0 * hash11(fi * 1.1);
        float t = fract((iTime + hash11(fi * 9.5) * life) / life);
        float y = mix(iResolution.y + 10.0, -10.0, t);
        float drift = sin(t * 9.42 + fi) * 14.0;
        vec2 d = abs(fragCoord.xy - vec2(x + drift, y));
        if (d.x <= 1.0 && d.y <= 1.0) {
            float fade = 1.0 - smoothstep(0.7, 1.0, t);
            col += vec3(1.0, 0.45, 0.10) * fade * 0.50;
        }
    }
}
