...
#define BODY_COL     vec3(0.520, 0.580, 0.660)
#define OUTLINE_COL  vec3(0.150, 0.170, 0.200)
#define FOOT_COL     vec3(0.520, 0.580, 0.660)
...
#define BELL_COL     vec3(0.784, 0.671, 0.216)
#define BELL_OUTLINE vec3(0.150, 0.170, 0.200)

#define SHOW_BELL    1.0
#define SHOW_BELL_ID 1
#define BG_COL       vec3(0.098, 0.098, 0.098)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Scattered pixel-art pebbles: randomly placed dots that twinkle individually.
    // Positions are fixed per-pebble (no animation drift) so it reads as stone, not snow.
    for (int i = 0; i < 30; i++) {
        float fi = float(i);
        vec2 pos = vec2(hash11(fi * 4.1), hash11(fi * 7.3)) * iResolution.xy;
        vec2 d = abs(fragCoord.xy - pos);
        float twinkle = 0.40 + 0.60 * sin(iTime * 0.45 + fi * 1.7);
        if (d.x <= 0.8 && d.y <= 0.8) {
            col += vec3(0.62, 0.66, 0.74) * twinkle * 0.20;
        }
    }
}
