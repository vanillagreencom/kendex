...
#define BODY_COL     vec3(0.620, 0.380, 0.840)
#define OUTLINE_COL  vec3(0.090, 0.060, 0.140)
#define FOOT_COL     vec3(0.620, 0.380, 0.840)
...
#define BELL_COL     vec3(0.945, 0.400, 0.702)
#define BELL_OUTLINE vec3(0.090, 0.060, 0.140)

#define SHOW_BELL    0.0
#define SHOW_BELL_ID 0
#define BG_COL       vec3(0.082, 0.078, 0.106)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Subtle drifting motes — sparse purple/pink pixels rising slowly with
    // gentle horizontal sway.  No machines, no lasers, no patterns.
    for (int i = 0; i < 12; i++) {
        float fi = float(i);
        float x = hash11(fi * 4.71) * iResolution.x;
        float life = 26.0 + 12.0 * hash11(fi * 1.9);
        float t = fract((iTime + hash11(fi * 6.3) * life) / life);
        float y = mix(iResolution.y + 12.0, -12.0, t);
        float sway = sin(t * 7.85 + fi * 1.7) * 18.0;
        vec2 d = abs(fragCoord.xy - vec2(x + sway, y));
        if (d.x <= 1.0 && d.y <= 1.0) {
            float fade = sin(t * 3.14159);   // brighter mid-life
            vec3 hue = mix(vec3(0.62, 0.38, 0.84), vec3(0.96, 0.42, 0.78),
                           hash11(fi * 11.0));
            col += hue * fade * 0.22;
        }
    }
