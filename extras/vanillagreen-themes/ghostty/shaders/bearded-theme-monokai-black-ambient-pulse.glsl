...
#define ACCESSORY    6.0      // 0=none, 1=leaf-hat, 2=fangs, 3=flower-crown,
...
#define BODY_COL     vec3(0.651, 0.886, 0.180)
#define OUTLINE_COL  vec3(0.122, 0.165, 0.020)
#define FOOT_COL     vec3(0.470, 0.682, 0.118)
#define BLUSH_COL    vec3(0.0, 0.0, 0.0)
#define ACC_COL      vec3(0.976, 0.149, 0.447)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.060, 0.060, 0.060)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.976, 0.149, 0.447)
#define BELL_OUTLINE vec3(0.122, 0.165, 0.020)

#define SHOW_BELL    1.0
#define SHOW_BELL_ID 1
#define BG_COL       vec3(0.078, 0.078, 0.078)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Pop-art monokai syntax-color dots: random 1px dots in green/pink/cyan/yellow,
    // flickering on and off independently.
    for (int i = 0; i < 32; i++) {
        float fi = float(i);
        vec2 pos = vec2(hash11(fi * 2.7), hash11(fi * 5.3)) * iResolution.xy;
        vec2 d = abs(fragCoord.xy - pos);
        float pulse = step(0.40, sin(iTime * 1.4 + fi * 0.71));
        if (d.x <= 1.0 && d.y <= 1.0 && pulse > 0.5) {
            float pick = hash11(fi * 13.1);
            vec3 hue;
            if      (pick < 0.25) hue = vec3(0.65, 0.89, 0.18);   // green
            else if (pick < 0.50) hue = vec3(0.97, 0.15, 0.45);   // pink
            else if (pick < 0.75) hue = vec3(0.40, 0.85, 0.94);   // cyan
            else                  hue = vec3(0.99, 0.91, 0.27);   // yellow
            col += hue * 0.30;
        }
    }
}
