...
#define BODY_COL     vec3(0.937, 0.957, 0.992)
#define OUTLINE_COL  vec3(0.710, 0.745, 0.996)
#define FOOT_COL     vec3(0.937, 0.957, 0.992)
#define BLUSH_COL    vec3(0.957, 0.722, 0.894)
#define ACC_COL      vec3(0.0, 0.0, 0.0)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.710, 0.745, 0.996)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.906, 0.518, 0.518)
#define BELL_OUTLINE vec3(0.710, 0.745, 0.996)

#define SHOW_BELL    1.0
#define SHOW_BELL_ID 1
#define BG_COL       vec3(0.188, 0.204, 0.274)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Falling ice flakes: small diamond shapes (4-pixel) falling slowly.
    for (int i = 0; i < 15; i++) {
        float fi = float(i);
        float dx_base = hash11(fi * 2.71) * iResolution.x;
        float life = 24.0 + 8.0 * hash11(fi * 0.91);
        float t01 = fract((iTime + hash11(fi * 4.7) * life) / life);
        float dy_pos = mix(iResolution.y + 10.0, -10.0, t01);
        float drift = sin(t01 * 6.28 + fi * 1.5) * 14.0;
        vec2 d = floor(fragCoord.xy - vec2(dx_base + drift, dy_pos) + 0.5);
        // Diamond: 4 pixels around a center (no center pixel).
        bool diamond = (d.x == 0.0 && abs(d.y) == 1.0) || (abs(d.x) == 1.0 && d.y == 0.0);
        if (diamond) {
            col += vec3(0.78, 0.91, 0.92) * 0.42;
        }
    }
}
