...
#define BODY_COL     vec3(0.840, 0.780, 0.945)
#define OUTLINE_COL  vec3(0.498, 0.388, 0.749)
#define FOOT_COL     vec3(0.840, 0.780, 0.945)
#define BLUSH_COL    vec3(0.918, 0.463, 0.796)
#define ACC_COL      vec3(0.0, 0.0, 0.0)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.498, 0.388, 0.749)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.824, 0.059, 0.224)
#define BELL_OUTLINE vec3(0.498, 0.388, 0.749)

#define SHOW_BELL    0.0
#define SHOW_BELL_ID 0
#define BG_COL       vec3(0.937, 0.945, 0.961)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Light theme — use SATURATED, slightly-darker colored pixels that pop on cream bg.
    // Drifting confetti-style squares in mauve / red / blue.
    for (int i = 0; i < 13; i++) {
        float fi = float(i);
        ...
        float dy_pos = mix(iResolution.y + 10.0, -10.0, t01);
        float sway = sin(t01 * 12.56 + fi * 2.1) * 28.0;
        vec2 d = abs(fragCoord.xy - vec2(dx_base + sway, dy_pos));
        if (max(d.x, d.y) <= 2.0) {
            float pick = hash11(fi * 9.1);
            vec3 hue;
            if      (pick < 0.34) hue = vec3(0.918, 0.463, 0.796);  // pink
            else if (pick < 0.67) hue = vec3(0.824, 0.059, 0.224);  // red
            else                  hue = vec3(0.118, 0.400, 0.961);  // blue
            // Subtract to darken on light bg.
            col -= (vec3(1.0) - hue) * 0.16;
        }
    }
}
