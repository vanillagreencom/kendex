...
#define SPRITE_TYPE  1.0    // 0=blob, 1=cat, 2=splat,
                                        // 3=pirate corsair, 4=rocketship,
                                        // 5=WSB guy, 6=deer pair, 7=bunny,
                                        // 8=squirrel, 9=NES warrior
#define SPRITE_TYPE_ID 1
#define LOOP_DUR     28.0       // seconds for the blob walk pingpong

// Palette (substituted per theme).
#define BODY_COL     vec3(0.953, 0.961, 0.984)
#define OUTLINE_COL  vec3(0.788, 0.643, 0.965)
#define FOOT_COL     vec3(0.953, 0.961, 0.984)
#define BLUSH_COL    vec3(0.961, 0.745, 0.902)
#define ACC_COL      vec3(0.0, 0.0, 0.0)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.788, 0.643, 0.965)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.929, 0.529, 0.588)
#define BELL_OUTLINE vec3(0.788, 0.643, 0.965)

#define SHOW_BELL    1.0
#define SHOW_BELL_ID 1
#define BG_COL       vec3(0.141, 0.153, 0.227)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Spinning cinnamon-spec orbs: small 1px dots tumbling diagonally,
    // varying speeds and warm hues.
    for (int i = 0; i < 14; i++) {
        float fi = float(i);
        float x0 = hash11(fi * 4.7) * iResolution.x;
        float y0 = hash11(fi * 8.3) * iResolution.y;
        float life = 18.0 + 6.0 * hash11(fi * 1.9);
        float t01 = fract((iTime + hash11(fi * 6.3) * life) / life);
        // Move diagonally across the screen.
        float ang = (hash11(fi * 11.0) - 0.5) * 1.4;
        vec2 vel = vec2(cos(ang), sin(ang)) * 240.0;
        vec2 pos = vec2(x0, y0) + vel * (t01 - 0.5);
        // Wrap around screen.
        pos.x = mod(pos.x + iResolution.x, iResolution.x);
        pos.y = mod(pos.y + iResolution.y, iResolution.y);
        vec2 d = abs(fragCoord.xy - pos);
        if (d.x <= 1.0 && d.y <= 1.0) {
            float pick = hash11(fi * 13.1);
            vec3 hue;
            if      (pick < 0.40) hue = vec3(0.929, 0.529, 0.588);   // peach
            else if (pick < 0.70) hue = vec3(0.933, 0.831, 0.624);   // yellow
            else                  hue = vec3(0.835, 0.624, 0.388);   // tan
            col += hue * 0.28;
        }
    }
}
