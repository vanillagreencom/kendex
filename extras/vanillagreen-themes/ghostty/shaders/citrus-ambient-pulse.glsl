...
#define ACCESSORY    8.0      // 0=none, 1=leaf-hat, 2=fangs, 3=flower-crown,
...
#define MOVEMENT     7.0       // 0=walk, 1=fly, 2=hop, 3=slide,
...
#define LOOP_DUR     70.0       // seconds for the blob walk pingpong

// Palette (substituted per theme).
#define BODY_COL     vec3(0.984, 0.761, 0.176)
#define OUTLINE_COL  vec3(0.306, 0.153, 0.000)
#define FOOT_COL     vec3(0.851, 0.529, 0.118)
#define BLUSH_COL    vec3(0.0, 0.0, 0.0)
#define ACC_COL      vec3(0.404, 0.694, 0.255)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.060, 0.060, 0.060)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.902, 0.224, 0.275)
#define BELL_OUTLINE vec3(0.306, 0.153, 0.000)

#define SHOW_BELL    1.0
#define SHOW_BELL_ID 1
#define BG_COL       vec3(0.063, 0.078, 0.074)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Pop-art bursting fruit confetti: small sharp squares in citrus colors,
    // arcing across the screen in different directions.
    for (int i = 0; i < 14; i++) {
        float fi = float(i);
        float life = 9.0 + 5.0 * hash11(fi * 2.3);
        float t = fract((iTime + hash11(fi * 6.5) * life) / life);
        // Each particle takes an arc trajectory from a corner.
        vec2 origin_corner = vec2(
            step(0.5, hash11(fi * 1.3)) * iResolution.x,
            step(0.5, hash11(fi * 8.1)) * iResolution.y
        );
        vec2 launch = vec2(
            (1.0 - 2.0 * hash11(fi * 7.7)) * iResolution.x * 0.45,
            (1.0 - 2.0 * hash11(fi * 3.3)) * iResolution.y * 0.35
        );
        // Sign-correct launch toward screen center.
        vec2 to_center = (iResolution.xy * 0.5 - origin_corner);
        launch = abs(launch) * sign(to_center);
        // Arc: linear x, parabolic y.
        vec2 pos = origin_corner + launch * t + vec2(0.0, -120.0 * t * (1.0 - t));

        vec2 d = abs(fragCoord.xy - pos);
        if (max(d.x, d.y) <= 2.0) {
            float pick = hash11(fi * 13.1);
            vec3 hue;
            if      (pick < 0.33) hue = vec3(1.00, 0.78, 0.18);  // lemon yellow
            else if (pick < 0.66) hue = vec3(0.40, 0.69, 0.26);  // lime green
            else                  hue = vec3(1.00, 0.55, 0.13);  // orange
            col += hue * (1.0 - smoothstep(0.7, 1.0, t)) * 0.30;
        }
    }
}
