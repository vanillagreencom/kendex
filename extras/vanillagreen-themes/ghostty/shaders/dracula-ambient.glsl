...
#define EYE_STYLE    3.0      // 0=v-oval+hl, 1=round-dot, 2=slit, 3=red-glow
#define ACCESSORY    7.0      // 0=none, 1=leaf-hat, 2=fangs, 3=flower-crown,
...
#define MOVEMENT     1.0       // 0=walk, 1=fly, 2=hop, 3=slide,
...
#define BODY_COL     vec3(0.741, 0.576, 0.976)
#define OUTLINE_COL  vec3(0.110, 0.078, 0.184)
#define FOOT_COL     vec3(0.420, 0.300, 0.580)
#define BLUSH_COL    vec3(0.0, 0.0, 0.0)
#define ACC_COL      vec3(0.741, 0.576, 0.976)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(1.000, 0.333, 0.333)
#define EYE_HL_COL   vec3(1.0, 0.6, 0.6)
#define BELL_COL     vec3(1.000, 0.333, 0.333)
#define BELL_OUTLINE vec3(0.110, 0.078, 0.184)

#define SHOW_BELL    0.0
#define SHOW_BELL_ID 0
#define BG_COL       vec3(0.157, 0.165, 0.212)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Pixel-art bats flying across the screen at varying altitudes, wings flap.
    for (int i = 0; i < 5; i++) {
        float fi = float(i);
        float life = 14.0 + 6.0 * hash11(fi * 2.3);
        float t = fract((iTime + hash11(fi * 5.7) * life) / life);
        float y_anchor = (0.25 + 0.55 * hash11(fi * 9.1)) * iResolution.y;
        float dir = step(0.5, hash11(fi * 11.0)) * 2.0 - 1.0;
        float x_lin = mix(-60.0, iResolution.x + 60.0, t);
        float x = (dir > 0.0) ? x_lin : (iResolution.x - x_lin);
        float y = y_anchor + sin(t * 6.28 + fi) * 35.0;

        vec2 bp = (fragCoord.xy - vec2(x, y)) / 4.0;
        bp.x *= dir;
        vec2 bspx = floor(bp + 0.5);

        // 2-frame wing flap (8fps).
        float flap = mod(floor(iTime * 8.0 + fi * 1.7), 2.0);

        bool body  = (bspx.x == 0.0 && abs(bspx.y) <= 0.0);
        bool wL, wR;
        if (flap < 0.5) {
            // Level wings.
            wL = (bspx.y == 0.0 && (bspx.x == -1.0 || bspx.x == -2.0))
              || (bspx.y == 1.0 && bspx.x == -3.0);
            wR = (bspx.y == 0.0 && (bspx.x ==  1.0 || bspx.x ==  2.0))
              || (bspx.y == 1.0 && bspx.x ==  3.0);
        } else {
            // Wings raised (peak).
            wL = (bspx.x == -1.0 && bspx.y == 1.0)
              || (bspx.x == -2.0 && bspx.y == 2.0);
            wR = (bspx.x ==  1.0 && bspx.y == 1.0)
              || (bspx.x ==  2.0 && bspx.y == 2.0);
        }

        if (body || wL || wR) {
            // Bat yields to text (only opaque where bg shows through).
            col = mix(vec3(0.18, 0.08, 0.26), col, text_amt);
        }
    }
}
