...
#define SHAPE_W      5.5
#define SHAPE_H      5.0
#define SHAPE_SQUARE 0.0   // 0.0 = ellipse, 1.0 = rectangle

// Style selectors (substituted per theme).
#define EYE_STYLE    0.0      // 0=v-oval+hl, 1=round-dot, 2=slit, 3=red-glow
...
#define SPRITE_TYPE  6.0    // 0=blob, 1=cat, 2=splat,
                                        // 3=pirate corsair, 4=rocketship,
                                        // 5=WSB guy, 6=deer pair, 7=bunny,
                                        // 8=squirrel, 9=NES warrior
#define SPRITE_TYPE_ID 6
#define LOOP_DUR     28.0       // seconds for the blob walk pingpong

// Palette (substituted per theme).
#define BODY_COL     vec3(0.700, 0.494, 0.310)
#define OUTLINE_COL  vec3(0.235, 0.137, 0.067)
#define FOOT_COL     vec3(0.700, 0.494, 0.310)
#define BLUSH_COL    vec3(0.929, 0.443, 0.604)
#define ACC_COL      vec3(0.929, 0.443, 0.604)
#define ACC2_COL     vec3(1.000, 0.949, 0.286)
#define EYE_COL      vec3(0.310, 0.110, 0.220)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.780, 0.082, 0.522)
#define BELL_OUTLINE vec3(0.420, 0.176, 0.298)

#define SHOW_BELL    1.0
#define SHOW_BELL_ID 1
#define BG_COL       vec3(0.184, 0.267, 0.278)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // ===== Single hot air balloon drifting across the sky =====
    {
        float cross_dur = 80.0;
        float bt = fract(iTime / cross_dur);
        float bx = mix(-40.0, iResolution.x + 40.0, bt);
        float by = iResolution.y * 0.58 + sin(iTime * 0.5) * 10.0;
        vec3 c1 = vec3(1.00, 0.60, 0.72);
        vec3 c2 = vec3(1.00, 0.92, 0.62);
        vec4 bal = sampleBalloon(fragCoord.xy, vec2(bx, by), 4.0, c1, c2);
        if (bal.a > 0.5) col = mix(bal.rgb, col, text_amt);
    }

    // ===== Pixel-art trees clustered bottom-right =====
    // Drawn at 50% opacity (cityscape-style) so text/UI remains readable.
    vec4 t3 = sampleTree(fragCoord.xy, vec2(iResolution.x - 230.0, 0.0), 2.8, iTime * 0.6 + 2.7);
    vec4 t2 = sampleTree(fragCoord.xy, vec2(iResolution.x - 170.0, 0.0), 3.5, iTime * 0.5 + 1.3);
    vec4 t1 = sampleTree(fragCoord.xy, vec2(iResolution.x -  85.0, 0.0), 5.0, iTime * 0.4);
    if (t3.a > 0.5) col = mix(t3.rgb, col, 0.50 + 0.50 * text_amt);
    if (t2.a > 0.5) col = mix(t2.rgb, col, 0.50 + 0.50 * text_amt);
    if (t1.a > 0.5) col = mix(t1.rgb, col, 0.50 + 0.50 * text_amt);

    // ===== Berry bush — distinct leaf clumps with gaps + visible berries =====
    // Bush sits at the very left edge.  Built from 7 small leaf clusters
    // arranged in a loose canopy with negative space between them.
    vec2 bush_c = vec2(50.0, 0.0);
    vec2 bp = fragCoord.xy - bush_c;

    // Define each leaf cluster: (offset_x, offset_y, radius).
    // Spaced so gaps show through between them.
    float r1 = len2(bp - vec2(-25.0, 22.0));
    float r2 = len2(bp - vec2(-12.0, 30.0));
    float r3 = len2(bp - vec2(  3.0, 26.0));
    float r4 = len2(bp - vec2( 18.0, 22.0));
    float r5 = len2(bp - vec2(-20.0, 12.0));
    float r6 = len2(bp - vec2( -4.0, 14.0));
    float r7 = len2(bp - vec2( 14.0, 10.0));
    float rad1 = 5.5; float rad2 = 6.5; float rad3 = 6.0; float rad4 = 5.0;
    float rad5 = 4.5; float rad6 = 5.0; float rad7 = 4.5;
    float rad1_2 = rad1 * rad1; float rad2_2 = rad2 * rad2;
    float rad3_2 = rad3 * rad3; float rad4_2 = rad4 * rad4;
    float rad5_2 = rad5 * rad5; float rad6_2 = rad6 * rad6;
    float rad7_2 = rad7 * rad7;
    float out1_2 = (rad1 + 1.2) * (rad1 + 1.2); float out2_2 = (rad2 + 1.2) * (rad2 + 1.2);
    float out3_2 = (rad3 + 1.2) * (rad3 + 1.2); float out4_2 = (rad4 + 1.2) * (rad4 + 1.2);
    float out5_2 = (rad5 + 1.2) * (rad5 + 1.2); float out6_2 = (rad6 + 1.2) * (rad6 + 1.2);
    float out7_2 = (rad7 + 1.2) * (rad7 + 1.2);

    bool in_leaf = (r1 <= rad1_2) || (r2 <= rad2_2) || (r3 <= rad3_2) || (r4 <= rad4_2)
                || (r5 <= rad5_2) || (r6 <= rad6_2) || (r7 <= rad7_2);
    // Each cluster has its own outline ring (so internal gaps stay outlined).
    bool out_leaf = !in_leaf &&
                 ( (r1 > rad1_2 && r1 <= out1_2)
                || (r2 > rad2_2 && r2 <= out2_2)
                || (r3 > rad3_2 && r3 <= out3_2)
                || (r4 > rad4_2 && r4 <= out4_2)
    ...
    if (out_leaf) {
        col = mix(vec3(0.10, 0.22, 0.06), col, text_amt);
    }
    if (in_leaf) {
        // Two-tone leaves: lit side (upper-right of each cluster) brighter.
        // Find which cluster owns this pixel by smallest signed distance.
        // Approximate lit/shadow by Y-bias + hash dither.
        float dither = hash21(floor(bp / 2.0));
        vec3 leaf_base   = vec3(0.30, 0.56, 0.20);
        vec3 leaf_lit    = vec3(0.46, 0.74, 0.28);
        vec3 leaf_shadow = vec3(0.20, 0.40, 0.12);
        vec3 leaf_col = (dither > 0.65) ? leaf_lit
                       : (dither < 0.30) ? leaf_shadow
                       : leaf_base;
        col = mix(leaf_col, col, text_amt);
    }

    // ===== Brown twigs/branches at the base of the bush =====
    bool twig = (bp.y >= 0.0 && bp.y <= 7.0) &&
                ( (abs(bp.x - (-22.0)) <= 0.5)
               || (abs(bp.x - (-8.0))  <= 0.5)
               || (abs(bp.x -  ( 6.0)) <= 0.5)
               || (abs(bp.x -  ( 20.0)) <= 0.5));
    if (twig) {
        col = mix(vec3(0.32, 0.20, 0.10), col, text_amt);
    }

    // ===== Berries: red dots with pink highlight, placed inside clusters =====
    vec3 berry_col       = vec3(0.92, 0.18, 0.18);
    vec3 berry_highlight = vec3(1.00, 0.65, 0.65);
    vec3 berry_outline   = vec3(0.42, 0.06, 0.06);

    // Berry positions (hand-picked so each sits inside a leaf cluster).
    vec2 berries[10];
    berries[0] = vec2(-25.0, 22.0);  berries[1] = vec2(-22.0, 24.0);
    berries[2] = vec2(-13.0, 30.0);  berries[3] = vec2(-10.0, 32.0);
    berries[4] = vec2(  3.0, 26.0);  berries[5] = vec2(  6.0, 28.0);
    berries[6] = vec2( 18.0, 21.0);  berries[7] = vec2(-19.0, 12.0);
    berries[8] = vec2( -3.0, 13.0);  berries[9] = vec2( 14.0, 10.0);

    for (int bi = 0; bi < 10; bi++) {
        vec2 berry_pos = berries[bi];
        vec2 d = floor(bp - berry_pos + 0.5);
        float bdr2 = len2(d);
        // 3-pixel berry: center + tiny highlight + outline ring.
        if (bdr2 <= 0.36) {
            col = mix(berry_col, col, text_amt);
        } else if (d.x == 0.0 && d.y == 1.0) {
            // Highlight pixel (above center).
            col = mix(berry_highlight, col, text_amt);
        } else if (bdr2 <= 2.25 && bdr2 > 0.36) {
            // Outline pixels around berry.
            col = mix(berry_outline, col, text_amt);
        }
    }
}
