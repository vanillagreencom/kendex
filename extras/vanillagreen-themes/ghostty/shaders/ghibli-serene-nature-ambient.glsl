...
#define SPRITE_TYPE  8.0    // 0=blob, 1=cat, 2=splat,
                                        // 3=pirate corsair, 4=rocketship,
                                        // 5=WSB guy, 6=deer pair, 7=bunny,
                                        // 8=squirrel, 9=NES warrior
#define SPRITE_TYPE_ID 8
#define LOOP_DUR     28.0       // seconds for the blob walk pingpong

// Palette (substituted per theme).
#define BODY_COL     vec3(0.776, 0.471, 0.275)
#define OUTLINE_COL  vec3(0.180, 0.102, 0.055)
#define FOOT_COL     vec3(0.471, 0.306, 0.188)
#define BLUSH_COL    vec3(0.961, 0.435, 0.490)
#define ACC_COL      vec3(0.404, 0.604, 0.255)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.060, 0.060, 0.060)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.902, 0.435, 0.318)
#define BELL_OUTLINE vec3(0.180, 0.102, 0.055)

#define SHOW_BELL    0.0
#define SHOW_BELL_ID 0
#define BG_COL       vec3(1.000, 0.973, 0.863)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // === Softly falling leaves: chunky pixel-art shape, opaque on bg ===
    // Each leaf rendered at 4x scale so it stays visible on high-DPI.
    float LEAF_SCALE = 4.0;
    for (int i = 0; i < 16; i++) {
        float fi = float(i);
        float dx_base = hash11(fi * 2.31) * iResolution.x;
        float life = 38.0 + 14.0 * hash11(fi * 0.81);
        float t01 = fract((iTime + hash11(fi * 6.3) * life) / life);
        float dy_pos = mix(iResolution.y + 30.0, -30.0, t01);
        float sway = sin(t01 * 8.0 + fi * 1.7) * 42.0
                   + cos(t01 * 4.2 + fi) * 16.0;
        vec2 dpos = vec2(dx_base + sway, dy_pos);
        vec2 d = floor((fragCoord.xy - dpos) / LEAF_SCALE + 0.5);
        // Leaf shape (sprite-px): 3-wide center row + stem + tail.
        bool leaf = (d.y == 0.0 && abs(d.x) <= 1.0)
                 || (d.x == -1.0 && d.y == 1.0)
                 || (d.x == 1.0  && d.y == -1.0)
                 || (d.x == 0.0  && d.y == -1.0);
        if (leaf) {
            float pick = hash11(fi * 7.7);
            vec3 hue;
            if      (pick < 0.34) hue = vec3(0.46, 0.74, 0.22);
            else if (pick < 0.67) hue = vec3(0.92, 0.66, 0.18);
            else                  hue = vec3(0.95, 0.42, 0.16);
            col = mix(hue, col, text_amt);
        }
    }

    // === Forest trees clustered across the bottom (deciduous, sampleTree) ===
    vec4 t1 = sampleTree(fragCoord.xy, vec2(iResolution.x -  90.0,  0.0), 5.5, iTime * 0.30);
    vec4 t2 = sampleTree(fragCoord.xy, vec2(iResolution.x - 200.0,  0.0), 4.0, iTime * 0.40 + 1.7);
    vec4 t3 = sampleTree(fragCoord.xy, vec2(iResolution.x - 290.0,  0.0), 3.0, iTime * 0.55 + 3.1);
    vec4 t4 = sampleTree(fragCoord.xy, vec2(80.0,                   0.0), 3.5, iTime * 0.45 + 4.2);
    if (t3.a > 0.5) col = mix(t3.rgb, col, text_amt);
    if (t4.a > 0.5) col = mix(t4.rgb, col, text_amt);
