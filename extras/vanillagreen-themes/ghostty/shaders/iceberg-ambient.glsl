...
#define ACCESSORY    5.0      // 0=none, 1=leaf-hat, 2=fangs, 3=flower-crown,
...
#define MOVEMENT     3.0       // 0=walk, 1=fly, 2=hop, 3=slide,
...
#define BODY_COL     vec3(0.110, 0.165, 0.275)
#define OUTLINE_COL  vec3(0.039, 0.059, 0.122)
#define FOOT_COL     vec3(1.000, 0.647, 0.000)
#define BLUSH_COL    vec3(0.0, 0.0, 0.0)
#define ACC_COL      vec3(1.000, 0.647, 0.000)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.937, 0.949, 0.961)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.886, 0.471, 0.471)
#define BELL_OUTLINE vec3(0.039, 0.059, 0.122)

#define SHOW_BELL    0.0
#define SHOW_BELL_ID 0
#define BG_COL       vec3(0.086, 0.094, 0.129)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Pixel-art snowflakes: 3-pixel star shapes (cross) falling with slight drift.
    for (int i = 0; i < 22; i++) {
        float fi = float(i);
        float dx_base = hash11(fi * 1.31) * iResolution.x;
        float life = 26.0 + 10.0 * hash11(fi * 0.61);
        float t01 = fract((iTime + hash11(fi * 2.7) * life) / life);
        float dy_pos = mix(iResolution.y + 10.0, -10.0, t01);
        float drift = sin(t01 * 6.28 + fi * 1.3) * 18.0;
        vec2 dpos = vec2(dx_base + drift, dy_pos);
        vec2 d = floor(fragCoord.xy - dpos + 0.5);
        // Plus-sign 3-pixel snowflake.
        bool flake = (d.x == 0.0 && abs(d.y) <= 1.0)
                  || (abs(d.x) <= 1.0 && d.y == 0.0);
        if (flake) {
            col += vec3(0.90, 0.95, 1.00) * 0.55;
        }
    }
}
