...
#define ACCESSORY    3.0      // 0=none, 1=leaf-hat, 2=fangs, 3=flower-crown,
                                        // 4=ninja-band, 5=beak, 6=tongue, 7=wings,
                                        // 8=leaf-stem, 9=belly-patch
#define BLUSH_ON     1.0       // 0.0 or 1.0
#define FOOT_ON      1.0        // 0.0 or 1.0 (some characters float)
#define MOVEMENT     2.0       // 0=walk, 1=fly, 2=hop, 3=slide,
                                        // 4=roll, 5=hover, 6=march, 7=bounce
#define SPRITE_TYPE  0.0    // 0=blob, 1=cat, 2=splat,
                                        // 3=pirate corsair, 4=rocketship,
                                        // 5=WSB guy, 6=deer pair, 7=bunny,
                                        // 8=squirrel, 9=NES warrior
#define SPRITE_TYPE_ID 0
#define LOOP_DUR     28.0       // seconds for the blob walk pingpong

// Palette (substituted per theme).
#define BODY_COL     vec3(0.965, 0.835, 0.953)
#define OUTLINE_COL  vec3(0.337, 0.227, 0.541)
#define FOOT_COL     vec3(0.612, 0.290, 0.490)
#define BLUSH_COL    vec3(0.722, 0.355, 0.580)
#define ACC_COL      vec3(0.722, 0.355, 0.580)
#define ACC2_COL     vec3(0.408, 0.812, 0.639)
#define EYE_COL      vec3(0.337, 0.227, 0.541)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.612, 0.290, 0.490)
#define BELL_OUTLINE vec3(0.337, 0.227, 0.541)

#define SHOW_BELL    1.0
#define SHOW_BELL_ID 1
#define BG_COL       vec3(0.843, 0.933, 0.976)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Floating pixel-art HEARTS in pink + mint, drifting up gently on light bg.
    // Each heart is a 5-pixel cluster (classic 8-bit heart shape).
    for (int i = 0; i < 11; i++) {
        float fi = float(i);
        float x_base = hash11(fi * 4.71) * iResolution.x;
        float life = 28.0 + 10.0 * hash11(fi * 1.13);
        float t = fract((iTime + hash11(fi * 5.7) * life) / life);
        float y = mix(iResolution.y + 12.0, -12.0, t);
        float sway = sin(t * 12.56 + fi * 2.1) * 22.0;
        vec2 d = floor(fragCoord.xy - vec2(x_base + sway, y) + 0.5);
        // 8-bit heart shape (relative to center):
        //   #..#
        //   ####
        //   .##.
        //    ##
        bool heart = (d.y == 1.0 && (d.x == -1.0 || d.x == 1.0))
                  || (d.y == 0.0 && abs(d.x) <= 1.5)
                  || (d.y == -1.0 && abs(d.x) <= 0.5);
        if (heart) {
            float pick = hash11(fi * 9.1);
            vec3 hue;
            if      (pick < 0.5) hue = vec3(0.722, 0.275, 0.561);   // candy pink
            else if (pick < 0.8) hue = vec3(0.41,  0.81,  0.64);    // mint green
            else                 hue = vec3(0.49,  0.31,  0.69);    // lavender
            // Subtract toward darker for visibility on light bg.
            col -= (vec3(1.0) - hue) * 0.20;
        }
    }
    // Sparse twinkling stars (small + bg).
    for (int j = 0; j < 14; j++) {
        float fj = float(j);
        vec2 pos = vec2(hash11(fj * 3.7), hash11(fj * 8.1)) * iResolution.xy;
        vec2 d = abs(fragCoord.xy - pos);
        float tw = 0.4 + 0.6 * sin(iTime * 0.7 + fj * 1.7);
        if (d.x <= 0.6 && d.y <= 0.6) {
            col -= vec3(0.40, 0.20, 0.50) * tw * 0.15;
        }
    }
}
