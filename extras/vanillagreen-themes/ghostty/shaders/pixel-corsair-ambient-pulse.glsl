...
#define SPRITE_TYPE  9.0    // 0=blob, 1=cat, 2=splat,
                                        // 3=pirate corsair, 4=rocketship,
                                        // 5=WSB guy, 6=deer pair, 7=bunny,
                                        // 8=squirrel, 9=NES warrior
#define SPRITE_TYPE_ID 9
#define LOOP_DUR     28.0       // seconds for the blob walk pingpong

// Palette (substituted per theme).
#define BODY_COL     vec3(0.286, 0.157, 0.090)
#define OUTLINE_COL  vec3(0.157, 0.078, 0.039)
#define FOOT_COL     vec3(0.345, 0.196, 0.110)
#define BLUSH_COL    vec3(0.0, 0.0, 0.0)
#define ACC_COL      vec3(0.0, 0.0, 0.0)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.157, 0.078, 0.039)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.906, 0.506, 0.392)
#define BELL_OUTLINE vec3(0.157, 0.078, 0.039)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // === LEGO brick landscape across the bottom ===
    // 7 stacks with varied widths (1-6 cols) and heights (1-3 rows),
    // unevenly spaced via hash jitter so it reads as a landscape, not a row.
    float brick_w = 16.0;
    float brick_h = 12.0;
    for (int i = 0; i < 7; i++) {
        float fi = float(i);
        // Base position spread + per-stack jitter for uneven layout.
        float base   = 0.08 + 0.13 * fi;
        float jitter = (hash11(fi * 7.7) - 0.5) * 0.08;
        float stack_x = (base + jitter) * iResolution.x;
        // Width distribution skewed toward small but with occasional long stacks.
        float wp = hash11(fi * 5.3);
        float w_blocks;
        ...
        // Height distribution: mostly low, occasionally taller.
        float hp = hash11(fi * 9.1);
        float h_blocks;
        if      (hp < 0.35) h_blocks = 1.0;
        else if (hp < 0.75) h_blocks = 2.0;
        else                h_blocks = 3.0;

        float stack_w_px = w_blocks * brick_w;
        float stack_h_px = h_blocks * brick_h;
        float lx = fragCoord.x - stack_x;
        float ly = fragCoord.y;
        if (lx >= 0.0 && lx < stack_w_px && ly >= 0.0 && ly < stack_h_px) {
            float bx = floor(lx / brick_w);
            float by = floor(ly / brick_h);
            // Color per brick.
            float cpick = fract(hash11(fi * 11.0 + bx * 0.13 + by * 0.41));
            vec3 brick_col;
            if      (cpick < 0.25) brick_col = vec3(0.86, 0.20, 0.20);
            else if (cpick < 0.50) brick_col = vec3(0.95, 0.74, 0.20);
            else if (cpick < 0.75) brick_col = vec3(0.30, 0.50, 0.95);
            else                   brick_col = vec3(0.30, 0.70, 0.30);

            float bx_local = mod(lx, brick_w);
            float by_local = mod(ly, brick_h);
            bool edge = (bx_local < 1.0 || bx_local > brick_w - 1.0
                      || by_local < 1.0 || by_local > brick_h - 1.0);
            bool stud = (by_local >= brick_h - 3.0 && by_local <= brick_h - 1.0)
                     && (   (bx_local >= 2.0 && bx_local <= 5.0)
                         || (bx_local >= 9.0 && bx_local <= 12.0));
            if (stud)      col = mix(brick_col * 1.20, col, text_amt);
            else if (edge) col = mix(brick_col * 0.45, col, text_amt);
            else           col = mix(brick_col, col, text_amt);
        }
    }
}
