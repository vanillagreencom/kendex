...
#define ACCESSORY    13.0      // 0=none, 1=leaf-hat, 2=fangs, 3=flower-crown,
...
#define MOVEMENT     0.0       // 0=walk, 1=fly, 2=hop, 3=slide,
                                        // 4=roll, 5=hover, 6=march, 7=bounce
#define SPRITE_TYPE  4.0    // 0=blob, 1=cat, 2=splat,
                                        // 3=pirate corsair, 4=rocketship,
                                        // 5=WSB guy, 6=deer pair, 7=bunny,
                                        // 8=squirrel, 9=NES warrior
#define SPRITE_TYPE_ID 4
#define LOOP_DUR     28.0       // seconds for the blob walk pingpong

// Palette (substituted per theme).
#define BODY_COL     vec3(0.090, 0.467, 0.525)
#define OUTLINE_COL  vec3(0.063, 0.106, 0.180)
#define FOOT_COL     vec3(0.208, 0.275, 0.318)
#define BLUSH_COL    vec3(0.0, 0.0, 0.0)
#define ACC_COL      vec3(0.737, 0.776, 0.804)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(1.000, 0.890, 0.604)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.953, 0.608, 0.471)
#define BELL_OUTLINE vec3(0.063, 0.106, 0.180)

#define SHOW_BELL    0.0
#define SHOW_BELL_ID 0
#define BG_COL       vec3(0.063, 0.106, 0.180)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // === Static city skyline silhouette across the bottom ===
    // Each "building" is 32 px wide; height varies by hash per building.
    float building_w = 32.0;
    float bidx = floor(fragCoord.x / building_w);
    float bh = 40.0 + hash11(bidx * 3.7) * 90.0;       // 40..130 px tall
    bool in_building = fragCoord.y <= bh;
    if (in_building) {
        // Dark blue-black silhouette at 50% opacity (sky shows through).
        col = mix(vec3(0.018, 0.025, 0.045), col, 0.50 + 0.50 * text_amt);

        // Windows: small lit pixels scattered on the building face.
        // Use 6x10 cell grid PER building, gated by per-cell hash.
        float cx = floor((fragCoord.x - bidx * building_w) / 6.0);
        float cy = floor((fragCoord.y - 14.0) / 10.0);
        float cell_id = bidx * 100.0 + cx * 7.0 + cy * 31.0;
        bool window_cell = hash11(cell_id) > 0.62 && fragCoord.y > 15.0 && fragCoord.y < bh - 5.0;
        // Position the window pixel inside the cell at a deterministic offset.
        float win_local_x = mod(fragCoord.x, 6.0);
        float win_local_y = mod(fragCoord.y - 14.0, 10.0);
        bool is_win_px = window_cell
                     && win_local_x >= 1.0 && win_local_x <= 3.0
                     && win_local_y >= 1.0 && win_local_y <= 3.0;
        if (is_win_px) {
            float tw = sin(iTime * 0.6 + cell_id * 1.7);
            vec3 hue = (hash11(cell_id * 11.0) < 0.7)
                       ? vec3(1.0, 0.86, 0.41)
                       : vec3(0.47, 0.84, 0.84);
            float intensity = 0.55 + 0.45 * tw;
            // Windows at 50% opacity to match buildings.
            col = mix(hue * intensity, col, 0.50 + 0.50 * text_amt);
        }
    }

    // === Launch platform (under the rocket, at the very bottom) ===
    float plat_cx = iResolution.x * 0.72;
    float p_dx = fragCoord.x - plat_cx;
    float p_ax = abs(p_dx);
    float py = fragCoord.y;
    bool plat_top    = (py >= 30.0 && py <= 36.0 && p_ax <= 28.0);
    bool plat_upper  = (py >= 20.0 && py <= 30.0 && p_ax <= 36.0);
    bool plat_lower  = (py >= 10.0 && py <= 20.0 && p_ax <= 44.0);
    bool plat_base   = (py >=  0.0 && py <= 10.0 && p_ax <= 52.0);
    bool plat_legs   = false;
    for (int li = 0; li < 4; li++) {
        float lx = -42.0 + float(li) * 28.0;
        if (py >= 0.0 && py <= 30.0 && abs(p_dx - lx) <= 1.5) plat_legs = true;
    }
    bool plat_seam = (py == 30.0 || py == 20.0 || py == 10.0) && p_ax <= 52.0;
    // Blinking red warning lights at the corners of platform top.
    bool plat_warning = (py == 31.0 || py == 32.0)
                     && (p_ax == 27.0 || p_ax == 28.0);
    float blink = step(0.0, sin(iTime * 4.0));

    if (plat_top || plat_upper || plat_lower || plat_base) {
        vec3 plat_col;
        if (plat_top)        plat_col = vec3(0.20, 0.22, 0.27);
        else if (plat_upper) plat_col = vec3(0.14, 0.15, 0.18);
        else if (plat_lower) plat_col = vec3(0.10, 0.11, 0.14);
        else                 plat_col = vec3(0.08, 0.09, 0.12);
        // Apply seam highlight (lighter pixel along each tier boundary).
        if (plat_seam) plat_col = vec3(0.32, 0.34, 0.40);
        // Vertical leg pixels are slightly brighter.
        if (plat_legs) plat_col = vec3(0.28, 0.30, 0.36);
        col = mix(plat_col, col, text_amt);
    }
    if (plat_warning && blink > 0.5) {
        col = mix(vec3(1.0, 0.20, 0.20), col, text_amt);
    }

    // === Moon (hard-edged disc with crater spots, right side, behind city) ===
    vec2 moon_c = vec2(iResolution.x * 0.78, 165.0);
    vec2 md = fragCoord.xy - moon_c;
    float mr2 = len2(md);
    if (mr2 <= 2304.0 && fragCoord.y >= bh) {
        vec3 moon_col = vec3(0.78, 0.74, 0.66);   // bone-white
        // Pixel-art craters (darker patches scattered on face).
        bool crater1 = len2(md - vec2(-14.0,  6.0)) <= 49.0;
        bool crater2 = len2(md - vec2( 11.0, -8.0)) <= 25.0;
        bool crater3 = len2(md - vec2(  5.0, 18.0)) <= 16.0;
        bool crater4 = len2(md - vec2( -6.0, -16.0)) <= 16.0;
        bool crater5 = len2(md - vec2( 18.0, 12.0)) <= 25.0;
        bool crater6 = len2(md - vec2(-20.0, -2.0)) <= 9.0;
        if (crater1 || crater2 || crater3 || crater4 || crater5 || crater6) {
            moon_col = vec3(0.45, 0.42, 0.36);    // crater grey
        }
        col = mix(moon_col, col, text_amt);
    }

    // === Static stars in the sky (fixed positions, slow twinkle) ===
    for (int s = 0; s < 22; s++) {
        float fs = float(s);
        vec2 spos = vec2(hash11(fs * 2.71), 0.55 + hash11(fs * 7.13) * 0.42) * iResolution.xy;
        // Skip stars that land in the building strip.
        if (spos.y < bh + 6.0) continue;
        vec2 sd2 = floor(fragCoord.xy - spos + 0.5);
        // Tiny + shape (1px or 3px depending on hash).
        float sz = (hash11(fs * 13.7) < 0.30) ? 1.0 : 0.0;
        bool star = (sd2.x == 0.0 && abs(sd2.y) <= sz)
                 || (abs(sd2.x) <= sz && sd2.y == 0.0);
        float tw = 0.5 + 0.5 * sin(iTime * 0.6 + fs * 1.9);
        if (star) {
            vec3 hue = (hash11(fs * 9.1) < 0.7)
                       ? vec3(1.0, 0.93, 0.78)   // warm white
                       : vec3(0.49, 0.88, 0.94); // cool teal
            col = mix(hue * tw, col, text_amt);
        }
    }

    // === Occasional shooting stars (streaks zip diagonally) ===
    for (int i = 0; i < 3; i++) {
        float fi = float(i);
        float life = 8.0 + 4.0 * hash11(fi * 7.1);
        float seg = floor((iTime + hash11(fi * 3.1) * life) / life);
        float t01 = fract((iTime + hash11(fi * 3.1) * life) / life);
        // Random angle + start position per segment.
        float ang = hash11(seg * 1.7 + fi) * 0.5 - 0.9;   // diagonal down-right
        vec2 start = vec2(-30.0 + hash11(seg * 5.3 + fi) * iResolution.x * 0.6,
                          iResolution.y * (0.75 + 0.15 * hash11(seg * 11.0 + fi)));
        vec2 vel = vec2(cos(ang), sin(ang)) * 800.0;
        vec2 pos = start + vel * t01;
        // Streak: 1 px wide perpendicular distance, length ~32 px behind head.
        vec2 to_p = fragCoord.xy - pos;
        vec2 along = vec2(cos(ang), sin(ang));
        float behind = -dot(to_p, along);
        vec2 perp = to_p - along * (-behind);
        float perp_d2 = len2(perp);
        if (behind >= 0.0 && behind <= 32.0 && perp_d2 <= 0.49) {
            float fade = 1.0 - behind / 32.0;
            col = mix(vec3(1.0, 0.95, 0.78) * fade, col, text_amt);
        }
    }
}
