...
#define BODY_COL     vec3(0.922, 0.737, 0.729)
#define OUTLINE_COL  vec3(0.149, 0.090, 0.227)
#define FOOT_COL     vec3(0.922, 0.737, 0.729)
#define BLUSH_COL    vec3(0.922, 0.435, 0.573)
#define ACC_COL      vec3(0.0, 0.0, 0.0)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.149, 0.090, 0.227)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.922, 0.435, 0.573)
#define BELL_OUTLINE vec3(0.149, 0.090, 0.227)

#define SHOW_BELL    0.0
#define SHOW_BELL_ID 0
#define BG_COL       vec3(0.051, 0.051, 0.051)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Drifting rose petals on the darker bg.
    for (int i = 0; i < 9; i++) {
        float fi = float(i);
        ...
        if (d.x <= 1.0 && d.y <= 0.5) {
            ...
            vec3 hue = (pick < 0.5) ? vec3(0.92, 0.43, 0.57)
                                    : vec3(0.92, 0.74, 0.73);
            col += hue * 0.32;
        }
    }
