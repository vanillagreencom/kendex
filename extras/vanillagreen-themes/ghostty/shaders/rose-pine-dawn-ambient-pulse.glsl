...
#define SPRITE_TYPE  7.0    // 0=blob, 1=cat, 2=splat,
                                        // 3=pirate corsair, 4=rocketship,
                                        // 5=WSB guy, 6=deer pair, 7=bunny,
                                        // 8=squirrel, 9=NES warrior
#define SPRITE_TYPE_ID 7
#define LOOP_DUR     28.0       // seconds for the blob walk pingpong

// Palette (substituted per theme).
#define BODY_COL     vec3(0.843, 0.510, 0.494)
#define OUTLINE_COL  vec3(0.341, 0.322, 0.475)
#define FOOT_COL     vec3(0.843, 0.510, 0.494)
#define BLUSH_COL    vec3(0.706, 0.388, 0.478)
#define ACC_COL      vec3(0.0, 0.0, 0.0)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.341, 0.322, 0.475)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.706, 0.388, 0.478)
#define BELL_OUTLINE vec3(0.341, 0.322, 0.475)

#define SHOW_BELL    1.0
#define SHOW_BELL_ID 1
#define BG_COL       vec3(0.980, 0.957, 0.929)
    ...
    // ============================================================
{
    ...
    for (int i = 0; i < 7; i++) {
        float fi = float(i);
        float dx_base = hash11(fi * 4.71) * iResolution.x;
        float life = 40.0 + 10.0 * hash11(fi * 1.13);
        float t01 = fract((iTime + hash11(fi * 5.7) * life) / life);
        float dy_pos = mix(iResolution.y + 12.0, -12.0, t01);
        float sway_x = sin(t01 * 12.56 + fi * 2.1) * 28.0;
        vec2 d = abs(fragCoord.xy - vec2(dx_base + sway_x, dy_pos));
        if (d.x <= 1.0 && d.y <= 0.5) {
            float pick = hash11(fi * 9.1);
            vec3 hue = (pick < 0.5) ? vec3(0.71, 0.39, 0.48)
                                    : vec3(0.92, 0.62, 0.49);
            col -= (vec3(1.0) - hue) * 0.18;
        }
    }

    // === Pine trees in the bottom-right ===
    vec3 pine_light = vec3(0.32, 0.50, 0.30);
    vec3 pine_dark  = vec3(0.18, 0.34, 0.18);
    vec3 trunk_col  = vec3(0.35, 0.22, 0.12);
    vec3 pine_out   = vec3(0.12, 0.20, 0.10);
    vec4 t1 = samplePineTree(fragCoord.xy, vec2(iResolution.x -  90.0, 0.0), 7.0,
                              pine_light, pine_dark, trunk_col, pine_out);
    vec4 t2 = samplePineTree(fragCoord.xy, vec2(iResolution.x - 180.0, 0.0), 5.0,
                              pine_light, pine_dark, trunk_col, pine_out);
    vec4 t3 = samplePineTree(fragCoord.xy, vec2(iResolution.x - 250.0, 0.0), 3.5,
                              pine_light, pine_dark, trunk_col, pine_out);
    if (t3.a > 0.5) col = mix(t3.rgb, col, text_amt);
    if (t2.a > 0.5) col = mix(t2.rgb, col, text_amt);
    if (t1.a > 0.5) col = mix(t1.rgb, col, text_amt);

    // === Horizontal sunrise gradient bands at the bottom ===
    float y_px = fragCoord.y;
    vec3 band_col = vec3(0.0);
    float band_alpha = 0.0;
    if (y_px < 18.0) {
        band_col = vec3(0.96, 0.42, 0.30); band_alpha = 0.45;
    } else if (y_px < 36.0) {
        band_col = vec3(0.97, 0.58, 0.38); band_alpha = 0.38;
    } else if (y_px < 56.0) {
        band_col = vec3(0.96, 0.74, 0.52); band_alpha = 0.28;
    } else if (y_px < 78.0) {
        band_col = vec3(0.96, 0.84, 0.66); band_alpha = 0.18;
    }
    if (band_alpha > 0.0) col = mix(col, band_col, band_alpha);

    // === Partial sun disc rising from below the horizon ===
    vec2 sun_c = vec2(iResolution.x * 0.55, -30.0);
    float sd2 = len2(fragCoord.xy - sun_c);
    if (sd2 <= 12100.0) {
        vec3 sun_inner = vec3(0.99, 0.82, 0.50);
        vec3 sun_rim   = vec3(0.88, 0.50, 0.36);
        vec3 sun_layer = (sd2 > 10000.0) ? sun_rim : sun_inner;
        float alpha = (sd2 > 10000.0) ? 0.60 : 0.50;
        col = mix(col, sun_layer, alpha);
    }

    // === Butterflies fluttering around (darker tones for light bg) ===
    for (int i = 0; i < 5; i++) {
        float fi = float(i);
        vec2 base = vec2(
            0.15 + 0.70 * hash11(fi * 3.1),
            0.30 + 0.50 * hash11(fi * 7.7)
        ) * iResolution.xy;
        float fx = 0.4 + 0.3 * hash11(fi * 11.0);
        float fy = 0.25 + 0.25 * hash11(fi * 13.0);
        vec2 pos = base + vec2(
            sin(iTime * fx + fi) * 60.0,
            cos(iTime * fy + fi * 1.7) * 35.0
        );
        // Dawn theme: keep butterflies WARM only — no purple/lavender tones
        // to avoid muddy mid-band colors against the cream background.
        float pick = hash11(fi * 9.1);
        vec3 wing_col;
        if      (pick < 0.34) wing_col = vec3(0.71, 0.39, 0.48);  // rose
        else if (pick < 0.67) wing_col = vec3(0.85, 0.42, 0.30);  // coral
        else                  wing_col = vec3(0.86, 0.55, 0.30);  // amber
        vec4 bf = sampleButterfly(fragCoord.xy, pos, 3.0, wing_col, 7.0, fi * 1.3);
        if (bf.a > 0.5) col = mix(bf.rgb, col, text_amt);
    }
}
