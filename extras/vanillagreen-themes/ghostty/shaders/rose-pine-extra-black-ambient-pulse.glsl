...
#define BODY_COL     vec3(0.984, 0.769, 0.663)
#define OUTLINE_COL  vec3(0.094, 0.063, 0.094)
#define FOOT_COL     vec3(0.984, 0.769, 0.663)
#define BLUSH_COL    vec3(1.000, 0.357, 0.537)
#define ACC_COL      vec3(0.0, 0.0, 0.0)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.094, 0.063, 0.094)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(1.000, 0.357, 0.537)
#define BELL_OUTLINE vec3(0.094, 0.063, 0.094)

#define SHOW_BELL    1.0
#define SHOW_BELL_ID 1
#define BG_COL       vec3(0.000, 0.000, 0.000)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Sparse drifting rose petals on pure-black void.
    for (int i = 0; i < 7; i++) {
        float fi = float(i);
        float dx_base = hash11(fi * 4.71) * iResolution.x;
        float life = 42.0 + 10.0 * hash11(fi * 1.13);
