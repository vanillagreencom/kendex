...
#define BODY_COL     vec3(0.918, 0.604, 0.593)
#define OUTLINE_COL  vec3(0.243, 0.180, 0.290)
#define FOOT_COL     vec3(0.918, 0.604, 0.593)
#define BLUSH_COL    vec3(0.922, 0.435, 0.573)
#define ACC_COL      vec3(0.0, 0.0, 0.0)
#define ACC2_COL     vec3(0.0, 0.0, 0.0)
#define EYE_COL      vec3(0.140, 0.125, 0.192)
#define EYE_HL_COL   vec3(1.0, 1.0, 1.0)
#define BELL_COL     vec3(0.922, 0.435, 0.573)
#define BELL_OUTLINE vec3(0.140, 0.125, 0.192)

#define SHOW_BELL    1.0
#define SHOW_BELL_ID 1
#define BG_COL       vec3(0.137, 0.129, 0.212)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Same drifting rose petals, cooler-tinted under moonlight.
    for (int i = 0; i < 9; i++) {
        float fi = float(i);
        float dx_base = hash11(fi * 4.71) * iResolution.x;
        float life = 38.0 + 10.0 * hash11(fi * 1.13);
        float t01 = fract((iTime + hash11(fi * 5.7) * life) / life);
        float dy_pos = mix(iResolution.y + 12.0, -12.0, t01);
        float sway_x = sin(t01 * 12.56 + fi * 2.1) * 30.0
                     + cos(t01 * 6.28 + fi) * 12.0;
        vec2 d = abs(fragCoord.xy - vec2(dx_base + sway_x, dy_pos));
        if (d.x <= 1.0 && d.y <= 0.5) {
            float pick = hash11(fi * 9.1);
            vec3 hue = (pick < 0.5) ? vec3(0.77, 0.66, 0.91)    // lavender
                                    : vec3(0.92, 0.61, 0.59);    // rose-pink
            col += hue * 0.28;
        }
    }
    // A faint moon disc in the upper-right.
    vec2 moon_c = vec2(iResolution.x * 0.82, iResolution.y * 0.78);
    float mr2 = len2(fragCoord.xy - moon_c);
    if (mr2 <= 484.0) {
        col = mix(vec3(0.77, 0.66, 0.91), col, text_amt * 0.7 + 0.3);
    }
    ...
    for (int i = 0; i < 5; i++) {
        ...
        float pick = hash11(fi * 9.1);
        vec3 wing_col;
        if      (pick < 0.33) wing_col = vec3(0.95, 0.65, 0.78);
        else if (pick < 0.67) wing_col = vec3(0.78, 0.65, 0.91);
        else                  wing_col = vec3(0.96, 0.86, 0.65);
