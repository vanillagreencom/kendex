...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // === Pop-art neon city lights (twinkling pixels across the sky) ===
    for (int i = 0; i < 24; i++) {
        float fi = float(i);
        vec2 pos = vec2(hash11(fi * 1.9), hash11(fi * 3.7)) * iResolution.xy;
        vec2 d = abs(fragCoord.xy - pos);
        float pulse = 0.45 + 0.55 * sin(iTime * 1.2 + fi * 1.3);
        if (d.x <= 1.0 && d.y <= 1.0) {
            float pick = hash11(fi * 7.1);
            vec3 hue;
            if      (pick < 0.34) hue = vec3(0.49, 0.81, 1.00);   // cyan
            else if (pick < 0.67) hue = vec3(0.94, 0.55, 0.98);   // pink
            else                  hue = vec3(1.00, 0.70, 0.40);   // amber
            col += hue * pulse * 0.34;
        }
    }

    // === Sporadic neon city silhouette: 4 buildings at hash-fixed positions ===
    // Not across the whole bottom — scattered with gaps.
    float cx = fragCoord.x;
    float cy = fragCoord.y;
    for (int b = 0; b < 4; b++) {
        float fb = float(b);
        // Spread buildings across width with hash jitter; leave gaps.
        float bx_left  = (0.05 + 0.25 * fb + 0.06 * hash11(fb * 3.7)) * iResolution.x;
        float bw       = 50.0 + 30.0 * hash11(fb * 5.3);
        float bh       = 60.0 + 60.0 * hash11(fb * 9.1);
        float bx_right = bx_left + bw;
        bool in_bldg = (cy >= 0.0 && cy <= bh) && (cx >= bx_left && cx <= bx_right);
        if (in_bldg) {
            col = mix(vec3(0.06, 0.08, 0.14), col, text_amt);
            // Neon-lit windows (2px squares, hash-gated).
            float cell_x = floor((cx - bx_left) / 6.0);
            float cell_y = floor(cy / 9.0);
            float cell_id = fb * 100.0 + cell_x * 13.0 + cell_y * 31.0;
            bool window_cell = hash11(cell_id) > 0.55 && cy > 10.0 && cy < bh - 6.0;
            float lx = mod(cx - bx_left, 6.0);
            float ly = mod(cy, 9.0);
            bool is_win_px = window_cell && lx >= 1.0 && lx <= 3.0
                                         && ly >= 1.0 && ly <= 3.0;
            if (is_win_px) {
                // Neon color per cell.
                float wpick = hash11(cell_id * 13.0);
                vec3 win_col;
                if      (wpick < 0.34) win_col = vec3(0.49, 0.81, 1.00);   // cyan
                else if (wpick < 0.67) win_col = vec3(0.94, 0.45, 0.98);   // pink
                else                   win_col = vec3(0.78, 0.55, 1.00);   // purple
                col = mix(win_col, col, text_amt);
            }
        }
        // Building top antenna/sign (thin vertical line on some buildings).
        if (hash11(fb * 11.0) > 0.5) {
            float antenna_x = (bx_left + bx_right) * 0.5;
            if (abs(cx - antenna_x) < 0.7 && cy >= bh && cy <= bh + 8.0) {
                col = mix(vec3(0.94, 0.45, 0.98), col, text_amt);
            }
        }
    }
}
