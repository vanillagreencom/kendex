...
#define BODY_COL     vec3(1.000, 0.580, 0.760)
#define OUTLINE_COL  vec3(0.150, 0.060, 0.090)
#define FOOT_COL     vec3(0.780, 0.260, 0.420)
#define BLUSH_COL    vec3(0.880, 0.300, 0.500)
...
#define BELL_COL     vec3(1.000, 0.510, 0.447)
#define BELL_OUTLINE vec3(0.100, 0.030, 0.060)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ============================================================
{
    // Pop-art warp-speed streaks: 1px-tall diagonal lines streaming across.
    for (int i = 0; i < 16; i++) {
        float fi = float(i);
        float speed = 70.0 + hash11(fi * 3.1) * 240.0;
        float life = (iResolution.x + 80.0) / speed;
        float t01 = fract((iTime + hash11(fi * 7.7) * life) / life);
        float head_x = mix(-40.0, iResolution.x + 40.0, t01);
        float y = hash11(fi * 5.3) * iResolution.y;
        float streak_len = 16.0 + hash11(fi * 11.0) * 32.0;
        float dx = fragCoord.x - head_x;
        float perp = abs(fragCoord.y - y);
        if (dx <= 0.0 && -dx <= streak_len && perp <= 1.0) {
            float fade = 1.0 - (-dx / streak_len);
            float pick = hash11(fi * 13.1);
            vec3 hue = mix(vec3(0.65, 0.83, 1.0), vec3(0.82, 0.82, 1.0), pick);
            col += hue * fade * 0.50;
        }
    }
}
