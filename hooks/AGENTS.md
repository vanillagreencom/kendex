...
- A refusal or a notice carries the line `<hook-name>: <key>=<value>`; the rule it follows — a stable key, the value acted on, English below it, the text in one place, and what a suite may assert about it — is [`../skills/code-quality/SKILL.md`](../skills/code-quality/SKILL.md). A reader matches the prefix `^<hook-name>: `, not line 1: a command the hook runs may write its own diagnostic to the same stream first, and that line names a cause the keyed one does not carry, so it is kept. A suite reads the keyed line the same way.
