...
# line is the contract a reader parses, `block-bare-cd: <key>=<value>`: a
# stable key for the condition and the value acted on — the missing tool, why
# the payload could not be read, or the line that is only a cd. The English
# explanation and the rewrite follow on later lines.
refuse() { # KEY VALUE
...
# has not read cannot be shown to scope its directory change. jq leads so the
...
BARE_STATUS=0
BARE=$(printf '%s\n' "$STRIPPED" | grep -E '^cd([[:space:]]+[^&|;]*)?$') || BARE_STATUS=$?
if [ "$BARE_STATUS" -eq 0 ]; then
  # The first such line is the one named; a later one is the same refusal.
  refuse refused "${BARE%%$'\n'*}"
fi
