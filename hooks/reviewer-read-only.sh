...
refuse() { # KEY VALUE
  {
    ...
    case "$1=$2" in
      tools=*)
        echo "$2 is required to read the hook payload and judge the path; refusing rather than skipping the guard"
        ;;
      payload=unreadable)
...
# is refused rather than let through. jq leads so the world with no tools at
# all names it.
