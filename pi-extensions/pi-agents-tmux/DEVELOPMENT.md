...
Child spawning resolves the pi entry through `pane.ts::getPiInvocation` (vstack#192). `argv[1]` is self-re-invoked only when its basename looks like a pi entry (`pi`, `pi.js`, `pi.mjs`, `pi.ts`, `cli.js`, `cli.mjs`, `cli.ts`) AND the nearest `package.json` above the realpathed script proves pi package identity (name `@earendil-works/pi-coding-agent`, a bin map exposing `pi`, or a package named `pi` with string-form bin); a process that merely imported these modules — a standalone harness or test calling `runSingleAgent`, even one named `cli.ts` — falls back to `pi` on PATH instead of forking itself. Direct-import callers can set `PI_SUBAGENT_ENTRY` to pin the entry explicitly; script overrides are resolved to absolute paths against the parent cwd because children spawn from the delegated agent cwd. Independently of entry resolution, every spawned child (bg one-shot env and pane launcher script) carries `PI_SUBAGENT_DEPTH`, and `assertSubagentSpawnDepth` refuses any spawn past depth 3 so a mis-resolved entry dies loudly instead of fork-bombing the machine.
