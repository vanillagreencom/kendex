...
Parallel dispatch runs through a flat worker pool capped at `maxConcurrency` (default 4); there is no separate batch boundary. The earlier `maxParallelTasks` setting is a deprecated no-op kept for setting-file compatibility.
