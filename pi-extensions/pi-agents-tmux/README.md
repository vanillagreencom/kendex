...
- Persistent panes auto-resume after detected provider rate limits. Scheduling prefers structured quota snapshots (Claude usage endpoints, Codex/OpenAI `wham/usage` fixtures/seams, SDK reset/retry fields) and records `resetSource`; localized `resets <time>` prose is only a degraded fallback before plain backoff.
