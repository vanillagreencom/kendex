...
- Task registry locking is cross-process and stale-lock aware; if a killed or stalled Pi process leaves a lock behind, dashboard and Monitor refreshes wait for stale recovery and skip transient lock contention instead of exiting Pi. Pane completion collection leaves outbox files in place until terminal task state is persisted, so lock contention can retry without losing completions.
