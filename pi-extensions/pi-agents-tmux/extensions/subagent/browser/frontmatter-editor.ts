...
import { type ExtensionContext, type Theme, withFileMutationQueue } from "@earendil-works/pi-coding-agent";
...
function refreshVstackManagedAgent(agent: AgentConfig, tomlPath: string): { ok: boolean; message?: string } {
	...
	if (result.status !== 0) {
		...
		const detail = (result.stderr || result.stdout || reason).trim();
		return { ok: false, message: `${reason}: ${detail.split(/\r?\n/).slice(-4).join(" ")}` };
	}
...
export async function showAgentEditConfirmation(ctx: ExtensionContext, message: string): Promise<void> {
	...
	const refreshFailed = /refresh failed/i.test(message);
	await ctx.ui.custom<void>((tui: TUI, theme: Theme, _kb, done) => ({
		...
		render(width: number): string[] {
			...
			const lines = [
				theme.fg(refreshFailed ? "warning" : "success", refreshFailed ? "Agent metadata update needs refresh" : "Agent metadata updated"),
				"",
