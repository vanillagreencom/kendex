...
- If a bg subagent returns `needs_completion` with `reason: "compact-then-empty"`, upstream Pi disposed the runtime during compact recovery before a final message persisted. Inspect `cwdSnapshot.head` (current HEAD sha), `cwdSnapshot.dirty` (true if worktree has uncommitted changes), and `cwdSnapshot.lastCommit.subject` (most recent commit subject) before deciding whether the subagent's work completed.
