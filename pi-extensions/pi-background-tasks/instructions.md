...
- If the recorded child pid is still alive at restore time AND its process identity (start time + comm name captured at spawn) still matches, the task is rehydrated as `running` (not stopped) and no fake exit is fired — use `bg_task log <id>` to inspect; the child is an orphaned process group that the spawning Pi can no longer signal directly. A background liveness watcher (default 30s poll) keeps watching that pid; when it disappears OR the kernel recycles the pid for an unrelated process (identity mismatch), the canonical exit event fires so the agent still gets a turn.
