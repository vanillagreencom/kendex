...
- Integrity diagnostics are written to `<piUserDir>/claude-bridge-diag.log` (an absolute `PI_CODING_AGENT_DIR`, else `~/.pi/agent`) with counts, affected tool names, and sampled tool-call IDs — only when `CLAUDE_BRIDGE_DEBUG=1`, the same gate as the debug log, and never with user-authored message content (VST-15).
