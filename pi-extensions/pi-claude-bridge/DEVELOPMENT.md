...
- Integrity diagnostics are written to `<piUserDir>/claude-bridge-diag.log` (`PI_CODING_AGENT_DIR` when it names a root-anchored path, else `~/.pi/agent`) with counts, affected tool names, and sampled tool-call IDs — only when `CLAUDE_BRIDGE_DEBUG=1`, the same gate as the debug log, and never with user-authored message content (VST-15).
