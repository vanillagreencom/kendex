...
The Flightdeck skill owns state mutation; the daemon owns wake delivery; `pane-respond` owns sending input to inner panes. pi-flightdeck only renders active status from on-disk state and delegates full inspection/control to the Rust app. It reads the durable run-store active pointer first, honoring `FLIGHTDECK_RUN_STORE_ROOT` from the project `.env.local`/`.env` via non-executing parsing of that variable and its referenced variables. Unsupported assignment forms for that variable fail closed rather than falling back to stale legacy state. It falls back to legacy project-local state/archive files only when no active run exists.
