...
| End-of-turn clippy | `turn_end` | If `.rs` files were touched during the turn, runs workspace clippy once. The summary steers the run, so the agent reads it in the turn that follows in every mode, headless included; an interactive session also gets a UI notification. A run that produced exactly what the last steered one produced is notified but not steered again, so an agent that cannot fix an error is told once rather than looped; the comparison is over everything the run wrote, not the report, and over line order canonicalized, since cargo interleaves parallel diagnostics differently on every run, so an edit that only moves an error still steers and so does a second timeout that collected different partial output. A session boundary other than a reload clears it, so suppression never crosses into a new conversation. A run that established nothing — no workspace root, a timeout, a clippy failure printing no error line — says so rather than passing as clean. Advisory only. |
