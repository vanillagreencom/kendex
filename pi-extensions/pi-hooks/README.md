...
| End-of-turn clippy | `turn_end` | If `.rs` files were touched during the turn, runs workspace clippy once. The summary steers the run, so the agent reads it in the turn that follows in every mode, headless included; an interactive session also gets a UI notification. A summary identical to the last one steered is notified but not steered again, so an agent that cannot fix an error is told once rather than looped. A run that established nothing — no workspace root, a timeout, a clippy failure printing no error line — says so rather than passing as clean. Advisory only. |
