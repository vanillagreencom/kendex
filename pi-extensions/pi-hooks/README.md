...
| Pre-commit gate | `tool_call` (bash) | On a bash command whose `git` argv names `commit`, defers to the working directory's git hooks when kendex armed them (`kendex guard install`: `pre-commit` and `commit-msg` both executable and marked, `core.hooksPath` unset), so git validates the commit exactly once. Refuses a git argv that sidesteps an armed hook (`--no-verify`, `-n`, a global `-c` or `--config-env`, a `GIT_CONFIG_*` assignment, a `git config` line naming `core.hooksPath`) and a commit in a repository nothing armed, naming `kendex guard install`. Never runs a check of its own. The command splits into simple commands and only a `git` argv is judged, so a heredoc body, another program's `-n` or `-c`, and prose in a quoted commit message are text, not flags; `sh -c '…'` and git aliases are invisible to it, and `$(…)`, backticks and `cd "$dir"` pass through. Gates the working directory only; a commit aimed at another repository is that repository's own hook's to gate: an armed working directory defers silently, an unarmed one refuses naming the directory it judged, and a working directory that is not a repository passes with a TUI notice naming it. |
...
| Clippy timeout | Max ms for the end-of-turn clippy run, abandoned past it so the turn can continue. |
