...
The tool is named and its input keyed the way the hook was authored to read them: Pi's `bash` is Claude Code's `Bash`, its `find` is `Glob`, its `ls` is `LS`, and the `path` a `read`, `write` or `edit` call carries reaches the hook as `file_path`. That translation is kendex's own (`crates/core/src/render/vocab/mod.rs`), held to it by a test.
