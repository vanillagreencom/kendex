...
import { globalCommand, projectCommand } from "./harness.ts";

for (const row of [
	{ name: "global", root: "/x/.pi/kendex", command: 'bash "/x/.pi/kendex/hooks/guard.sh"', anchor: undefined, expected: "guard" },
	{ name: "global with env", root: "/x/.pi/kendex", command: globalCommand("/x/.pi/kendex/hooks/guard.sh", { GUARD_RULES: "crates/ui/**/*.rs=iced-rs" }), anchor: undefined, expected: "guard" },
