...
- Nothing leaves the process unbounded. Every event is sanitized to a compact envelope (`extensions/event-sanitizer.ts::sanitizeBridgeEvent`): the events in `COMPACT_EVENT_COMPACTORS` keep counts and previews and spill the whole payload, and a shrunk envelope carries `truncated`, `originalBytes`, `rawEventPath` and `rawEventRef`. The events in `STREAMING_DELTA_COMPACTORS` are delta-only: their descriptor never measures or keeps Pi's cumulative `message`, `partial` or `partialResult`, and they never spill, because Pi fires `message_update` once per token and `tool_execution_update` once per partial tool result, so the whole value would be rewritten to disk each time. A `message_update` descriptor keeps the delta and reports its bytes as `originalBytes`; a `tool_execution_update` descriptor keeps the call name and id alone and reports `originalBytes` 0, since Pi's update payload carries no delta and nothing of `partialResult` is retained. `message_end` and `tool_execution_end` carry the whole value instead. A `--raw` response sets `rawError` on a truncated envelope that spilled nothing, so it does not read as a failed spill. That note is optional detail charged against the response budget: when it does not fit it is dropped and `responseTruncated` is set, while a reported failure is never dropped. The stored envelope carries no such text, keeping a streaming turn free of per-event bytes. `extensions/event-history.ts::BridgeHistory` evicts by count and by total bytes, caps each `history` response and reports `responseTruncated`, and spills raw payloads to `<bridgeDir>/raw/<pid>.jsonl` under `maxRawSpillBytes`. A spill the live slots cannot fit is refused with `rawError` without touching the file; the sidecar is rewritten only to reclaim the bytes of evicted envelopes, so one event costs at most one rewrite or one refusal. Sidecars are removed at `session_shutdown` and process exit, and `cleanupStaleSpills` removes those of dead pids at start.
