...
- **Single-object `edit` inputs accepted as one-edit arrays ([#7835](https://github.com/earendil-works/pi/issues/7835))** — no vstack extension ships an edits-array tool (grep zero); the validation lives in Pi's coding-agent and harness edit tools.
...
- **`nanoid` transitive dev-dependency DoS bump** — no vstack extension lockfile contains `nanoid`.
...
- `pi update` no longer treats older registry versions as updates ([#8226](https://github.com/earendil-works/pi/issues/8226)) — `pi update` reconciles `git:`/`npm:` entries only; vstack path packages stay out of scope.
