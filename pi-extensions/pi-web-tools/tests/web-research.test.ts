...
for (const expanded of [false, true]) {
	test(`research source tree: expanded=${expanded}`, () => {
		...
		assert.deepEqual(expanded ? { first: text.includes("├─ [1] Source 1"), url: text.includes("https://example.com/1"), last: text.includes("[20] Source 20"), hidden: text.includes("[21] Source 21"), remaining: /2.*20\/22/.test(lines.at(-1) ?? "") } : lines, expanded ? { first: true, url: true, last: true, hidden: false, remaining: true } : []);
	});
}
