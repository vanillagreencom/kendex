...
**Ownership is that the first line IS the marker.** Not the marker anywhere
in the file: a hand-written file at a generated path that quotes or preserves
it further down would otherwise read as managed, and `render` would overwrite
bytes `adopt` never took over. Byte for byte, with the version and the
recorded paths the only fields free — and free only inside the classes below,
which is what makes the rest of the line a constant. Both generated markers
put that head on their first line: the HTML form is one line entire, and the
`#` form opens with it and closes on a second line.

One rule, and one no containment test can express: a quotation of the marker,
a line reading `# not generated by bot-instructions`, an HTML comment closed
before the token or never closed at all, and a token on a later line of a `#`
run are each the repo's own file, refused. `render`, `adopt` and `orphan` ask
that one question, and the marker's own insertion writes through it, so the
place it goes and the place it is sought cannot come apart.

The first line, **or the first line after a leading prologue**. A leading YAML
frontmatter block is skipped, and so is the `yaml-language-server` schema line
`.coderabbit.yaml` opens with. One rule at every path, so a file that was
moved or copied still answers — a rendered
`.macroscope/correctness/<surface>.md` copied to `.macroscope/approvability.md`
keeps its frontmatter, and that copy is `orphan`'s case rather than an exotic
one.
...
output uses an HTML comment on **one line**, so the whole marker is the one
line `owns` tests. YAML and TOML use `#`, over two lines, and so does
`.macroscope/ignore.md`, whose grammar is glob-per-line with `#` comments
rather than markdown's.
...
**One open decides and replaces.** A generated file's bytes do not depend on
the file, so its replacement has nothing to read. `AGENTS.md` is a
read-modify-write — the splice, and `adopt` taking a region or a file over —
and there the new bytes are computed from the content the write phase itself
read. Computing them from an earlier, separate read would silently discard
whatever landed between the two: the ownership check, the region and the bytes
spliced all come from one read for that reason.

That read decodes strictly, and so does every other read in this package.
**Text this run judges by, or writes back, must round-trip.** A file holding a
byte that is not UTF-8 is refused naming the path, before anything is written:
substituting it would put U+FFFD into content this package does not own, and
report success. Every validator read refuses on the same rule, in the working
tree and in the index alike, or the pre-commit lane passes a commit no render
can produce.

**A path list is neither content nor display text, and round-trips.** `git
ls-files` emits whatever names the repo holds, and a name is bytes to git; a
repo tracking one that is not UTF-8 is a working repo, not a repo to refuse.
Nor may it substitute: a name is a thing this run REOPENS, and a substituted
`b"\xff/AGENTS.md"` still passes the nested-`AGENTS.md` filter and then
addresses a different path. The path list decodes with surrogates, which `os`
calls and `subprocess` argument lists encode back to the bytes git gave. That
is the one exception to the rule above.

**Write phase.** The generator builds and validates a complete scratch tree,
then replaces each path. A failure part way through names what it replaced
before the failure, and `check` reds on every path still carrying the old bytes
until a re-run finishes the set. What the design does not claim is an atomic
multi-file replacement: no filesystem offers one, and a mixed tree that says so
...
**The scratch tree is in memory, never on disk**: the build holds each output's
bytes and the byte validators read the structure those bytes were serialised
from. That removes the "where does it live, and how is it unique per run"
question rather than answering it — a validator failure leaves the repo
...
rendered as a closing PARAGRAPH of the instruction text, one blank line under
it, naming the paths the rules do not cover. A blank line rather than a space,
because `instructions` ending in a fenced code block end on the closing fence,
and a fence with text after it on the line closes nothing — the sentence and
everything after it then rendered as code. Those bots load the instructions for the excluded files and
...
wants. `renders.md` fixes the key and that value for such a surface
...
`repo-toml.md` § The glob dialect rather than left to the author.
...
emitted document is what `coderabbit-schema` and
...
**`.macroscope/ignore.md`.** One glob per line, `#` comments, blank lines
ignored: the grammar Macroscope documents, cited in `references/limits.md`
§ Macroscope. The file is markdown by extension only, so the marker takes the
`#` form here rather than the HTML form every other markdown output uses, and
each glob's reason is a `#` comment on the line above it, taken from the same
two sources `path_filters` uses: the entry's `reason`, or the fixed derived
string. An HTML comment in this file would be a pattern matching nothing.

A `reason` is single-line, which is what stops a reason from putting a line of
the author's choosing into a file whose every non-comment line is a pattern.
`bot-instructions.toml` refuses a break at input time.
...
the render never emits. Macroscope evaluates `exclude` after `include`,
...
`waitsForDiscoveryTimeout` for the whole correctness run. This package
