...
**Which prologue a file may carry is decided by its path**, never by what its
first bytes look like. The two named above belong to `.coderabbit.yaml` and to
the frontmatter-carrying markdown outputs; every other output's marker is the
file's first line. Reading a `---` block as frontmatter wherever one appeared
would let a hand-written `REVIEW.md`, `best_practices.md` or `AGENTS.md`
region open with one and choose what the ownership test read next. The
generator writes the marker through the same rule it reads it by, so the place
it goes and the place it is sought cannot come apart.
