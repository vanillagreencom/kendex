...
all judge every output by that one line.

**Ownership is that the first line IS the marker**, byte for byte, with the
version and the recorded paths the only fields free — and free only inside the
classes below, which is what makes the rest of the line a constant. Both
generated markers put that head on their first line: the HTML form is one line
entire, and the `#` form opens with it and closes on a second line. One rule
for every comment syntax, and a rule no containment test can express: a
quotation of the marker, a line reading `# not generated by bot-instructions`,
an HTML comment closed before the token or never closed at all, and a token on
a later line of a `#` run are each the repo's own file, refused.

**Two questions are asked of that line, and they are not the same question.**
The line is identical for both; what differs is what may precede it.

*May this package overwrite this file?* — asked wherever bytes are about to be
replaced, by `render`, by `adopt`, and by the marker's own insertion. **Strict
about what the path decides**: only the prologue named above for that path may
precede the marker, and only that format's comment syntax may carry it.
Reading a `---` block as frontmatter wherever one appeared would let a
hand-written `REVIEW.md`, `best_practices.md` or `AGENTS.md` region open with
one and choose what the test read next. The generator writes the marker
through this same rule, so the place it goes and the place it is sought cannot
come apart.

*Did this package write this file?* — asked by `orphan`, and **generous about
both**: any prologue this package writes, and either comment syntax. The path
there is where the file ENDED UP rather than where it was written, and a file
that was moved or copied is `orphan`'s case rather than an exotic one. A
rendered `.macroscope/correctness/<surface>.md` copied to
`.macroscope/approvability.md` or under `.macroscope/check-run-agents/` keeps
its frontmatter, and neither destination's format allows one — judged by the
destination's rule it read as unmarked, and the validator went quiet about a
live file carrying this package's marker.

Generosity belongs to the read side and to nothing else. The line still has to
BE the marker on both, so nothing but a copy of this generator's own output
answers either; what differs is the cost of being wrong. Too generous on the
write side is a file overwritten. Too strict on the read side is silence about
a file that is still being read by a bot.
...
**A path list is neither, and round-trips.** `git ls-files -z` emits whatever
names the repo holds, and a name is bytes to git; a repo tracking one that is
not UTF-8 is a working repo, not a repo to refuse, so the rule above does not
reach here. Nor does substitution: a name is a thing this run REOPENS, and a
substituted `b"\xff/AGENTS.md"` still passed the nested-`AGENTS.md` filter and
then addressed a different path, so `agents-section` reported nothing about a
live nested policy file. The path list decodes with surrogates, which `os`
calls and `subprocess` argument lists encode back to the bytes git gave. That
is the one exception to the rule above, and it is the only one outside the
write path.
