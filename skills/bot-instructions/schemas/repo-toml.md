...
| `code_review_path` | string | no | Where the complete review doctrine is written, and the path the `AGENTS.md` directive, the Copilot pointer and CodeRabbit's `code_guidelines.filePatterns` all name. Defaults to `.github/instructions/code-review.md`. Six clauses, each its own refusal: the marker path class `[A-Za-z0-9._/-]`, a `.md` suffix, no empty, `.` or `..` component, a parent directory of exactly `.github/instructions`, no `AGENTS.md` basename in any case, and no upper-case basename. A path this render already writes is refused where the whole output set exists, naming it |

**The parent-directory clause is what keeps a retired path findable.** `orphan` walks `.github/instructions`, so a file left behind at a path an earlier value named is scanned and reported, and `render` refuses rather than writing the second one. A freely placed path would leave a marked file carrying the whole doctrine that nothing looks at once the key moves or `codex` goes false, with `check` reporting clean. The same clause keeps every path this package writes inside the tree its `repo-effects.writes` declares. An `AGENTS.md` basename is refused separately, because every harness loads one at the start of every session for the directory it sits in — the cost this file exists to move out of the root one. Case-insensitively, since a case-insensitive filesystem loads `agents.md` as that file.

**The upper-case clause keeps two namespaces from aliasing.** `[[bot-instructions.surface]] name` is lower-case by its own refusal, so every surface renders a lower-case basename into this same directory. An upper-case basename here case-folds onto one of them, and `render`'s collision clause compares path strings: the two would pass it, a case-insensitive filesystem would keep whichever file the write phase put down last, and the run would report writing both. Refusing the basename keeps that clause a plain string comparison.
...
| Key | Renders |
|-----|---------|
| `codex` | the pointed `code_review_path` file, which Codex, Copilot and CodeRabbit reach, and the `AGENTS.md` section, which is the directive line sending Codex there. Which bots that is, and which two keep restated surfaces, is SKILL.md § What reads what |
...
**Render-side second checks.** The `doctrine block text` row is enforced where that value is read — when the generator parses the `## Doctrine` section — and it carries **every** class the row marks, not the heading class alone. Doctrine text does not come through this file, so no input refusal covers it, and each of the three markdown classes is reachable on its own route: a heading line or an underline under a text line renders into `code-review.md`, `REVIEW.md` and `.macroscope/correctness/doctrine.md`, where blocks are `##` sections with paragraphs preserved, forging a section in files whose Escaping paragraphs exist so a string cannot forge one; a line that is exactly `---` opens frontmatter where a block reaches byte 0; and the marker text would forge ownership of a file the repo wrote. Doctrine text reaches no `.instructions.md` file on any route — the routing table gives it eight destinations and that is not one of them — so nothing about the generator emitting that file's frontmatter itself covers any of the three.
...
Both tables use doctrine block IDs as keys. `append` adds text to a block; `replace` substitutes the whole block. Unknown block IDs fail validation. Values follow their row in § The content refusals. Line breaks are preserved on every destination that carries the block, per [the render contract](renders.md) § Common rules; no doctrine reaches the `AGENTS.md` region at all.
