...
**`[bot-instructions.repo] code_review_path` is the second such entry, and it is outside that table.** Its clauses are shapes rather than content refusals — the marker path class, a `.md` suffix, no empty, `.` or `..` component, the `CODE_REVIEW_TREE` parent, and no `AGENTS.md` basename — and `config._code_review_path` encodes them. `repo-toml.md` § `[bot-instructions.repo]` states them on the key's own row. Five clauses, six controls in `toml-schema.test.sh`, the parent-directory clause carrying two: a sibling directory and a directory nested below the tree.

Reading the table's marked cells against its Enforced column, plus the two path-shape entries named above, is how the control count is checked; that is what a validator restating its own copy of the predicates would defeat.

**Rejects, the cross-flag set.** `qodo_best_practices` or `qodo_review_md` true while `qodo` is false. `copilot` or `coderabbit` true while `codex` is false, because that flag is what writes the pointed `code-review.md`, where both of those bots get their doctrine and where the Copilot pointer sentence aims. A non-empty `[[bot-instructions.surface]]` set while `copilot`, `coderabbit`, `macroscope` and `qodo_best_practices` are all false: surface text has no route to any other bot, so those surfaces are instructions the author wrote and nothing will ever read.
...
- A missing block in the `code-review.md` or `macroscope doctrine.md` column. Both columns carry every block, because the first is where Codex, Copilot and CodeRabbit are all sent and the second is Macroscope's only surface, and the table states that as an invariant of itself. Delete the `8` from `reply-contract`'s `code-review.md` cell and three bots lose the reply contract with every other validator green.
...
**Rejects, also.** A non-empty exclusion set that a destination the routing table marks as carrying the paths does not carry: `code-review.md`, `pr_agent issues`, `pr_agent extra`. Each is read as the value the bot reads — the rendered pointed file, `[review_agent] issues_user_guidelines`, `[pr_reviewer] extra_instructions` — never as the whole rendered file, because `.pr_agent.toml` also carries `[ignore] glob` listing every exclusion, and asking whether a path appears anywhere in that file is answered by the unrelated mechanism beside the guidance. That is what holds the requirement in SKILL.md § Every rendered config excludes the render trees — without it a render could drop the paths from the one surface Codex reads and violate nothing checkable.
...
**Silent failure.** `AGENTS.md` § Code Review Rules is the only file content Codex is documented to load, and that section is where the directive naming the pointed file sits. Rename the heading, move the section, or delete the file, and the directive is gone: Codex reviews every pull request with no repo rules at all, posting normally throughout. Whether it then follows the directive is `references/limits.md`'s question, not this validator's.
...
**The heading predicate over the region's content is not a clause here.** The region carries one directive line the generator writes, and the only value interpolated into it is the configured path, whose class `toml-schema` fixes. No doctrine or repo string reaches the region on any route, so a clause here would be unreachable, and § Controls' own warning is that an unproven clause which is dead or unreachable stays green for good. `repo-toml.md` § The content refusals owns that predicate for the surfaces repo and doctrine text do reach.

**The region's length is not a clause here either, and `adopt` is why.** A region carrying more than the directive line is a finding, but `check` already reds it under `drift` — a longer region is a byte difference from a fresh render — and a second judge here would red beside it on every repo and say the same thing twice. What `drift` cannot cover is `adopt`, which runs no validator at all and would otherwise take eight blocks of doctrine over and report only that the adoption succeeded. That clause is `verbs._region_finding`, reported under `agents-region`, adopt-only, with its own red control and its green counterpart on an already-rendered region.
...
That covers a retired surface's `.instructions.md` and `correctness/*.md`, the root-level files of a bot whose flag went false (`.coderabbit.yaml`, `.pr_agent.toml`, `best_practices.md`, `REVIEW.md`, `.github/copilot-instructions.md`, the `.macroscope/` tree), the pointed `code-review.md` at whatever path `[bot-instructions.repo] code_review_path` names, and the marked `## Code Review Rules` region inside `AGENTS.md` when `[bot-instructions.bots] codex` goes false. That last one is why the region carries a marker at all: it is never a whole file, so a whole-file rule would leave live doctrine in the one place Codex reads, reported by nothing.
...
**The scan is recursive, over what each bot reads rather than what the generator writes.** `.github/instructions/**`, `.macroscope/correctness/**`, `.macroscope/check-run-agents/**` and `.macroscope/approvability.md`, since Copilot and Macroscope read all four and the policy set already counts them. `code_review_path` needs no entry of its own: `toml-schema` refuses a value outside the first of those trees, so a pointed file at a path the key used to name is already in the walk. That refusal and this scan are one rule read from two ends, and `repo-toml.md` § `[bot-instructions.repo]` states it. A marked file one directory down, left by a hand move or a directory rename, is at no path the generator would write, so a flat scan would miss it while Copilot kept loading it — and the two `.macroscope/` paths this package never writes are exactly where a marked generated file could be moved and stay active, particularly after `macroscope = false`, with neither this nor `drift` reporting it.
