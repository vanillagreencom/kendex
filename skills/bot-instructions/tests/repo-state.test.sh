...
# Ownership is the token on the FIRST LINE of the first comment, one rule for
# every comment syntax. Four shapes read further than that line and overwrote
# a file `adopt` never took over, one per way of reading further: past the
# `-->` on a one-line comment, past an unterminated `<!--`, and — at the hash
# carriers, with and without the prologue their format requires — past the
# first `#` line into the rest of the opening `#` run. Each asserts the bytes
# survive AND that render refuses naming the path, because either alone would
# pass on a run that did nothing.
quoted() {
  local repo label path
  repo="$(bi_rendered_repo "quoted-$1")" || return 1
  label="$2"
  path="$3"
  printf '%s' "$4" > "$repo/$path"
  printf '%s' "$4" > "$BI_TMP/quoted-$1.expected"
  git -C "$repo" add -A >/dev/null 2>&1
