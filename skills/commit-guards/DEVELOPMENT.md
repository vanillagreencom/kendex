...
- The base is the remote oid where this repository has that commit; otherwise the first boundary commit of `git rev-list --boundary <local-oid> --not --remotes=<remote>`. That boundary is read off `refs/remotes/<name>/*`, which were fetched from `remote.<name>.url`, so it is taken only where git's own push URL — the hook's second argument, after `pushurl` and every `insteadOf` rewrite — is that same URL. A fetch-upstream, push-to-a-fork triangle otherwise bounds the range by what the other repository holds. With no base to vouch for, the whole tree is the scope, which is also where a remote spelled as a URL lands.
