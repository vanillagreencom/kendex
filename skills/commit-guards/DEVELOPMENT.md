...
- The base is the remote oid where this repository has that commit; otherwise the first boundary commit of `git rev-list --boundary <local-oid> --not --remotes=<remote>`. That boundary is read off `refs/remotes/<name>/*`, which were fetched from `remote.<name>.url`, so it is taken only where that key holds exactly one value and it is git's own push URL — the hook's second argument, after `pushurl` and every `insteadOf` rewrite. The key is multi-valued when a remote pushes to two places, a fetch uses the first, and git runs the hook once per URL, so `--get-all` asks it and more than one value means no boundary can be vouched for. `core.hooksPath` and `core.bare` stay on `--get`: those ask which value git uses, and for a scalar that is the last, which is what `--get` returns. A fetch-upstream, push-to-a-fork triangle otherwise bounds the range by what the other repository holds. With no base to vouch for, the whole tree is the scope, which is also where a remote spelled as a URL lands.
