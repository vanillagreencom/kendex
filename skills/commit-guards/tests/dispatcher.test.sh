...
# twice is refused. The render inventory the guards read is an empty list.
repo() { # NAME
  R="$TMP/$1"
...
run_rows \
  ...
  "commit-msg in the batch list is exit 2 with the hook pointer|clean list-commit-msg|COMMIT_GUARDS_CHECKS=commit-msg||rc=2 ${ERR}commit-msg reads a message and cannot run in the batch; wire 'commit-guards commit-msg \"\$1\"' into the commit-msg hook instead" \
  "an unknown name in the check list is exit 2 naming the known set|clean list-unknown|COMMIT_GUARDS_CHECKS=no-such-check||rc=2 ${ERR}COMMIT_GUARDS_CHECKS names unknown check 'no-such-check' (known: $KNOWN)" \
...
run_rows \
  ...
  "--base=REF is the same scope|grown base-2|$BC=1,COMMIT_GUARDS_CHECKS=byte-ceiling|all --base=base|rc=1 $(steps base:base byte-ceiling)$VIOLATIONS" \
...
echo "=== a single check runs alone, flags and exit status passed through ==="
...
single_rows \
  ...
  "flags pass through to the named check|clean single-2||byte-ceiling --all||rc=0 byte-ceiling: OK — 2 tracked file(s) (full sweep) checked, ceiling 200 KB" \
