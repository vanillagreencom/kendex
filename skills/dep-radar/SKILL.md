...
**First run** (no inventory): sweep the repo for pins — package manifests and
lockfiles, `vendor/` dirs, SHA-256 constants near download or pin code, model
manifest scripts, version constants referencing upstream releases — write the
inventory, wire an upstream check per surface, and have the owner review the
tiers.
...
| Patched vendor forks of large upstreams, with no script-gated sync | Upstream releases | Report — owner-decided | — |
