...
- `--issue` is the delegation's `Artifact Key:` line, the workflow-state key whose forms `workflow-state --help` § Keys enumerates, never the tracker-native `OWNER/REPO#N` or a bare number. `--round-id` is its `Round ID:` line.
