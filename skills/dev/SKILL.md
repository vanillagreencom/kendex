...
- **Claude Code** — the Bash tool caps at ~10 min and a turn has no wall-clock primitive, so start the run with `run_in_background`, **end your turn**, and wait for either the harness completion wake or the orchestrator's report-only tail nudge. Two caveats make this safe, and both bite exactly here: (1) **never pipe the run** to `head`/`tail`/`grep` to bound its output — piping is a general property of every result-bearing command, not a per-tool rule: the recorded status becomes the pager's, always 0, while the printed text still says FAILED (and a piped `ci-wait --json | tail` truncates the JSON head, silently losing the verdict). Redirect to a log file and read the FILE afterwards; (2) **the completion wake reports the WRAPPER's exit code, not the command's** — a wake saying `completed (exit code 0)` proves nothing about the validate result; the tool's own `END OF OUTPUT — exit status: N` block in the log is the result. That wake is also not reliably delivered to in-process teammate sub-agents, so it may never arrive: **idling after backgrounding is normal, not a stall** — the orchestrator's wall-clock watchdog closes the round (orch SKILL.md § Wait for Agent Return Before Acting). Do not poll for status or exit code; a poll is an instant no-op turn that advances no wall clock.
