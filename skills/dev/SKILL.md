...
- When a change deletes a call, apply [code-quality § Cleanup](../code-quality/SKILL.md#cleanup) to its callee. A deletion found by that sweep maps to the same Done-when item as the call removal that orphaned it.
- A field, setting, or view member added by the change has a real producer and consumer. When its producer is external, name and document that producer and add its consumer in the change; otherwise, add both sides in the change.
