...
**If you were respawned mid-round and no longer have the `Review items:` list, do not reconstruct it from the raw review JSONs and do not guess** — the delegated set was a curated, renumbered subset, and a guessed mapping puts fabricated reasoning into the durable record. The orchestrator persisted your round's item set at delegation: read `[WORKTREE_PATH]/tmp/dev-round-[ARTIFACT_KEY]-[DEV_ROUND_ID].json` (each `items[]` entry carries the delegated number `n` and the item's full text) and write one `--item` per entry. If that file is missing too, report the gap to the orchestrator instead of writing an artifact you cannot back.
...
**Read-only analysis round.** If your delegation was explicitly investigate-and-recommend — no items to apply, no implementation — do NOT force `--kind fix` with fabricated item decisions, and do NOT skip the artifact. Run `dev-return-write` with `--kind analysis` and your recommendation/evidence as `--summary '[TEXT]'` (inline, single-quoted plain text with no backticks, embedded apostrophes spelled `'\''` — the sanctioned route when the harness refuses a file write) or `--summary-file [FILE]` (exactly one of the two), and no `--commit`/`--validate`/`--item` (all three are rejected for this kind; the artifact omits the commit/validate keys, so it is truthful by construction — see [`../../orch/schemas/dev-return.md`](../../orch/schemas/dev-return.md) § Analysis rounds). Then return with your recommendation instead of the item table below.
