...
**Read-only analysis round.** If your delegation was explicitly investigate-and-recommend — no items to apply, no implementation — do NOT force `--kind fix` with fabricated item decisions, and do NOT skip the artifact. Run `dev-return-write` with `--kind analysis` and your recommendation/evidence as `--summary "$(cat <<'EOF' … EOF)"` (inline, quoted-heredoc so free-form prose passes literally — the sanctioned route when the harness refuses a file write, vstack#1236) or `--summary-file [FILE]` (exactly one of the two), and no `--commit`/`--validate`/`--item` (all three are rejected for this kind; the artifact omits the commit/validate keys, so it is truthful by construction — see [`../../orch/schemas/dev-return.md`](../../orch/schemas/dev-return.md) § Analysis rounds). Then return with your recommendation instead of the item table below.
