...
**Read-only analysis round.** If your delegation was explicitly investigate-and-recommend — no implementation — you have no commit and ran no validation, and the checklist rows about commit/validate/QA labels do not apply. Do NOT force `--kind implement` (that would assert a validation outcome that did not occur) and do NOT skip the artifact (a missing artifact reads as an unfinished round). Write your recommendation and evidence to a file (e.g. `tmp/analysis-[ARTIFACT_KEY].md`), then use the analysis kind — it rejects `--commit`/`--validate` and omits those keys from the artifact, so it is truthful by construction:
