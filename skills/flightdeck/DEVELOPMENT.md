...
- **Merge-permission monitor**. Implemented in `src/daemon/merge-permission-monitor.ts` and called from the daemon run loop. It scans the active run state for ready entries with `merge_blocked_permission` markers and emits a synthetic canonical `merge-permission-blocked` wake at most once per marker per 60s, so watch workflows can re-run authoritative GitHub state checks even when the child pane is quiet.
