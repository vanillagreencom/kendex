...
| `flightdeck-repo-sync` | Safe post-merge git helper for local default-branch reconciliation. It fetches/prunes, computes ahead/behind, fast-forwards clean unambiguous `main`, blocks dirty/ahead/diverged cases, and emits `repo.main_*` activity rows when managed. |
