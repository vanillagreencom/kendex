...
| `flightdeck-repo-sync` | Safe post-merge git helper for local default-branch reconciliation. It validates remote/branch ref components, fetches/prunes with `--refmap=` plus an explicit remote-tracking refspec instead of configured fetch refspecs, computes ahead/behind, fast-forwards clean unambiguous `main`, blocks dirty/ahead/diverged cases, and emits `repo.main_*` activity rows when managed. |
