...
Flightdeck core is the generic tmux-session manager. It owns `TrackedEntry` lifecycle, owner metadata, daemon wake routing, generic prompt handling, and stable pane/window ids for any harness session. Issue orchestration is a domain layer on top: Linear adds GitHub/Linear/worktree metadata under `entry.domain.issue`; GitHub issue mode adds numeric GitHub issue metadata under `entry.domain.github_issue`. Both issue lanes use issue-specific lifecycle states (`merge-ready` / `merged` / `aborted`), while Linear alone owns PR conflict graphs, merge queues, and next-cycle Linear recommendations.
