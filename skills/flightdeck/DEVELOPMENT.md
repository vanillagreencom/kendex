...
Each run directory contains `metadata.json`, `state.json`, `activity.jsonl`, optional `summary.md`, and `snapshots/`. `active-run.json` is project-scoped and points at the current run only. `flightdeck-state run create` initializes these files and sets the pointer; `run ensure --tmux-session <name>` is the session lifecycle entry point used by `flightdeck-session start` / `attach` (it passes `checkStale: true` so tmux-liveness rotates orphan runs). Helper invocations (`get`/`set`/`tracked-entries`/`activity` …) call into `ensureActiveRun` with `checkStale: false`, so an idle helper does not surprise-rotate a run when no tmux session is up. `run ensure` reuses only when the active pointer and metadata match the requested tmux session, finalizes stale runs only when recorded pane ids are proven absent, and fails closed on missing metadata, session mismatch, or tmux liveness failure. `run terminate` marks metadata and state terminated, writes a timestamped snapshot pair (`<TS>.json` + `<TS>.activity.jsonl`) under `snapshots/`, copies the summary into `summary.md` when the referenced file is under the project root, clears the pointer only when it still targets that run, and renames any surviving legacy project-local files to `.migrated` so dashboards never re-read them. `run terminate-active` requires both the active pointer and metadata to match the requested tmux session. `flightdeck-state archive` is a thin wrapper around `terminateActiveRun` plus the `session.completed` activity append and the daemon-stop call. Dashboard self-launch still passes the `FLIGHTDECK_SKIP_ACTIVE_RUN=1` env, but because helper CLIs auto-ensure on first write, the dashboard's pane-registry entry ends up in the same active run that subsequent linear/github entries land in (vstack#227 unified state).
