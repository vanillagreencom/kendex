...
- **Window-name refresh** piggybacks on mid-session reconcile. The daemon calls `pane-registry refresh-window-names`, which records live `tmux display-message -p -t <pane> '#W'` output in `entry.window_name_current` without mutating the original spawn `title`.
...
Each run directory contains `metadata.json`, `state.json`, `activity.jsonl`, optional `summary.md`, and `snapshots/`. `active-run.json` is project-scoped and points at the current run only. `flightdeck-state run create` initializes these files and sets the pointer; `run ensure` is the session lifecycle entry point used by `flightdeck-session start` / `attach` and decides reuse vs fresh run creation without treating the tmux session name as the run id. It reuses only when the active pointer and metadata match the requested tmux session, finalizes stale runs only when recorded pane ids are proven absent, and fails closed on missing metadata, session mismatch, or tmux liveness failure. `run terminate` marks metadata and state terminated, writes a timestamped snapshot, copies the summary into `summary.md` when the referenced file is under the project root, and clears the pointer only when it still targets that run. `run terminate-active` requires both the active pointer and metadata to match the requested tmux session, then syncs the matching project-local state/activity before snapshotting. `flightdeck-state archive` invokes `run terminate-active --tmux-session <name>` before rotating the legacy files, so explicit terminate workflows and generic archive callers both finalize durable history while preserving project-local archives; `flightdeck-session start` / `attach` abort instead of archiving if tmux liveness or archive finalization fails. Dashboard self-launch is explicitly marked with `FLIGHTDECK_DASHBOARD_LAUNCHING=1`, `FLIGHTDECK_SKIP_ACTIVE_RUN=1`, and `--no-active-run` so lifecycle reconciliation must not create a user run for the `flightdeck-dashboard` self-entry. Dashboard history UI remains separate from this status-shell work.
...
Write affordances live under `src/actions.rs` plus confirmation handling in `app/update.rs`: prune shells to `pane-registry remove <entry_id>`, focus shells to `tmux select-window -t <pane_target>`. Settings persistence lives in `src/settings_catalog.rs` and is limited to validated writes of `<project-root>/tmp/flightdeck-settings.toml`; keep its path/symlink rejection, async save flow, visible status/error handling, and no-live-env-mutation behavior intact. Stale detection comes from `src/tmux/panes.rs` and caches `tmux list-panes -a -F '#{pane_id}'` for `TMUX_PROBE_TTL` seconds. Do not add new destructive dashboard writes without a confirm popup and a canonical script/helper owner; do not add new direct-file writes without explicitly documenting the target, safety checks, and user-visible failure path.
