...
Each run directory contains `metadata.json`, `state.json`, `activity.jsonl`, optional `summary.md`, and `snapshots/`. `active-run.json` is project-scoped and points at the current run only. On this branch head, `flightdeck-state run create` initializes a fresh run; `run terminate <run-id>` marks metadata and state terminated, writes a timestamped snapshot, copies a matching activity artifact when present, and clears the pointer only when it still targets that run. Matching-run reuse, summary-copy lifecycle plumbing, and tmux-session-scoped active termination are PR #165/#166 follow-up work, not available commands here. The Rust dashboard uses this store for startup and the `H` History popup: matching active pointer means live project-local view, pointer absence/mismatch means no-active landing, and terminated/imported snapshots are loaded read-only only after explicit selection.
