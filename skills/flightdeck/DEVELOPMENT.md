...
- **Pi adapter event identity dedupe**. Pi subscriber wake rows for assistant `message_end` / `agent_end` events carry `event_identity` from bridge timestamp/raw ref/sequence when available. The daemon uses that identity for adapter-event dedupe instead of final-text hash alone, so repeated identical handshakes such as `PRE-PR-REVIEW-READY: tmp/ready-for-review.txt` wake once per new Pi event while duplicate processing of the same raw event still suppresses.
