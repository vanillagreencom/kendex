...
| `flightdeck-repo-sync` | Safe post-merge git helper for local default-branch reconciliation. It validates remote/branch ref components, checks the remote branch with `ls-remote`, fetches/prunes with `--refmap=` plus an explicit remote-tracking refspec instead of configured fetch refspecs, computes ahead/behind, fast-forwards clean unambiguous `main` only after checking ignored/untracked collisions with incoming tracked paths, blocks missing-remote/dirty/ignored-collision/ahead/diverged cases, and emits `repo.main_*` activity rows when managed. |
