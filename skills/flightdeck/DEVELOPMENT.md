...
- **Pi adapter event identity dedupe**. Pi subscriber wake rows for assistant `message_end` / `agent_end` events prefer a shared `turnId` as `event_identity`, then raw bridge refs/sequence/timestamp when available. The daemon uses that identity for adapter-event dedupe instead of final-text hash alone, while suppressing same-turn `message_end` + `agent_end` echoes for the same final text. Repeated handshakes in later turns, such as `PRE-PR-REVIEW-READY: tmp/ready-for-review.txt`, still wake once per new turn; duplicate processing of the same raw event stays suppressed.
...
- **Merge-permission monitor**. Implemented in `src/daemon/merge-permission-monitor.ts` and called from the daemon run loop. It scans the active run state for ready entries with `merge_blocked_permission` markers and emits a synthetic canonical `merge-permission-blocked` wake at most once per marker per 60s, so watch workflows can re-run authoritative GitHub state checks even when the child pane is quiet. Adapter subscribers must pass `FD_ENTRY_KIND` to `prompt-classify --entry-kind` (or `--entry-kind-unknown`) so issue-only no-footer shapes are not downgraded to `domain-mismatch` before the marker is armed.
...
Use the TrackedEntry seam everywhere new code reads tracked sessions. Core helpers (`readTrackedEntries`, `writeTrackedEntry`, `entryIdForIssue`, `issueIdForEntry`) live under `lib/flightdeck-core/src/state/`; `pane-registry list --format json` and `flightdeck-state tracked-entries` expose the same normalized view to scripts, including flattened PR/worktree fields from `domain.plan_item`. Subscriber classifier calls must carry tracked-entry context (`FD_ENTRY_KIND` / `FD_ENTRY_HARNESS`) into `prompt-classify`; otherwise issue-only adapter sentinels fail closed as `domain-mismatch` and issue lanes can miss workflow handshakes. The Rust dashboard (`lib/flightdeck-dashboard/`) is the canonical read-only consumer for active state and durable history. Live startup/focus calls the run-history helper (`flightdeck-state run ensure --project-root <root> --tmux-session <S>`) when no explicit `--state-file` was supplied, then reads the returned durable `paths.state_json`; this keeps dashboard launch/focus on the same all-dead stale-run rotation contract as `flightdeck-session start` / `attach`. `tui --session <name>` uses `flightdeck-state run active` to validate the requested tmux session and load that durable state path, while `--run-id` / `--archive` remain explicit read-only history paths. Any new dashboard work goes through `state/run_history.rs` for run-store command boundaries — don't add a parallel reader or duplicate `flightdeck-state` lookup.
...
The live activity sidecar for current runs is durable run-store `<run-dir>/activity.jsonl` beside `<run-dir>/state.json`. `flightdeck-state path` / `flightdeck-state activity path` resolve through the active-run pointer, so appenders target the run directory rather than project-local `tmp/flightdeck-activity-<session>.jsonl`. Termination copies final state/activity into `<run-dir>/snapshots/` while preserving the run's canonical `activity.jsonl`; legacy tmp sidecars and `*.jsonl.archive` files are read only as compatibility/import sources and are migrated to `.migrated` on first contact when the active durable path exists. Live retention is capped at 5,000 events and 10 MiB; oversized details are collapsed to `{original_bytes,truncated}`. The append path must keep `activity.jsonl`, truncation replacements, and the sidecar lock at `0600` even under a permissive process umask so durable run-store strict-mode reads do not fail after permission migration.
...
The Rust dashboard crate lives in `skills/flightdeck/lib/flightdeck-dashboard/`; the trampoline at `scripts/flightdeck-dashboard` prefers `target/release/flightdeck-dashboard` and falls back to `cargo run --release`. The crate declares an empty `[workspace]` so a host project's root workspace does not absorb it. The trampoline also runs a fast `find -newer` staleness check against `Cargo.{toml,lock}` and `src/` and triggers `cargo build --release --quiet` before exec when the prebuilt binary predates committed source (e.g. across vstack#234-class state-location changes). Set `FLIGHTDECK_DASHBOARD_NO_REBUILD=1` to suppress the staleness check on systems where the binary is intentionally pinned and cargo is unavailable. If rebuild fails while the existing binary is still executable, the trampoline warns and launches the existing binary instead of blocking child pane startup.
...
| `flightdeck-dashboard` | Rust/ratatui dashboard trampoline. `launch` registers and verifies `.entries.flightdeck-dashboard` via `flightdeck-session start --kind workflow`; `focus-or-launch` takes the same launch lock, focuses existing app panes, and includes stale-probe path/command/stderr diagnostics in JSON/plain errors. Both launch paths run `flightdeck-state run ensure --tmux-session <S>` through `state/run_history.rs` before probing when no explicit state file was supplied, so all-dead stale active runs are rotated first. `--no-daemon` keeps file-mode behavior, while `FLIGHTDECK_DAEMON_RUST=1` starts the Rust daemon. `tui --demo[=NAME]` uses compiled fixtures; `tui --state-file <path>` reads a concrete master-state JSON; `tui --session <name>` reads the matching active run's durable `state.json`, otherwise shows a no-active landing; `tui --run-id <id> [--snapshot <timestamp>]` and `tui --archive <path>` load read-only history/legacy views. Live TUI mode watches state/archive paths with debounce, tails daemon/wake JSONL into the Activity tab, shows cost/source-state indicators, and shells confirmation-gated focus/prune writes to canonical helpers. |
