...
Each run directory contains `metadata.json`, `state.json`, `activity.jsonl`, optional `summary.md`, and `snapshots/`. `active-run.json` is project-scoped and points at the current run only. `flightdeck-state run create` initializes a fresh run; `run ensure` is the lifecycle entrypoint and reuses a live matching active run for the requested tmux session or creates a fresh run when no safe match exists. `run terminate` marks metadata and state terminated, writes a timestamped snapshot, copies matching activity/summary artifacts when present, and clears the pointer only when it still targets that run. `run terminate-active --tmux-session <name>` requires pointer and metadata tmux-session match before terminating. Normal start/attach calls `run ensure`; archive/terminate flows call `run terminate-active` after syncing final project-local state into the durable run. The Rust dashboard uses this store for startup and the `H` History popup: matching active pointer means live project-local view, pointer absence/mismatch means no-active landing, and terminated/imported snapshots are loaded read-only only after explicit selection.
