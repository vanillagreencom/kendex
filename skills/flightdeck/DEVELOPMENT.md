...
Dashboard activity: the Rust dashboard reads structured JSONL through `JsonlActivitySource`, not the legacy daemon/wake sources. It validates `schema_version: 1`, skips malformed lines with diagnostics, tracks device/inode for same-path rotation, falls back to newest activity archive filename when the live activity file is gone, and file-watches live activity sidecars for debounced reloads. Durable history views may bind directly to a run-store `activity.jsonl` path.
...
Each run directory contains `metadata.json`, `state.json`, `activity.jsonl`, optional `summary.md`, and `snapshots/`. `active-run.json` is project-scoped and points at the current run only. `flightdeck-state run create` initializes these files and sets the pointer; `run terminate` marks metadata and state terminated, writes a timestamped snapshot, and clears the pointer only when it still targets that run. The Rust dashboard uses this store for startup and the `H` History popup: active pointer present means live view, pointer absent means no-active landing, and terminated/imported snapshots are loaded read-only only after explicit selection.
...
- `flightdeck-dashboard` — Rust/ratatui standalone dashboard; `launch` opens the tracked workflow dashboard window and optionally starts the Rust daemon. Also supports demo fixtures plus `tui --state-file <path>`, `tui --session <name>` active/no-active durable run loading, `tui --run-id <id> [--snapshot <timestamp>]`, `tui --archive <path>`, debounced live file watching, History popup browsing/import, source-state banners, Activity feed scaffolding, cost/token totals, and confirmation-gated prune/focus actions that are blocked in read-only archive views.
...
| `flightdeck-dashboard` | Rust/ratatui dashboard trampoline. `launch` registers and verifies `.entries.flightdeck-dashboard` via `flightdeck-session start --kind workflow`; `--no-daemon` keeps file-mode behavior, while `FLIGHTDECK_DAEMON_RUST=1` starts the Rust daemon. `tui --demo[=NAME]` uses compiled fixtures; `tui --state-file <path>` reads a concrete master-state JSON; `tui --session <name>` loads the durable active run or a no-active landing; `tui --run-id <id> [--snapshot <timestamp>]` and `tui --archive <path>` load read-only history/legacy views. Live TUI mode watches state/archive paths with debounce, tails daemon/wake JSONL into the Activity tab, shows cost/source-state indicators, and shells confirmation-gated focus/prune writes to canonical helpers. |
