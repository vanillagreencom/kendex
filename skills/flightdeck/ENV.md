...
| `FLIGHTDECK_PRE_PR_REVIEW_MAX_ROUNDS` | `3` | Soft review-then-fix audit cap. Rounds beyond this continue autonomously; after the cap only `P1`/`P2` or explicitly safety-critical `category="fix"` blockers prevent approval, while `P3`/`P4` nits become non-blocking suggestions. The workflow logs `pre-pr-review autonomous-override` instead of asking the user. |
| `FLIGHTDECK_PRE_PR_REVIEW_HARD_CAP` | `MAX+2` | Hard-cap policy for repeated pre-PR blockers. At or beyond this cap, `workflows/shared/pre-pr-review.md` writes a focused blocker/follow-up artifact and routes the child deterministically instead of opening a routine user decision. |
...
| `FLIGHTDECK_MASTER_HARNESS` | auto | Override the supervisor harness passed to `flightdeck-daemon start --master-harness`. Auto-detection checks owner env, harness-specific env, `PI_CODING_AGENT`, then tmux `pane_current_command`; use this only when a wrapper hides the real master harness. |
...
| `FLIGHTDECK_DASHBOARD_NO_REBUILD` | `0` | Set to `1` to suppress the `scripts/flightdeck-dashboard` trampoline's `find -newer` staleness check against `Cargo.{toml,lock}` and `src/`. By default, when cargo is on PATH and source files are newer than the prebuilt binary, the trampoline runs `cargo build --release --quiet` before exec so vstack#234-class state-location changes can't be missed by a pre-existing local build. If rebuild fails while an executable binary already exists, the trampoline warns and uses that binary instead of aborting pane launch. |
