...
| `FLIGHTDECK_PRE_PR_REVIEW_MAX_ROUNDS` | `3` | Soft review-then-fix audit cap. Rounds beyond this continue autonomously when concrete `category="fix"` blockers remain or a verification pass is needed; the workflow logs `pre-pr-review autonomous-override` instead of asking the user. |
