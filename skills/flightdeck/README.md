...
- Pause for humans on risky choices: scope creep, force-merge, issue aborts, domain mismatch, or novel prompt shapes; routine pre-PR round caps and merge-permission blocks are handled as autonomous workflow states, with late-loop `P3`/`P4` review nits downgraded to non-blocking suggestions.
