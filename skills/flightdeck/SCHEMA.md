...
`flightdeck-state run create` writes a fresh `metadata.json`, `state.json`, `activity.jsonl`, and `active-run.json`. Durable readers validate `metadata.json` against its containing project and requested/path-derived run id before returning or mutating a run. `run active` reports the active pointer and metadata, or `null`. `run terminate <run-id>` sets `terminated=true`, writes `terminated_at`, creates a final `snapshots/<timestamp>.json`, copies a matching activity snapshot when present, and clears `active-run.json` only when it still points at the requested run. `run import-legacy` copies `flightdeck-state-*.json.archive` and matching activity archives into deterministic imported run ids without deleting source archives. Matching-run reuse and tmux-session-scoped active termination wrappers are PR #165/#166 follow-up lifecycle work, not commands available on this branch head. PR #167 dashboard live mode therefore treats durable metadata as an active-run gate/label and keeps project-local `flightdeck-state-<session>.json` as the live source. No retention policy deletes durable runs or legacy imports by default; operators must explicitly remove old run directories when they choose to expire history.
