...
Master state lives at `~/.vstack/flightdeck/projects/<project-id>/runs/<run-id>/state.json` (vstack#227). Activity history lives beside it as `<run-dir>/activity.jsonl` and is exposed through `flightdeck-state activity path|append|tail|export`. Both survive compaction; terminate writes a snapshot pair under `<run-dir>/snapshots/<terminated_at>.{json,activity.jsonl}` via `flightdeck-state archive` (see `terminate.md § 6`). The run dir preserves the full session history (including merged-issue `decisions_log`, `pr_number`, `merge_commit`) so post-completion dashboards and post-mortem inspection have the whole session history — do not call `pane-registry remove-merged` between `set terminated true` and `archive`. Dashboard startup shows a no-active-run state and requires explicit History (`H`), `--run-id`, or `--archive` selection for archived data. Daemon-private files in `FD_STATE_DIR` are keyed by `SESSION_KEY=s<N>` instead (see `patterns/tmux-monitoring.md`). The legacy `<project>/tmp/flightdeck-state-<TMUX_SESSION>.json` is migrated to `.migrated` on first contact and never read or written again.
  ...
  "entries": {
    "<ENTRY_ID>": {
      ...
      "domain": {
        ...
        //   "pr_number": 0,
        //   "merge_commit": null,
        //   "merge_blocked_permission": {"reason":"MergePullRequest permission denied","pr":0,"ready":true,"last_checked_at":"<ISO8601>"},
        //   "scope_files_actual": 27
        // }
