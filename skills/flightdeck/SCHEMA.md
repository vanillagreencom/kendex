...
`flightdeck-state run create` writes a fresh `metadata.json`, `state.json`, `activity.jsonl`, and `active-run.json`. `run ensure` is the lifecycle-facing command: it reuses the existing live active run when the active pointer and metadata match the requested tmux session, and creates a fresh run when there is no active pointer, the pointer targets another session, metadata is missing, or the pointed run is terminated. Normal `flightdeck-session start` / `attach` flows call `run ensure` before registering entries so durable metadata exists without making the durable `state.json` the live mutation target. Durable readers validate `metadata.json` against its containing project and requested/path-derived run id before returning or mutating a run. `run terminate` sets `terminated=true`, writes `terminated_at`, creates a final `snapshots/<timestamp>.json`, copies a matching activity snapshot when present, and clears `active-run.json` only when it still points at the requested run. `run terminate-active --tmux-session <name>` first requires the active pointer and run metadata to match that tmux session, then performs the same termination flow, copies the final summary when present, and clears the pointer. `flightdeck-state archive` / terminate workflows sync final project-local state, activity, and summary into the active durable run before rotating legacy project-local archives. `run import-legacy` copies `flightdeck-state-*.json.archive` and matching activity archives into deterministic imported run ids without deleting source archives. No retention policy deletes durable runs or legacy imports by default; operators must explicitly remove old run directories when they choose to expire history.
