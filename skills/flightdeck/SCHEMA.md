...
The `flightdeck-daemon` pi-subscriber binder reads adapter metadata from `pane-registry list --format json` (the flattened view that surfaces `entry.adapter.*` at the top level). For `harness="pi"` entries the daemon requires **all three** of `pi_bridge_pid`, `pi_bridge_socket`, `pi_session_id` along with `cwd`; missing any field forces a `pi-subscriber-bind-skip` and the subscriber is never spawned. Launch paths MUST persist all three before the next daemon reconcile tick:
