...
`flightdeck-state run create` writes `metadata.json`, `state.json`, `activity.jsonl`, and `active-run.json`. `flightdeck-state run ensure` is the lifecycle entry point used by `flightdeck-session start` / `attach`: it reuses a non-terminated active run, creates one when none exists, creates a fresh one when the active run is already terminated, and finalizes a stale active run before creating a replacement when tracked entries exist but no recorded pane id is live in tmux. Durable readers validate `metadata.json` against its containing project and requested/path-derived run id before returning or mutating a run. `run terminate` sets `terminated=true`, writes `terminated_at`, creates a final `snapshots/<timestamp>.json`, copies a matching activity snapshot when present, copies `summary_path` / `--summary-path` to the run `summary.md` when the summary file is present under the project root, and clears `active-run.json` only when it still points at the requested run. `run terminate-active --tmux-session <name>` applies the same termination path only when the active pointer belongs to that tmux session, and syncs the matching project-local state/activity before the final durable snapshot. `run import-legacy` copies `flightdeck-state-*.json.archive` and matching activity archives into deterministic imported run ids without deleting source archives.

Auto-archive on session start: `flightdeck-session start` and `attach` roll the live file to a `.json.archive` sibling before fresh init when (a) `terminated == true` or (b) the file has tracked entries but ZERO `pane_id` is currently alive in tmux. The archive command first terminates the matching active durable run, syncs the project-local state/activity into the durable run snapshot, clears the active pointer, then leaves the legacy `.json.archive` / `.jsonl.archive` files readable for old tooling. This removes the need to manually prune leftover state from prior tmux sessions or crashed masters. `flightdeck-session start` also exports `FLIGHTDECK_ENTRY_ID` into the launched child environment (consumed by `github.sh` / `linear.sh` wrappers to auto-bind activity events to the right entry) and captures the current `git rev-parse --abbrev-ref HEAD` of the entry's cwd into `entry.branch` (informational; not refreshed when the agent switches branches mid-session) and onto every `pr.*` activity row's `refs.branch`.
