...
Daemon staleness contract (vstack#213): `flightdeck-session start` and `attach` are the canonical entry points that arm wake delivery. After `ensure_active_run_for_session` and entry registration they invoke a `ensure_daemon_for_session` helper that compares the per-session `flightdeck-daemon health` output (specifically the `staleness` field, `master_pane_id`, and `subscribed_pane_ids` set) against the live tracked entries and the current `master_pane_id`. If the daemon is missing, master diverged, or staleness is `stale-state` / `stale-inner` / `pre-active-run`, the helper stops the existing daemon and starts a fresh one with the current alive inner panes + harnesses (`pane-registry list --format inner-panes-live` / `inner-harnesses-live`, excluding the dashboard self-entry). `FLIGHTDECK_ENSURE_DAEMON=0` disables the helper for tests; `FLIGHTDECK_DAEMON_BIN` can override the binary used (mirrors `FLIGHTDECK_DASHBOARD_BIN`). `FLIGHTDECK_ARCHIVE_SKIP_DAEMON_STOP=1` opts the archive helper out of its post-archive daemon-stop call. The reconciler in `daemon/reconcile.ts` remains a best-effort safety net that picks up panes added mid-tick — it is not the primary mechanism for arming wake delivery on a new session.

Daemon metadata file: while running, the daemon writes `fd-daemon-<SESSION_KEY>.meta.json` next to its pid file (under `FD_STATE_DIR`) capturing `pid`, `started_at`, `master_pane_id`, `master_harness`, `inner_targets`, `inner_harnesses`, `subscribed_pane_ids`, `state_file_path`, `state_file_inode`, `active_run_id`, `session_id`, `session_key`, `session_name`, and `updated_at`. The loop rewrites `subscribed_pane_ids` + `updated_at` whenever reconcile adds or removes a pane. Graceful exit removes the file; max-lifetime handoff preserves it for the successor to refresh. `flightdeck-daemon health` reads the meta file and derives `staleness: fresh | stale-state | stale-inner | pre-active-run | meta-missing` against the current state-file inode + active-run pointer.
