...
`pi-rate-limit-skipped`: the Pi subscriber runs every non-custom `message_end` through the rate-limit decider. Classifier rejections append an activity-only row with `event_type:"rate_limit_skipped"`, `classifier_tag:"pi-rate-limit-skipped"`, and `reason:"non-assistant"|"no-stopreason"|"stopreason-mismatch"|"no-prose"`; the daemon records it as `agent.rate_limit_skipped` and never wakes master.
