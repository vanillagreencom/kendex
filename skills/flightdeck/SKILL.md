...
- **Classifier + activity signals** — rate-limit decisions tag as `pi-rate-limit-skipped` (classifier rejection; activity-only, reason is `non-assistant` / `no-stopreason` / `stopreason-mismatch` / `no-prose`), `pi-rate-limit-retry` (scheduled), and `pi-rate-limit-exhausted` (ladder spent). Treat these as advisory; do not pause master or prompt user unless exhausted tag fires.
