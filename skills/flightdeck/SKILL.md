...
- **Post-merge local main sync** — after a PR is observably `MERGED`, run `flightdeck-repo-sync main --project-root <PROJECT_ROOT> --remote origin --branch main --json`. The helper fetches/prunes, fast-forwards only clean unambiguous local `main`, and reports `repo.main_synced`, `repo.main_sync_blocked`, or `repo.main_sync_failed`. Do not sync queued auto-merge until a later poll sees real `MERGED`; never reset, stash, discard, or force-push to reconcile local `main`.
