...
- **rate-limit** (`VSTACK_RATE_LIMIT_WATCHDOG`, default on; `VSTACK_RATE_LIMIT_MAX_ATTEMPTS`=5, `VSTACK_RATE_LIMIT_BACKOFF_LADDER`=`60,120,300,600,1800` seconds) — on a detected Claude API rate-limit error, schedules an exponential-backoff steer-retry. Emits `agent.rate_limit_skipped` for classifier rejections with reason `non-assistant` / `no-stopreason` / `stopreason-mismatch` / `no-prose`, plus `agent.rate_limited` / `agent.rate_limit_retry` / `agent.rate_limit_exhausted` for the recovery path, and short-circuits the canonical wake path while a retry is pending.
