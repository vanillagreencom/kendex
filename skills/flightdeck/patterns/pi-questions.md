...
- Spawn commands can still use `pi '/skill:<name> ...'` (see `open-terminal`) because Pi's CLI initial prompt goes through the native expansion path. Mid-session flightdeck daemon wakes for Pi now use `pi-bridge send "/skill:flightdeck linear watch --from-daemon"`.
