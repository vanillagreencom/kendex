...
2. **Re-drain on `bridge_hello`** — `pi-session-bridge` sends `{"type":"bridge_hello","protocol":"pi-session-bridge.v1","state":{"sessionId":"..."}}` the instant the stream socket is accepted. The subscriber logs the connected Pi session id, emits a `pi-session-connected` guard row when the daemon supplied an expected session id, and runs the same drain again. This closes the race window between the initial snapshot and the stream subscription registering with the bridge. `seen_qids` carries forward, so a question seen by both drains (or by a drain and the live `event:"question" action:"opened"`) wakes master exactly once.
