...
5. the daemon wakes the Pi master through `pi-bridge send` with `/skill:flightdeck linear watch --from-daemon`, observable in `pi-bridge history`, with `harness=pi via=pi-bridge` in the daemon log. The test fails if that daemon log is absent.
