...
`flightdeck-daemon stop` terminates the external wake daemon (validates PID + flock holder before killing; refuses on stale PID file). `run terminate-active` marks the matching durable active run terminated, copies the final project-local state/activity into the run snapshot, copies the summary into the run directory when present, and clears `active-run.json`. `archive` rotates the live state file to `tmp/flightdeck-state-<SESSION>-<terminated_at>.json.archive` so the next session in the same tmux name (e.g. `HT`) starts clean instead of inheriting this session's entries, merge queue, and `terminated` flag. The archive preserves the full `.entries` map for post-mortem inspection and dashboard rendering. `archive` also attempts the same active-run termination as an idempotent safety net when a caller omits the explicit `run terminate-active` line.
