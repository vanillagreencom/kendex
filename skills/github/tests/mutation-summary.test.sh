...
# filter that reports `success` from the failed count. jq parses `key: a == b`
# inside `{}` as a syntax error, so an unparenthesized comparison there exits
# non-zero after every mutation has already landed, and a caller that routes on
# the exit status — merge-pr's thread pass — reads a completed resolve as a
# failure.
#
...
run_table "a completed batch mutation renders its summary and exits 0" "\
...
# The inverse: a batch whose mutations did not land reports them under `failed`
# with success false, so `success` reads the mutations and not the rendering.
echo "=== the inverse: a batch that did not land ==="
build resolve-two
gh_stub_answer api-graphql "$(jq -nc --arg a "$ID_A" --arg b "$ID_B" \
  '{data: {t0: {thread: {id: $a, isResolved: true}}, t1: {thread: {id: $b, isResolved: false}}}}')"
assert_eq "$(run "$ID_A $ID_B")" \
  "rc=0 out={\"success\":false,\"resolved\":[\"$ID_A\"],\"failed\":[\"$ID_B\"]} err=- calls=$THREAD_CALLS" \
  "one thread left unresolved reports success false with that id under failed"
...
# unparenthesize FILE LIVE — replace the parenthesized comparison LIVE with its
# unparenthesized inner text, asserting the file carried exactly one and
# carries none after. LIVE's inner text is a substring of LIVE, so the count of
# LIVE itself going 1 to 0 is what establishes the edit.
unparenthesize() {
  local file="$1" live="$2" bare="${2#(}" content before after
  bare="${bare%)}"
  before="$(grep -F -c -- "$live" "$file")"
  [[ "$before" == "1" ]] || {
    printf 'control: %s carries %s occurrences of the live form, not 1\n' "$file" "$before" >&2
    exit 2
  }
  content="$(cat "$file")"
  printf '%s\n' "${content//"$live"/"$bare"}" >"$file"
  after="$(grep -F -c -- "$live" "$file" || true)"
  [[ "$after" == "0" ]] || {
