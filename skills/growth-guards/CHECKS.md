...
- A code span holding `<path>.md § Heading` must name a tracked file with a heading whose text equals `Heading` case-insensitively after trimming; one holding `<path>.md#anchor` must name a tracked file with that slug or explicit anchor; one holding a path alone, `<dir>/<name>.md`, must name a tracked file. The path resolves against the citing file's directory first, then against the repository root. A bare `NAME.md` with no directory is a file name being mentioned, not a citation, and is not judged.
...
The scopes are md-format's, over `GROWTH_GUARDS_MD_REFS_PATHS` minus `GROWTH_GUARDS_MD_EXCLUDES`, with the same `GROWTH_GUARDS_MD_SCOPE`; the default list is the prose lane's plus `docs/architecture/*.md`. Targets resolve against the index whatever the scope, so a link into a file the commit deletes is dead, and a heading is read from the index copy of a file no commit is touching. A tracked path holding a newline can be no link target.
