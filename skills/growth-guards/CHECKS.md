...
The grammar, judged line by line, is `scripts/lib/md-blocks.awk`'s, with its line-shape predicates in `scripts/lib/md-shapes.awk`, and both lanes and the reflow read by that one pair:
...
- A tag whose name holds `_` is no HTML element but a prompt section (`<output_format>`, `<delegation_format>`), and its lines are a template rather than prose: alone on its line it opens a block that runs to the line holding its closing tag, blank lines included, and one never closed is refused like an unterminated comment. An opener sharing its line with prose is a paragraph line, and a lone closing tag is the one-line block CommonMark makes it.
...
- A line whose first non-blank character is `|` opens a table, and so does a one-line paragraph holding `|` over a delimiter row (cells of `-` with an optional `:` at either end, separated by `|`, the outer pipes optional, at least one pipe present). The table runs to the next blank line: every line until then is a row, skipped, except a heading, a fence or a thematic break, which is judged as itself. A table is a boundary.
- A heading is `#` to `######` followed by a space, a tab or the end of the line, or a `=` or `-` underline directly under a paragraph line (setext). A heading needs a blank line before it and after it, whatever the neighbour is (a paragraph, a list, a fence, a table, a thematic break, an HTML block, a definition, a blockquote at any depth, another heading), with one exception: a one-line HTML comment directly over it.
...
- A definition, `[label]: destination` at the line start, is a boundary, so definitions stack without blank lines. A `[label]:` whose destination sits on the next line is a paragraph line and its wrap; the reflow joins it into the one-line form.
- A thematic break (`---`, `***`, `___`, spaces allowed between) is a boundary.
- A blockquote's `>` markers are stripped and its content judged by the same rules. A change of depth is a boundary, except that a paragraph line at a lower depth directly under a quoted paragraph line is that paragraph's lazy continuation and judged as one; a heading or a fence closer on either side of the change still needs its blank line, which the reflow places at the shallower depth so that it ends neither quote.
...
An unterminated fence, front matter, HTML comment or prompt-section block is exit 2 naming the file and the line that opened it: what follows cannot be judged, and skipping it would pass a file this reading never finished.
...
- A file that ends inside a block comment, a heredoc body or a string spanning lines is not extractable: exit 2 naming the file and the line the opener sits on, never a clean count, because every comment after the opener would otherwise be swallowed. A JavaScript regex literal holding an odd number of backticks or quotes leaves the file in that state.
