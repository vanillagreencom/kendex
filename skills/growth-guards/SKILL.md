...
| **byte-ceiling** | A tracked file a change puts over the ceiling (default 200 KB) fails. `--staged` (default) gates the staged additions and changes, `--base REF` the additions since merge-base, `--all` sweeps every tracked file. Lockfiles are exempt built-in. |
