...
echo "=== scopes: with neither flag, GROWTH_GUARDS_MD_SCOPE decides ==="
run_mdf
[ "$RC" -eq 0 ] && case "$OUT" in *"nothing staged to judge"*"GROWTH_GUARDS_MD_SCOPE=all"*) true ;; *) false ;; esac \
...
OUT="$(cd "$R" && GROWTH_GUARDS_MD_SCOPE=all "$MDF" 2>&1)" && RC=0 || RC=$?
[ "$RC" -eq 1 ] && case "$OUT" in *"committed.md:2:"*) true ;; *) false ;; esac \
  && ok "GROWTH_GUARDS_MD_SCOPE=all is --all" || bad "scope=all is --all" "rc=$RC out=$OUT"
...
git -C "$R" add -A
OUT="$(cd "$R" && GROWTH_GUARDS_MD_SCOPE=sometimes "$MDF" 2>&1)" && RC=0 || RC=$?
...
OUT="$(cd "$R" && GROWTH_GUARDS_MD_PATHS='docs/*.md' "$MDF" --all 2>&1)" && RC=0 || RC=$?
[ "$RC" -eq 1 ] && case "$OUT" in *"vendor/wrapped.md"*) false ;; *"docs/wrapped.md:2:"*) true ;; *) false ;; esac \
  && ok "GROWTH_GUARDS_MD_PATHS replaces the list" || bad "path list replaces" "rc=$RC out=$OUT"
mkdir -p "$R/tools"
...
OUT="$(cd "$R" && GROWTH_GUARDS_MD_PATHS=' ' "$MDF" --all 2>&1)" && RC=0 || RC=$?
...
OUT="$(cd "$R" && GROWTH_GUARDS_MD_PATHS='docs/*.md' "$MDF" --staged 2>&1)" && RC=0 || RC=$?
...
put skills/growth-guards/wrapped.md $'Wrapped\ntext.\n'
run_mdf --all
