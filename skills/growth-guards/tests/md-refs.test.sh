...
unset GROWTH_GUARDS_MD_REFS_PATHS GROWTH_GUARDS_MD_EXCLUDES GROWTH_GUARDS_MD_SCOPE GROWTH_GUARDS_SETTINGS_FILE \
...
OUT="$(cd "$R" && GROWTH_GUARDS_MD_REFS_PATHS='docs/*.md' "$MDR" --all 2>&1)" && RC=0 || RC=$?
...
OUT="$(cd "$R" && GROWTH_GUARDS_MD_REFS_PATHS='*/growth-guards/*.md' "$MDR" --all 2>&1)" && RC=0 || RC=$?
...
put skills/growth-guards/SKILL.md "$(cat "$SKILL_DIR/SKILL.md")"$'\n\nSee [gone](nowhere.md).\n'
run_refs --all
