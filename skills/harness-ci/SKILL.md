...
**The judged checkout is read, never written, and it must be the commit.** A checkout with anything uncommitted is refused the `render` class: the proof would otherwise attest to whatever a step before the classifier left on disk. Nothing in the wiring writes there.
