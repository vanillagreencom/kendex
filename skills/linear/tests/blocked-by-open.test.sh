...
cache_issues="$(jq -cn --argjson main "$main" --argjson child "$child" '[$main, $child]')"
printf '%s\n' "$cache_issues" >"$TMP_ROOT/.cache/linear/issues.json"
printf '%s\n' '{"synced_at":"2026-09-01T00:00:00-07:00"}' >"$TMP_ROOT/.cache/linear/meta.json"
...
while IFS= read -r source; do
  [[ -n "$source" ]] || continue
  source_count="$(tr -d '[:space:]' <"$source" | awk -v needle='inverseRelations{nodes{idtypeissue{' '
    { text = text $0 }
    END { while ((at = index(text, needle)) > 0) { count++; text = substr(text, at + length(needle)) } print count + 0 }
  ')"
...
assert_eq "production GraphQL has one inverse relation projection owner" "$projection_count" 1
