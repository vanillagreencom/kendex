...
# fifteen lines above its own fail-closed arm, so all three were accepted at
# rc 0 and did nothing: on a cache holding more than one team, a request that
# named one got the whole workspace back with nothing in the output naming the
# scope. `cache_list_labels` and `cache_list_cycles` ended their loops with
...
# swallowed the same way. All three live twins refuse the same input.
#
# This locks in:
#   A. `issues list --team X` returns only that team's issues.
#   B. `--assignee` and `--created-since`, which the cache does not implement,
#      refuse instead of returning every issue.
...
# unfiltered read on a single-team fixture.
...
assert_contains "B: --assignee is refused rather than returning every issue" \
  "$assignee_out" "Unknown flag"
...
assert_contains "B: --created-since is refused rather than returning every issue" \
  "$created_out" "Unknown flag"
...
assert_contains "C: labels --team=KEN is refused rather than returning every team's labels" \
  "$labels_inline_out" "Unknown flag"
...
assert_contains "D: cycles --bogus is refused rather than returning every cycle" \
  "$cycles_out" "Unknown flag"
