...
- Everything unmeasurable is refused, never assumed idle. A lane is pickable only at `status: ok` with at least one parsed window; `no_credentials`, `expired`, `rate_limited`, `unreachable`, `no_usage_data` and `error` all yield a null headroom and are skipped. `pick` exits `3` when nothing qualifies, distinct from 1 for a real failure.
