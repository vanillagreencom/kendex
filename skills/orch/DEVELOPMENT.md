...
- **Review-gate reference scripts** (vendor, don't fork): the canonical pair lives in this skill at `scripts/ci/review-predicate.sh` and `scripts/ci/approval-refire.sh` (reference implementation: memsira PR #334). `review-predicate.sh` is the single source of truth for "is this PR head reviewed?" — the full evidence OR-group above (non-author review object / trusted check-run / trusted commit status / outage attestation) plus the changes-requested and unresolved-thread terms, with the reviewThreads `first:100` overflow read failing CLOSED to threads-open and every evidence read failing LOUD with no verdict (exit 2 = take no action, never "awaiting"). `approval-refire.sh` is the convergence described above. Consumers copy both files into `.github/scripts/` (the refire sources the predicate from its own directory — keep them in lockstep) and parameterize by env: required `GH_REPO`/`PR_NUMBER`/`HEAD_SHA`; optional `GATE_CONTEXT` (default `Review gate`), `REVIEW_CHECK_NAME` (trusted reviewer check/status name, default `Devin Review`; empty disables those surfaces), `OUTAGE_CONTEXT` (default `vstack-reviewer-outage`; empty disables), `PR_AUTHOR`, `QUIESCE`, `MAX_ATTEMPTS`. Wiring: the classifier job runs the predicate and posts the gate status from its verdict (success only when its own attempt runs gate-open); the event-driven and scheduled convergence workflows run the refire. Regression tests: `tests/review_predicate.sh`, `tests/approval_refire.sh`.
