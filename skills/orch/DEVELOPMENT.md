...
Statuses: `approved` / `reviewed` (exit 0); `changes_requested`, `comments`, `timeout` (exit 1); `error` (exit 1, or 3 on auth failure — same auth contract as `ci-wait`). Every exit path emits a final stdout result; `--json` always prints one well-formed object (review mode adds `mode`, `head_sha`, `reviews_at_head`).
...
- `approval_wait.sh` — GitHub-native approval verdict detection, review-mode gating (head pinning, author/DISMISSED exclusion, standing CHANGES_REQUESTED, thread resolution), `--resolve-mode` precedence + output contract.
