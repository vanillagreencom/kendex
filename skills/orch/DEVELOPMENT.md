...
- Review mode (`--mode review`): the REST `pulls/{n}/reviews` listing (it carries `commit_id`; `latestReviews` does not) — reviewed when a submitted review is pinned to the current `headRefOid` (re-read every poll, so a force-push resets the wait), is not DISMISSED/PENDING, and is not the PR author's own. Any state counts; an approval is also a review. A non-author reviewer whose latest submitted review stands at CHANGES_REQUESTED blocks until dismissed or re-reviewed, and the gate additionally requires zero unresolved threads. When `PR_REVIEW_CHECK` names the trusted review bot's check-run (vstack#654), a `success` conclusion of the newest check-run with exactly that name on the current head (REST `commits/{sha}/check-runs`, consulted only when no review object is pinned there) is accepted as alternative review evidence — for bots that submit a review object only when they have findings, whose clean re-analyses would otherwise deadlock the gate — under the same standing-CHANGES_REQUESTED and zero-unresolved-threads conditions. Check-runs are matched by NAME, and any GitHub App or Actions workflow can publish a check under any name, so the setting must name a check produced by the trusted review bot — the same user-configured trust model as `PR_REVIEW_NUDGE`; the matched run's `app.slug` is reported in the text output for auditing, never filtered on. No reviewer-name-specific logic anywhere — the mode is configuration, not bot detection.
...
- **Review-mode gate condition** (repos with commenting-only review bots, `PR_REVIEW_GATE = "review"`): the repo-side gate job must accept **either** signal — a non-author review object pinned to the current head, **or** a `success` conclusion of the trusted review bot's check-run on that head. Such bots submit a review object only when they have findings; a clean re-analysis passes their check-run without posting a review, so a gate requiring a review object alone deadlocks every PR that becomes clean after a push (vstack#654). Trigger on `pull_request`, `pull_request_review: types: [submitted]`, and `check_run: types: [completed]`. The check name is repo configuration and is matched by name only — gate on the trusted bot's check (the same trust model as orch's `PR_REVIEW_CHECK` setting), e.g. with `Devin Review` as the configured name:
...
- `approval_wait.sh` — GitHub-native approval verdict detection, review-mode gating (head pinning, author/DISMISSED exclusion, standing CHANGES_REQUESTED, thread resolution, `PR_REVIEW_CHECK` check-run evidence), `--resolve-mode` precedence + output contract.
