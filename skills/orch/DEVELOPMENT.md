...
A bare argument selects and `!name` rejects, so a set of fragments and that set negated split the battery: no suite runs in both halves and none runs in neither. CI runs the battery as three shards built that way, and `tools/tests/orch-shard-partition.test.sh` holds them to it.
...
- Shard coverage. CI runs the battery as three shards and a shard is nothing but a `run-all.sh` name filter, so a filter that stopped matching would drop its suites out of every shard while all three stayed green. `tools/tests/orch-shard-partition.test.sh` reads those filters out of `.github/workflows/skill-tests.yml`, replays them over the real suite names in a sandbox of empty files, and requires each suite in exactly one shard; its controls drop a shard and repeat a shard.
...
A merge queue re-runs CI on the merge group before it lands the PR. Where the PR head already contains the base head, that run tests the tree the PR head already tested, so it costs a second CI pass and finds nothing new. `ORCH_MERGE_BYPASS=fast-path` is the per-repository opt-in that lets a lane skip it, and `merge-pr.md` § 5 step 1 gates the skip on the head containing the base head. Freshness is `base-freshness`'s `behind`, which is zero exactly when the merge-base equals the base head; the workflow binds that verdict to the commit being merged by requiring the worktree HEAD to be the prepared head and the PR base to be the branch the script measured. Nothing in the setting grants the merge. GitHub decides it from the ruleset bypass on the merging account, so the refusal path is the ordinary one and leads back to the queue. A precondition the lane itself refuses skips the direct attempt and arms auto-merge, and the PR body's `## Merge decision` names which cause it was.
