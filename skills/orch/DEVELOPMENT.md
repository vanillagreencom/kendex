...
- **Reviewer-outage recognition** (repos that gate merges on review evidence AND want `PR_REVIEW_ON_TIMEOUT=proceed` to be end-to-end, vstack#795): the `proceed` setting governs only orch's `approval-wait` — the orch-side wait stops idling and records a reviewer-down override. It does NOT by itself open the review gate: the writer runs on GitHub events, never on orch's wait deadline, so it has no notion of "orch decided the reviewer is genuinely down". The bridge: orch attests the outage as a commit STATUS. When `approval-wait` proceeds in **review mode** — and ONLY then, so it inherits the proceed path's genuine-silence guarantee (zero unresolved threads, no review/check/status engagement at head, head unchanged during the wait) — it posts the context named by `PR_REVIEW_OUTAGE_CONTEXT` as `success` on the proceeded head, with the reason in the description (the predicate REQUIRES a non-empty reason). (The marker is review-mode only: approval mode's engagement signal excludes reviewer check-runs/statuses, and a commit status cannot satisfy native required-approvals branch protection anyway.) The repo side is configuration, not code — the vendored engine ships both halves: the predicate's override term reads the context named by `REVIEW_GATE_OVERRIDE_CONTEXT` (the orch key is an alias over the legacy `REVIEW_GATE_OUTAGE_CONTEXT` spelling), and the writer's `status` event leg converges the gate when the attestation lands. **Both sides must name the SAME exact context string** (e.g. `vstack-reviewer-outage`): orch posts `PR_REVIEW_OUTAGE_CONTEXT`, the predicate reads `REVIEW_GATE_OVERRIDE_CONTEXT`, nothing else compares them, and a value configured on only one side silently disables the fallback — `proceed` then never unblocks the gate. (Consumer repos pin this equality with a settings drift test; hyprtrade's `tools/test-agent-policy` is the reference.)

  **SECURITY — this is a deliberate branch-protection relaxation:** the outage status lets a merge complete with no human/bot review. Three bounds make it acceptable, and all three must hold: (a) orch only attests on the hardened genuine-silence predicate (a pending or failed reviewer check is engagement, not silence, so an in-flight reviewer never triggers it; a force-push invalidates the head-pinned status); (b) the status is match-by-context and posted by orch's trusted bot token — so enable it ONLY on repos where every status publisher is trusted, and configure `REVIEW_GATE_STATUS_PUBLISHER_REJECT` so workflow-minted statuses can never satisfy it; and (c) it is a triple opt-in — `PR_REVIEW_ON_TIMEOUT=proceed` AND the outage/override context pair set AND the reject-list posture reviewed. Leaving `PR_REVIEW_OUTAGE_CONTEXT` empty keeps `proceed` orch-side-only (no marker, gate untouched). The status names orch as its creator, so a reviewer-outage merge is auditable in the commit's status history.
