...
- **Convergence is the writer, not repo-side machinery.** A `success` commit status re-fires no PR workflow, so v1 needed repo-side convergence/re-fire scripts to make review-state transitions reach the gate. The v2 writer IS that convergence: its `status`/`check_run`/review event legs and cron floor reach live state and converge the gate — downward transitions (dismissed review, reopened thread, changes-requested) close it event-fast. Since VST-210 the PR-attached legs reach it INDIRECTLY: they run a group-less relay job that dispatches a converge pass and exits, and the predicate itself runs on `workflow_dispatch` and `schedule` (`merge_group` reaches the writer too but bypasses the predicate entirely, posting its queue success unconditionally). The relay exists so an eviction from the single-writer concurrency group can never leave a `CANCELLED` check pinning a PR at `mergeStateStatus UNSTABLE`; it is also the one job holding `actions: write`, scoped solely to dispatching this same workflow's converge leg, so "the writer never re-runs CI" still holds of the job that evaluates anything — the write job carries no `actions` scope at all. Nothing is held back either way (heavy lanes execute in the merge queue regardless of review state). Migration for an already-adopted repo: review-gate `references/adoption.md` § "Updating an already-adopted copy". Its safety conventions — fail loud act never on read failure, ordering guard on success posts, converge-all enumeration so no head is stranded — are engine-internal and pinned by the skill's own tests, not consumer obligations.
