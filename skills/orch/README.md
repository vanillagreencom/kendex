...
| `ORCH_OVERSEER_PREFERENCE` | Comma-separated `harness:rank:effort` entries `oversee-succeed` tries in order, `rank` a kendex tier ladder position, 1 the top: `oversee-succeed --help` | empty |
| `ORCH_QUESTION_TOOL` | `off`: a launched overseer drops its question tool where a lane does ([skill-rules.md](references/skill-rules.md) § Coordination); `overseer`: keeps it | `off` |
