...
| `ORCH_OVERSEER_PREFERENCE` | Comma-separated `harness:rank:effort` entries that `oversee-succeed` tries in order. `rank` is a position on the kendex tier ladder, 1 for the top tier. A named entry writes its harness's model, effort and permission posture. The caller fallback keeps the flags after `--`. See `oversee-succeed --help` for lane selection. | empty |
