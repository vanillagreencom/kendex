...
| `ORCH_LANE_HOST` | Provider `lane-host` runs: an executable script path or `local`; `ORCH_LANE_HOST_MAX_CALLS` (`4`) and `ORCH_LANE_HOST_BUSY_WAIT_SECS` (`30`) bound its calls. [Host protocol](schemas/lane-host.md) | `local` |
