...
| `REVIEW_MAX_CYCLES` | Max internal re-review cycles per issue in review-pr § 4; the `rereview_panel` write raises `rereview_cycles` and refuses once that count is past it. Nothing else spends it — § 7 QA re-checks write `qa_recheck_panel`, and fix rounds tally in `cycles` | `4` |
