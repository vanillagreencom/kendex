...
Use `skills/orch/scripts/dev-artifact-check WORKTREE_PATH ISSUE_ID DEV_DELEGATED_AT_EPOCH` to deterministically validate a dev agent's on-disk completion artifact (existence, `mtime >=` delegation epoch, and the required non-empty fields `.kind` — `implement` or `fix` — `.issue`, `.branch`, `.commit`, `.validate`). It prints `{ok, path, reason}` with reason `valid`/`missing`/`stale`/`invalid`. The dev agent writes this artifact to `WORKTREE_PATH/tmp/dev-return-ISSUE_ID.json` after its commit/labels/summary (`dev/workflows/dev-implement.md` § 10, `dev/workflows/dev-fix.md` § 6) — it is the durable completion record. When a long `tools/validate`-class command exceeds the harness tool timeout mid-tail, the turn ends and the live return message is lost; dev-start (§ 3) and dev-fix accept the completion on a fresh valid artifact instead of burning the stall-escalation ladder and re-delegating (vstack#770). `dev-artifact-check --file <json_path> [delegated_at_epoch]` validates one explicit artifact; the optional boundary applies the same freshness gate. The script proves only that the agent finished its tail — git and tracker corroboration stay in the orch workflow, so a fresh valid artifact triggers the § 3 checks, it does not replace them.
