...
| `REVIEW_RISK_COMMAND` | Opt-in risk classifier: a repo-relative command `review-pr` runs against the change set before the review cycle; prints `high`/`medium`/`low` (exit 0) and the reviewer fan-out is sized from it (`low` → small panel + first-clean-round convergence). Any failure or unset = full fleet — fails open to depth, never shallowness | (unset) |
