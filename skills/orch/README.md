...
| `CI_FIX_MAX_CYCLES` | Max automatic standalone ci-fix cycles for one PR; `workflow-state head-budget take` counts them across the heads the cycles push, and a passing or unconfigured CI run clears the count. Managed submission and merge recovery keep their own cycle caps | `6` |
...
| `REVIEW_MAX_EXTERNAL_ROUNDS` | Max external comment-triage passes and automatic review-wait restarts on one PR head. Comment triage owns `pr_comment_review.iterations`; `workflow-state head-budget take` separately spends every automatic wait restart, starting the count over on a changed head, and accepted gate evidence clears it | `4` |
...
| `ORCH_DECISION_MODE` | `ask` presents decision points; `auto-recommended` executes the recommended option without asking. Review findings disposition is by rule in EVERY mode — no mode presents a selection menu over findings. The always-ask set in [SKILL.md § The Cycle](SKILL.md#the-cycle) applies in every mode | `auto-recommended` |
| `ORCH_MERGE_AUTONOMY` | `auto` merges without asking once every merge gate is green; `ask` presents the merge decision. A `MERGE_READY = false` state never auto-merges | `auto` |
| `PR_REVIEW_ON_TIMEOUT` | `proceed` advances only when no reviewer engaged and no review thread is open; `block` reports the timeout | `proceed` |
