...
| `ORCH_POST_MERGE_CMD` | Bash command that `scripts/post-merge` runs in the base checkout after synchronization. It receives `ORCH_POST_MERGE_BEFORE` and `ORCH_POST_MERGE_AFTER`, the commit IDs before and after synchronization. A failed command stops the run before project refresh and verification | empty |
