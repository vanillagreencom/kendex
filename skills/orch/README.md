...
- `lane-mail` carries questions, notices and directives between a lane and the overseer as files in the lane's worktree, so messages need no tmux pane and also reach a lane on another machine. The overseer's own mailbox is the owner channel, each ask closed once by `resolve`.
...
| `ORCH_QUESTION_TOOL` | `off`: no launched session gets its harness question dialog. `overseer`: a launched overseer keeps it | `off` |
| `ORCH_ASK_WAIT_MINUTES` | Minutes an owner ask with a recommendation waits before the watch takes that option | `120` |
