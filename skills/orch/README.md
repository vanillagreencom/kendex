...
| `ORCH_HANDOFF_CONTEXT_TOKENS` | Context tokens at or past which the `lane-mail-check` turn-end hook refuses a lane's turn end until its handoff record stands, and at or past which the overseer's `lanes context` backstop poll marks a lane it can still read a pane for. The hook resolves it through `orch-env`, so a settings file sets it; the poll reads the process environment alone | `500000` |
