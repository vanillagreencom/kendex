...
| `PR_REVIEW_CHECK` | Exact name of the check-run the repo's trusted review bot publishes on analyzed heads (e.g. `Devin Review`). When set, `review` mode also accepts a `success` conclusion of that check on the current head as review evidence — for bots that submit a review object only when they have findings, whose clean re-analyses would otherwise never satisfy the gate. Matched by check name only, so name a check produced by the trusted review bot | empty |
