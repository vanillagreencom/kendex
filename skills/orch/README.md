...
Use `skills/orch/scripts/dev-return-write --worktree PATH --kind implement|fix --issue ID --round-id RID --branch BRANCH --commit SHA --validate STR [--validate-note TEXT] [--qa-label LABEL]... [--bundled] [--no-summary] [--summary TEXT | --summary-file PATH] [--item N DECISION REASONING]...` for the dev agent to write its round-scoped completion artifact deterministically (atomically, well-formed) instead of hand-authoring JSON; it prints the artifact path and exits 2 on invalid input. A read-only analysis round (investigate + recommend, no implementation) uses `--kind analysis` with exactly one of `--summary TEXT` (inline) or `--summary-file PATH` instead — no `--commit`/`--validate` (both rejected; the artifact omits those keys). Canonical schema — fields, kind rules, `items[]` shape: [`schemas/dev-return.md`](./schemas/dev-return.md); validation and round-id internals: [`DEVELOPMENT.md`](./DEVELOPMENT.md).

Use `skills/orch/scripts/dev-round-write --worktree PATH --issue ID --round-id RID (--item N TEXT ... | --items-file JSON_PATH)` for the orchestrator to persist a fix round's delegated item set at the moment the round is stamped, so the set survives the orchestrator's context (vstack#1230): a respawned dev agent reads its items from `tmp/dev-round-ID-RID.json` instead of guessing, and the acceptance check gains an on-disk expected set. Inline `--item N TEXT` suits plain text; shell-hostile item text (backticks, quotes — real review blocks) goes through `--items-file`, a JSON array of `{n, text}` built with the harness file-write tool. Records are immutable per round: an identical re-invocation is idempotent, different content under the same round id exits 2 (a changed delegation mints a new round id). It prints the record path and exits 2 on invalid input. Canonical schema: [`schemas/dev-round.md`](./schemas/dev-round.md); validation internals: [`DEVELOPMENT.md`](./DEVELOPMENT.md).

Use `skills/orch/scripts/dev-artifact-check --worktree WT --issue ISSUE --round-id RID [--expect-items N,N,... | --expect-items-from-round]` to deterministically validate a dev agent's round-scoped completion artifact; it prints `{ok, path, reason}` (`valid`/`missing`/`invalid`/`incomplete`). Fix rounds prefer `--expect-items-from-round`, which derives the exact expected item set from the persisted `dev-round-write` record (validating the record's full schema and internal round id; a missing or unusable record exits 2 rather than degrading the gate) — `--expect-items N,N,...` stays available for typing the numbers directly. A fresh valid artifact lets `dev-start` § 3 / `dev-fix` accept a completion whose live return never arrived because the validation outlasted the turn (vstack#770) without re-delegation; git/tracker corroboration stays in the orch workflow. Schema: [`schemas/dev-return.md`](./schemas/dev-return.md); gate ordering, type-strict fields, and the `--expect-items`/`--expect-items-from-round`/`--file` modes: [`DEVELOPMENT.md`](./DEVELOPMENT.md).
