...
| `REVIEW_RISK_COMMAND` | Opt-in risk classifier. Holds a repo-relative command (any name); the `review-pr` workflow runs it via `scripts/review-risk` against the change set before the review cycle. Contract: prints `high`/`medium`/`low` (exit 0); the reviewer fan-out is sized from the answer (`low` → small panel + first-clean-round convergence). Any failure or unset = full fleet — fails open to depth, never shallowness | (unset) |
