...
**Ancestor gate (direct selection).** Any directly selected issue — start, start-worktree, handoff, dev-start, parallel-check — walks its FULL `parent_id` chain to the top, fetching and classifying each ancestor (a `(one PR)` title always wins). An enclosing `(one PR)` ancestor makes THAT bundle the work item (the child only on explicit user choice) — promotion picks the IDENTITY only: the promoted bundle is then gated exactly like any other selection, on the union of its own `blocked_by` and every container ancestor's ABOVE it (mixed ancestry drops nothing — a blocked top container stops a promoted single-PR bundle too). Otherwise the ancestors are containers: union the issue's own `blocked_by` with EVERY container ancestor's, fetch all blocker states (`issues bulk-get`), and only a non-terminal `state_type` blocks. A blocked issue is never dispatched, leased, or marked safe — one hop is not enough: a cross-bundle blocker on the top-level container must stop a directly selected grandchild.
