...
**Classify every wake mechanically — never from its wording (vstack#803).** For a dev/QA delegation (one carrying a `dev_round_id`), the task-list glance above is not sufficient. On EVERY wake — `finished`, `idle`, or any teammate notification — run **A** (`dev-artifact-check`) and the **B** git/tracker checks and classify via the A/B decision table below BEFORE going idle. A "finished"/"idle" wake is not evidence the round completed: a dev agent that idles mid-completion-tail (after validate, before `dev-return-write`) still emits a `finished` wake, yet its round never wrote the artifact. Reading the wake's text — or elapsed time — and idling without running A/B is the exact stall in vstack#803. Message text and timers are never acceptance inputs; the scripts are.

**Arm a wall-clock watchdog at delegation — do not depend on the sub-agent's wakes (vstack#803).** The wake handler only runs when a wake arrives, so a sub-agent that idles mid-tail and then goes silent emits no further wakes, nothing re-runs A/B, and the session stalls indefinitely. When `dev_delegated_at` is stamped at delegation, also arm a **single** wall-clock re-check for `dev_delegated_at + quiet_window` (the 10-minute window) using the harness's background-task facility, so the A/B check + escalation ladder runs even if NO wake ever arrives. The requirement is uniform; only the mechanism is harness-specific — Claude Code: a background shell that re-invokes on exit; Pi: a `bg_task` timer (its no-exit-wake limitation is Pi `bg_task` #15 — pair it with the mechanical per-wake check as backstop); Codex/OpenCode: the orchestrator's own scheduled re-entry / session-file poll. One-shot, not a busy poll: it fires once at the deadline, runs A/B if the round is still outstanding (else no-op), and re-arms only if the round entered a new nudge/escalation step. This closes the wake-dependency hole; the Pi part is a harness limitation (#15) noted upstream, not a vstack-fixable gap.
