...
> If you are running in **Codex**: Spawn generated vstack agents with `agent_type` set to the actual generated agent name and `fork_context: false`. Reviewers returned by `list-review-agents` must spawn as `agent_type=<reviewer-name>`; dev agents selected from `agent:X` labels must spawn as `agent_type=X`. Use `worker` only for an intentional generic-worker fallback when no matching generated agent exists or the selected agent is deliberately generic, and call that fallback out in status and workflow state. Two-step pattern: (1) spawn the selected runtime agent with the `<bootstrap_format>` message, (2) `send_input` a `DELEGATION:` prefixed message containing exactly the filled `<delegation_format>` content — nothing more.
>
