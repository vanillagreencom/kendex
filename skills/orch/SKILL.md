...
**Multi-PR watching (optional review-gate consumption).** The waiters are single-PR *foreground* waits inside one workflow lifecycle. A session shepherding several PRs across hours (steward loops, queue babysitting, harness wake-up models) must NOT hand-roll a monitor keyed on gate-state transitions — a PR sitting steadily at "pending with open review threads" transitions nothing and the session sleeps through it. When the review-gate skill is installed (check `.agents/skills/review-gate/scripts/pr-watch.sh` exists, the same existence-check pattern as second-opinion in submit-pr § 1.2), use it as the single state reducer: `GH_REPO=<owner>/<repo> .agents/skills/review-gate/scripts/pr-watch.sh --heal` — exit 0 = nothing needs attention, exit 1 = tab-separated attention lines (unresolved threads with queued-PR dequeue warnings, standing objections, a stale gate with a bounded writer self-dispatch, a disarmed mergeable PR, reviewer silence past `PR_REVIEW_WAIT_SECS`), exit 2 = a PR could not be read. Without the review-gate skill, fall back to polling `approval-wait`/`queue-wait` per PR — they detect the same conditions one PR at a time.
