...
- Whether glob mode may answer a rejection with an older sibling is decided by the rejecting GATE, not by its reason name: a self-report about THIS run (no review happened, nothing was measured, the declaration says nothing, the finding items use wrong field names) ends the search, while a rejection that could be a torn or truncated write (a gate that could not run, finding arrays missing wholesale, `stale`, a missing `.verdict`) falls back to an intact sibling. `incomplete` lands on both sides, which is why the disposition is a property of the gate.
