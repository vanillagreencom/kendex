...
- **A watchdog that ends without a verdict leaves the round unwatched.** Either check writes a keyed first line on stderr and exits nonzero when its own probe broke rather than reaching an answer — `<script>: exit=<status>` for a helper that died, `dev-artifact-check: verdict-unreadable …` for a verdict it could not read. Neither is a verdict and neither is `wait`: nothing is now clocking that round, so arm a fresh watchdog before the next wait. Statuses are in each check's `--help`.
