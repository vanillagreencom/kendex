...
- An artifact citing a measurement that produced no samples — a mutation/stability citation whose sample count (denominator) or thread count is zero, or a `perf_qa` payload whose `percentiles` carry no value above zero — is rejected with reason `zero_sample` plus a `detail` field quoting the citation. A zero RESULT is not a zero sample: `stability: 0/10` stays valid. The gate applies to every artifact, `qa_metadata` or not, and is suppressed for one declaring `qa_metadata.measurement_failed` — a non-empty string naming the instrument, echoed back as an additive `measurement_failed` field on the result so a review that ran on a broken instrument is visible rather than silently accepted or silently rewritten.
- A gate that could not run at all (a torn read of a non-atomically written artifact, a broken `jq`) is rejected as `invalid` carrying the underlying diagnostic in `detail`. No gate reports its own failure as "nothing found".
