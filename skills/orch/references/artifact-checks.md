...
- An artifact declaring `qa_metadata` whose `blockers[]`/`suggestions[]` are missing or not arrays, or whose present items omit a required `review-finding` field — including the routing-critical `category ∈ {fix,issue}` on suggestions — is rejected with reason `incomplete` plus a `detail` field naming the array and what it was, or the first offending item and field. Artifacts without `qa_metadata` are unaffected.
- An artifact citing a measurement that produced no samples — a mutation/stability citation whose sample count (denominator) or thread count is zero, or a `perf_qa` payload whose `percentiles` carry no value above zero — is rejected with reason `zero_sample` plus a `detail` field quoting the citation. A zero RESULT is not a zero sample: `stability: 0/10` stays valid. Only `.summary` and `.qa_metadata` are scanned, the places an artifact states its own measurements; numbers quoted inside a `blockers[]`/`suggestions[]` item describe the code under review and are out of scope.
- A TOP-LEVEL `measurement_failed` string declares that the reviewer's own instrument produced nothing. The zero_sample gate still runs and anything it finds is recorded as `measurement_suppressed` rather than rejecting; the accepting reason becomes `valid_undermeasured` with the declaration echoed back — so an orchestrator branching on `reason` cannot record an undermeasured domain as a clean one. A declaration that names no instrument (wrong type, blank, a null token, bare punctuation, too short) is rejected as `invalid_declaration`.
- A gate that could not run at all (a torn read of a non-atomically written artifact, a broken `jq`) is rejected as `invalid` carrying the underlying diagnostic in `detail`. No gate reports its own failure as "nothing found", and jq's stderr never reaches the answer channel. When jq cannot run at all, every mode still prints a parseable `invalid` rejection rather than exiting empty.
- Every `ok=false` result carries a `detail` naming its cause. § 3.1 relays that text on the single permitted re-delegation, so a rejection that does not say why is a dead end.
- Whether glob mode may answer a rejection with an older sibling is decided by the rejecting GATE, not by its reason name: a self-report about THIS run ends the search, while a rejection that could be a torn or truncated write falls back to an intact sibling. Among the content gates only "the gate could not run" is on the fallback side — a truncated JSON object is not a JSON object, so every torn write fails the `.verdict` parse before any content gate sees it. An unset disposition is treated as terminal, so a gate added later fails closed.
