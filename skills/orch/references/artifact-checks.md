...
- **A watchdog that ends without a verdict leaves the round unwatched.** A verdict is exit 0 or 1; exit 2 is a keyed refusal. Any other status leaves the round unwatched whether or not a keyed line arrived, so judge it on the status and arm a fresh watchdog before the next wait. Each check's `--help` owns the statuses and what stderr does and does not carry.
