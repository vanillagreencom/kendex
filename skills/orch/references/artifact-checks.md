...
- A TOP-LEVEL `measurement_failed` string declares that the reviewer's own instrument produced nothing. It replaces the zero_sample gate, and the accepting reason becomes `valid_undermeasured` with the string echoed back — so an orchestrator branching on `reason` cannot record an undermeasured domain as a clean one. A declaration that names no instrument (wrong type, blank, a null token, bare punctuation, too short) is rejected as `invalid_declaration`.
