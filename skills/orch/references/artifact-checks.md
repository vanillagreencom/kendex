...
- **A watchdog that ends without a verdict leaves the round unwatched — judge that on the exit status, never on stderr.** A verdict is exit 0 or 1 with the result on stdout. Exit 2 is a refusal, keyed on the first line of stderr: a usage error, or `dev-artifact-check: verdict-unreadable …` when the poll could not read its own answer. ANY OTHER STATUS is an abort, and the backgrounded task's exit is the whole signal: the keyed `<script>: exit=<status>` line lands only where bash reaches the EXIT trap, and under real fork exhaustion — the poll loop's `sleep` unable to fork — bash ends the shell with status 127 and writes no line at all. None of these is a verdict and none is `wait`: nothing is clocking that round, so arm a fresh watchdog before the next wait. Statuses are in each check's `--help`.
