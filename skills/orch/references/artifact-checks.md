...
- A declaration does not delete what it silences: the measurement gate still runs, and anything it found is echoed as `measurement_suppressed` beside `measurement_failed`, so one declaration covering an unrelated perf payload is visible rather than blanket.
