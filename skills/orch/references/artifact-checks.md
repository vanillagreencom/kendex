...
- `no_review` and `zero_sample` are TERMINAL in glob mode: they are the reviewer's self-report about the run, so the newest fresh artifact carrying one ends the search. `stale`, `invalid` and `incomplete` describe one write, so the search still falls back past them to an intact sibling.
