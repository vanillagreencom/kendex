...
Size tripwire, checked before every round's dispositions: `dev-round-write` refuses the round when the branch diffstat exceeds twice workflow state `pr.baseline_lines`, which the first implementation receipt records. Cut back to the Done-when before starting another fix round, and close every thread on cut code with the deleting sha.
