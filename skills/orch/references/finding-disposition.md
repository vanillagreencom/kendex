...
Every filing carries the `Reached by:` line project-management's creation bar requires (its SKILL.md § Disposition). A review-born finding clears the bar only where that reach is a user or a shipped run — priority 2 with a `Symptom:` line naming the run, user, or red check that already showed it. Below that, a true defect is fixed here when it is tiny (est-1, no new helper, no new file) and is otherwise declined with its reason; a review-born P3 does not file. `linear.sh issues create` refuses a body with no reach where `LINEAR_REQUIRE_REACH` is set, and a `--review-born --priority 2` body with no symptom.
