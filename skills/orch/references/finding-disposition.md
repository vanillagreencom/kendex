...
1. **Does it claim a defect** (a failing state, broken path, wrong output)? Verify the mechanism yourself:
   - **Establish age.** Before deciding to fix a finding in a file the branch did not create, use `git blame` only to establish the age of the finding's lines and inspect the current diff to decide whether the branch arms the defect.
...
Size report: `branch-size-check` records counts and a verdict in `pr.size_check`; `dev-round-write` stores the same report in the round. The issue's optional `**Expected delta**` line supplies the comparison. A reviewer or the orchestrator uses the recorded verdict and counts to decide whether a cut serves the Done-when. Size alone never refuses launch, fix delegation, or push. Close every thread on cut code with the deleting sha. Record a chosen cut with `dev-round-write --cut`; [dev-round.md § Declared cuts](../schemas/dev-round.md#declared-cuts) owns its acceptance rule.
