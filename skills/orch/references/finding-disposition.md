...
Size tripwire, enforced twice. Before the push, `branch-size-check` exits 3 when the branch's production lines exceed the allowance the issue states, or its test lines exceed one control per Done-when surface plus its must-fail control ([`../workflows/submit-pr.md`](../workflows/submit-pr.md) § 1.2). Before fix delegation, `dev-round-write` exits 3 when the branch diffstat exceeds twice workflow state `pr.baseline_lines`. Stop the item-by-item fix path, report the current and baseline counts, and cut back to the Done-when before minting another round. Close every thread on cut code with the deleting sha. The cut is a round like any other, minted and delegated as one; its record is stamped `dev-round-write --cut`, the one declaration the tripwire lets through, and acceptance refuses it unless the branch came back to the cap.
