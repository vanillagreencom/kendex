...
Size tripwire, enforced before fix delegation: `dev-round-write` exits 3 when the branch diffstat exceeds twice the immutable baseline behind workflow state `pr.baseline_lines`. Stop the item-by-item fix path, report the current and baseline counts, and cut back to the Done-when before minting another round. Close every thread on cut code with the deleting sha.
