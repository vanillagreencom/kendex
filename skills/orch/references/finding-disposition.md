...
1. **Does it claim a defect** (a failing state, broken path, wrong output)? Verify the mechanism yourself:
   - **Establish age.** Before deciding to fix a finding in a file the branch did not create, use `git blame` on the finding's lines to establish whether the branch introduced or armed the defect.
