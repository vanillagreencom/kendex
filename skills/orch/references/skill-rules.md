...
**Tracked issue creation.** A delegated orchestration session (lane) never creates a tracked issue. TPM analysis from [audit-issues](../../project-management/workflows/audit-issues.md) § 2.1 / § 4.1 runs in a lane or delegated subagent, never in the overseer session. A lane records each new candidate as one Linear comment on its own issue with `.agents/skills/linear/scripts/linear.sh comments create [ISSUE_ID] --body-file [PATH]`. The first line is `Proposal: [TITLE]`. The comment also carries `Reached by: [BEHAVIOR]`, `Reason: [WHY TRACKED WORK IS NEEDED]`, and `Evidence: [REPOSITORY_PATH]`. The evidence path uses a semantic anchor, never a line number, so another checkout can verify it. The lane status file carries no proposal. Project-management's [proposal-sweep](../../project-management/workflows/proposal-sweep.md) workflow owns fleet analysis. Its TPM lane produces issue-mode audit JSON and files nothing. The overseer checks that output against live fleet work before it runs [audit-issues](../../project-management/workflows/audit-issues.md) §§ 5-7; project-order resumes at § 2.2. It replies to the proposal comment with the created issue ID or the one-line decline reason. A non-delegated primary session routes creation through TPM, except for `plan-issues`, `start-new`, and the `merge-pr` rebundle, which it may run directly with their workflow-specified label sets.
