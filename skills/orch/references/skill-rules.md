...
**Tracked issue creation.** A delegated orchestration session (lane) never creates a tracked issue, including through `audit-issues`. At any workflow step requesting either, it records the proposal, reached behavior, reason, and any audit-input path in the status file named by its launch brief. It hands the step to the overseer and waits for the outcome before dependent replies or worktree removal. The overseer returns the issue IDs or decline reasons; the lane records that outcome and resumes the caller's continuation. [oversee.md](../workflows/oversee.md) owns the launch and status-file handoff. A non-delegated primary session, including the overseer, routes creation through TPM, except for `plan-issues`, `start-new`, and the `merge-pr` rebundle, which it may run directly with their workflow-specified label sets.
