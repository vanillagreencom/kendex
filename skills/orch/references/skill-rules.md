...
**Compaction.** A command `open-terminal` or `oversee-succeed` builds carries the words that turn the harness's own auto-compaction off wherever an adapter names its model's window, so a lane hands off at `ORCH_HANDOFF_CONTEXT_PCT` of its own window before its harness compacts it. A claude command on a model whose window the claude adapter does not name keeps its compaction, since no mark would hand it off; a fleet launch onto one is refused as `launch-window-unknown`, and a succession onto one as `model-window-unknown`. A fleet `--cmd` launch on claude or codex carries them inside its command or is refused as `launch-compaction-missing`. Claude Code takes `--settings='{"env":{"DISABLE_AUTO_COMPACT":"1"}}'`, quoted so the shell passes its JSON whole, and Codex `-c model_auto_compact_token_limit=9223372036854775807`; Pi has no launch word, and its settings file turns compaction off. The words live in `scripts/lib/lane-launch.sh` beside the question-tool words.
