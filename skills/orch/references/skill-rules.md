...
| Overseer, any harness | The harness's own tool stays on by default, because the owner answers there | `ORCH_OVERSEER_QUESTION_TOOL=off` gives every successor `oversee-succeed` launches that harness's words. An overseer the owner starts by hand takes them on its own command line, and its successors keep them |
