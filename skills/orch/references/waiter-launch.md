...
Load from `submit-pr.md` or `merge-pr.md` before running `approval-wait`, `ci-wait` or `queue-wait`, and from `oversee.md` § Watch delivery before launching the repeat watch. Run each long waiter under `setsid`. Keep the lane active until its completion file arrives. Never start the waiter as a harness background command.
