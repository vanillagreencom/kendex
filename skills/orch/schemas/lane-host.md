...
- The credential a hosted lane runs on is the provider's own, injected at `create`. The caller's `CLAUDE_CONFIG_DIR` never reaches the host, so the control machine's copy of an account and the provider's copy are independent: either can expire while the other works. `accounts` is the only place a provider reports its copy. A provider that omits the verb fails it, and `lanes list` then reports only the control machine's reading.
...
It implements no `accounts` verb. Its account files are copied from the control machine at `create`, so the credential it uses is the one `lanes list` already measures locally.
