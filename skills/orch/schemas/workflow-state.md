...
| `launch_queue` | string[] | The items the overseer's last selection chose and has not launched, in launch order, each a `lanes[]` item key. Rewritten whole at each selection ([oversee-events.md § Judgement rules](../references/oversee-events.md#judgement-rules)); `oversee-report` renders its head as the status report's Next row |
| `owner_items` | object[] | One `{id, text, answered_at}` per question the overseer sent the user: `id` names it for the answer, `text` is the question in one line, `answered_at` the UTC time its answer arrived, absent until then. `oversee-report` renders each entry without `answered_at` as the status report's Waiting on you row. The report keeps no marker here: the newest file in `progress-reports/` beside this file is the last report (`oversee-report --help`) |
...
- `workflow-state-oversee.json` and its lock, and the watch's own files beside it that `scripts/lib/watch-pid.sh` names: `oversee-watch.pid`, `.argv`, and the `.log` and `.err` a succession-restarted watch writes.
