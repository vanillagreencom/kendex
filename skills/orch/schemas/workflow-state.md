...
- Every path of that directory whose name carries, as a whole token, the item of a workflow state that stands there, with or without a fleet state, while that state was written within `ORCH_RECORD_RETENTION_DAYS` or the worktree it names still exists, by its `worktree` field or, for a state that records only a `branch`, the linked worktree `git-context branch-worktree` resolves for that branch, never the main checkout. The item's close-out takes that state when the item ends, and a lane closed while its item was open leaves it for a later resume. A state past the retention whose worktree is gone belongs to an item no close-out reached and nothing resumes, and it goes with its files like any other entry.
