...
| `near_ceiling` | string[] | `byte-ceiling` `near-ceiling` lines from the last accepted dev return, each naming a file within reach of the byte ceiling. Each accepted round REPLACES the list, so it describes the branch as that round left it, never an accumulation across rounds; the next round's delegation renders one `Near-ceiling:` line per entry |
