...
| `pr` | object | Pull-request growth state. `baseline_lines` is the one authoritative value. `dev-artifact-check --record-baseline` writes it after full implementation acceptance; a fresh standalone route writes it only after both first-fix round records land. The write binds the active round to the same current HEAD used for measurement. Rebases and later rounds do not move it |
