...
| `tier` | string | The cycle the item runs, `micro`, `small` or `standard`, as `item-tier` assigned it; `set` refuses any other value as `tier-value`. [`small.md`](../workflows/small.md) § 1 is the writer. `small` holds `REVIEW_MAX_CYCLES` and `REVIEW_MAX_EXTERNAL_ROUNDS` to the tier ceilings `workflow-state --help` lists, and refuses a `first_panel`, `rereview_panel` or `verification_panel` of more than three reviewers as `panel-bound`. Absent, no tier bound applies |
