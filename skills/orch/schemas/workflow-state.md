...
| `pr_review` | object | Reviewer-gate mode tracking: `mode` ("approval"/"review"/"exempt"/"off" as printed by `approval-wait --resolve-mode`) and `head_sha`, the commit that mode was resolved for. A mode belongs to one head, because a later push can change the class the review gate waives, so the pair is written together and a reader binds the mode to a head by `head_sha` exactly as it binds `pr.size_check` by its own. A reader whose live head differs re-resolves before it applies the recorded mode |
