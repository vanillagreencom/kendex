...
| `overseer` | object | The overseer session itself: `{pane, window, launch_line}`. `pane` is the tmux pane the overseer runs in, `window` that pane's window id, and `launch_line` the whole command a successor of it is launched with — lane prefix, harness, model, effort, permission flags and brief — null where none is recorded. `oversee-watch` writes `pane` and `window` for the `$TMUX_PANE` it was started from wherever the record names another pane, and fills an absent `launch_line` from `oversee-succeed --print-launch-line`, leaving a line already recorded alone; `oversee-succeed` writes the line of each successor it chooses. `oversee-watch` hands the recorded line back to `oversee-succeed --dead-pane` through a file on `overseer-dead` ([oversee-events.md § Event kinds](../references/oversee-events.md#event-kinds)) |
