...
| `overseer` | object | The overseer session itself, the session record [overseer-host.md](overseer-host.md) § The session record names: `{runtime, generation, server, pane, window, account, launch_line}`. `runtime` is the value `overseer-host resolve` answered when the session was opened, `tmux` today, and a session never changes runtime. `generation` counts the sessions recorded on this fleet: `oversee launch` and `oversee-succeed` write one more than the record they replace, before the new session's first turn, and put the prior record back when the launch is abandoned; `oversee register` writes the next generation for a session a person opened by hand and keeps it where the record already names that pane. `server` and `pane` are the `<tmux server pid> <pane id>` key `lib/lane-context.sh` builds a session's own row on, which the turn-end hook compares its own against; `window` is the pane's window; `account` is the config directory the session was launched on, null where none was picked. `launch_line` is the whole successor command: lane prefix, harness, current model, effort, permission flags and brief. Each live `oversee-watch` invocation replaces `server`, `pane`, `window` and `launch_line` once at startup through `oversee-succeed --print-launch-line`, including after a manual restart in the same pane and after the restart `oversee-succeed` makes from a successor's pane, so `pane` names the pane the running watch serves; it keeps `runtime`, `generation` and `account` where the record names its own pane on its own server and drops them where it names another session's. Watch recording always receives the current model and effort, even when `ORCH_OVERSEER_PREFERENCE` is set. Live self-succession uses that preference instead. `oversee-succeed` writes the exact successor command before it launches, which covers the interval before that successor starts its watch. `oversee-watch` hands the last live-owned line back through a file on `overseer-dead` ([oversee-events.md § Event kinds](../references/oversee-events.md#event-kinds)) |
