...
| `round_prunes` | object | Round id to `{action, used_pct, mark_pct, bytes}`, one entry per round `round-prune` recorded, which a usage, state, setting or disk-read refusal is not: `action` is `pruned`, `below-mark`, `failed` or `no-worktree`, `bytes` what the prune freed, 0 below the mark or with no worktree. `round-prune` is the one writer |
