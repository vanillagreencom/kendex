...
ot_message() { # REASON FIELD=VALUE...
  ...
  case "$reason" in
    ...
    launch-compaction-missing) text='This fleet --cmd command leaves the harness'"'"'s own compaction on, so it would compact the lane before its handoff mark. Nothing was launched. Add the words this line names, in that order, inside the command, each quoted so the shell passes it whole: the claude word as --settings='"'"'{"env":{"DISABLE_AUTO_COMPACT":"1"}}'"'"'.' ;;
