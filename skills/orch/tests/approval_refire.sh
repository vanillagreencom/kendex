#!/usr/bin/env bash
# Regression tests for orch/scripts/ci/approval-refire.sh (vstack#949) — the
# vendorable status-convergence script behind the pending-status review gate
# (DEVELOPMENT.md § CI Triggering Patterns, "Status convergence").
#
# The refire is exercised against a STUBBED review-predicate.sh placed in its
# own directory (it resolves the predicate relative to itself), so these tests
# pin the convergence branches independently of predicate behavior:
...
#   r12. custom GATE_CONTEXT: a prior        -> rerun, not direct success
#        success under another context is       (proof is per-context), and
#        not proof                              posts name the custom context
set -euo pipefail
...
# Sandbox: the real refire next to a stubbed predicate.
mkdir -p "$TMP_ROOT/scripts" "$TMP_ROOT/bin"
cp "$REPO_ROOT/skills/orch/scripts/ci/approval-refire.sh" "$TMP_ROOT/scripts/"
...
echo "=== gate context is parameterized ==="

out=$(run_refire STUB_VERDICT_LINE="$APPROVED" GATE_CONTEXT="CI Required" \
  STUB_GATE_HISTORY='[{"context":"Review gate","state":"success","description":"ok"}]' \
...
out=$(run_refire STUB_VERDICT_LINE="$AWAITING" GATE_CONTEXT="CI Required" STUB_GATE_HISTORY='[]')
