...
# bans issue-number citations (`vstack#NNN`, bare `(#NNN)`) in instruction-flow
...
SCRATCH="$TMP_ROOT/inject-vstack-cite.md"
cp "$REPO_ROOT/skills/orch/SKILL.md" "$SCRATCH"
printf '\nAlways ask before merge (vstack#944).\n' >> "$SCRATCH"
if [[ -n "$(scan "$SCRATCH")" ]]; then
  pass "lint flags an injected vstack#NNN citation"
else
  fail "lint MISSED an injected vstack#NNN citation (no teeth)"
fi
