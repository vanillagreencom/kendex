...
rule_fenced "admin eligibility is exact-head classified" "$SUBMIT" "### 2.1 Consumer Admin-Merge Offer" 'admin-merge classify' '[BASE_SHA]' '[HEAD_SHA]'
rule "automatic decisions never select admin merge" "$SUBMIT" "### 2.1 Consumer Admin-Merge Offer" '`ORCH_DECISION_MODE` never answers it' '`merge_mode: admin`' '§ 1-7'
rule_fenced "merge accepts only exact-head authorization" "$MERGE" "" 'admin-merge verify' '[PREPARED_BASE]' '[PREPARED_HEAD]'
