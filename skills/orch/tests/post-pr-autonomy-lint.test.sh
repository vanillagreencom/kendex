...
route_rule() {
  ...
  if ! grep -qxF -- "$expected" "$file"; then
    fail "$name: route is missing"
    return
  fi
  pass "$name"
  awk -v old="$expected" -v new="$inverse" '{ if ($0 == old) print new; else print }' "$file" > "$scratch"
  if cmp -s "$file" "$scratch" || grep -qxF -- "$expected" "$scratch"; then
    fail "$name: inverse control did not make the route guard red"
  else
...
rule "the skill owns the named-stop record" "$SKILL_DIR/SKILL.md" "## The Cycle" \
  '**Post-PR autonomy.**' '`workflow-state post-pr-stop record`' '`ORCH_MERGE_AUTONOMY` controls merge consent only'

rule_fenced "comment triage reads the decision mode" "$COMMENTS" "" \
  'orch-env ORCH_DECISION_MODE auto-recommended'
route_rule "decision mode controls managed comment triage" "$COMMENTS" \
  'Decision route: `auto-recommended` -> `continue-to-§8`; `ask` -> `return-pending-choice`.' \
  'Decision route: `auto-recommended` -> `return-pending-choice`; `ask` -> `continue-to-§8`.'
rule_fenced "submission reads the decision mode" "$SUBMIT" "" \
  'orch-env ORCH_DECISION_MODE auto-recommended'
route_rule "submission spends review retries before its cap" "$SUBMIT" \
  ...
  '   Decision route: `auto-recommended` + `retry-budget` -> `record-review-round-cap`; `auto-recommended` + `retry-cap` -> `restart-review-wait`; `ask` -> `present-review-choice`.'
route_rule "start-worktree preserves the final stop before summaries" "$START" \
  'Stop route: `upstream-stop` -> `preserve`; `no-upstream-stop` -> `record-merge-gates-unmet`; `final-stop` -> `render-summaries`.' \
  'Stop route: `upstream-stop` -> `record-merge-gates-unmet`; `no-upstream-stop` -> `preserve`; `final-stop` -> `render-summaries`.'
...
rule_fenced "ci-fix reads the decision mode" "$CI" "## 3. Classify And Route" \
  'orch-env ORCH_DECISION_MODE auto-recommended'
route_rule "ci-fix spends retries before its cap" "$CI" \
  ...
  'Decision route: `auto-recommended` + `retry-budget` -> `record-ci-fix-cap`; `auto-recommended` + `retry-cap` -> `restart-ci-fix`; `ask` -> `present-ci-fix-choice`.'
