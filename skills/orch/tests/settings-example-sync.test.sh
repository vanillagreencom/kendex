...
ORCH_KEYS="PR_REVIEW_GATE REVIEW_GATE_MODE PR_REVIEW_WAIT_SECS PR_REVIEW_NUDGE_SECS PR_REVIEW_NUDGE PR_REVIEW_CHECK PR_REVIEW_ON_TIMEOUT CI_WAIT_NO_CHECKS_GRACE CI_FIX_MAX_CYCLES REVIEWER_SLOT_BUDGET PR_REVIEW_REFIX_MAX_LINES ORCH_DECISION_MODE"
...
for key in $ORCH_OPTIN_KEYS; do
  check "$key commented example present in skill template" "grep -q \"^# $key = \" \"\$SKILL_TEMPLATE\""
  check "$key commented example present in root template" "grep -q \"^# $key = \" \"\$ROOT_TEMPLATE\""
done
