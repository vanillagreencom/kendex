...
assert_file_contains "$submit_workflow" '.agents/skills/orch/scripts/approval-wait [PR_NUMBER] 30 --json --mode [GATE_MODE]' "submit-pr polls the review gate via approval-wait (no max_wait positional — PR_REVIEW_WAIT_SECS resolves the budget)"
assert_file_not_contains "$submit_workflow" '30 900 --json' "submit-pr no longer hardcodes the 900s review-wait budget"
