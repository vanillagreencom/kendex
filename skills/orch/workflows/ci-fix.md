...
1. **Re-confirm the review gate at the new head** — resolve the mode through the existing gate vocabulary, never by detection:
   ...
   ```
   - `approved` / `reviewed` / `proceeded` → step 2. (`proceeded` is the `PR_REVIEW_ON_TIMEOUT=proceed` reviewer-down degrade: the re-confirm deadline passed with zero unresolved threads and no reviewer evidence; record `pr_approval.reviewer_down true` and continue.)
