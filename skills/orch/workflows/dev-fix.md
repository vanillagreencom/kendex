...
**Caller context** (via `⤵`): `worktree`; `lifecycle` — `"managed"` (return at § 3) or `"self"` (default); `dev_agent` — a live dev agent; `issue_id` — the workflow-state key, the normalized issue ID (`issue-N` for GitHub, `PROJ-123` for Linear), never the bare GitHub issue number; `items` — formatted review items; `source` — `pr-review` | `qa-review` | `review` | `local-review` (default `conversation`); `qa_agent`; `convergence` — present only when the caller's review reached its third cycle, carrying the `choice` (`cut`, `structural`, or `split`) and its `reason`. When it arrives, the round's answer takes that shape: a `structural` choice fixes the recurring class rather than each item, and a `cut` or `split` has already narrowed `items` to what survives the decision. Carry it into the delegation so the agent is told what is being asked of it, not just what was reported.
