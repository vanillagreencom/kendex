...
3. **Container guard** — Linear work items (explicit `[ISSUE_ID]` or branch-derived), standalone lifecycle only (managed callers run their own preflight before delegating here). ORDERING: with an EXPLICIT issue ID (argument or caller context), run this guard BEFORE step 1 — `session-init` normalizes/creates the issue-named branch for explicit worktree issues, which would manufacture exactly the container branch artifact this guard forbids; only a branch-derived ID (no explicit argument — the branch already exists, nothing new is created) defers the guard until after step 1 resolves it. The guard itself: sync the cache (`linear.sh sync --reconcile`; step 6 below then just re-verifies) and fetch `cache issues get [ISSUE_ID] --with-bundle`. Apply the container classification (SKILL.md → Coordination): a `(one PR)` title marker always wins; without it, children present or `agent:multi` label → CONTAINER. Container → STOP HERE, before any state exists — no lease claim, no workflow state: containers are never initialized (they hold no implementation state; each child is the session/PR unit). Report the container verdict and its unblocked children instead. A LEAF whose `parent_id` is set gets the full Ancestor gate (SKILL.md → Coordination) the same way: walk the chain, classify each ancestor (an enclosing `(one PR)` bundle claims the work item), and gate on the union of the leaf's own and every container ancestor's blockers (states fetched, only non-terminal `state_type` blocks) — a blocked child STOPS here too, before the lease exists, with its live blockers named.
