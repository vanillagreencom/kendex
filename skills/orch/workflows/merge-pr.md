...
3. **Sync issue tracker cache, then close a finished container** — **Linear only** (merged PRs close issues via magic words; cache must reflect done states):
   ...
   **Container completion — the container closes LAST.** The merge just closed `[ISSUE]` (from § 4.1); if that was the final open child of a container parent, complete the container now. Skip if no `[ISSUE]` was extracted in § 4.1. Mechanical check:
   ...
   c. Confirm nothing is still open, then complete:
      ...
      ```
      Pending children remain → report "container [PARENT_ID] stays open ([N] children pending)" in § 7 and continue to step 4. Empty → FIRST capture the canceled set (both branches below need it; the validation payload deliberately excludes canceled children, and post-completion reads only observe overwritten state):
      ```bash
      [MAIN_REPO_ROOT]/.agents/skills/linear/scripts/linear.sh cache issues children [PARENT_ID] --recursive
      ```
      Collect every entry whose `state_type` is `canceled` as `[CANCELED_IDS]`. On entry to this step ALWAYS check for leftover `[MAIN_REPO_ROOT]/tmp/container-canceled-[PARENT_ID]-*.lst` files from interrupted prior runs and run the cascade repair on the UNION of their ids IMMEDIATELY — before validation — restoring any that now read `state_type == "completed"` back to Canceled (the interrupted completion already cascaded them, and validating first would count them as completed bundle children and fail on their missing summaries); delete each repaired file, then re-run the children listing so `[CANCELED_IDS]` reflects the restored states. PERSIST the fresh set before anything mutates — create the scratch dir first (`[MAIN_REPO_ROOT]/tmp` is git-ignored and absent on fresh checkouts) and write one id per line with the harness file tool to `[MAIN_REPO_ROOT]/tmp/container-canceled-[PARENT_ID]-[RUN_TS].lst`, where `[RUN_TS]` is THIS run's `git-context timestamp compact` (skip when empty). The name is per-run on purpose: concurrent close attempts must never share a snapshot — a losing attempt deleting a shared file would strand the winner mid-sequence with no recovery ids. Then validate, and gate the completion on the result:
      ```bash
      ...
      ```
      Capture the JSON. The command exits 0 even when validation fails, so the gate is the payload: proceed ONLY when `.all_ok == true` (a nonzero exit is the fail-closed container guard — treat it the same as `all_ok=false`). Otherwise DELETE this run's just-persisted `container-canceled-[PARENT_ID]-[RUN_TS].lst` (nothing mutated, so there is no interrupted cascade to recover — a leftover here would make a later merge revert children completed legitimately in between), report the failing `.results[]` entries (id, state, what failed) in § 7, and stop — do not complete this container and do not climb to its parent.

      On `all_ok == true`, check the container's own entry in `.results[]` first: `state_type == "completed"` means a concurrent sibling merge (or a retried run) already closed it — skip the completion with a note in § 7 instead of re-posting a summary. Otherwise — and ONLY on this branch (`issues complete` never runs on the skip branch; `[SUMMARY_FILE]` exists only here) — ensure `[MAIN_REPO_ROOT]/tmp` exists, write the bundle summary with the harness file-write tool to `[MAIN_REPO_ROOT]/tmp/container-summary-[PARENT_ID]-[TIMESTAMP].md` (timestamp via `git-context timestamp compact`; starts with `## Bundle Complete`, one line per child with its PR), use that path as `[SUMMARY_FILE]`, and complete NOW — the completion runs BEFORE the cascade repair below, because it is exactly what the repair repairs:
      ```bash
