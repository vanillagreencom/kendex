...
3. **Sync issue tracker cache, then close a finished container** — **Linear only** (merged PRs close issues via magic words; cache must reflect done states):
   ...
   **Container completion — the container closes LAST.** The merge just closed `[ISSUE]` (from § 4.1); if that was the final open child of a container parent, complete the container now. Skip if no `[ISSUE]` was extracted in § 4.1. Mechanical check:
   ...
   c. Confirm nothing is still open, then complete:
      ...
      ```
      Collect every entry whose `state_type` is `canceled` as `[CANCELED_IDS]`. On entry to this step ALWAYS check for a leftover `[MAIN_REPO_ROOT]/tmp/container-canceled-[PARENT_ID].lst` from an interrupted prior run and run the cascade repair on ITS ids IMMEDIATELY — before validation — restoring any that now read `state_type == "completed"` back to Canceled (the interrupted completion already cascaded them, and validating first would count them as completed bundle children and fail on their missing summaries); delete the file once repaired, then re-run the children listing so `[CANCELED_IDS]` reflects the restored states. PERSIST the fresh set before anything mutates — create the scratch dir first (`[MAIN_REPO_ROOT]/tmp` is git-ignored and absent on fresh checkouts) and write one id per line with the harness file tool (skip when empty). Then validate, and gate the completion on the result:
      ```bash
      ...
      ```
      Capture the JSON. The command exits 0 even when validation fails, so the gate is the payload: proceed ONLY when `.all_ok == true` (a nonzero exit is the fail-closed container guard — treat it the same as `all_ok=false`). Otherwise DELETE the just-persisted `container-canceled-[PARENT_ID].lst` (nothing mutated, so there is no interrupted cascade to recover — a leftover here would make a later merge revert children completed legitimately in between), report the failing `.results[]` entries (id, state, what failed) in § 7, and stop — do not complete this container and do not climb to its parent.
