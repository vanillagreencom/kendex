...
3. **Sync issue tracker cache, then close a finished container** — **Linear only** (merged PRs close issues via magic words; cache must reflect done states):
   ...
   **Container completion — the container closes LAST.** The merge just closed `[ISSUE]` (from § 4.1); if that was the final open child of a container parent, complete the container now. Skip if no `[ISSUE]` was extracted in § 4.1. Mechanical check:
   ...
   c. Confirm nothing is still open, then complete:
      ...
      ON BOTH BRANCHES, when `[CANCELED_IDS]` is non-empty, run the cascade repair (the § 4.3 cascade — this run's completion or a concurrent completer's — can flip a canceled child to a completed state, and a canceled child must stay canceled): `issues bulk-get [CANCELED_IDS]`, and restore every id whose `state_type` now reads `completed` (terminality by type, never state names) with `.agents/skills/linear/scripts/linear.sh issues update [CHILD_ID] --state "Canceled"`, noting restorations in § 7. And STILL climb on the skip branch: an already-completed container's own parent may be waiting, so continue to the grandparent check either way:
      ```bash
