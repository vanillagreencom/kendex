...
4. **Clean up branches and worktrees**, scoped to this PR by default — never enumerate unrelated branches or sibling worktrees.
   ...
   **The merged predicate is `worktree cleanup`'s**: ancestry into the base branch, or a merged pull request for the branch when ancestry fails. A squash merge leaves no ancestry, so the second proof is the one that applies to every PR landing through the queue. `worktree remove` applies it itself when deleting the branch — a nonzero exit after the tree is gone means the branch survived and the diagnostic names which proof failed; carry that as `kept` in the § 6 `Branch` row.
