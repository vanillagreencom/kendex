...
3. **Sync issue tracker cache, then close a finished container** — **Linear only** (merged PRs close issues via magic words; cache must reflect done states):
   ...
   **Container completion — the container closes LAST.** The merge just closed `[ISSUE]` (from § 4.1); if that was the final open child of a container parent, complete the container now. Skip if no `[ISSUE]` was extracted in § 4.1. Mechanical check:
   ...
   c. Confirm nothing is still open, then complete:
      ...
      On `all_ok == true`, check the container's own entry in `.results[]` first: `state_type == "completed"` means a concurrent sibling merge (or a retried run) already closed it — skip the completion with a note in § 7 instead of re-posting a summary — but run the canceled-child check either way: the concurrent completer's cascade may already have flipped canceled children, so compare the captured canceled set against a fresh `issues bulk-get` and restore any that read Done. And STILL climb: an already-completed container's own parent may be waiting on it, so continue to the grandparent check below. Otherwise write the bundle summary with the harness file-write tool to `[MAIN_REPO_ROOT]/tmp/container-summary-[PARENT_ID]-[TIMESTAMP].md` (timestamp via `git-context timestamp compact`; starts with `## Bundle Complete`, one line per child with its PR) and use that path as `[SUMMARY_FILE]`; then complete with it — and capture any CANCELED children FIRST — from the step-3a children listing run WITHOUT `--pending` (`.agents/skills/linear/scripts/linear.sh cache issues children [PARENT_ID] --recursive`), collecting every entry whose `state_type` is `canceled`; the validation payload deliberately excludes canceled children, and post-completion reads only observe the overwritten state: the § 4.3 cascade can flip a canceled child to Done when its parent closes, and a canceled child must stay canceled — after completing, restore each captured id with `.agents/skills/linear/scripts/linear.sh issues update [CHILD_ID] --state "Canceled"` (verify with `issues bulk-get [CAPTURED_IDS]`) and note the restoration in § 7:
      ```bash
