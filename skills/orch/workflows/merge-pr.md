...
3. **Sync issue tracker cache, then close a finished container** — **Linear only** (merged PRs close issues via magic words; cache must reflect done states):
   ...
   **Container completion — the container closes LAST.** The merge just closed `[ISSUE]` (from § 4.1); if that was the final open child of a container parent, complete the container now. Skip if no `[ISSUE]` was extracted in § 4.1. Mechanical check:
   ...
   c. Confirm nothing is still open, then complete:
      ...
      On `all_ok == true`, check the container's own entry in `.results[]` first: `state_type == "completed"` means a concurrent sibling merge (or a retried run) already closed it — skip the completion with a note in § 7 instead of re-posting a summary. Otherwise write the bundle summary with the harness file-write tool to `[MAIN_REPO_ROOT]/tmp/container-summary-[PARENT_ID]-[TIMESTAMP].md` (timestamp via `git-context timestamp compact`; starts with `## Bundle Complete`, one line per child with its PR), use that path as `[SUMMARY_FILE]`, and complete with it.
