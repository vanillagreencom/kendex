...
3. **Sync issue tracker cache, then close a finished container** — **Linear only** (merged PRs close issues via magic words; cache must reflect done states):
   ...
   **Container completion — the container closes LAST.** The merge just closed `[ISSUE]` (from § 4.1); if that was the final open child of a container parent, complete the container now. Skip if no `[ISSUE]` was extracted in § 4.1. Mechanical check:
   ...
   c. Confirm nothing is still open, then complete:
      ...
      ```
      Pending children remain → report "container [PARENT_ID] stays open ([N] children pending)" in § 7 and continue to step 4 (this return happens before the lock below is taken — nothing to release). Empty → FIRST capture the canceled set (both branches below need it; the validation payload deliberately excludes canceled children, and post-completion reads only observe overwritten state):
      ```bash
      ...
      **Serialize per parent BEFORE touching any snapshot** — create the scratch dir first (`[MAIN_REPO_ROOT]/tmp` is git-ignored and absent on fresh checkouts), then take the per-parent lock with `mkdir [MAIN_REPO_ROOT]/tmp/container-close-[PARENT_ID].lock` (mkdir is atomic — it either creates or fails). If it already exists: a concurrent sibling merge is mid-sequence — UNLESS the lock dir's mtime is over 60 minutes old (a crashed run; remove it and take it fresh), skip this container's completion entirely with a § 7 note ("container close deferred: concurrent attempt holds the lock") and continue to step 4 — the concurrent attempt or the next merge closes it. Everything below through the final snapshot deletion runs while holding the lock; release it (`rmdir`, after removing this run's snapshot) on EVERY exit path — completion, skip, validation-fail stop, and pending-children return.
