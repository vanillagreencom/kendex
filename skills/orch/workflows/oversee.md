...
- `merged` → wake the owning lane when `.merge_queue_watch.status` is still `"watching"`; that lane runs merge-pr's tracker, base-sync, and cleanup steps, then returns to its outer post-merge build/install/verify flow. The overseer only confirms the lane completed those steps. Mark it done and launch the next unblocked item after the worktree is gone. When the fleet's LAST item finishes, run the `heartbeat` triage pass before closing out, whatever the tracker; items living in Linear also get `.agents/skills/orch/scripts/reconcile-work-items` (a GitHub-item fleet skips that with a note). Report both with the close-out.
...
- `idle-after-return` → read its pane tail. An armed-return lane is available for another fleet launch, but its session stays reachable for the detached merge verdict. When that verdict lands, send the lane a boundary tick; close its session only after it reports post-merge completion and its worktree is gone.
