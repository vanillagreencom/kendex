...
- `peer-note [REPOSITORY] [ID]` → a peer repository's overseer wrote to this one; its text follows. A line carrying `re=[ASK_ID]` replies to this overseer's own ask: record it against that ask, do not answer it. Answer anything else, or put a question to a peer, per [references/peer-mail.md](../references/peer-mail.md). Record the exchange in the fleet log.
