...
Placement: before each launch, read `lane-host resolve`. Any value but `local` makes every lane remote, whatever its harness: add `--host [HOST]`. The credential reaches the sandbox per [schemas/lane-host.md](../schemas/lane-host.md) § Provider protocol, with no local `CLAUDE_CONFIG_DIR` prefix.
...
- **Hand off a lane before it runs out.** On the § 1 tmux surface, at every § 4 event, read each live lane's context use:
  ...
  With `ORCH_LANE_HOST` set, also run `lane-host list`: a row whose repository/item is not in the watch's items file is a sandbox with no live claim; surface it to the user.
