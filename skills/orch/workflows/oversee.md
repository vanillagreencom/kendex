...
2. Size: read the item's body once and make a quick judgement of its complexity (a colour, data, docs or bounded one-function fix; a mechanism change across one subsystem; a correctness predicate with several interacting writers or a review already past its round bound), and pick the model and the reasoning effort that complexity needs. Every launch names both; `open-terminal` refuses a lane launch that names either none, one keyed refusal per missing half, because a harness default changes without notice. The model is sized BEFORE the lane, because it is what the lane is judged on: an account with plan-wide room can have none left for one model, and choosing the lane first picks an account the launch then opens a usage banner on. Never pick a weaker model because a lane is near its wall: pick another lane.
...
4. Launch: the `handoff.md` § 2 `open-terminal` invocation plus `--lane [CONFIG_DIR]` from the picked record, the sized `--launch-flags` and `--state-dir [OVERSEE_STATE_DIR]` (§ Lane record), one item per launch. `open-terminal` reads the model and the effort out of those flags, or out of `--cmd` when they name either none, and refuses the launch that names either nowhere. It then judges the named lane on that model a second time, so a lane picked without `--model` can be refused here. A hosted `--relaunch` is not judged on usage: it runs on the provider's copy of the credential and reports `host-relaunch-credential`. It still names a model and an effort. The gate emits six refusals, each with its own answer:
   - `launch-model-missing` and `launch-effort-missing` (naming `harness`, `lane` and the `spellings` that harness takes): the launch made only part of the choice of lane, model and effort. Add the missing flag; never let the harness pick.
   ...
   - `host-credential-dead`: this machine's copy of the account expired and could not be renewed, while the provider holds a live credential for the same account. Renew the login on this machine for that config dir. A relaunch of an already hosted item is not blocked by it.
...
A dead or walled terminal lane uses native resume. Start with the `handoff.md` § 2 terminal command. Add `--relaunch`, the selected `--lane`, the chosen `--launch-flags` and `--state-dir [OVERSEE_STATE_DIR]`. Keep the tracker, repository, harness, and item arguments. Do not pass `--cmd`, because a custom command bypasses session lookup. A record carrying `host` adds `--host [HOST]`: the provider keeps its tree and the harness continues natively. A hosted relaunch runs on the provider's own copy of the account credential, injected when the host was created, so the launcher does not judge it on the usage windows this machine reads and an account reading `expired` here does not stop it; the launcher reports `host-relaunch-credential` and the resumed session reports its own usage banner, which § 4 reads as `usage-limit`. A fresh hosted launch is still judged on this machine's reading. Where that copy has expired and cannot be renewed while the provider holds the same account, the refusal is `host-credential-dead` and the remedy is to log in again on this machine for that config directory. The launcher delivers the continuation line in the resumed command itself and keeps a merged item's tree as it stands, so nothing is pasted into the pane after the resume. A hosted codex lane is the exception: `codex resume` refuses a prompt beside `--last`, so it resumes with no line and the launcher reports `resume-lineless`. Paste that lane's continuation line into its pane through § Talking to a lane, Pane paste, the way a walled lane gets its nudge.
