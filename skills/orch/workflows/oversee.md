...
4. Launch: the `handoff.md` § 2 `open-terminal` invocation plus `--lane [CONFIG_DIR]` from the picked record, the sized `--launch-flags` and `--state-dir [OVERSEE_STATE_DIR]` (§ Lane record), one item per launch. `open-terminal` reads the model out of those flags, or out of `--cmd` when they name none, and judges the named lane a second time; where neither names a model it judges the account's binding bucket. A lane picked without `--model` can therefore be refused here. A hosted `--relaunch` is not judged at all: it runs on the provider's copy of the credential and reports `host-relaunch-credential`. The gate emits four refusals, each with its own answer:
