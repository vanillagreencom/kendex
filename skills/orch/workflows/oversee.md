...
4. Launch: the `handoff.md` § 2 `open-terminal` invocation plus `--lane [CONFIG_DIR]` from the picked record and the sized `--launch-flags`, one item per launch. `open-terminal` reads the model out of those flags and judges the named lane on it a second time, so a lane picked without `--model` can be refused here. The gate emits four refusals, each with its own answer:
   ...
   - `lane-claims-unreadable`: the in-flight claim store refused, not the allowance. Fix the claims directory the `lane-claims:` line names in its `dir=` field, or set `OVERSEE_WATCH_STATE_DIR`; re-running the pick refuses again.
