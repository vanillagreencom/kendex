...
- `queue-verdict` → wake the named lane. The lane reads its verdict file and routes it per `merge-pr.md` § 5 step 1; the overseer neither reads nor removes the file. The event repeats until the lane consumes it, which is the first act of the route it takes.
- `merged` → wake the owning lane; a GitHub merge alone is not lane completion. The lane reads its verdict file, runs `merge-pr.md` § 5 steps 2-6, and reports a failure there instead of advancing. When the fleet's LAST item completes, run the `heartbeat` triage pass before closing out, whatever the tracker; items living in Linear also get `.agents/skills/orch/scripts/reconcile-work-items` (a GitHub-item fleet skips that with a note). Report both with the close-out.
...
- `idle-after-return` → read its pane tail. An armed-return lane is available for another fleet launch, but its session stays reachable until its PR has merged and `merge-pr.md` § 5 steps 2-6 have run, or it has handed back a non-merged verdict.
