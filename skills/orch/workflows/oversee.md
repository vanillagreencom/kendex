...
- `pr-watch` → handle every attention line. What each kind MEANS is `pr-watch.sh --help`; what follows is only what the overseer does about it. A detail ending in `(QUEUED: dequeue before pushing)` rides several kinds and outranks all of them: the lane dequeues before it can push anything.
  ...
  - `error` → two producers, and they take opposite actions. A read or predicate failure: clear what blocked the read and re-run. A writer-dispatch failure (`writer dispatch failed for …`): the gate is unhealed and a re-run only re-attempts the same failing dispatch, so fix the dispatch path first. Either way an unevaluated PR is never healthy.
