...
- `lane-closed` → the watch closed the merged hosted lane's sandbox: record the `kept=` line that follows in the fleet log, the provider's `kept=PATH` or the watch's `kept=none` for a close that archived no tmp records.
- `lane-close-refused` → a checkout of the merged hosted lane, its clone or its worktree, has uncommitted files and the sandbox stays: surface the `path=` line that follows to the user. Never run `lane-host close --force`.
