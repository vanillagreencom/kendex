...
| `unjudged` from `lanes state` | No process is read at all. The pane settled nothing: no pane on this server carries the name, because the window closed or two windows share it, or the pane's screen carries no marker. | A closed window is relaunched under § Recovery relaunch. A shared name is resolved by renaming one window, which is the guess the observer refuses to make. A markerless screen is read at the pane. |
