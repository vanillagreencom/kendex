...
- **Lane proposals.** Partition pending `Proposal:` comments by resolved tracker and GitHub repository. Use one fleet issue only after its owning lane returns and releases its worktree as the carrier for one batch. Launch the low-cost TPM [proposal-sweep](../../project-management/workflows/proposal-sweep.md) lane with `open-terminal --cmd`; its brief supplies the batch tracker, repository, issue IDs, resolved comment IDs, status file, and mailbox. Add the carrier to `oversee-watch` and wait for its `lane-notice` whose text is exactly `Proposal sweep complete: [STATUS_FILE]`; read no output file before that durable event. For a hosted carrier, run `.agents/skills/orch/scripts/lane-host touch --item [CARRIER_ID]`, then use its `cat` verb to read the status and transfer its named audit. Stop on any probe or read failure. Write the audit bytes to `tmp/proposal-audit-[CARRIER_ID].json` in the overseer's checkout. For a local carrier, copy the named audit there. Read no verification text. Check the local rows against live fleet work, then run [audit-issues](../../project-management/workflows/audit-issues.md) §§ 5-7 with only that local file under `PM_CREATE_AUTONOMY`. For Linear, reply with `.agents/skills/linear/scripts/linear.sh comments create [ISSUE_ID] --parent [COMMENT_ID] --body-file [PATH]`. For GitHub, write an issue comment with `gh issue comment [NUMBER] --repo [OWNER/REPO] --body-file [PATH]`; its first line is `Proposal outcome for [COMMENT_URL]:`. Each outcome carries the created issue ID or one-line decline. Record its source comment and outcome in the fleet log. The completion notice is the sweep lane's worktree release signal. After transferring the files, release the carrier with `.agents/skills/orch/scripts/lane-host close --item [CARRIER_ID]` when hosted or `.agents/skills/worktree/scripts/worktree remove [CARRIER_ID]` when local. A failed release blocks reuse; record a successful release before reuse.
- **Heartbeat backstop for lane proposals.** Include every fleet issue in its tracker and repository batch at each `heartbeat` and before fleet close. This catches a comment posted without another event. Never launch on a worktree its owning lane holds or reuse a carrier whose release is not recorded.
