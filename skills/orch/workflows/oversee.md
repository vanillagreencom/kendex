...
- `peer-note [REPOSITORY] [ID] kind=[KIND]` → a peer repository's overseer wrote to this one; its text follows. Answer `kind=ask` alone, per [references/peer-mail.md](../references/peer-mail.md); `kind=answer` carries `re=[ASK_ID]` and is recorded against that ask of this overseer's, and `kind=directive` needs no reply. Record the exchange in the fleet log.
