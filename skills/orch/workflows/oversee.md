...
- `lane-closed` → the watch closed the merged hosted lane's sandbox: record the `kept=` line that follows in the fleet log when one is present; a close that archived no tmp records prints none.
