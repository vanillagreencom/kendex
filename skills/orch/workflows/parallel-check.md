...
**Expand containers to children.** A parent that is a CONTAINER (`agent:multi` label, or children present with no `(one PR)` title marker) is not a dispatch unit — replace it in the item list with its unblocked children (non-terminal `state_type`, no incomplete blocker) and analyze those. Independent children of one container may dispatch concurrently; blocking relations between siblings sequence the dependent ones. Only an explicit single-PR bundle (`(one PR)` marker) stays in the list as one item.
