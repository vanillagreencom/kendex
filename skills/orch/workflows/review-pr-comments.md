...
**Delegate the fix set.** Ensure the worktree exists (`worktree exists`/`worktree path`, creating with `--pr [PR_NUMBER]` when missing). A worktree this step created carries no lease, so claim it — `worktree-session-guard claim [WORKTREE_PATH] --owner [ISSUE_ID]`, exit 75 meaning another session holds it — then group the `fix set` by `agent` and stamp the round per group as separate tool calls immediately before delegating, arming the watchdog per [SKILL.md § Round Closure](../SKILL.md#round-closure):
