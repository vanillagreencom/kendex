...
`[AGENTS]` is the caller's `agents` context when provided. Otherwise it is the first-cycle panel: the reviewers this harness exposes whose domains the whole diff, `origin/[BASE_BRANCH]...HEAD`, or the issue's Done-when touches, judged by [§ 4's scoped-panel rule](#bounded-re-review). On a § 1 entry, record the panel before any spawn, `[PANEL_REASON]` naming the domains touched, or `caller panel` when the caller passed `agents`:
...
Omit empty categories. Read `patched_causes` and `frozen_causes` first, with the command [finding-disposition.md § Recurrence](../references/finding-disposition.md#recurrence) states. Decline any item that cannot affect real usage with a one-line reason here, per [SKILL.md § The Cycle](../SKILL.md#the-cycle) — it is neither fixed nor filed, and it is reported in § 8. An item a check result lists under `repeats` is a candidate: it sits at a location where § 4 or § 7 recorded a decline, and the entry carries each recorded description and reason. Compare it with them. The same defect stays declined on the recorded reason, is not a new finding, and is recorded under its own location and description by the write below with that reason copied, so § 8 finds it. A different defect is dispositioned like any other finding. Record every decline, from this section or any disposition step, in one write per item, which replaces an entry with the same (location, description), `[SOURCE]` being `pr-review` here and `qa-review` where § 7 follows this pattern:
...
A domain is already reviewed when its reviewer has an accepted artifact in `json_paths`, whichever panel ran it: the first panel is scoped, so a fix can reach a domain no reviewer has read, and a panel member that went unresponsive read nothing.
...
Omit `[OWNER/REPO]` when `TRACKER=linear`. Stamp `review_delegated_at` before each QA delegation as § 2.2 does, and accept the return through § 3.1's check, `--issue` included; on `ok == true`, append the artifact path to `json_paths` and carry the result's `repeats` to § 7; when the agent reports a `benchmark_commit` other than `none`, confirm it resolves with `git -C [WORKTREE_PATH] log -1 --oneline [SHA]`. A performance QA agent's `qa_metadata.perf_qa` block is posted as an issue comment — Linear via `linear.sh comments create [ISSUE_ID] --body-file`, GitHub via `gh issue comment ${ISSUE_ID#issue-} --body-file` — written to a file first. A `pass` verdict continues to the next QA agent. After all QA agents complete, → § 7 — every verdict, every time: § 7 owns the exit and no branch here decides one around it, carrying the blockers and the `category == "fix"` suggestions not already in `escalated_items`. An item this round's QA artifact reports again is retained even when `fixed_items` lists it — that entry is stale, and dropping the item here would hide a live blocker behind the earlier fix.
