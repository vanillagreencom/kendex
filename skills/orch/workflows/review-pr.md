...
```bash
.agents/skills/orch/scripts/workflow-state update [ISSUE_ID] '.fixed_items = ((.fixed_items // []) | map(select(.location != "[LOC]" or .description != "[DESC]"))) | .escalated_items = ((.escalated_items // []) + [{"description":"[DESC]","location":"[LOC]","reason":"outstanding at the review cycle cap","outcome":"blocked","source":"[SOURCE]"}])'
```
