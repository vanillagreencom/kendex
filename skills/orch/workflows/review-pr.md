...
Three is the last cycle that delegates. At `cycles` 4 the cap below governs instead: its verification pass may surface blockers, and those are reported at § 5 rather than answered by another round — so this gate is the final chance to change the shape of the work, not one checkpoint among many.
...
- `split` moves a surface into its own change, so findings on it are still live: record each one in `escalated_items` with the split named as its reason before narrowing `items`, so § 8 reports them as outstanding and the audit files them against the new change. A finding absent from both `fixed_items` and `escalated_items` reads as declined, and a blocker does not stop being one because it moved.
