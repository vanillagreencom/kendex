...
This section owns its exit; it borrows nothing from § 4's cap. **Converged** means a re-check surfaced no new finding of any category — a blocker, a `category == "fix"` suggestion, or a `category == "issue"` suggestion first raised on that re-check is a new finding, and the loop is not converged. One clean re-check is the criterion: § 6 routes a clean pass to § 8 and never back here, so a second confirming pass is not a thing this loop can schedule. The exit stops repeated patching of a finding already answered; it never discards one. The loop otherwise runs on while each round finds a DISTINCT defect this diff introduced, each of those earning its fix, and stops the moment one root cause reappears at a new site: that round's answer is SKILL.md § Review must converge's structural close, cutting the surface that generates the class, never another patch round.
