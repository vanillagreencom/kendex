...
```bash
.agents/skills/orch/scripts/workflow-state update [ISSUE_ID] --argjson-file art [ARTIFACT_PATH] --arg src [SOURCE] '$art.[ARRAY][[INDEX]] as $item | .fixed_items = ((.fixed_items // []) | map(select(.location != $item.location or .description != $item.description))) | .escalated_items = ((.escalated_items // []) + [{description: $item.description, location: $item.location, reason: "outstanding at the review cycle cap", outcome: "blocked", source: $src}])'
```
