...
Omit empty categories. **Disposition is by rule, not by prompt** — never present a selection menu over the findings. Every surviving blocker and `category == "fix"` suggestion is fixed; declines are reported in § 5. That disposition is `mode-independent`: `ORCH_DECISION_MODE` never reintroduces the menu, in `ask` and `auto-recommended` alike. The always-ask set in [SKILL.md § The Cycle](../SKILL.md#the-cycle) still applies.
