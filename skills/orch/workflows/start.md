...
- **Child of a container** → the parent's classification decides, and the child's own fetch carries only `parent_id` — so fetch the parent first (`cache issues get [PARENT_ID]`; `--with-bundle` on the child does NOT include the parent's title or labels) and check ITS title/labels. Parent without the `(one PR)` marker → the child is the PR unit — but gate it on the SAME unblocked test used for container expansion before proceeding (its own `blocked_by` plus the parent's, blocker states fetched, only non-terminal `state_type` blocks): a directly-selected child must not skip the sequencing its siblings enforce — if blocked, stop and name the live blockers instead of preparing a worktree. Parent WITH the marker → the old rule applies: use the parent as the work item unless the user explicitly chose the child.
