...
Then read the engine's one-switch gate disable, which overrides that resolution:

```bash
.agents/skills/orch/scripts/orch-env REVIEW_GATE_MODE enforce
```

When `REVIEW_GATE_MODE` resolves to `off` (standard orch-env precedence, the same as every other key here), the repo's review gate is disabled by settings — the engine predicate answers `approved` before any evidence read and every status it posts says the gate is disabled — so set `GATE_MODE = off` regardless of the reviewer-gate resolution above: the wait loop is skipped and the recording and § 6.1 gate 4 handling follow the existing `off` path. `enforce` (the default) leaves the resolution above unchanged. Any other value is a config error on the engine side (a typo cannot disable a merge gate) — do not treat it as `off`; keep the resolved `GATE_MODE` and surface the bad value.
...
1. **Review wait loop.** Poll for the gate verdict and new review comments together (skip when `GATE_MODE` is `off`):
   ...
   | `proceeded` | Reviewer-down degrade (`PR_REVIEW_ON_TIMEOUT=proceed`): the deadline passed with **zero unresolved threads** and no reviewer evidence — every configured reviewer stayed silent (e.g. credits exhausted). Record it under its own state field (kept distinct from a user Force merge so provenance is preserved) so the § 6.1 gate 4 check reads it as an automated reviewer-down proceed, then → step 2. CI (§ 5) and the comment-hygiene gate (§ 6.1 gate 3) still apply in full: `.agents/skills/orch/scripts/workflow-state set [ISSUE_ID] pr_approval.reviewer_down true`. The proceed is a LOCAL verdict only — orch posts no status and manufactures no review evidence; a repo whose CI independently gates on review evidence stays blocked unless its gate is disabled via the engine's `REVIEW_GATE_MODE=off` or an operator posts the engine's manual override status. |
   ...
   | `timeout` | No gate verdict within the review-wait budget (`PR_REVIEW_WAIT_SECS`, default 900 s; only when `PR_REVIEW_ON_TIMEOUT` is `block`/unset — with `proceed` a no-evidence deadline arrives as `proceeded` above) → Ask user: "No [GATE_MODE]-gate verdict on PR #[PR_NUMBER] after [ELAPSED] min" — `Force merge` \| `Keep waiting` \| `Stop here` |
...
2. **Gate 2** — verify the recorded § 5 result: met when the final CI result was `status=complete`, `verdict=pass`. Do not re-run ci-wait.
   ...
   **`ci-wait verdict=pass` is NOT the same as "every check in the rollup is green".** `ci-wait` correlates the head's checks with its Actions runs and judges the current substantive run; `gh pr checks` reads the whole rollup, which can still hold checks from a SECOND same-head workflow run — e.g. a duplicate dispatch cancelled by concurrency, whose jobs come back as zero-second failures. `github.sh pr-merge --check` applies the same correlation as `ci-wait` (one shared implementation, vstack#881), so the two agree; raw `gh pr checks` output does not and is not the gate.

   So a `pr-merge --check` refusal immediately after a green § 5 does **not** invalidate the § 5 verdict, and it is not a reason to re-run ci-wait. Identify which run each reported failure belongs to (`gh pr checks [PR_NUMBER] --json name,state,link,workflow` — the run id is in `link`) before treating it as a real CI failure. Note that run id does **not** order runs by execution: a rerun executes under the ORIGINAL run's id, so a lower-numbered run can be the current one (vstack#699, vstack#876). If a refusal survives and every reported failure belongs to a superseded or duplicate run while the repo's required aggregate is green, report it to the user with the run ids rather than silently forcing or silently abandoning the merge.
