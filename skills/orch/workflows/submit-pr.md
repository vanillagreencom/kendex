...
The printed value is `GATE_MODE` — `approval`, `review`, or `off`. The resolution order is implemented once in approval-wait, never re-derived here: the engine's one-switch gate disable wins first (`REVIEW_GATE_MODE=off`, orch-env precedence, prints `off` — the repo's gate is disabled by settings, so the wait loop is skipped and recording and § 6.1 gate 4 follow the existing `off` path; only the exact value `off` narrows — any other value leaves the reviewer keys authoritative, and the engine predicate is where a bad value fails loud); then `PR_REVIEW_GATE` when set; otherwise the legacy `PR_APPROVAL_GATE` maps `on` → `approval` and `off` → `off`; all unset defaults to `approval`. Set the keys in `vstack.settings.toml` `[env]`.
