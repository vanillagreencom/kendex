...
**Gate 2** — verify the recorded result; do not re-run ci-wait. `ci-wait verdict=pass` is not the same as "every check in the rollup is green": ci-wait correlates the head's checks with its Actions runs and judges the current substantive run, while `gh pr checks` reads the whole rollup, which can still hold checks from a second same-head run — a duplicate dispatch cancelled by concurrency comes back as zero-second failures. `github.sh pr-merge --check` applies the same correlation as ci-wait, so those two agree; raw `gh pr checks` output does not and is not the gate. A `pr-merge --check` refusal right after a green § 5 therefore does not invalidate the § 5 verdict. Identify which run each reported failure belongs to (`gh pr checks [PR_NUMBER] --json name,state,link,workflow` — the run id is in `link`) before treating it as real; run id does not order runs by execution, since a rerun executes under the original run's id. If a refusal survives and every reported failure belongs to a superseded or duplicate run while the required aggregate is green, report it with the run ids rather than silently forcing or abandoning the merge.
