...
After every managed `review-pr-comments` return, rebuild the PR body from § 2 step 3 and update it with § 2 step 4 before continuing or reporting a terminal validation failure. This carries each stored proposed rule into the existing `## Proposed rules` section once.
