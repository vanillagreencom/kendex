...
- The Linear cache holds the whole workspace: `sync` sends no team filter, and `cache issues list` neither filters by team nor returns one. Scope every cached read to the configured team, resolved per tpm-audit § 1.1.1 — an issue outside it is never an input, a comparison, a duplicate, an obsolescence candidate, or a cancellation, and a project outside it is never a placement.
