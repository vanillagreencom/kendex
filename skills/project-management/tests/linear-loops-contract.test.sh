...
require_fixed 'human-filed issues are born in Triage and stay eligible at any age' 'human creation-time pairs still bundle'
require_fixed 'on the leader FIRST, then apply the "re-triage"' 'the handoff comment precedes the trigger label'
require_fixed 'the comment must exist before the label can fire Loop 2' 'handoff ordering rationale is stated'
require_fixed 'any issue named in a `bundle-handoff from <ID>` comment on' 'handoff-named issues are exempt from the age rule'
