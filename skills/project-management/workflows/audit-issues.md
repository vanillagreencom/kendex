...
**Carried approval (roadmap-create only).** When the analyzed input carries `approved_at_plan_gate: true` — set only by the roadmap-create wrapper, in this same session, after the user answered `Approve` at roadmap-plan § 5 on the identical set — the "Create these issues?" question is already answered: every unmodified `create` item is approved, and the question is asked only for entries marked `"reapprove": true` (changed since that answer). The flag has no authority from a subagent, another session, or any input file roadmap-create did not just write; cancellations not already executed by roadmap-create § 2 (those arrive as `skip`, reason `executed at § 2`), declined follow-ups, and the fail-closed rule above stand in full.
