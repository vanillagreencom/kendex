...
| `add_relations` | `depends_on_proposed` titles → `blocked_by: ["#N"]` by index; `depends_on_existing` → `blocked_by: ["[ISSUE_ID]"]`. A dependency on an entry § 2 omitted (an executed `expand`/`descope`) retargets to that entry's existing `target` issue id — never a dangling `#N`. Relations are already lifted to parent level — preserve them |
