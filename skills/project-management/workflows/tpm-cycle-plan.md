...
**Blocking level rule**: Child→external relations must be lifted to parent level — before adding a cross-bundle relation, resolve each endpoint by walking its FULL `parent_id` chain to its top-level ancestor (one hop is not enough: with `P → C → G`, a proposed `P blocks G` one-hop-resolves to `P blocks C` and slips through). If one endpoint's chain passes through the other endpoint at ANY depth, the relation is ancestor↔descendant — reject it (see below). Otherwise lift each endpoint to the child of the LAST common ancestor when the chains share one (peer nodes where the subtrees separate), or to each side's top-level ancestor when they share none. Keep the relation at CHILD level (sibling child-blocks-child) only when the two lifted endpoints are distinct issues that share the same non-empty `parent_id`: the parent is a container by default and dependent children are sequenced by exactly these sibling relations — selection dispatches only unblocked children. A relation whose endpoints are ancestor and descendant (at any depth, per the chain walk above) is NOT sibling sequencing — reject it instead of adding it: a child→parent block is meaningless (the container closes last anyway), and a parent→child block is inherited by every child, deadlocking the bundle as permanently undispatchable. (Inside an explicit `(one PR)` single-PR bundle the sibling relation is still valid but only informs the delegated session's internal order.)
