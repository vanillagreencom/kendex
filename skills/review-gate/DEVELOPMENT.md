...
- A pass reads the open-PR listing once, so a push before a PR's turn can leave the predicate a head it cannot fetch. When a single-head evaluation fails, the writer re-reads that PR once. It evaluates a moved head on the current head in the same pass, and skips a PR closed since the listing; both log `writer-head-moved`. A failure on the head the PR still has, or a failed re-read (`writer-pr-read-failed`), still fails the pass. The writer does not re-read after an overrun of the per-PR share.
