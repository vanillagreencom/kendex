...
`--adopt` is the remedy's writer. It runs every version of the template in the repository's history through the same two allowances, and re-installs the current template only over a copy one of those versions equals. The re-install writes the template's bytes with the copy's two allowances applied, so a comment-only edit, which equality compares out, is replaced. A copy whose code lines no version equals is a copy a person edited, and it is never overwritten: that verdict is `workflow-edited`, and its remedy is a hand re-copy.

The check re-derives nothing by design. Deriving the contract (this job's permissions, that expression's terms, these activity types) is a YAML-and-expressions parser written in bash, and every new spelling is a new hole. Equality has no such gap, because the template carries no per-repo values: a copy that differs from every shipped version is a copy someone edited.
...
A review fails as `suppressed-findings` when its body carries a section titled `Suppressed comments (N)` or `Previously missed (N)`. Copilot writes that block instead of posting comments when it judges a finding to be in code the current diff did not change, so no review thread exists and the unresolved-thread term reads zero. The title is read off a markdown heading or a `<details>` summary, and an entry off a bold or a backticked `file:line` line, because the reviewer has written the block on both surfaces. The term runs over the rows the evidence select accepts, before the `REVIEW_GATE_REVIEW_OBJECT_MIN_STATE` reduction, so a COMMENTED row carrying findings is read under `approved` too. It reads them at the commit the gate relies on: the head, and `carry_base` as well once carry-forward decided the evidence — the carry candidate is the same trust chain at a different commit, so the row carrying the block is itself carry-eligible and head-only reading would let a carry approve over it.

The block ends at the next heading of any level, or at the `</details>` that closes the section the title opened. That close is depth-counted, because the summary-titled shape wraps each entry in a `<details>` of its own and ending at the first one would keep one entry and drop the rest. An entry inside the block is a whole line of the form `**path:line**` or `` `path:line` ``, stripped of the zero-width space Copilot writes into a path for display. Lines inside a fenced snippet are skipped, because the offending code pasted under an entry carries `#` comment lines a heading rule would otherwise read as the end of the block. The fence records its opening delimiter's character and length and closes only on a run of the same character at that length or longer with nothing after it, so a three-backtick fence quoted inside a four-backtick one does not end it. The section title is the sentinel: one extractor turns a markdown heading, or a `<details>` summary with its inner tags stripped, into its plain title text, and both title arms read that whatever the fence state says and close any fence they find open, so no run of fence-looking lines earlier in the body can leave the scan inside a fence and swallow the block that follows. That errs toward finding a block, never toward missing one — the same direction as the quoting limit below. The title's `N` is the count reported; the entries are the file:line list. A disagreement between the two, a title whose count is not a number, and a parse bash cannot read all refuse instead of reporting a smaller number, and all refuse to the verdict rather than to exit 2, which would tell the writer to take no action and leave an earlier success standing. The verdict detail carries a bounded list because a commit-status description holds about 140 characters; when it drops entries it counts them, and the full list goes to stderr under `review-gate-notice=predicate-suppressed`.

There is no dedicated settings key. A class policy `none` decision skips the term. `REVIEW_GATE_MODE = "off"` skips it when the class policy is inactive or resolves to `current`. The known limit is the errored-attestation filter's mirror: a body quoting the title at the start of a line counts as a real block.

An entry is subtracted when the PR author has answered it, because no thread exists to reply into. The reply is an issue comment by the author carrying a line that opens with the entry's own `file:line` token — bare as the status detail prints it, or bold or backticked as the review body prints it — and continues with the reply. The comment is stripped of the display zero-width space the body carries, so an author copying the token out of the rendered review answers the entry. Every spelling goes through one test, equality with a token the scan extracted, so the scan holds the only definition of what an entry token is and a path it admits is a path the author can copy back, a space in it included. The bare form needs the character after the token to be no letter or digit, so `a/b.ts:1` cannot claim the line `a/b.ts:12`; the decorated forms need no such test, their closing `**` or backtick being the terminator. The comment binds this head through a literal marker, `Dispositions at <sha>` (case-insensitive), followed by a sha at or above `REVIEW_GATE_SHA_PREFIX_FLOOR` that the head starts with, so a comment written for an earlier head does not survive a push. The shape is the comment-form matcher's: the literal, decoration after it ignored as non-hex, then the sha. Nothing else in the comment binds. A run shaped like a sha asserts no commit — the one a `Fixed in <sha>` names, which the ordinary commit-reply-push order makes the head; a tracking claim's `#1234567`; the entry token where a path opens with hex; a line disposing an entry the newest review dropped, which no current entry claims — and a free scan let any of them carry a comment written for the earlier head onto a diff no reviewer re-read. A marker cannot be written by accident, which ends the class instead of excluding its members one at a time. That reply is read by the same `disposition`, `tracking`, `names_issue`, `declined` and `reason_left` definitions the thread terms read, spelled once in `REPLY_FORMS_DEF`: a reply that is neither a disposition nor a tracking claim, a tracking claim naming no issue, and a decline whose reason strips to nothing all leave the entry standing. The newest line naming an entry decides. Every finding stands when the disposition read produces no count, and the verdict says so; the term still clears whole when the commit the gate relies on carries no block.
...
When the class policy is inactive, `REVIEW_GATE_DOCS_ONLY = "none"` calls the harness-ci `harness-only --mode docs` classifier for the PR base and head. A true verdict substitutes for missing review evidence only when no changed path matches `REVIEW_GATE_CARRY_FORWARD_EXCLUDE`. A false verdict or an excluded policy path takes the normal evidence path. This substitution never bypasses changes requested, suppressed findings, or unresolved threads.

`REVIEW_GATE_CLASS_POLICY` is active by default: `review-policy` holds the default value, and an explicit empty assignment turns it off. `review-policy --check-choice` says how the repository chose: the default, the default assigned, custom rows or off, comparing normalized rows. `validate.sh` reads that record and refuses custom rows or off unless `REVIEW_GATE_CLASS_POLICY_DECISION` names a tracked decision record, so any departure is a recorded choice and never an inherited default. The review-gate suites that pin the evidence engine or a legacy lane assign the key empty, because those lanes run only under an inactive policy.

When the class policy is active, `review-policy` calls the existing `harness-ci` classifier and returns one evidence policy. The [README class table](README.md#class-policy) owns the result for each class. A `none` result returns before every read the gate would make, which the [README class table](README.md#class-policy) states the scope of. Other results use the existing evidence engine. A fallback is not a result. `standard` is both a rule's verdict and the classifier's answer where it could not read a diff, an inventory, a render proof, the branch measurer or the narrow-change list, and its `class:` line carries `measured=true|false` saying which. `review-policy` reads that marker and keeps no list of causes; a `false` answer exits 2 with `policy-unmeasured` naming the classifier's own cause, and a class line with no marker exits 2 with `policy-classifier-protocol`. Every caller materializes the range before asking.

Changes requested and unresolved threads fail closed whenever the selected class policy reads review evidence. Every evidence read fails loud with exit 2 and no verdict.
...
- The `pull_request_target` job never executes PR-controlled code. Every workflow checkout pins a shallow copy of the default branch with stored credentials disabled. For an active class policy, the predicate fetches the base and head objects, creates a separate detached checkout for the pull request head, prepares its kendex source mirror without GitHub credentials, and calls the trusted default-branch classifier against that checkout. The pull request checkout is judged data and supplies no executable classifier or install script.
