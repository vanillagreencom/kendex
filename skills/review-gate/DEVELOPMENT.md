...
When `REVIEW_GATE_CLASS_POLICY` is active, `review-policy` calls the existing `harness-ci` classifier and returns one evidence policy. The [README class table](README.md#class-policy) owns the result for each class. A `none` result returns before every review evidence and thread read. Other results use the existing evidence engine. An endpoint the checkout cannot resolve is not a result: the classifier answers `standard` there and reports `cause=unresolved-endpoint`, and `review-policy` exits 2 with `policy-endpoint` rather than pass a class nothing measured to a caller. Every caller materializes the range before asking.
