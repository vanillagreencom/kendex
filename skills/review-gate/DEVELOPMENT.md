...
| `tests/lib/merged-sweep-fixtures.sh` | The `gh` stub, the GraphQL fixture builders, the assertions and the pass runners, which the merged-sweep suite sources, so the suite file holds only its test arms. |
