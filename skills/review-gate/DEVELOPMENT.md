...
`--adopt` is the remedy's writer. It runs every version of the template in the repository's history through the same two allowances, and re-installs the current template only over a copy one of those versions equals. A copy no version equals is a copy a person edited, and it is never overwritten: that verdict is `workflow-edited`, and its remedy is a hand re-copy.
