...
| `scripts/validate-workflow.sh` | The adopted-workflow contract alone. Split at the one seam the check has — everything here reads `.github/workflows/` and nothing else does — and usable on its own when only the workflow copy changed. |
...
evaluate. Every configuration rule sits above that stop; moving one below it
is a visible edit, not a silent hole in what the flag covers.
