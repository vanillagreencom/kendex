...
| `scripts/validate-standard.sh` | Does this repository's GitHub side match the organization standard? Read-only GETs against the rulesets, the app installations and the environment holding the app secrets. `validate.sh` does not run it, because CI's token cannot read bypass actors, installations or secret names. |
| `standard.json` | The organization standard's values: the required contexts, the app, the environment and its secret names. `validate-standard.sh` reads them from here and nowhere else. |
