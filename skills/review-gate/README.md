...
A missing package is refused, never read as an inactive policy. `review-policy` exits 2 as `policy-classifier` when the `harness-ci` classifier is not installed beside this skill, and as `policy-unmeasured` when the classifier could not measure a class: no `orch` narrow-change list or measurer, or no `kendex` command to prove a render diff (`cause=no-verifier`). `validate.sh` reports the first as `class-policy-unresolved`.

To opt out, a repository assigns `REVIEW_GATE_CLASS_POLICY = ""`, which turns this table off and keeps the gate behavior from before the class policy. It also sets `REVIEW_GATE_CLASS_POLICY_DECISION` to the tracked decision record that gives the reason. `validate.sh` refuses an empty value with no decision record as `class-policy-inactive`.
