...
The class policy decides before `REVIEW_GATE_MODE`. With the mode `off`, a `small` change still needs its bot review round. A repository with the mode `off` that inherits the default is reported as `class-policy-mode-off`: assign `REVIEW_GATE_CLASS_POLICY` explicitly to accept the bot round, or opt out with a decision record. The legacy `REVIEW_GATE_DOCS_ONLY` and `REVIEW_GATE_RENDER_PATHS` lanes below run only under an inactive policy, so under an active one `validate.sh` reports a configured lane, `REVIEW_GATE_DOCS_ONLY = "none"` or a non-empty `REVIEW_GATE_RENDER_PATHS`, as `class-policy-inert-lane`.
