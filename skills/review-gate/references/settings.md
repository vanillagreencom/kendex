...
| `REVIEW_GATE_API_RETRY_DELAY_SECONDS` | `2` | Pause between retry attempts. The selftest fails on a committed non-integer (the live predicate would exit 2 on every evaluation) but replays its battery with the delay pinned to 0 — the pause paces production reads and decides no verdict. |
