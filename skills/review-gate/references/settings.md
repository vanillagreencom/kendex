...
| `REVIEW_GATE_EVIDENCE_AT_FILE` | (unset) | ENV-ONLY per-invocation seam, never a settings key (like `REVIEW_GATE_SETTINGS_FILE` and `REVIEW_GATE_STATUS_SNAPSHOT_FILE`): a path the predicate writes the evaluated evidence's creation instant to on approved verdicts — the latest instant across every contributing source, empty when any of them lacks a timestamp or the verdict is not approved. The v2 writer sets it and uses the instant for the ordering fast path (a completed attempt that started after the evidence AND skipped no jobs needs no proof rerun). Deliberately a FILE rather than a second stdout line: every consumer parses the predicate's stdout positionally. |
| `REVIEW_GATE_STALL_MINUTES` | `120` | How long a head may sit in ONE waiting disposition before the writer stops waiting (0 disables). Waits are correct only while the awaited event can still arrive; two shapes exist where it cannot (a head that never produces a `pull_request` run, and a proof attempt cancelled before completing — the marker branch never re-issues on its own). Past the bound the writer re-issues the proof rerun where that helps (no double-bill: the replaced attempt produced no completion) and escalates ONCE where it cannot help (a `STALLED:` gate description plus a red run for the sustained-failure incident). Fail-closed: a stalled head is reported, never opened. |
