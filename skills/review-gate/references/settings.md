...
| `PR_REVIEW_WAIT_SECS` | `900` | Review-wait quiet period, SHARED with the orch skill (approval-wait's absent-positional budget). Read by `pr-watch.sh` as the `awaiting-stale` threshold — an un-reviewed head older than this is attention. One key so the watcher and the waiter agree on what "silence too long" means. |
