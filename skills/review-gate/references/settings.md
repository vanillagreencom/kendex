...
| `REVIEW_GATE_MERGED_SWEEP_STATE_DIR` | `tmp/review-gate-merged-sweep` | Directory holding `merged-sweep.sh`'s per-repo dedupe state — one file per repo, named for `GH_REPO` with the slash written `%2F` so two repositories can never share a file, holding the finding keys of the previous pass. A RELATIVE path is anchored on the repository root (`git rev-parse --show-toplevel`), never the process cwd, so a poll loop that changes directory between passes keeps its rising edge; outside a git working tree a relative value is a configuration error naming this key. RESOLUTION BOUNDARY: this key alone is resolved with the settings files read from the repository root rather than the process cwd, because the ladder reads `.env.local` and both TOMLs as cwd-relative paths and an off-root pass would otherwise take the built-in default and anchor a different directory under the same root — re-announcing every finding on every pass, which is the failure the anchoring exists to stop. The default writes INSIDE the repository, so a consumer must gitignore it or point this key somewhere else. Explicitly empty is a configuration error, not a disable — `--no-state` is how a caller runs without state. |
