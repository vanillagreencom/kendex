...
| `REVIEW_GATE_REVIEW_OBJECT_ERROR_PATTERNS` | `encountered an error and was unable to review` | Case-insensitive substrings marking a review row's BODY as an errored-run attestation → not evidence and never a carry candidate. Matched at the start of the body only (first line, after trimming whitespace and markdown quote markers): a review that quotes a pattern in later text is still evidence. Patterns can never reach past line one — when adopting a new reviewer bot or after a bot update, verify its errored-run body still leads with the configured marker. Never a blocker: the changes-requested reduction ignores this list. A configured value replaces the default list. Empty disables. |
