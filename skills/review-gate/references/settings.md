...
| `REVIEW_GATE_OVERRIDE_CONTEXT` | (absent = fall back to `REVIEW_GATE_OUTAGE_CONTEXT`) | v2 name for the operator override status context (the outage attestation generalized), read by the single writer (`review-writer.sh`). Implemented as an alias: when the key is present its value is exported to the predicate as `REVIEW_GATE_OUTAGE_CONTEXT` (and wins over a file-level `REVIEW_GATE_OUTAGE_CONTEXT`); when absent, the predicate's own outage-context resolution applies unchanged. Empty disables the override source. |
