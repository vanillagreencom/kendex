...
# Dispatch is by request shape (the endpoint path, or the GraphQL query
# text); the switches GH_SHIM_FAIL, GH_SHIM_FAIL_TIMES and GH_SHIM_EMPTY
# drive the fail-loud paths, and <name>.page2.json models a second page.
# Every request URL is appended to .urls.log so a case can pin read shapes.
set -euo pipefail
url=""; filter=""; paginate=0; graphql_page2=0; graphql_after=""; query=""
while [ $# -gt 0 ]; do
  case "$1" in
    api|--slurp) ;;
    --paginate) paginate=1 ;;
...
case "$url" in
  ...
  *"/issues/"*"/comments"*) name=comments ;;
  graphql)
    # A blob query names its objects inline (one alias per sha:path); the
    # threads query never mentions a Blob. Dispatch on that, not on the
    # request shape, which is the same `gh api graphql` for both.
    case "$query" in
      *"on Blob"*) name=blobs ;;
      *) name=graphql ;;
    esac
...
if [ "$name" = "blobs" ] && [ -f "$GH_SHIM_FIXTURES/blobs.errors.json" ]; then
  # A GraphQL error envelope in place of the objects — served verbatim, the
  # shape GitHub answers a bad query or a partial outage with.
  cat "$GH_SHIM_FIXTURES/blobs.errors.json"
  exit 0
fi
if [ "$name" = "blobs" ]; then
  # Each alias resolves to its own fixture, blob-<sha>-<path with / as
  # __>.json, holding the Blob object or null (absent at that sha). A
  # missing fixture refuses loudly, like a missing page: a case must plant
  # every object it reads, or an unplanted one would read as absent and a
  # refusal could pass for a proof.
  resolved='{}'
  while IFS=$'\t' read -r alias expr; do
    [ -n "$alias" ] || continue
    sha="${expr%%:*}"; path="${expr#*:}"
    blob_file="$GH_SHIM_FIXTURES/blob-$sha-${path//\//__}.json"
    [ -f "$blob_file" ] || { echo "shim: no blob fixture $blob_file (expression $expr)" >&2; exit 94; }
    resolved="$(jq -c --arg a "$alias" --slurpfile v "$blob_file" '. + {($a): $v[0]}' <<<"$resolved")"
  done <<EOF_ALIASES
$(grep -oE '[A-Za-z_][A-Za-z0-9_]*:object\(expression:"[^"]*"\)' <<<"$query" | sed -E 's/^([A-Za-z0-9_]+):object\(expression:"([^"]*)"\)$/\1\t\2/')
EOF_ALIASES
  response="$(jq -n --argjson r "$resolved" '{data:{repository:$r}}')"
  if [ -n "$filter" ]; then jq -r "$filter" <<<"$response"; else printf '%s\n' "$response"; fi
  exit 0
fi
