...
+b')]" '' '' '' "$(jq -cn --argjson r "$(review reviewer APPROVED '2026-01-01T00:00:00Z' "$OTHER")" '[$r]')"
  ...
  carry_row "carry: a bodyless COMMENTED ancestor review that opened no thread is not a carry candidate" awaiting 0 "docs" "$ACTIVE_CARRY_EXCLUDE" "$ACTIVE_VENDORED_PATHS" "" "any" "$ACTIVE_ERROR_PATTERNS" "" "ahead" "[$DOCS_DELTA]" '' '' '' "$(jq -cn --args '$ARGS.positional | map(fromjson)' "$(review "reviewer" COMMENTED "2026-01-01T00:00:00Z" "$OTHER")")"
