...
# The provenance reads of the vendored carry class go through GraphQL blob
# objects, one fixture per (sha, path): blob SHA PATH CONTENT plants a text
# blob the way GitHub serves it (byteSize is the UTF-8 length, never a
# number a case picks); blob_absent SHA PATH plants "no object at that
# sha", and blob_shape lets a case write any other shape explicitly.
blob_fixture_path() { # sha, path -> the fixture file for that object
  printf '%s/blob-%s-%s.json' "$fixtures" "$1" "${2//\//__}"
}
blob() { # sha, path, content
  jq -n --arg t "$3" '{text:$t,isBinary:false,isTruncated:false,byteSize:($t|utf8bytelength)}' \
    >"$(blob_fixture_path "$1" "$2")"
}
blob_absent() { # sha, path
  printf 'null\n' >"$(blob_fixture_path "$1" "$2")"
}
blob_shape() { # sha, path, JSON object
  printf '%s\n' "$3" >"$(blob_fixture_path "$1" "$2")"
}
# A kendex lock naming what it rendered: lock_fix SHA PATH=HASH... writes
# .kendex-lock.json at that sha with one entry carrying those renderedFiles;
# sha256_of prints the hash the lock would record for CONTENT.
sha256_of() { # content -> plain sha256 hex, what `sha256sum` prints
  printf '%s' "$1" | sha256sum | cut -d' ' -f1
}
lock_fix() { # sha, [path=hash]...
  local sha="$1" pair files='{}'
  shift
  for pair in "$@"; do
    files="$(jq -c --arg p "${pair%%=*}" --arg h "${pair#*=}" '. + {($p): $h}' <<<"$files")"
  done
  blob "$sha" ".kendex-lock.json" "$(jq -n --argjson f "$files" \
    '{version:5,entries:{"skill:hello:claude":{name:"hello",kind:"skill",harness:"claude",renderedFiles:$f}}}')"
}
