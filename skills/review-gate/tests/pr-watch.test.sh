...
set +e
out=$(cd "$TMP_ROOT/cwd" && env -u GH_REPO "$TMP_ROOT/scripts/pr-watch.sh" --help 2>"$TMP_ROOT/help.err")
rc=$?
set -e
...
set +e
out=$(cd "$TMP_ROOT/cwd" && env -u GH_REPO "$TMP_ROOT/scripts/pr-watch.sh" -h 2>/dev/null)
rc=$?
set -e
