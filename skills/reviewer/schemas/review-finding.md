...
Declaring `qa_metadata` also commits the artifact to usable findings. `review-artifact-check` rejects such an artifact with reason `incomplete` when its `blockers[]` or `suggestions[]` is missing or not an array (a truncated write can keep `verdict`/`summary` while silently losing the findings, so the verdict cannot be trusted), AND when a present `blockers[]`/`suggestions[]` item omits a required item field below — `id`, `title`, `location`, `description`, `recommendation`, `priority`, `estimate`, plus `category ∈ {fix,issue}` for suggestions (the orchestrator routes suggestions on `category`, so a category-less item is unroutable; the rejection's `detail` field names the offending item and field). `questions[]` is exempt: it is PR-comment-triage-only. Artifacts without `qa_metadata` keep the tolerant existence + `verdict` validation.
