...
| `SECOND_OPINION_CURRENT_MODEL` | `"claude"` (or `"opus"`, `"gpt-5.6-sol"`, `"openai-codex/gpt-5.6-sol"`, `"none"`) | The model your session runs — set per session, not in the shared file. Detected for Claude Code and Codex; required in Pi, OpenCode, Cursor and undetected shells — `none` says there is no session model (CI, plain terminal) |
...
| `SECOND_OPINION_CURRENT_MODEL` | (unset) | The model your session runs, when the CLI cannot tell (Pi, OpenCode, Cursor, undetected — required there; `none` = no session model); Claude Code and Codex are detected. **Session-scoped: export it in the session, not in `.env`, `vstack.settings.toml`, `.vstack/settings.toml` or `.env.local`** — a project file is read by every session in the repo, so a value in one is judged by what it adds: it is **ignored** when it agrees with a detected Claude Code or Codex session (detection already knew), **refused naming both values** when it contradicts one, and **refused naming the file** in a session whose harness cannot be detected (Pi, OpenCode, Cursor), where a shared file cannot describe a per-session model and nothing could catch a stale one. Model ids normalize, provider prefix included (`opus`, `anthropic/claude-opus-4` → claude; `gpt-*`, `openai-codex/gpt-*` → codex); a value you set that the roster does not know is refused (a detected one is not — the roster may name only the target) |
