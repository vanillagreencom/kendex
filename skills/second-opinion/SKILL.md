...
**Review is multi-lane by default.** With no `--target` and no `SECOND_OPINION_TARGET`, `review` runs every available lane in `SECOND_OPINION_REVIEW_TARGETS` in parallel on one pinned scope and writes a single union artifact: findings deduplicated across lanes, with per-lane provenance in `qa_metadata.lanes`. With `--output`, each lane's own artifact is kept beside the union as `<output>.<target>.json`. One failed lane does not fail the run — it is recorded in `qa_metadata.lanes` and `qa_metadata.coverage` becomes `"degraded"`; only when every lane fails is there no artifact and the run exits 4 or 5 (see Error Handling). Lane resolution, merge rules, artifact placement and permissions, scratch durability, and the failure taxonomy: [references/multi-lane.md](references/multi-lane.md).
