...
**No exit leaves a previous run's output behind.** Every mode that writes `--output` — `review`, `audit`, `challenge`, `quick` — clears that path and its managed sidecars (`.raw.txt`, `.retry.txt`, `.failed.json`, `.noreview.json`, `.incomplete.json`) as soon as the arguments are read: ahead of a bad-flag error, the `jq` check, the mode check and `--timeout` validation, and before target selection or any CLI run. So a refusal, timeout, or unusable response cannot be read as this run's result by a caller that continues past the advisory non-zero exit. `--output` is a path the caller designated for this run's output, so no question of authorship arises there; `--help` and `detect` never reach the write and clear nothing.

`review` additionally clears the `<output>.<target>.json` lane artifacts it alone can produce: the roster's own names unconditionally, plus any left by a target since renamed or retired — those only when the file's **content** carries this skill's own `external-…` agent marker alongside the review schema, since a lane path is *inferred* rather than designated. A sibling that merely shares the name shape (`<output>.notes.json`), or shares the *schema* under another agent (an internal `reviewer-*` artifact), is left untouched.
