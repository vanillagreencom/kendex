...
| `SECOND_OPINION_CURRENT_MODEL` | (unset) | Model identity of this session; overrides harness detection. **Session-scoped — set it in the environment of the sessions that need it, not in shared `vstack.settings.toml` or `.env.local`**: a repo-wide value reaches every session in the repo, and one that contradicts a detected `claude`/`codex` session is refused rather than allowed to mark that session's own model cross-model. Required in Pi/OpenCode/Cursor and undetected shells (refused until set); `none` = no session model. Model ids normalize; a value the roster does not know is refused |
