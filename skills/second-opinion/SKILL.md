...
|----------|---------|-------------|
| `SECOND_OPINION_MODELS` | `claude codex` | Priority-ordered target roster, space/comma separated; the first eligible entry wins |
...
| `SECOND_OPINION_CURRENT_MODEL` | (unset) | Model identity of this session; overrides harness detection. Required in Pi/OpenCode/Cursor and undetected shells (refused until set); `none` = no session model. Model ids normalize; a value the roster does not know is refused |
...
|-----------|---------|--------|
| 1 | No eligible cross-model target (refused — the error lists every candidate and why), CLI not found, missing prompt, invalid JSON response | Report the stated reason; a refusal is fixed in settings (`SECOND_OPINION_MODELS`, `SECOND_OPINION_CURRENT_MODEL`), never by forcing the same model |
...
Runtime failures emit the JSON error object above; pre-flight configuration errors — unknown flag, missing mode, non-integer `--timeout` or `SECOND_OPINION_COUNT`, missing `jq`, an uncreatable `--output` parent directory, a missing `--cwd` — are plain `Error:` lines on stderr with exit 1.
