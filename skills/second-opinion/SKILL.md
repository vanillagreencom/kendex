...
**No exit leaves a previous run's artifact behind.** `--output`, every `<output>.<target>.json` lane artifact beside it — the roster's own names unconditionally, plus any left by a target since renamed or retired, and those only when the file's **content** is a review this skill emitted, so a sibling that merely shares the name shape (`<output>.notes.json` holding your own text or JSON) is never touched — and the managed sidecars of both (`.raw.txt`, `.retry.txt`, `.failed.json`, `.noreview.json`, `.incomplete.json`) are cleared as soon as the arguments are read — ahead of a bad-flag error, the jq check, the mode check and `--timeout` validation, and before target selection or any CLI run — so a refusal, timeout, or unusable response cannot be read as a pass by a caller that continues past the advisory non-zero exit. `--help` and `detect` are the exclusions: neither writes anything.
