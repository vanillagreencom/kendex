...
| `SECOND_OPINION_TIMEOUT` | `1080` | CLI timeout in seconds — one **total** per lane: the one retry on a malformed response runs inside the remainder (floored at 60s), so a lane never runs longer than the budget plus that floor. The default exceeds the ~600s ceiling most agent harnesses put on a foreground shell call: run the script in the background, or a foreground call must pass `--timeout` at or below that ceiling — otherwise the harness kills the shell first, no artifact is written and the exit-5 path never runs |
