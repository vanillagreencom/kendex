...
| `SECOND_OPINION_COUNT` | `1` | Opinions a `review` collects; 2 or more selects that many distinct models and unions their findings |
| `SECOND_OPINION_CURRENT_MODEL` | (unset) | Model identity of this session; overrides harness detection. Set it in Pi/OpenCode |
...
| `SECOND_OPINION_ARTIFACT_DIR` | `tmp/second-opinion` | Home for `review`/`audit` records written without `--output` (preserved raw, failed, and rejected responses); relative to `--cwd`, created owner-only. `--output` is the explicit override for the artifact itself |
