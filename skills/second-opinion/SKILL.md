...
| `SECOND_OPINION_CURRENT_MODEL` | (unset) | Model identity of this session; overrides harness detection. **Session-scoped — export it in the session, never in `.env`, `vstack.settings.toml`, `.vstack/settings.toml` or `.env.local`**: those reach every session in the repo, so a value from one is not a declaration for any of them and is refused (naming the file). Required in Pi/OpenCode/Cursor and undetected shells (refused until set); `none` = no session model. Model ids normalize; a value the roster does not know is refused |
...
**No exit leaves a previous run's artifact behind.** `--output`, every `<output>.<target>.json` lane artifact beside it — the roster's own names unconditionally, plus any left by a target since renamed or retired, and those only when the file's **content** carries this skill's own `external-…` agent marker alongside the review schema — so a sibling that merely shares the name shape (`<output>.notes.json`), and one that shares the *schema* under another agent (an internal `reviewer-*` artifact), are both left untouched — and the managed sidecars of both (`.raw.txt`, `.retry.txt`, `.failed.json`, `.noreview.json`, `.incomplete.json`) are cleared as soon as the arguments are read — ahead of a bad-flag error, the jq check, the mode check and `--timeout` validation, and before target selection or any CLI run — so a refusal, timeout, or unusable response cannot be read as a pass by a caller that continues past the advisory non-zero exit. `--help` and `detect` are the exclusions: neither writes anything.
