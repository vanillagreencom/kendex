...
Worktrees are created under `<parent-of-checkout>/.worktrees/<checkout-name>/` — beside the checkout, not inside it, so editor file watchers never ingest worktree build outputs and sibling repos cannot collide. The default branch comes from `origin/HEAD` (fallback `main`). After creation the configured symlinks, copies, and scratch directories are applied.
...
Run from the main checkout of a git repo with an `origin` remote. New-work claims need authenticated `gh` and `flock`. Put project defaults in committed `vstack.settings.toml` under `[env]`; keep secrets and personal overrides in `.env.local`. Load order is `.env`, then `vstack.settings.toml`, then `.env.local`.
