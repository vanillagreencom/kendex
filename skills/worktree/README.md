...
Bare `create <ID>` claims new work only. If a matching worktree, local/remote branch, or open PR already exists, it exits 75, reports the ownership signals, and does not rebase or modify a local branch. Inspect or monitor that work instead of launching another implementer. Run each issue create separately and check its result; do not batch creates in a shell loop whose last success can mask an earlier active-work exit.

The owning session can opt in with `create <ID> --reuse`, which rebases the existing branch onto `origin/<default>` and refreshes its setup. If that rebase conflicts, it aborts back to the clean pre-rebase state and prints the conflicting files plus two recovery paths: `create <ID> --restack` re-runs the rebase and pauses in the conflict state so you can resolve the files, `git -C <path> add` each one, and `GIT_EDITOR=true git -C <path> rebase --continue` (or `rebase --abort` to back out), then re-run `create <ID> --reuse` to finish setup; alternatively `remove <ID>` + `create <ID>` recreates the worktree fresh from `origin/<default>`, discarding the conflicting local commits. Use `create <ID> --pr <N>` to explicitly inspect an open PR when no worktree exists.
