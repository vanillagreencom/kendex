...
Staleness is heartbeat age with **no liveness check**. The heartbeat is stamped when the lease is claimed and refreshed only by `worktree create --reuse`, so lease age measures time since the worktree was claimed, not time since the session last worked in it: a session that has been working steadily for longer than its TTL is stale by this measure and will be unlocked by `release --stale` or `sweep`. Nothing refreshes a lease on a timer and nothing runs `sweep` automatically; confirm the owner is really gone before releasing.
