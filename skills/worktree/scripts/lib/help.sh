...
lives at <parent-of-checkout>/.worktrees/<checkout-name>/<WORKTREE_HOSTED_NAME>
(default lane) whatever WORKTREE_BASE_DIR says, one path in every lane of the
repository.
...
Path arguments and canonicalization:
  ...
  then the worktree registered for the issue branch, then the hosted lane
  path when the tree there records the issue, which finds a hosted tree whose
  HEAD a paused restack detached; there is no auto-migration. Path
  comparisons are canonical (physical, symlink-resolved on both sides). Direct path arguments for mutating commands must be
...
personal overrides):
  ...
  WORKTREE_HOSTED_NAME        The last segment of the path create --hosted
                              gives a new worktree, under
                              ../.worktrees/<checkout-name> whatever
                              WORKTREE_BASE_DIR says; one path segment in the
                              issue-ID alphabet. Default: lane.
...
Options:
  ...
  --hosted        The create runs for a hosted lane, on a clone that holds
                  one lane worktree: a new tree lands at
                  ../.worktrees/<checkout-name>/<WORKTREE_HOSTED_NAME> beside
                  the checkout (path --hosted), the same path in every lane
                  whose clone sits at the same path, so a build there hits
                  compile-cache entries keyed by another lane's identical
                  source path. The tree records its issue, so the issue ID
                  finds it with HEAD detached. With --reuse or --restack a
                  tree the issue already has is reused where it stands;
                  without them create refuses it (exit 75) as for any
                  existing tree. A path another issue's tree holds exits 75.
                  Without --hosted, the new tree is keyed by the issue ID so
                  several can coexist
