...
assert_not_contains "$(cat "$AUTH_PUSH_ROOT/standalone-push.args")" "kendex.test-github-helper=loaded" "standalone worktree push does not require a GitHub helper"
...
assert_contains "$(cat "$AUTH_PUSH_ROOT/helper-push.args")" "kendex.test-github-helper=loaded" "worktree push delegates git invocation to sibling GitHub helper"
