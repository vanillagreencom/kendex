...
#           here-document;
#           `printf-top` and `printf-split` metadata changed by `printf -v`;
#           `command-top` an ordinary command; `literal-hash` a stale hash;
#           `scalar-comment` a true comment; `after-function` a later pkgrel; `default-expansion` an unmodeled operator; `compound-index` an array index;
...
world() { # NAME — a fresh copy of the pristine world at $TMP/w-NAME, defect planted
  ...
  case "$name" in
    ...
    heredoc-brace-function)
      ...
      header_line "$recipe/PKGBUILD" 'pkgrel=2' "^  cat <<-'DATA'$"
      header_line "$recipe/PKGBUILD" '}' "^  cat <<-'DATA'$"
      sed -i.bak 's/pkgrel = 1/pkgrel = 2/' "$recipe/.SRCINFO" && rm -- "$recipe/.SRCINFO.bak"
      grep -q 'pkgrel = 2' "$recipe/.SRCINFO" || { echo "check-aur-sync.test: the here-document brace function edit did not take" >&2; exit 1; }
      ;;
    heredoc-source-top)
      header_line "$recipe/PKGBUILD" "source /dev/stdin <<'DATA'" '^sha256sums_aarch64='
...
unsupported SHA-512 checksum|sha512|-|2|unreadable=packaging/arch/kendex/PKGBUILD
