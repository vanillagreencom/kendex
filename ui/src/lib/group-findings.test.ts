...
describe("groupFindings", () => {
  ...
  it("keeps findings separate when the rule or the sentence differs", () => {
    const rows = [
      row({ name: "a", findings: [FINDING] }),
      row({ name: "b", findings: [{ ...FINDING, message: "different" }] }),
      row({ name: "c", findings: [{ ...FINDING, rule: "rce" }] }),
    ];
    expect(groupFindings(rows)).toHaveLength(3);
  });

  // One decision covers one rule and one sentence wherever it was found, so
  // two places are one row with both places under it — drawing two would
  // let acting on the one a person read clear the one they did not.
  it("draws one decidable row for one identity found in two places", () => {
    const groups = groupFindings([
      row({ name: "a", findings: [FINDING] }),
      row({ name: "a", findings: [{ ...FINDING, location: "other:1" }] }),
    ]);
    expect(groups).toHaveLength(1);
    expect(groups[0].locations).toEqual([FINDING.location, "other:1"]);
