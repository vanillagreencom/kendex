...
/** How much of an author's summary a preview shows. Chosen for about six
 *  lines at the preview's width — long enough that a written summary
 *  arrives whole, short enough that a page of prose does not become a
 *  popup. */
...
export function previewSummary(summary: string): PreviewSummary {
  ...
  if (text.length <= PREVIEW_SUMMARY_CHARS) {
    return { shown: text, truncated: false };
  }
  const window = text.slice(0, PREVIEW_SUMMARY_CHARS);
