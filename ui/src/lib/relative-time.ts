...
// not second-level precision. The steps below a month are that footer's;
// the two above it are for the commit dates the marketplace surfaces show,
// which are routinely years old and read as a day count without them
// ("731d ago" is not a date anybody parses).
//
...
//     attribute by hand, which is how a site ends up without one.
...
//
// `grep -rn 'relativeTime(' ui/src` is the list.
 ...
 *  because every caller would otherwise carry a copy of it, and one that
 *  forgets crashes a page. */
