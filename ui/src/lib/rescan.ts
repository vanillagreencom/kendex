...
//
// Every write runs this after it: an install, an update, a subscription
// change, a registry write. Adding a project, dropping one, or moving a
// harness's folder changes which scopes the audit reads, and a scope with no
// view of its own counts zero unmanaged items — which is how a project card
// ends up hiding the only way to the ones it holds.
import { useAuditStore } from "@/stores/audit";
import { useScanStore } from "@/stores/scan";

export async function rescanEverything(opts?: {
  /** Say so when the scan fails, however many times running. "Scan again"
   *  is somebody waiting on an answer; a rescan behind a write is not, and
   *  the scan store's own once-only notice covers it. */
  announce?: boolean;
}): Promise<void> {
  await Promise.all([
