...
//
// A write that can change what is installed runs this after it, and so does
// every button offering to look again. Adding a project, dropping one, or
// moving a harness's folder changes which scopes the audit reads, and a
// scope with no view of its own counts zero unmanaged items — which is how a
// project card ends up hiding the only way to the ones it holds.
