...
//
// A write whose effect reaches past the surface that made it runs this
// after it: an install, a registry change, a package coming current. A
// write that reads its own surface back does not need it — the Follow-source
// flip and the audit's item actions each land a fresh answer for the scope
// they touched — and the buttons offering to look again run it because
// nothing else knows what changed. Adding a project, dropping one, or moving
// a harness's folder changes which scopes the audit reads, and a scope with
// no view of its own counts zero unmanaged items — which is how a project
// card ends up hiding the only way to the ones it holds.
